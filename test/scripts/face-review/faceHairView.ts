 ...
 * photograph's mask compares like with like. The mask is the viewport's
 * width and `rows` rows (the viewport's height by default): a model's hair
 * may hang below a photograph's frame, and rows beyond it keep its end
 * measurable where the photograph's own is. Pure.
 */
export function faceHairVisibleMask(props: {
  ...
  const raster = (
    ...
    for (let t = 0; t < surface.indices.length; t += 3) {
      ...
      const y1 = Math.min(rows - 1, Math.ceil(Math.max(a.y, b.y, d.y)));
      for (let y = y0; y <= y1; ++y)
        for (let x = x0; x <= x1; ++x) {
          const px = x + 0.5;
          const py = y + 0.5;
  ...
  const skin = new Float64Array(size * rows).fill(Infinity);
  raster(props.skin, (at, z) => {
    if (z < skin[at]!) skin[at] = z;
  });
  const data = new Uint8Array(size * rows);
  for (const part of props.hair)
    raster(part, (at, z) => {
      if (z <= skin[at]! + props.slack) data[at] = 1;
    });
  return { width: size, height: rows, data };
}

/**
 * One document's model as a hair index reads it under a photograph's
 * camera: the hair the camera sees (`faceHairVisibleMask`, over three
 * viewport heights so hair hanging below the frame still ends), each anchored
