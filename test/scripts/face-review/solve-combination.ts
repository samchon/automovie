...
export const solveCombination = (
  ...
  // A push can hand a crossing on: the lining pushed clear of the gum block
  // carries the lip seam with it, and a lip that was clear of the teeth is
  // through them. So after the listed pairs, every part that moved is read
  // against every other part, and a pair now crossing that neither the rest
  // nor the singles cross (`baseline`) is solved in the same pass, up to
  // twice, before the combination is called repaired.
  const solvedPairs = new Set<string>();
  const touched = new Set<string>();
  const handedOn = (): [string, string][] => {
    const found: [string, string][] = [];
    const seen = new Set<string>();
    for (const mover of touched)
      for (const other of parts.keys()) {
        if (other === mover) continue;
        const key = [mover, other]
          .sort((a, b) => a.localeCompare(b))
          .join(" x ");
        if (baseline.has(key) || solvedPairs.has(key) || seen.has(key))
          continue;
        seen.add(key);
        if (parts.get(mover)!.surface === parts.get(other)!.surface) continue;
        if (
          measureAutoMovieMeshCrossings(partMesh(mover), partMesh(other))
            .length === 0
        )
          continue;
        const [a, b] =
          rank(mover) < rank(other) ? [mover, other] : [other, mover];
        found.push([a, b]);
      }
    return found;
  };
  let queue = ordered;
  for (let pass = 0; pass < 3 && queue.length > 0; pass++) {
    for (const [firm, soft] of queue) {
      solvedPairs.add(
        [firm, soft].sort((a, b) => a.localeCompare(b)).join(" x "),
      );
      if (rank(soft) < RIGID) {
      ...
      if (!solved && !sameSurface && YIELDS.indexOf(firm) >= RIGID) {
        ...
        for (const [vertex, delta] of second.moved)
          for (let k = 0; k < 3; k++)
            other.positions[vertex * 3 + k] += delta[k];
      ...
      if (moved.size > 0 || shared > 0) {
        touched.add(soft);
        if (shared > 0 || sameSurface) touched.add(firm);
      }
    }
    queue = handedOn();
  }
