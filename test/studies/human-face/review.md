...
### Current-matrix linear solver comparison

Profiling the same joint pucker problem identifies direct factorization and continuous collision detection as the main costs. Splitting Eigen's direct solve leaves the first twelve numerical steps and their paths exact. Symbolic analysis takes 0.593 seconds and numeric factorization/back substitution 7.089 seconds, so symbolic reuse alone addresses a small fraction of the measured linear cost. Timers are observational and never select a numerical branch.

The research successor uses Eigen's public ConjugateGradient on each current Lower-triangle search matrix, with an earlier checked LDLT factor used only as a preconditioner within that problem. It independently recomputes the current relative residual, requires at most 1e-10 and a descending direction, and falls back to a fresh direct solve on failure. A potentially terminal direction is recomputed directly before applying the existing 1e-6 mm vertex-movement criterion. The assembly supplies positive mass plus PSD/Gauss-Newton terms; residual and descent tests do not independently certify the whole current matrix's positive definiteness. The [Eigen solver documentation](https://libeigen.gitlab.io/eigen/docs-nightly/group__TopicSparseSystems.html) describes these library operations. The [Eisenstat/Walker study](https://users.wpi.edu/~walker/Papers/forcing_terms,SISC_17,1996,16-32.pdf) motivates residual control, but its true-Jacobian convergence assumptions do not directly apply to this projected search metric.

Independent dense-oracle cases verify changed matrices and sparsity, zero and small right-hand sides, exhausted-iteration fallback, reuse of the replacement factor, invalid factorization, nonfinite directions and recovery. The first refusal run exposed an experiment-build mistake: exception catching was enabled only at link time. Enabling it during compilation made the refusal cases observable; both logs remain preserved. These standalone research checks do not replace repository unit coverage.

Sequential full pucker runs use identical inputs and retained kernels. The direct baseline converges in 139 steps and 195.591 seconds, reproducing its earlier solution bytes exactly. CG with a 50-iteration work budget converges in 139 steps and 212.021 seconds, about 8.4% slower. Linear work rises from 81.568 to 94.771 seconds because failed iterative attempts precede 29 fresh solves. Its maximum terminal coordinate difference is 7.785e-8 mm. All 21 same-camera initial/target/candidate PNGs are byte-identical to the direct comparison.

A second controlled run changes only that work budget to 20, limiting the cost of unsuccessful attempts without relaxing accuracy. It converges in 137 steps and 192.638 seconds, but linear work is still 81.690 seconds. This single 1.5% wall difference does not establish a practical speed improvement. The terminal maximum difference is 0.192867 mm at the floor/vestibule attachment, with whole-compound RMS 0.004807 mm. Right/left eyelid maximum target errors are 0.012143/0.010111 mm, and the lip maximum is 2.575559 mm. Its final original objective is slightly higher than the direct result. A small linear residual alone therefore does not ensure an identical terminal state for this nonconvex geometric problem.

Both candidates retain exact fixed coordinates and have zero full native double/Float32 intersections. Independent exact final-Float32 censuses check 467,385 and 467,220 inclusive AABB pairs respectively, with 403,204 contacts limited to declared shared features and zero forbidden intersections in each. Analytic and previous-census controls run before both audits. The main agent directly inspected all 42 calibrated AMD GPU captures. The second candidate retains the earlier visible form without a demonstrated visual improvement; its seven candidate PNGs differ, while the fourteen initial/target PNGs remain exact. The no-op control also preserves every coordinate exactly.

The reference-localized direct formulation remains the integration basis: these two numerical variants establish neither a material speed improvement nor better facial form. Their source, binaries, logs, failed run, receipts, regional measurements, GLBs and captures remain available under `ipc-before-pcg`, `ipc-pcg50`, `ipc-pcg20`, `joint-cpu-profile`, `rest-locality-pucker-direct-paired`, `rest-locality-pucker-pcg-full`, `rest-locality-pucker-pcg20-full` and the corresponding diagnostic directories within the retained investigation root. The current `joint_linear` files are research only. The comparison does not add a product solver, general initialization, other-subject coverage or a likeness acceptance.

### Initial enclosure from current native inputs

The next research step separates the successful initial enclosure from retained meshes. `general-maxillary-initialization.mjs` takes the current dental recipe, attachment and constructor-owned cervical cycles, plus authored outer/ridge/buccal topology. `general-oral-initialization.mjs` consumes current pre-seal lip topology, native interiors and sealer identities. It calls the existing maxillary, FEM, cheek, floor and complete-overlap owners. No saved native mesh, person name or coordinate-based join enters either pure constructor. Hidden tissue dimensions remain explicit author estimates.

A fresh read-only observation of the canonical document's actual build leaves the complete model unchanged. The new initializer preserves its inputs and reproduces the earlier raw floor, every constrained block, the 32,778-vertex/65,602-triangle compound, full model and 7,148,596-byte GLB exactly. Previous artifacts are comparison oracles only. This removes a saved-mesh dependency; it does not establish an arbitrary-face initialization or a new facial improvement.

Two independent changed-input cases retain the same authored tissue profile: intermediate face width increases by 0.02, or dental arch half-width increases by 0.25 mm. Their actual source builds produce 32,771/65,588 and 32,780/65,606 vertices/triangles respectively, with all 320 native cervical junctions. Complete projected floor/enamel constraints number 52,108 and 52,582. Both full compounds have zero native double/Float32 intersections. Independent exact final-Float32 audits check 455,734 and 457,059 inclusive AABB pairs, with zero forbidden intersections; the analytic and earlier floor-census controls pass first.

The main agent directly inspected all 46 calibrated AMD Radeon 8060S captures, comparing each changed input's original model with its complete enclosure and isolated oral parts. Exterior silhouette and features remain visually consistent while the connected interior replaces the detached backdrop. The simplified enamel, bulky eyelids, angular nasal base and missing lower enamel/tongue remain. These are two initialization probes, not population, expression or likeness acceptance.

Seven input-refusal probes cover missing cavity/enamel, duplicate and disconnected profile ownership, stale dental geometry/cervical IDs and incorrect upper-lip controls; every input remains unchanged, and a subsequent valid construction reproduces all baseline blocks exactly. One initial disconnected-path probe accidentally duplicated another vertex and tested the earlier guard instead; its script and failure remain preserved. A stale-mesh refusal run exceeded 116 CPU seconds; stage tracing identified the deep assertion, whose Node implementation constructs a full textual array diff. The bounded diagnostic now uses the same complete strict comparison without materializing that diff. Both interrupted observations and the successful seven-case/recovery run are retained.
