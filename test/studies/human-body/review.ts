 ...
 * @evidenceReview {@link Human.IAutoMovieHumanBodyBasis} #fabbf21 Read the schema and admission against the shipped CC0 asset: sparse endpoints, shared topology, one exact material partition, landmarks, joints and four-influence skin, and the frame shared with the face at the neck ring. Reread after the rest angle joined the joint record; nothing else in the schema moved.
 ...
 * @evidenceReview {@link Human.IAutoMovieHumanBodyBasis.correctives} #5a67c83 Read driver-side admission, the (0,1] gain and the product activation the shipped macro pair residuals fire under.
 ...
 * @evidenceReview {@link Human.assertHumanBodyBasis} #f2f0381 Read identity, envelope, mirror, corrective, topology, sparse-row, partition and endpoint-population admission, then the rig hand-off.
 ...
 * @evidenceReview {@link Human.assertHumanBodyRig} #9d3979c Read landmark, tree-order, reference, sign-versus-constraint, range and skin admission against the shipped rig and the analytic negatives. Reread after the rest angle: it is refused when nonfinite, nonzero on a held axis, or outside its range, so the empty document is never a refused pose; the unconstrained root declares every sign.
 ...
 * @evidenceReview {@link Human.humanBodyBasisWeights} #ceddd46 Read envelope and membership refusals, identity row admission and the product corrective activation shared by builder and measurement.
