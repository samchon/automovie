 ...
 * its eye owner with weight one. Articulation lists `leftEye` and
 * `rightEye`; the jaw fields are inert here because the pigment rule reads
 * only the eye ids.
 */
export function humanFaceIrisGlobeFixture(): IAutoMovieHumanFaceBasis {
  const size = 256;
