...
for (const file of files) {
  for (const { anchor, body } of sections(read(`materials/${file}`))) {
    ...
    for (const id of ids) {
      const owners = modelLinks.filter((model) => modelIds.get(model)?.has(id));
      ...
      results.push({ material: `${file}#${anchor}`, id, owners, spaceLinks, declaredAnywhere, mask });
    }
...
const table = [
  "| material H2 | face id 또는 응답 | 만드는 설계 owner |",
  "|---|---|---|",
  ...results.map((row) => {
...
if (process.argv.includes("--write")) fs.writeFileSync(ledgerFile, document);
  ...
  if (check !== document) { console.error("material face ledger differs from current producer output"); process.exitCode = 1; }
  else console.log(JSON.stringify(summary));
}
else if (process.argv.includes("--json")) console.log(JSON.stringify({ summary, results, unowned }, null, 2));
else {
