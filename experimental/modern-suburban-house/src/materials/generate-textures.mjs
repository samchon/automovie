/**
 * Deterministic authored colour tiles for the house material bindings.
 * Run `node src/materials/generate-textures.mjs`; no external image input.
 * Coordinates and random lattice wrap at 512, so every PNG tiles on both axes.
 * Base colours are the sRGB fallbacks in docs/materials; the viewer marks each
 * loaded colour map sRGB and falls back to the binding's hex if loading fails.
 */
...
/** Each source expression is periodic in u/v; module sizes live in bindings.ts. */
...
const textures = {
  ...
  "timber.png": (x, y, u) => {
    ...
    return base(edge ? 0x624b34 : 0x8c6a48, vein + (noise(x, y, 32, 10) - 0.5) * 10);
  },
  "oak.png": (x, y, u, v) => {
    const joint = distanceToEdge(u) < 0.012 || distanceToEdge(v) < 0.006;
    const grain = Math.sin((u * 15 + noise(x, y, 8, 11) * 0.33) * 2 * Math.PI) * 7;
    return base(joint ? 0x815b39 : 0xb08050, grain + (noise(x, y, 64, 12) - 0.5) * 9);
  },
  "tile.png": (x, y, u, v) => {
