...
@evidence principles/design/materials.md#material-binding-interface 검은 도장 금속의 결합 vocabulary는 난간의 `baluster`·`bottom-rail`, 문·수납의 `handle`·`hinge`·`rail`, 등기구 외장의 `fixture-housing`·`fixture-canopy`·`fixture-stem`, 주침실 커튼의 `rod`·`bracket`이며 부재별 방향과 단면/양면은 00 면 결합 규칙을 따른다.
