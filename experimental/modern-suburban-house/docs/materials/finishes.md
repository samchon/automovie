...
The module producer computes each row and column from its law, applies its row offset, starts brick rows at `y=0.15 m` and siding rows at `y=0.90 m` above the `0.75 m` skirt, and clamps facade module centres to their host half-module extents. Roof module centres are clamped to the transformed host slab. The law retains the declared stagger rule as source metadata, while the current producer's visible placement operation is the measured row/column offset. The resulting population is measured and regenerated, not a hand-copied set of face records. A count, host surface, or span that cannot be derived from these law inputs remains unverified.
