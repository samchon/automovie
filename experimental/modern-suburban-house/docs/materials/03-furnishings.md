...
@evidence contracts/texture-readability.md#material-texture-readability 수납장 `front`·`carcass`의 반무광 도막 결은 0.10 m 모듈이다. 원형의 각 판 왼쪽 아래를 원점으로 U 폭·V 높이를 쓰고 서랍·문짝 경계에서 끊어 패널 간 그림자를 지우지 않는다.
...
표면 결속 계획: 수납장 `front`·`carcass`의 반무광 도막 결은 0.10 m 모듈이다. 원형의 각 판 왼쪽 아래를 원점으로 U 폭·V 높이를 쓰고 서랍·문짝 경계에서 끊어 패널 간 그림자를 지우지 않는다. 아직 생성된 맵과 GPU 근접·리뷰 거리 판정은 없으므로 unverified다.
