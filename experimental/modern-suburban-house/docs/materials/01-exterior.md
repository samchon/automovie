...
@evidence contracts/texture-readability.md#material-texture-readability `shingle-face`의 광물 입자 거칠기와 미세 색 차를 결정론적 맵으로 만든다. 각 지붕 면의 처마를 V 원점, 처마 평행선을 U로 놓고 탭 폭 0.333 m·course 노출 0.14 m에 맞춘다. 골짜기·용마루·굴뚝 절단에서 pattern은 실제 잘린 부재와 함께 끝난다.
...
표면 결속 계획: `shingle-face`의 광물 입자 거칠기와 미세 색 차를 결정론적 맵으로 만든다. 각 지붕 면의 처마를 V 원점, 처마 평행선을 U로 놓고 탭 폭 0.333 m·course 노출 0.14 m에 맞춘다. 골짜기·용마루·굴뚝 절단에서 pattern은 실제 잘린 부재와 함께 끝난다. 아직 생성된 맵과 GPU 근접·리뷰 거리 판정은 없으므로 unverified다.
...
@evidence contracts/texture-readability.md#material-texture-readability 기단·굴뚝·벽난로의 벽돌 면에는 0.20 × 0.065 m 벽돌과 0.01 m 줄눈의 색·거칠기 맵을 쓴다. 벽 길이 U·높이 V, 외벽 바닥선과 굴뚝 하단을 원점으로 하며 모서리에서 수평 줄눈 높이를 맞추고 개구부에서 끊는다.
...
표면 결속 계획: 기단·굴뚝·벽난로의 벽돌 면에는 0.20 × 0.065 m 벽돌과 0.01 m 줄눈의 색·거칠기 맵을 쓴다. 벽 길이 U·높이 V, 외벽 바닥선과 굴뚝 하단을 원점으로 하며 모서리에서 수평 줄눈 높이를 맞추고 개구부에서 끊는다. 아직 생성된 맵과 GPU 근접·리뷰 거리 판정은 없으므로 unverified다.
...
@evidence principles/design/materials.md#material-construction-appearance charcoal 창틀과 굴뚝 cap은 구성을 '분체 도장 알루미늄'로, 외관의 #2E3033·roughness 0.40·metallic 0.0을 기준값으로 두고 아래 표면 결속 계획의 결·광학 응답으로 최상층 마감을 표현한다고 적는다.
...
@evidence contracts/texture-readability.md#material-texture-readability `frame`·`sash`·`mullion`·`muntin`과 굴뚝 cap의 도장 금속 미세결은 각 부재 길이를 U로 한 0.05 m 모듈이다. 국소 부재 끝을 원점으로 하고 맞댐마다 결 방향을 새로 잡으며 모서리 하이라이트를 유지한다.
