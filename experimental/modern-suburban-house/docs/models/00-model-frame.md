...
@evidence principles/core/common.md#substantive-completion 원점을 뒤쪽 바닥선 중앙, +Z를 사용 앞쪽으로 정하고 걸레받이의 뒤·옆 하단 0.015×0.10 m 띠를 비우는 규칙 및 모든 model 면의 기본 UV 축·원점·척도·이음을 적는다.
...
개구부를 채우지 않는 가구·설비·수납 원형은 [제작 좌표](../settings/00-production.md#coordinate-units)의 단위와 축을 쓰고 국소 원점을 바닥에 닿는 뒤쪽 모서리 선의 가로 중앙에 둔다. 국소 +Z는 사용자가 서서 쓰는 앞쪽, 국소 +Y는 world +Y, 국소 +X는 앞에서 보아 오른쪽이다. 뒤쪽 모서리 선은 벽에 붙는 원형이면 벽 마감 면에, 섬·식탁처럼 벽에서 떨어진 원형이면 사용 방향 반대쪽 외곽에 둔다. 이 원점은 점유 외곽의 기준일 뿐 고체 부재의 평평한 뒤판을 뜻하지 않는다. 노출 걸레받이가 있는 벽에 뒤 원점을 붙이는 원형은 바닥 위 국소 Y=[0,0.10] m와 벽에서 방 쪽 Z=[0,0.015] m의 띠에 메시를 만들지 않는다. 가구의 왼쪽 또는 오른쪽 끝도 걸레받이 벽에 닿으면 그 끝에서 안쪽 0.015 m까지의 국소 X 띠와 Y=[0,0.10] m를 비운다. 옆면 홈은 해당 벽과 마주 보는 측면의 전체 깊이에 적용하며 뒤쪽 홈과 만나는 모서리는 두 홈의 합집합으로 한 번만 뺀다. 각 원형은 해당하는 뒤·옆 벽을 명명하고 발 위치·뒤판 시작 높이·하부 홈을 본문에서 지정한다. 상부의 뒤면 및 앞면·손잡이·작동 최대치는 받은 예약 외곽을 넘지 않는다. 벽 앞 붙박이도 실제 벽 개구부가 없는 한 걸레받이를 뒤에서 이어 가고 같은 홈을 쓴다. 외곽 치수는 [예약 맞춤 계약](../contracts/reservation-fit.md#reservation-fit)에 따라 room owner의 상한 박스와 사용 공간 예약에서 받고, world 배치 회전은 Y축 회전 하나로 instances가 room owner의 좌표에서 계산한다. 소스 owner는 `src/models/frame.ts`이며 각 가구 H2가 이 규칙을 링크로 소비한다.
...
01–19의 창·문·계단·수납·걸레받이·가구·설비·외피 반복·소품 원형은 각 H2가 별도 UV를 정한 경우를 제외하고 다음 미터 UV를 상속한다. 원점은 각 독립 부재의 국소 왼쪽 아래 뒤 모서리이며 평면마다 (0,0)에서 다시 시작한다. 수평 상·하면은 X 방향 U·Z 방향 V, 세로 앞·뒤면은 X 방향 U·Y 방향 V, 세로 옆면은 Z 깊이 방향 U·Y 방향 V다. 절단 끝면은 그 면의 가장 긴 실제 변을 U, 다음 변을 V로 삼아 새로 시작한다. 기운 평면은 그 면에서 국소 +X를 투영해 얻은 접선(영벡터면 +Z의 투영)을 U로 삼고 법선과 U의 외적으로 얻은 접선을 V로 삼는다. 원통·원뿔대는 축 실제 길이 U·둘레 실제 길이 V로 두며, 축이 Y이면 +Z, 축이 Z이면 +X, 축이 X이면 +Y 방위에서 둘레 seam을 시작한다. 축이 기울어 어느 축과도 평행하지 않으면 부재의 시작점에서 축에 직교 투영한 +Y(영벡터이면 +Z)를 둘레 0 방향으로 정하고 오른손 방향의 실제 둘레 호길이로 V를 편다. 구는 +Y 극을 기준으로 경도 실제 호길이 U·북극에서의 위도 실제 호길이 V=Rθ(θ는 +Y에서 잰 라디안)를 주고 경도 seam은 +Z 방향이다. 구의 경도 U는 각 위도에서 R sinθ·φ의 실제 호길이로 두며 극점의 U는 0이다. 자유 곡면은 부재가 선언한 시작 경계의 첫 꼭짓점에서 경계 호길이 U를 누적하고 그 경계에 직교하는 표면 최단거리 V를 누적하며, 분기점·날카로운 모서리에서 seam을 끊는다. 모든 좌표는 1.00 UV 단위가 부재 표면의 1.00 m이며 음수 축을 향하는 반대 면에서는 U의 물리 방향을 유지하되 원점만 그 면의 대응 모서리로 옮긴다. 따라서 왼쪽·오른쪽 세로면의 결은 거울상으로 반전되지 않는다. 같은 평면의 연속 판은 UV를 이어 쓰고 창틀·문설주·문선·문짝·경첩·레일·봉·선반·서랍·쿠션·기구 갓·지붕널 줄처럼 실제 분리되는 부재의 절단 끝에서만 이음을 끊는다. 목재 부재의 별도 긴 결은 길이 방향 U로 회전하고, 지붕 경사 부재의 국소 축과 seam은 해당 H2의 예외가 우선한다. 뒷면·밑면·오목한 면·두께 면에도 UV를 주며 UV 없는 면을 허용하지 않는다. 이 models 소유 좌표는 결 방향과 물리척도를 전달하고 반복 길이와 광학값은 materials가 정한다.
