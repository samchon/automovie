...
이 미닫이 옷장은 독립 이동 가구가 아니라 벽 개구부 안에 고정하는 붙박이 몸통이다. 뒤판·옆판이 바닥부터 벽에 닿는 대신, [걸레받이 원형](06-interior-trim.md#wall-baseboard)의 노출 run은 옷장 양쪽 바깥 옆판 앞 끝에서 각각 수직 마개로 끝나고 옷장 뒤쪽 벽에는 반복하지 않는다. 따라서 옷장 1.50×0.60×2.20 m 외곽과 두 트랙·문짝 이동 0.72 m는 변하지 않으며 옷장 뒤에 감춘 걸레받이와 몸통이 중복하지 않는다.
