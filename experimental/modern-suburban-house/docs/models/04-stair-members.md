...
아래 flight는 `src/spaces/stair.ts`의 열린 옆면 X=−0.65 m에서 현관 쪽 +X로 두께 0.015 m만 차지한다. 길이축은 시작 Z=−1.45 m에서 참 Z=−3.41 m까지 −Z이고, 바닥 위 높이의 아래 모서리는 `Y=0.17×(−1.45−Z)/0.28` m, 위 모서리는 그 값+0.10 m다. 판의 네 바깥 단면 꼭짓점은 (Z,Y)=`(−1.45,0)`, `(−1.45,0.10)`, `(−3.41,1.29)`, `(−3.41,1.19)` m이며 X=−0.65와 X=−0.635 m 사이에서 압출한다. 각 0.28 m 단의 내부에서는 아래 모서리가 그 단 윗면보다 낮아 구조 옆면에 붙는다. 상부 flight는 열린 앞 옆면 Z=−3.41 m에서 현관 쪽 +Z로 두께 0.015 m만 차지한다. 길이축 X=−0.65→1.87 m에서 아래 모서리는 `Y=1.36+0.17×(X+0.65)/0.28` m, 위 모서리는 그 값+0.10 m다. 네 끝점의 Y 범위는 시작 [1.36,1.46], 도착 [2.89,2.99] m다. 마지막 디딤에서 상층 바닥 Y=3.06 m로 올라가는 0.17 m 마지막 챌판은 기존 spaces 부재이며 측판을 그 바닥까지 억지로 늘리지 않는다. 실제 단별 구조 옆면에 접하기만 하며 계단 안쪽 0.075 m 난간 예약에는 들어가지 않는다.

두 판 사이 참의 바깥 0.015×0.015 m 정사각 모서리 X=[−0.65,−0.635], Z=[−3.41,−3.395], Y=[1.19,1.46] m는 같은 원형의 세 번째 닫힌 연결판이다. 아래 판은 그 정사각형의 Z 시작면 중 Y=[1.19,1.29] m에, 위 판은 X 시작면 중 Y=[1.36,1.46] m에 접하고 어느 판도 공유 부피가 없다. 그 사이 0.07 m는 참 구조 옆면에 기대는 흰 연결판이 덮는다. 아래 첫 챌판 앞에서는 수직 끝면으로, 상층 도착에서는 X=1.87 m의 [계단 도착 막음](../spaces/02-stair.md#stair-boundary-heights)에 닿는 수직 끝면으로 닫는다. 세 판의 앞·뒤·윗면·아랫면·끝면은 모두 `stair-skirt` face id를 받고, 그 흰 도장은 [실내 trim](../materials/02-interior-shell.md#interior-trim-white)이 맡는다. UV는 각 경사 판의 첫 아래 모서리에서 실제 경사 길이 U·판 높이 V를 1 UV/m로 투영하고, 두께 면과 끝면은 해당 면의 왼쪽 아래에서 같은 미터 척도로 다시 시작한다. 참 연결판에서는 각 면의 왼쪽 아래에서 별도 투영해 세 부재의 접선마다 이음을 끊는다. 기하와 UV, 현관 쪽 실제 순폭, GPU 읽힘은 modelSources 이전이라 unverified다.
