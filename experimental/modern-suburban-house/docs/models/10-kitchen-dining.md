...
`leaf` 문짝은 국소 X=[−0.34,0.34] m·Y=[0.10,0.62] m·Z=[0.61,0.65] m의 폭 0.68 m·두께 0.04 m 판이며 피벗은 Y=0.10 m·Z=0.65 m의 국소 X축이다. 손잡이 `handle`은 문짝 윗단보다 0.04 m 아래의 X=[−0.24,0.24] m 수평 막대로 지름 0.02 m이고, 양 끝의 지름 0.015 m 짧은 지지축이 전면에서 0.03 m를 띄운다. 90° 열린 문에 대한 위의 0.55 m 점유는 피벗에서 문짝 높이와 손잡이 최전점까지의 보수적인 합이며 닫힌 Z 외곽은 0.65 m다.
...
식기세척기 `appliance-body`의 닫힌 외곽은 0.60×0.60×0.88 m이고 전면 문 자리는 빈다. `leaf`는 폭 0.60 m·높이 0.60 m·두께 0.02 m로 Y=[0.10,0.70] m에 놓이고, `control-panel`은 바로 위 Y=[0.70,0.88] m의 폭 0.60 m·두께 0.02 m 띠다. `handle`은 문 윗변 중앙에 폭 0.12 m·깊이 0.012 m로 파는 홈이라 전면 바깥으로 돌출하지 않는다. 열린 문 뒤의 `appliance-interior`는 외곽 안쪽에서 폭 0.56 m·깊이 0.50 m·높이 0.68 m의 빈 상자 내벽이며 외부 덮개와 부피를 겹치지 않는다.
