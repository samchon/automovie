...
벽에 닿는 소파·책장·서랍장·책상·벤치·냉장고·레인지·세탁기/건조기·주방 L형 하부장·벽 앞 미닫이 옷장의 뒤에서는 걸레받이 run을 연속한다. 이 원형들은 [가구 하부 빈 띠](00-model-frame.md#model-furniture-local-frame)대로 바닥 위 0.10 m·벽에서 0.015 m의 부피를 비우고, 건조기·벤치처럼 앞벽에 옆면이 닿으면 국소 X 끝 0.015 m도 비운다. 차고 공유 벽 문선에서는 걸레받이가 문선 바깥 끝에서 멈추고 상판·상부장·팬트리 선반·세탁기는 해당 문선의 실제 부피를 비운다. 각 run의 종단은 컴파일된 벽 경계와 문선 외곽에서만 산출하며 방 예약 길이를 줄이거나 가구를 전방으로 옮기지 않는다. 열린 문에서는 외부 문의 `casing`·실내 문의 `casing-a/b`의 바깥쪽 가장자리에서 종단하고, 문선이 없는 개구부에서는 벽 끝에서 종단한다. 이 판단은 도장 벽을 다시 만들지 않고 노출 걸레받이의 부피만 분할한다.
