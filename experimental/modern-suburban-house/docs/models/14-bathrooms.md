...
@evidence principles/design/models.md#model-observable-style-basis 스타일 라벨을 두지 않고 폭 0.50 m 물탱크 위의 뚜껑과 폭 0.38 m 둥근 끝 몸통이라는 실루엣·비례만 정하며, 재료 선택 없이 표면 id `ceramic`·`toilet-seat`·`lid`·`handle`을 남긴다.
...
@evidence principles/design/models.md#model-scale-layer-completion 공통 상면 0.85 m, W·D 세 쌍, 판 두께와 문 수·틈, 16분할 볼의 위아래 단면, 수전 접점, 샤워 욕실 소품 세 개의 국소 위치를 확정한다. 배관·배수는 제외하고 실제 메시·프레임은 unverified다.
...
부품은 걸레받이(0.10 m), 수납 몸통, 문 전면, 상판(0.03 m), 매립 세면볼, 수전이다. 국소 원점은 뒤쪽 벽과 바닥의 가로 중앙이고 X=[−W/2,W/2], Z=[0,D]다. 걸레받이는 X=[−W/2+0.02,W/2−0.02], Z=[0,D−0.07], Y=[0,0.10] m의 닫힌 받침이다. 몸통은 Y=[0.10,0.82] m이며 두께 0.02 m의 좌우 옆판·뒤판·바닥판으로 닫고 전면과 상면은 문·상판을 위해 비운다. 상판은 X·Z 전체 W × D, Y=[0.82,0.85] m다. 세면볼은 상판 중심의 W−0.20 m × D−0.20 m 타원 절개 아래 0.15 m 깊이의 열린 그릇이다. 그릇의 16개 둘레 꼭짓점은 위에서 반지름 ((W−0.20)/2,(D−0.20)/2), 바닥에서 그 0.70배이고, 바닥은 Y=[0.69,0.70] m의 두께 0.01 m 판으로 닫는다. 상판의 구멍 둘레와 그릇 윗끝은 한 번만 맞댄다. 수전 바닥판은 상판 뒤 테의 국소 X=0, Z=0.05 m에 접하며 지름 0.05 m·두께 0.008 m다. 지름 0.025 m 수직 기둥은 판 위에서 0.20 m이고 지름 0.018 m 토출관은 그 끝에서 볼 쪽으로 0.10 m 뻗어 토출 끝을 상판 위 0.20 m에 둔다. 폭 0.60 m 파우더룸에는 폭 0.56 m 문 한 장, 폭 0.70·0.85 m인 두 욕실에는 각각 폭 0.32·0.395 m 문 두 장을 둔다. 몸통 옆판과 각 문 사이 0.02 m, 두 문 사이 0.02 m를 남긴다. 따라서 0.70 = 2 × 0.32 + 3 × 0.02 m, 0.85 = 2 × 0.395 + 3 × 0.02 m다. 각 문은 Y=[0.10,0.82] m, 두께 0.02 m이고 윗변에서 깊이 0.012 m·높이 0.018 m의 홈 손잡이를 파며 추가 전면 돌출이 없는 강체다. 표면 id는 `plinth`, `carcass`, `leaf`, `countertop`, `ceramic`, `faucet`, 홈 안쪽의 `handle`이다.

파우더룸·두 욕실의 도장 벽에 닿는 `plinth`는 뒤쪽 국소 Z=[0,0.015] m·Y=[0,0.10] m를 실제 판에서 빼고, 옆면이 도장 벽에 닿는 배치는 해당 국소 X 끝 0.015 m도 같은 높이에서 뺀다. 두 홈은 합집합으로 한 번만 제거한다. 상판과 세면볼의 외곽은 움직이지 않으며 타일 벽에서는 걸레받이 입력이 없으므로 홈을 만들지 않는다.

[샤워 욕실 설정](../settings/10-house.md#shower-bathroom)이 요구하는 세면 소품은 샤워 욕실 배치에서만 켜는 선택 부품으로 두며, 상판 뒤쪽 Z=[0,0.12] m 띠 안에 비누 받침(0.12 × 0.08 × 0.02 m), 펌프 병(지름 0.07 m, 높이 0.18 m), 칫솔 컵(지름 0.07 m, 높이 0.10 m)을 놓는다. 국소 상판 위 접점의 (X,Z)는 차례대로 (−0.25,0.05), (0.18,0.065), (0.28,0.065) m다. 세 소품의 끝은 폭 0.70 m 상판의 X=[−0.35,0.35] m와 뒤쪽 Z=[0,0.12] m 안에 들고 X=0 수전 판과 분리된다. 앞끝은 볼 절개 시작선 Z=0.10 m를 넘지 않는다. 표면 id는 `accessory`다. 배관·배수는 표현하지 않는다. 소스 owner는 `src/models/furnishings/bathrooms.ts`다. 관찰은 샤워 욕실 배치에서 세 소품이 볼과 수전 뒤에 보이는지, 세 매개변수 쌍의 평면 외곽이 각 예약과 같은지, 측면에서 상면 0.85 m가 읽히는지다. 모든 관찰은 unverified다.
...
@evidence principles/design/models.md#spatial-convention 가구 국소 원점의 world X=1.525 m와 로컬 +X=world +X를 적용해 뒤벽 수전 부모 구간 world X=[1.00,1.30] m를 로컬 X=[−0.525,−0.225] m, 중심 −0.375 m로 옮긴다. 세 패널 피벗은 로컬 +X를 따라 미끄러지며 트랙은 부스 외곽을 넘지 않는다.
...
@evidenceExclude upstream/design/models.md#settings-and-space-revision-from-model-work 샤워 욕실의 부스 외곽·높이 2.10 m·통과 폭 0.80 m 이상과 뒤벽 수전 world X=[1.00,1.30] m를 검토했다. 가구 원점 world X=1.525 m를 빼 로컬 X=[−0.525,−0.225] m·중심 −0.375 m로 놓고 통과 폭 0.80 m를 남겨 부모를 고칠 결함은 없었다.
@evidence spaces/rooms/shower-bath.md#shower-fixture-use 샤워 욕실 예약의 부스 폭 1.25 m·깊이 1.10 m·높이 2.10 m를 원형 외곽으로, 뒤벽 수전 world X=[1.00,1.30] m를 가구 원점 기준 로컬 X=[−0.525,−0.225] m로, 통과 폭 0.80 m 이상을 최대 열림 목표로 받는다.
...
샤워부스 `shower-tray`는 1.25×1.10×0.018 m 바닥판에서 둘레 0.010 m 가장자리를 남기고 안쪽 네 평면을 중심 배수구 쪽으로 0.004 m 내린 닫힌 부재다. 뒤벽의 `faucet`은 설치 중심 로컬 X=−0.375 m·Y=1.05 m의 지름 0.08 m·깊이 0.025 m 원판과 지름 0.025 m·길이 0.075 m 토출관, Y=2.03 m의 지름 0.025 m·길이 0.175 m 관 및 지름 0.14 m·두께 0.025 m 헤드로 이루어진다. 두 물건의 완성 외곽은 위에서 정한 부스·벽 예약 안에 머문다.

왼쪽 벽 쪽 `rail` 프로파일은 국소 X=[−0.625,−0.605] m, 세계 Z=[−7.790,−7.760] m, 상층 바닥 위 Y=[0,2.10] m인 닫힌 각관이다. 가장 뒤 패널의 닫힘 X 시작점은 부스 왼쪽 경계 −0.625 m로 두고 그 유리 두께 구간은 세계 Z=[−7.760,−7.752] m라 프로파일 앞면과 면으로만 닿는다. 나머지 두 패널은 각자의 트랙 깊이에서 움직이고, 세 장의 X 합집합은 앞 문단대로 [−0.625,0.625] m 전체 1.25 m다. 프로파일 폭 0.020 m는 열린 상태의 유효 통과 폭에서만 빼므로 1.25−0.43−0.02=0.80 m가 남는다. 위·아래 트랙과의 맞댐도 면 접촉으로 끝내며 별도의 겹친 덮개를 만들지 않는다.
...
@evidence principles/core/common.md#substantive-completion 레일 지름 0.025 m·전체 길이 1.90 m·커튼 사용 길이 1.80 m·높이 2.05 m, 매달림 봉 지름 0.02 m, 레일 중심 2.05 m를 감싼 윗단부터 바닥 위 0.60 m까지 내려오는 두께 0.01 m 커튼 판을 정한다.
...
커튼 레일은 [방 문서](../spaces/rooms/tub-bath.md#tub-fixture-use)의 Z=[−8.70,−6.90] m 예약을 그대로 받아 욕조 길이 1.80 m 전체를 따른다. [가구 국소 좌표](00-model-frame.md#model-furniture-local-frame)의 예외로 선형 부재이므로 원점을 뒤쪽 레일 끝 높이 2.05 m에 두고 +Z를 욕조의 긴 축을 따라 둔다. 배치 yaw는 욕조의 긴 축에서 계산한다. 레일은 지름 0.025 m·길이 1.80 m이고 양 끝은 천장까지 내려오는 지름 0.02 m 매달림 봉 둘이 받친다. 벽에서 욕조 뒤끝까지의 0.10 m에는 별도의 레일이나 브래킷을 만들지 않는다. 천장 높이는 방 owner에서 받으며 두 봉의 X/Z는 레일 양 끝 축과 같다.

커튼은 레일 중심 2.05 m를 감싼 윗단부터 바닥 위 0.60 m까지 내려오는 두께 0.01 m의 판이고 매개변수 `curtain-open`이 펼친 길이를 1.80 m에서 0.25 m까지 바꾼다. 걷은 상태에서는 레일 뒤쪽 끝에서 앞쪽 0.25 m 안에 모이며 이때 주름을 네 번 꺾인 판으로 표현한다. 레일의 모든 면은 `rail`, 천장에 닿는 받침봉은 `rod`, 커튼의 양면과 절단 끝은 `curtain`이다. 걷은 네 접힘의 욕조 전면 법선 변위는 각 절점에서 0, +0.015, 0, -0.015, 0 m이고 천 두께 0.01 m를 더해도 중심선에서 ±0.02 m여서 부모의 폭 0.10 m 띠 안에 머문다. 레일·봉의 UV는 길이 U·둘레 V, 커튼은 긴 방향 U·높이 V를 미터로 둔다. 소스 owner는 `src/models/furnishings/bathrooms.ts`다. 관찰은 펼친 상태와 걷은 상태의 사선 투시에서 커튼이 욕조 사용 범위로 늘어지지 않는지다. 모든 관찰은 unverified다.
