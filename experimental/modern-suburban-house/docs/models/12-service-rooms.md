...
세 공구의 `tool-steel`은 망치 머리 0.12×0.035×0.035 m, 렌치 0.06×0.30×0.025 m 몸통과 양끝 턱, 톱의 국소 X=[0,0.24]·Y=[0.02,0.14]·깊이 0.030 m 날을 각각 닫힌 부재로 만든다. `tool-grip`은 망치 자루 0.025×0.32×0.025 m와 톱의 X=[0.24,0.32]·Y=[0.03,0.13]·깊이 0.030 m 손잡이이고 금속 끝에 면으로 접한다. 두 `bin`의 외곽은 각각 0.16×0.12×0.10 m다. 재질 id는 모양과 치수가 정해진 이 부재에만 붙으며 타공판 `board`는 1.20×1.00×0.02 m다.
