...
레퍼런스 04의 거실 밝은 소파와 03의 공용부 밝은 소파를 한 구조 원형의 두 배치로 채택한다. 보조 쿠션과 담요는 별도 소품 원형이다.
...
두 소파 배치 모두 벽에 닿는 외곽 뒤 기준선은 유지한다. 네 발 중 뒤쪽 두 발의 뒤 경계를 국소 Z=0.015 m로 옮기고 앞쪽 발은 그대로 둔다. Y=[0.08,0.10] m의 `base` 뒤 하단은 Z=[0,0.015] m를 파내 걸레받이 앞면에 맞댄다. 등받이와 팔걸이는 바닥 위 0.10 m보다 높은 구간만 뒤 벽에 닿고, 앞 가장자리와 2.10×0.95×0.90 m 외곽은 유지한다. 이 relief가 없던 평평한 뒤 아래판은 만들지 않는다.

국소 X 외곽은 [−1.05,1.05] m, Z 외곽은 [0,0.95] m다. 네 `leg`는 각각 단면 0.06×0.06 m·Y=[0,0.08] m이고 X=[−0.98,−0.92]·[0.92,0.98] m에 두 쌍으로 둔다. 뒤 다리의 깊이는 Z=[0.015,0.075] m, 앞 다리는 Z=[0.87,0.93] m다. `base`는 X=[−1.05,1.05]·Z=[0,0.95]·Y=[0.08,0.30] m에서 뒤 하단의 Z=[0,0.015]·Y=[0.08,0.10] m만 빼 닫는다. 두 `arm`의 국소 X는 [−1.05,−0.90]·[0.90,1.05] m, Z=[0,0.95] m, Y=[0.30,0.62] m다. `back`은 X=[−0.90,0.90]·Z=[0,0.20]·Y=[0.30,0.90] m다. 세 좌석 쿠션은 남은 X 폭 1.80 m의 세 구간과 Z=[0.20,0.95]·Y=[0.30,0.43] m를 차지한다. 그러므로 낮은 벽 접촉에서는 뒤 다리 0.015 m와 받침 홈이 걸레받이를 비우고, 팔걸이·등받이는 Y≥0.30 m에서만 뒤벽까지 닿는다.

세 `seat-cushion`은 팔걸이 사이 X 폭 1.80 m를 각 0.596 m와 두 틈 0.006 m로 나눈 닫힌 패브릭 부재이고, 각 부재의 깊이는 Z=[0.20,0.95] m·높이는 Y=[0.30,0.43] m다. 부재는 받침 위에서 면으로 닿으며 팔걸이 `arm`이나 등받이 `back`의 부피를 공유하지 않는다.
...
레퍼런스 04 벽난로 앞 안락의자를 채택한다. 의자 옆 작은 탁자는 별도 예약이 없어 이 원형에 숨겨 넣지 않는다.
...
부품은 다리 넷, 받침, 좌석 쿠션 하나, 등받이, 팔걸이 둘이다. 국소 외곽은 X=[−0.425,0.425]·Z=[0,0.85]·Y=[0,0.90] m다. 다리 `leg`는 단면 0.06×0.06 m 네 각기둥이고 중심 X=±0.365·Z=0.08/0.77 m, Y=[0,0.08] m다. 받침 `base`는 X=[−0.425,0.425]·Z=[0,0.85]·Y=[0.08,0.30] m 판이다. 뒤 `back`은 X=[−0.425,0.425]·Z=[0,0.18]·Y=[0.30,0.90] m, 두 팔걸이 `arm`은 X=[−0.425,−0.305]·[0.305,0.425]·Z=[0.18,0.80]·Y=[0.30,0.60] m다. `seat-cushion`은 X=[−0.305,0.305]·Z=[0.18,0.80]·Y=[0.30,0.43] m다. 이 판들은 지정한 경계면에서만 접하고 내부 체적은 공유하지 않는다. 좌석 폭 0.61 m는 한 사람용 예약을 채운다. 재질 경계는 `leg`, `base`, `seat-cushion`, `back`, `arm`이고 관절은 없다.
...
@evidence spaces/rooms/common.md#common-family-reservation 가족실 러그 예약의 X 폭 1.55 m·Z 길이 1.70 m를 원형 인자로 받고 위치는 부모와 instances에 맡긴다.
