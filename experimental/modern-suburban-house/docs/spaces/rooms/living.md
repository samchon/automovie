...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 본문이 living-room 경계, entry-living-door, 문 앞 가구 금지 바닥, 문짝 없는 공용부 개구부, 굴뚝 접면, 두 창 void, 관찰 질문 셋을 이 한 H2에 모두 두어 거실 평면 범위에 빈 owner가 없음을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 공용부 개구부를 common-room-plan, 벽난로 접면을 chimney-roof-interface, 두 창을 living-front-window·living-left-window 링크로만 받고 방 안에서 창 좌표를 다시 쓰지 않아 중복 저작이 없음을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-verification-address #a143ab1 문·창의 같은 경계 포함, 소파/벽난로의 앞뒤 통행 차단, 현관에서의 거실 가시성이라는 세 관찰 질문이 개구부·창 void 소비·직접 출입 주장을 각각 반증할 수 있고 순폭·binding은 unverified로 남겼음을 확인했다.
...
@evidenceReview settings/10-house.md#living #a70aad2 설정의 1층 전면 왼쪽, 현관 직접 가시, 01 전면 창의 거실 외벽 귀속, 왼쪽 외벽 굴뚝 조건을 본문의 ground-storey 배치, entry-living-door, living-front-window void, 굴뚝 접면 소비와 하나씩 대조했다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work #46f1b62 설정 living의 전면 왼쪽 위치·현관 직접 가시·후면 공용부 연결을 X = [-5.50, -1.95]·Z = [-6.05, -0.25] 경계와 앞쪽 문·뒤쪽 개구부에 대조해 설정 수정 없이 두 출입이 성립함을 확인했다.
-->
...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 설정의 소파·테이블·안락의자·책장은 네 행 표로, 러그는 X = [-4.00, -2.00]·두께 0.008 m 예약으로, 앉고 일어서는 사용과 벽난로 돌출 상한은 본문 문단으로 각각 owner가 있음을 확인했다.
...
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 네 가구 표가 평면·높이·방향 세 열을 빈칸 없이 채우고 소파 앞·의자 발·책장 앞 사용 범위와 선반 산출 규칙까지 있어, 다음 층이 거실 가구 배치를 새로 정할 필요가 없음을 확인했다.
@evidence principles/core/common.md#declared-basis 높이는 1층 완성 바닥 기준이며 수치는 제품 치수나 제작 완료가 아닌 최대 점유 입력이라고 밝힌다.
@evidenceReview principles/core/common.md#declared-basis #7ccd1cb 표의 world X/Z가 몸체·손잡이 최대 점유 입력이고 높이가 storey-datums의 1층 완성 바닥 기준이라 밝혀, 좌면 0.43 m·책장 1.90 m 같은 값이 제품 치수로 오인되지 않는 근거를 확인했다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation #0632226 설정은 가구 목록만 주지만 본문은 소파의 오른쪽 계단 분리벽 등대기와 -X 방향, 안락의자 +Z 방향, 책장 Z = [-5.90, -4.90]의 뒤쪽 배치를 더해 공간층 고유의 배치 결정을 추가했음을 확인했다.
@evidence principles/design/spaces.md#space-topology 모든 가구와 사용 영역을 living-room 안에 두고 의자 사용 발 점유를 책장 횡단 통로에 합산하지 않는다.
@evidenceReview principles/design/spaces.md#space-topology #3f8d925 표와 세 사용 범위 좌표가 모두 living-room의 X = [-5.50, -1.95]·Z = [-6.05, -0.25] 안에 들고, 의자 발 Z = [-4.80, -4.20]을 책장 횡단 통로에 합산하지 않는 방해 관계가 명시됨을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 벽난로 전체 돌출을 chimney-roof-interface의 기존 상한 링크로만 받고 이 H2가 돌출 치수를 다시 쓰지 않으며 바닥 hearth·목재 선반을 통로로 늘리지 않는다고 적어 owner가 하나임을 확인했다.
@evidence principles/design/spaces.md#space-verification-address 평면/단면에서 좌석·테이블·책장과 사용 점유를 겹쳐 읽고 후면까지의 동선을 함께 검사한다.
@evidenceReview principles/design/spaces.md#space-verification-address #a143ab1 좌석·테이블·책장과 사용 점유의 평면/단면 중첩 읽기, #living-through-route 동선 검사, spatial-observation-derivation 관찰 owner를 지목하고 충돌·착석 시야는 unverified로 둔 점을 확인했다.
@evidence settings/10-house.md#living 패브릭 소파·낮은 목재 테이블·작은 안락의자·짙은 책장·러그를 불이 꺼진 정적 상태의 공간 점유로 두고 소파가 왼쪽 벽난로를 향하게 한다.
@evidenceReview settings/10-house.md#living #a70aad2 설정의 패브릭 소파·낮은 목재 테이블·작은 안락의자·짙은 책장·러그를 표와 러그 평면에, '불은 꺼진 정적 상태'를 마지막 문단에 대응시키고 소파 -X 방향이 왼쪽 벽난로를 향함을 확인했다.
@evidence settings/00-production.md#use-profile 소파 앞 사용 영역에 사람 점유체의 깊이를 X, 폭을 Z로 적용한다.
@evidenceReview settings/00-production.md#use-profile #6ef5afe 소파 앞 X = [-3.45, -2.90]의 0.55 m가 use-profile 점유체 깊이 0.45 m를, Z = [-3.60, -1.80]의 1.80 m가 폭 0.60 m를 담아 깊이→X·폭→Z 적용이 수치로 성립함을 대조했다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work #46f1b62 설정 living의 가구 목록과 use-profile 점유체를 네 평면 예약과 세 사용 범위에 대조해 소파 앞 사용이 테이블 X = -3.45와 소파 X = -2.90 사이에 맞물려 방 안에서 설정 수정 없이 성립함을 확인했다.
-->
...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 현관 쪽 문→주 통행 띠→공용부 개구부 경로, 문 회전 중 대기, 옆 창·앞 창·소파·책장·독서 의자로의 분기가 본문 세 문단에 모두 서술되어 누락된 목적지가 없음을 확인했다.
...
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 주 통행 띠 X = [-4.90, -4.00]와 그 Z 범위(뒤쪽 안쪽 면~-1.45 m), 테이블 앞 Z = [-1.45, -0.45] 꺾임 바닥, 커튼 0.12 m 한계가 수치로 있어 경로를 다음 층이 새로 긋지 않아도 됨을 확인했다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation #0632226 설정의 '후면 공용부와 연결'에 없는 결정, 즉 벽난로 앞면과 테이블 사이 X = [-4.90, -4.00] 띠와 문 없는 경계에서 옆으로 이동해 냉장고·섬 사이로 잇는 경로를 본문이 더했음을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 뒤쪽 냉장고·섬 사이 진입은 common-clear-routes, 창대/손잡이는 06-openings의 external-opening-interface 링크로 받고 이 H2는 커튼 0.12 m 한계만 더해 중복 값이 없음을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-verification-address #a143ab1 문 없는 경계의 단일 점유체 모서리 회전과 90° 열린 문짝 두께/손잡이·전면 창대·커튼 사이 통과 폭을 반증 관찰로 명시하고 spatial-observation-derivation으로 넘기며 통행을 unverified로 둔 점을 확인했다.
...
@evidenceReview settings/00-production.md#use-profile #6ef5afe X = [-4.90, -4.00] 띠 폭이 use-profile의 연속 통로 0.90 m와 같고, 문 없는 경계에서는 폭을 더하지 않고 점유체 하나가 모서리를 도는 경로로 목표를 적용함을 대조했다.
@evidence settings/10-house.md#living 좌석이 문 접근을 막지 않도록 소파 착석 영역을 통과 경로로 세지 않는다.
@evidenceReview settings/10-house.md#living #a70aad2 설정의 '좌석이 문·계단 접근을 가로막지 않는다'를 소파 착석 영역을 통과 경로에서 빼고 띠를 소파 반대편 벽난로 앞면·테이블 사이에 둔 본문 배치와 대조해 성립함을 확인했다.
@evidence obligations/design/spaces.md#space-access-circulation 현관↔공용부의 연결을 거실 안의 구체 통행 띠로 배정하고 의자 사용·책장 횡단 중에도 그 띠를 유지한다.
@evidenceReview obligations/design/spaces.md#space-access-circulation #76c5e04 현관↔공용부 연결을 X = [-4.90, -4.00] 띠에 배정하고, 의자 발과 Z = [-4.65, -3.75] 책장 횡단 바닥의 순차 사용 중에도 왼쪽 띠가 유지된다는 본문을 과제 경로 요건과 대조했다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work #46f1b62 use-profile 0.90 m와 설정 living의 공용부 연결을 테이블 앞 Z = [-1.45, -0.45] 꺾임 바닥과 냉장고·섬 사이 후면 진입까지 이어 대조해 설정 수정 없이 경로가 성립함을 확인했다.
-->
