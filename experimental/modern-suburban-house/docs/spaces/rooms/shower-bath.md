...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 본문이 shower-bathroom 경계, 복도 벽과 세 공유 벽, hall-shower-door 하나, #shower-fixture-use 소비, 문턱에서 끝나는 카펫, 외벽 없는 곳의 창 금지를 한 H2에 모두 두었음을 확인했다.
...
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 마감 안쪽 X = [0.90, 3.07]·Z = [-8.80, -6.06] m, 앞쪽 벽 Z = [-6.06, -5.91]의 개구부 X = [1.05, 2.05]·Y = [3.06, 5.26] m와 0.90 m 목표가 있어 샤워 욕실 평면이 수치로 확정됨을 확인했다.
@evidence principles/core/common.md#declared-basis 예약 순내부 2.17 × 2.74 m가 설비 사용에 충분하다는 검증은 아직 없다고 밝힌다.
@evidenceReview principles/core/common.md#declared-basis #7ccd1cb 순내부 2.17 × 2.74 m가 X·Z 구간의 차와 일치하고, 이 예약이 설비 사용에 충분하다는 검증이 아직 없다고 본문이 미해결 입력으로 드러내 근거 상태가 명시됨을 확인했다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation #0632226 설정의 '상층 복도에서 직접 들어가는 첫 욕실'에 본문이 -X 문설주 경첩·실내 -Z 열림의 hall-shower-door와 네 면이 모두 내부 경계인 실이라는 부모에 없는 공간 결정을 더했음을 확인했다.
@evidence principles/design/spaces.md#space-topology 앞쪽 복도, 왼쪽 주침실, 오른쪽 욕조 욕실, 뒤쪽 옷방과 공유 벽으로 인접하고 주침실에 출입문을 추가하지 않는다.
@evidenceReview principles/design/spaces.md#space-topology #3f8d925 앞쪽 upper-hall-plan 복도, 왼쪽 주침실, 오른쪽 욕조 욕실, 뒤쪽 옷방 인접과 복도 쪽 단일 연결, '주침실에 출입문을 추가하지 않는다'는 차단이 본문에 있어 그래프가 복원됨을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 세 공유 벽 X = [0.75, 0.90]·X = [3.07, 3.22]·Z = [-8.95, -8.80]와 복도 벽을 경계로 명시하고 외벽이 없어 창을 넣지 않아, 샤워 욕실이 입면 창 owner를 새로 만들지 않음을 확인했다.
...
@evidenceReview settings/10-house.md#shower-bathroom #10c9aaa 설정의 '상층 복도에서 직접 들어가는 첫 욕실'과 '주침실을 통과해야 접근하는 전용 욕실로 바꾸지 않는다'를 복도 벽의 hall-shower-door와 주침실 쪽 문 금지에 대조했다.
...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 본문이 부스와 세 겹 미닫이, 부스 앞 대기, 변기·세면장 몸체와 앞 사용, 거울, 수전 1.05 m·헤드 2.05 m, 수건, 실문→세면대→변기 진입 순서를 모두 서술해 기구 사용에 빈 owner가 없음을 확인했다.
...
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 부스 X ≤ 2.15·Z ≤ -7.70, 유리 2.10 m, 대기 X = [0.95, 1.85]·Z = [-7.65, -7.05], 부스 앞 Z = -7.70과 세면장 Z = -6.80 사이 0.90 m, 변기·세면장 좌표가 있어 배치가 확정됨을 확인했다.
@evidence principles/core/common.md#declared-basis 외여닫이 유리문을 실문이나 오른쪽 기구 접근을 막을 수 있어 선택하지 않았다는 근거를 남기고 방수·배수 성능을 주장하지 않는다.
@evidenceReview principles/core/common.md#declared-basis #7ccd1cb 외여닫이 유리문을 실문 또는 오른쪽 기구 접근을 막을 수 있어 배제한 선택 근거와, 바닥/턱 0.02 m 예약이 방수·배수 성능 주장이 아니라는 구분이 본문에 있음을 확인했다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation #0632226 설정의 '샤워문과 실 출입문이 기구 사용 공간을 동시에 막지 않아야'에 본문이 오른쪽에 모이는 세 겹 미닫이와 90° 열린 실문이 들어오지 않는 대기라는 공간 결정을 더했음을 확인했다.
@evidence principles/design/spaces.md#space-topology 부스를 별도 방 id로 만들지 않고 실문에서 세면대·부스 앞을 거쳐 세면장 뒤로 돌아 변기에 닿는 순서를 정한다.
@evidenceReview principles/design/spaces.md#space-topology #3f8d925 샤워부스를 별도 방 id로 만들지 않고, 실문→세면대·부스 앞→세면장 뒤→부스 오른쪽→변기 순서와 샤워부스 비통과를 같은 shower-bathroom 안의 경로로 정했음을 확인했다.
@evidence principles/design/spaces.md#space-boundary-authority 수전/헤드 돌출은 미닫이 문 점유와 분리하고 수건은 세면장 앞 사용과 실문 회전에서 벗어나게 둔다.
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 수전 0.10 m·헤드 0.20 m 돌출을 뒤 벽 X = [1.00, 1.30]에서 미닫이 점유와 분리하고, 수건을 앞쪽 벽 X = [2.20, 2.45]에 둬 세면장 앞 X = [1.92, 2.52] 사용과 실문 회전에서 떼었음을 확인했다.
...
@evidenceReview settings/10-house.md#shower-bathroom #10c9aaa 설정의 유리 샤워부스·세면대·변기·거울·수건을 부스·세면장·변기 예약과 1.10–1.90 m 거울·수건 예약으로 대응시키고, 문 비충돌 조건을 미닫이와 실문 회전 밖 대기로 대조했다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work #46f1b62 설정 shower-bathroom 기구 목록과 use-profile을 뒤쪽 왼편 부스·뒤쪽 오른편 변기·앞쪽 오른편 세면장 좌표와 0.90 m 예약에 대조해 세 예약이 좌표상 겹치지 않아 설정 수정이 필요 없음을 확인했다.
-->
...
샤워 수전과 헤드는 부스의 뒤 벽 X = [1.00, 1.30] m 범위에 두고, 수전 높이는 상층 바닥 위 1.05 m, 헤드 높이는 2.05 m다. 벽에서 부스 안쪽으로의 최대 돌출은 수전 0.10 m·헤드 0.20 m이며 미닫이 문 점유와 분리한다. 수건은 실 앞쪽 벽 X = [2.20, 2.45] m, 높이 1.10–1.50 m, 돌출 0.08 m 이내에 두어 세면장 앞 사용과 실문 회전에서 벗어나게 한다. 수전/헤드·수건과 사용자의 실제 간섭은 부스 단면과 전신 점유로 다시 읽는다. 왼쪽 칸막이의 부스 안쪽 면에는 레퍼런스 05의 작은 병 벽감을 둔다. 벽감은 Z = [-8.55, -8.15] m, 상층 바닥 위 Y = [1.10, 1.50] m, 부스 안쪽 면 X = 0.90 m에서 벽 안으로 0.08 m 들어가 X = 0.82 m에 닫힌 뒤판을 둔다. 0.15 m 칸막이 중 뒤쪽 0.07 m는 그대로 남아 주침실과의 벽을 관통하지 않으며 새 개구부나 통로로 세지 않는다. 벽감의 뒤·옆·위·아래 타일 면은 이 공간 owner, 병의 표면은 models owner다. `src/spaces/rooms/shower-bath.ts`는 한 연결된 칸막이 메시에서 실제 벽감을 비운다. `auditHouseGeometry()`의 해당 part는 184 vertices(552 position scalars), 276 indices, 92 triangles, 연결 성분 1, 퇴화·열린·비다양체 모서리·비유한 수 0, 바깥 winding의 양수 부피 1.1143 m³, position 수에 맞는 normal·미터 UV 배열를 가졌다. 안정 part id는 `shower-primary-partition`, 전체 점유 범위는 X = [0.75, 0.90], Y = [3.06, 5.66], Z = [-8.95, -6.06] m이며 의도된 blind 벽감은 1개, 관통 opening은 0개다. 측정 producer는 `src/measurements/space-audit.ts`다. 병 원형의 실제 배치와 GPU 프레임은 아직 unverified다.
