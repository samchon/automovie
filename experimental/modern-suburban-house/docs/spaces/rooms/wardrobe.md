...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 본문이 primary-wardrobe 경계, 주침실 공유 벽의 primary-wardrobe-door 하나, 후면 0.55 m 수납 예약, 문 안쪽 X = [0.90, 2.10] 돌아설 공간, 전체 방 관찰 부담을 한 H2에 모두 두었음을 확인했다.
...
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 마감 안쪽·문 개구부·후면 수납 세 좌표 묶음과 0.90 m 목표가 있고, 수납 깊이 0.55 m가 Z = [-10.45, -9.90]의 차와 맞아 옷방 평면이 다음 층의 추정 없이 확정됨을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 뒤/오른쪽 외벽을 main-building-extent, 후면 창 없음을 envelope/rear.md의 rear-openings 링크로 소비해 이 H2가 외벽이나 창 배치를 다시 저작하지 않음을 확인했다.
@evidence principles/design/spaces.md#space-verification-address 자기 실 안 threshold/네 모서리/중심 네 방향, 끝 선반 접근, 주침실 쪽 문 회전을 검사한다.
@evidenceReview principles/design/spaces.md#space-verification-address #a143ab1 자기 실 안 threshold·네 모서리·중심 네 방향, 끝 선반 접근, 주침실 쪽 문 회전·조명을 0.95 m 산술과 단일 문 주장을 반증할 검사로 두고 unverified로 남겼음을 확인했다.
...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 본문이 X = 4.25까지의 옷걸이 구간, X = 4.40부터의 접은 옷/신발 선반, 봉과 2.05 m 상단 선반, 오른쪽 네 선반, 앞쪽 통로로 양 끝에 닿아 돌아오는 사용을 모두 서술했음을 확인했다.
...
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 걸기 X ≤ 4.25와 접기 X ≥ 4.40 사이 0.15 m 경계, 봉 1.65 m 위 상단 선반 2.05 m, 네 선반 산출 규칙이 모두 있어 옷방 수납 내부를 다음 층이 새로 정하지 않아도 됨을 확인했다.
@evidence principles/core/common.md#declared-basis 수납 평면은 primary-wardrobe-plan에서 받고 목재 선반·봉·의류·조명은 후속 구현이라고 밝힌다.
@evidenceReview principles/core/common.md#declared-basis #7ccd1cb 수납 평면을 #primary-wardrobe-plan 후면 예약에서, 높이를 상층 완성 바닥에서 받고 목재/밝은 선반·봉·의류·조명은 후속 구현이라 밝혀 근거와 미정 부분이 구분됨을 확인했다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation #0632226 storage 설정의 '옷걸이/선반과 실제 내부 깊이'에 본문이 X = 4.25/4.40 기준 걸기·접기 구간 분할과 네 선반 높이 반복 규칙이라는 공간 결정을 더했음을 확인했다.
@evidence principles/design/spaces.md#space-topology 문 안쪽 회전 바닥에서 앞쪽 통로를 따라 양 끝 수납에 닿고 같은 문으로 돌아온다.
@evidenceReview principles/design/spaces.md#space-topology #3f8d925 문 안쪽 회전 바닥 → 앞쪽 통로 → 양 끝 수납 → 같은 문 귀환 경로가 같은 primary-wardrobe 안에서 닫히고 바닥 바구니·서랍이 통로로 나오지 않는 관계를 본문에서 확인했다.
@evidence principles/design/spaces.md#space-boundary-authority 옷·옷걸이·용기의 깊이를 기존 수납 예약 안에 두고 넘치면 옷을 잘라 숨기지 않고 공간 배치를 고친다.
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 옷·옷걸이·용기 깊이를 #primary-wardrobe-plan의 0.55 m 예약 안에 두고 이 H2는 그 깊이를 다시 쓰지 않으며, 넘치면 옷을 잘라 숨기지 않고 공간 배치를 고친다는 원칙을 확인했다.
@evidence principles/design/spaces.md#space-verification-address 끝 수납 앞 사용체, 문/통로 단면, 전체 내부 관찰을 검사한다.
@evidenceReview principles/design/spaces.md#space-verification-address #a143ab1 끝 수납 앞 사용체, 문/통로 단면, 전체 내부 관찰을 spatial-observation-derivation에 넘겨 구간 분할과 깊이 예약 주장을 반증 가능하게 하고 geometry·순폭은 unverified로 둠을 확인했다.
...
@evidenceReview settings/10-house.md#storage #cc3fdd3 storage 설정의 옷걸이/선반·실제 내부 깊이를 봉 후면 0.28 m·1.65 m, X = 4.40부터의 선반, 0.55 m 예약 안의 옷·용기 깊이 규칙과 대조해 수납 정체성이 공간 입력으로 있음을 확인했다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work #46f1b62 설정 storage의 옷걸이/선반·내부 깊이 조건을 X ≤ 4.25 걸기·X ≥ 4.40 접기 구간과 바닥 바구니·서랍을 통로로 꺼내지 않는 규칙에 대조해 기존 후면 예약만으로 설정 수정 없이 성립함을 확인했다.
-->
