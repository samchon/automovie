...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 X = [-5.50, 5.50]·Z = [-10.45, -6.20] 외곽, 문짝 없는 두 개구부와 X = [3.22, 5.50] 닫힌 벽, 왼쪽 주방·중앙 식사·오른쪽 가족실, 네 외벽 void 바인딩, common.ts owner가 적혀 누락이 없음을 확인했다.
...
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 마감 안쪽 X = [-5.50, 5.50]·Z = [-10.45, -6.20], 앞 경계 Z = [-6.20, -6.05], 두 개구부 X = [-5.00, -2.15]·[-1.35, 3.07]과 Y = [0, 2.40]이 정해져 다음 층이 경계·개구부를 발명하지 않아도 됨을 확인했다.
@evidence principles/core/common.md#declared-basis 뒤·좌·우는 본채 외곽, 오른쪽 앞부분은 차고 접합에서 받고 세 기능의 위치는 03의 관계를 채택한 설정에서 받는다.
@evidenceReview principles/core/common.md#declared-basis #7ccd1cb 뒤·좌·우 변은 main-building-extent, 오른쪽 앞부분은 attached-garage-extent 링크, 세 기능 위치는 '설정이 03에서 채택한 관계' 링크에 근거가 붙어 각 배치 주장의 출처를 지목할 수 있음을 확인했다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation #0632226 설정 common-room의 '하나의 연속 공간'에 대해 문짝 없는 living-common-opening·service-common-opening과 팬트리 뒤 X = [3.22, 5.50] 닫힌 벽 구간이라는 앞 경계 결정이 부모에 없이 더해짐을 대조했다.
@evidence principles/design/spaces.md#space-topology 공용부 앞쪽이 거실·서비스 접근·팬트리와 만나고 두 개구부로 거실·서비스 접근에 직접 연결되며 정원문으로 외부에 연결된다.
@evidenceReview principles/design/spaces.md#space-topology #3f8d925 앞 Z = [-6.20, -6.05] 경계가 거실·서비스 접근·팬트리와 인접하고 두 개구부가 living-plan·service-access-plan을 직접 잇고 garden-door가 같은 방 외벽에서 외부로 이어지는 place graph를 확인했다.
@evidence principles/design/spaces.md#space-boundary-authority 네 창/문 void는 각 입면 owner에서 소비하고 오른쪽 앞부분을 노출 외벽으로 중복 처리하지 않는다.
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 네 외벽 void를 rear/right 입면 링크로 소비하고 차고 접합 앞부분을 노출 외벽으로 중복 처리하지 않으며 넓은 개구부의 상인방은 interior-boundary-ownership 배정 안에서 푼다는 점을 확인했다.
...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 표 일곱 행의 두 하부장 띠·냉장고·레인지와 오븐·전자레인지·두 상부장, 냉장고/오븐 문 +X 0.55 m 작동, 열린 기기 앞 두 작업 점유가 각각 적혀 누락 설비가 없음을 확인했다.
...
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 일곱 점유 owner의 평면·높이·방향, 상판 0.91 m, 냉장고 앞 X = [-4.15, -3.70]·오븐 앞 X = [-4.30, -3.70] 작업 점유가 수치로 닫혀 models가 벽 주방 배치를 새로 정할 필요가 없음을 확인했다.
@evidence principles/core/common.md#declared-basis 높이는 1층 완성 바닥 기준, 서벽·후벽은 공용부 안쪽 경계를 참조하고 수치는 제품 규격 인용이 아니라고 밝힌다.
@evidenceReview principles/core/common.md#declared-basis #7ccd1cb 높이를 storey-datums의 1층 완성 바닥, 서벽·후벽을 #common-room-plan 안쪽 면에 두고 수치가 제품 규격 인용이 아닌 공간 예약이라 밝혀 표의 각 값에 근거가 있음을 확인했다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation #0632226 설정 kitchen-equipment 목록에 대해 레인지 Z = [-9.50, -8.70]를 후면 창 아래가 아닌 왼쪽 벽에 두고 전자레인지 자리에 상부장을 겹치지 않는 배치 결정이 더해짐을 대조했다.
@evidence principles/design/spaces.md#space-topology 주방을 kitchen-dining-family 내부의 기능 구역으로 두고 별도 room이나 칸막이를 만들지 않는다.
@evidenceReview principles/design/spaces.md#space-topology #3f8d925 주방을 kitchen-dining-family/ground-storey 내부 기능 구역으로 두고 별도 room·칸막이 없이 섬과의 사이를 작업 통로로 잇는 포함·연결 관계가 메시 없이 읽힘을 확인했다.
@evidence principles/design/spaces.md#space-boundary-authority 뒤쪽 상판은 주방 창의 높이 상한을 소비하고 L형 두 띠의 교차 코너 몸체를 복제하지 않는다.
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 뒤쪽 상판이 kitchen-rear-window 높이 상한을 링크로 소비하고 L형 두 띠를 합집합으로 읽어 교차 코너 몸체를 복제하지 않으며 서벽·후벽은 plan 경계를 참조함을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-verification-address #a143ab1 기기 형상·열림 범위·점유 충돌·창/상부장 단면·03 읽힘과 문/서랍을 닫은 거실 쪽 진입을 벽 주방 예약의 반증 관찰로 두고 unverified로 남김을 확인했다.
@evidence settings/10-house.md#kitchen-equipment 왼쪽/후면 L형 하부장·두 상부장과 냉장고·레인지와 오븐·전자레인지를 벽 주방에 배정하고 두 번째 수도꼭지를 창 앞에 넣지 않는다.
@evidenceReview settings/10-house.md#kitchen-equipment #39c68f2 설정의 냉장고·레인지·전자레인지, 후면 보조 작업면, 두 수도꼭지 비필수 조건을 L형 하부장·두 상부장·세 기기 예약과 창 앞 두 번째 수도꼭지 금지에 대조해 성립함을 확인했다.
@evidence settings/00-production.md#use-profile 열린 냉장고/오븐 앞 작업에 사람 점유체의 깊이를 X, 폭을 Z로 적용한다.
@evidenceReview settings/00-production.md#use-profile #6ef5afe 냉장고 앞 X = [-4.15, -3.70]의 0.45 m와 오븐 앞 X = [-4.30, -3.70]의 0.60 m를 점유체 깊이 0.45 m에, 두 Z 구간 0.95·0.80 m를 폭 0.60 m에 대조해 작업 자세가 줄지 않음을 확인했다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work #46f1b62 설정 kitchen-equipment의 기기 목록·두 수도꼭지 비필수와 use-profile 점유체를 L형 벽 주방에 적용해 열린 냉장고/오븐 작업이 #common-clear-routes 별도 경로와 갈라져 부모 수정이 필요 없음을 확인했다.
-->
...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 섬 몸체의 서쪽 작업면·동쪽 좌석면, 싱크와 상판 위 0.40 m 이내 수도꼭지, 식기세척기와 열린 문 앞 작업, 세 스툴 사용 점유가 각각 적혀 섬 범위의 누락 항목이 없음을 확인했다.
...
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 섬 X = [-3.65, -2.60]·Z = [-8.70, -6.45], 싱크 X = [-3.55, -3.05], 식기세척기 Z = [-8.05, -7.45], 스툴 Z 중심 -8.30 m·0.70 m 간격 규칙이 정해져 다음 층이 섬 배치를 새로 정할 필요가 없음을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 식기세척기 앞 작업 Z = [-8.10, -7.40]이 벽 주방 오븐 앞 Z = [-9.50, -8.70]과 겹치지 않음을 대조하고 스툴 뒤 여유를 주 동선 폭에 더하지 않아 두 owner 구간을 중복 계상하지 않음을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-verification-address #a143ab1 개수·pitch에서 파생한 실제 점유와 가장 좁은 곳, 싱크/식기세척기와 맞은편 기기, 03의 싱크 섬·세 좌석 읽힘을 반증 관찰로 두고 서쪽 통로·뒤쪽 회전을 경로 예약과 대조하게 함을 확인했다.
...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 상판 X = [-0.35, 1.35]·Z = [-8.40, -7.50], 긴 변 네 좌석과 양 끝 두 좌석 규칙, 좌석 사용 점유, garden-door 대기와의 분리, 식탁 오른쪽·뒤쪽 주 동선이 각각 적혀 누락이 없음을 확인했다.
...
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 상판 높이 0.75 m, 긴 변 좌석 X 중심 0 m부터 1.00 m 간격, 양 끝 좌석 Z 양옆 0.325 m·바깥 0.75 m가 수치로 정해져 다음 층이 여섯 좌석 위치를 새로 배치할 필요가 없음을 확인했다.
@evidence principles/core/common.md#declared-basis 식탁 치수는 여섯 좌석 요구와 뒤쪽 문 대기를 함께 담기 위한 저작 선택이라고 밝힌다.
@evidenceReview principles/core/common.md#declared-basis #7ccd1cb 식탁 치수를 여섯 좌석 요구와 뒤쪽 문 대기를 함께 담는 저작 선택으로 밝히고 대기는 rear.md#garden-door, 주 동선은 #common-clear-routes에서 받아 근거가 드러남을 확인했다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation #0632226 설정의 가족·손님 6개 좌석에 대해 긴 변 네 좌석과 양 끝 두 좌석이라는 두 반복 규칙, 합집합 검사와 불투명 상자 금지라는 생성 결정이 더해짐을 대조했다.
@evidence principles/design/spaces.md#space-topology 식사 구역을 공용부 중앙에 두고 뒤쪽 좌석 끝과 garden-door 안쪽 대기를 분리한다.
@evidenceReview principles/design/spaces.md#space-topology #3f8d925 식사 구역을 공용부 중앙에 두고 뒤쪽 좌석 끝과 garden-door 안쪽 대기를 분리하며 주 동선이 식탁 오른쪽·뒤쪽을 지나는 인접·장애 관계가 메시 없이 읽힘을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 garden-door 안쪽 대기는 rear.md 링크, 주 동선은 #common-clear-routes 링크로 받아 재정의하지 않고 좌석 점유 합집합을 축정렬 bbox와 구별해 검사하게 함을 확인했다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work #46f1b62 설정 common-room의 6개 좌석·정원 출입 근접과 use-profile 폭을 대조해 상판 Z 끝 -8.40에서 0.75 m 바깥까지의 뒤쪽 좌석이 garden-door 대기 앞에서 끝나 설정 수정 없이 성립함을 확인했다.
-->
...
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 소파 뒤쪽 면을 #common-room-plan이 닫은 팬트리 경계에 대고 별도 벽을 세우지 않으며 family-rear-window·family-right-window를 rear/right 입면 링크로 소비함을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-topology #3f8d925 같은 kitchen-dining-family 안에서 벽/문/새 복도 없이 오른쪽 띠와 뒤쪽 띠가 Z = -9.25에서 꺾여 garden-door 대기에 닿고 후면 띠가 뒤쪽 띠에 합류하는 경로 그래프를 확인했다.
@evidence principles/design/spaces.md#space-boundary-authority 대기를 경로 위에 별도 바닥으로 겹치지 않고 열린 기기 앞 작업 공간을 통과 폭으로 중복 계상하지 않는다.
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 garden-door 대기를 경로 위 별도 바닥으로 겹치지 않고 열린 기기 앞 작업을 벽 주방 H2의 사용 자세로 남겨 통과 폭에 중복 계상하지 않으며 냉장고 몸체 외곽에서 순폭을 읽음을 확인했다.
