...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 attached-garage-extent 소비, 서쪽 경계의 머드룸 문, garage-front-door·garage-right-window, 차도까지 이어진 문턱, 천장 폐합, 낮은 바닥 바탕, 차량 배제와 garage-interior.ts owner가 모두 적혀 누락이 없음을 확인했다.
...
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 차량 점유를 비우고 선반·공구·구조를 #garage-storage-use와 #garage-use-routes에 배정하며 문 안쪽 무턱, 천장 임의 높임 금지, 문을 닫아도 머드룸으로 닿는 관찰까지 정해져 있음을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 두 번째 차고 상자나 외벽을 만들지 않고 외곽을 attached-garage-extent에서 소비하며 머드룸의 높은 문턱/챌면을 laundry owner 한 번 생성으로 두어 이 방에 단·판을 포개지 않음을 확인했다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work #46f1b62 설정 garage의 차량 금지·콘크리트 바닥·천장 요구와 delivery-scope 자동차 제외를 garage-ground-floor-base·garage-ceiling-closure 소비와 빈 바닥 배정에 대조해 부모 수정이 필요 없음을 확인했다.
-->
...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 후벽 선반, 공구 작업대와 서랍, 공구판, 선반·서랍 앞 사람 작업 바닥, 상부 문 이동 예약 뒤 배치와 측면 창 비가림이 각각 적혀 수납 범위에 누락이 없음을 확인했다.
...
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 선반 X = [7.15, 8.85]·Z -5.85 m까지·높이 2.05 m와 0.20 m부터 0.40 m 간격 다섯 선반, 작업대 X = [9.00, 10.20]·상면 0.90 m, 서랍 +Z 0.45 m, 공구판 1.10–2.10 m가 정해져 있음을 확인했다.
@evidence principles/core/common.md#declared-basis 높이는 본채 바닥이 아닌 차고 완성 바닥 기준이며 공구 종류·수·원형은 후속 저작이라고 밝힌다.
@evidenceReview principles/core/common.md#declared-basis #7ccd1cb 높이를 본채 바닥이 아닌 ground-threshold-datums의 차고 완성 바닥 기준으로 밝히고 공구 종류·수·원형은 후속 저작, 기계·전기 성능은 주장하지 않는다고 적어 근거 범위가 드러남을 확인했다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation #0632226 설정 garage의 선반·공구 수납에 대해 선반 X = [7.15, 8.85]·작업대 X = [9.00, 10.20]을 후벽에 모아 중앙 바닥과 머드룸 대기를 비우고 작업 바닥 Z = [-5.85, -4.80]를 두는 배치 결정을 대조했다.
@evidence principles/design/spaces.md#space-topology 수납을 garage 후벽에 두고 오른쪽 창을 가로지르는 높은 옆벽 수납을 두지 않는다.
@evidenceReview principles/design/spaces.md#space-topology #3f8d925 수납을 같은 garage/ground-storey의 후벽에 붙이고 오른쪽 창을 가로지르는 높은 옆벽 수납을 두지 않아 창 접근이 내부 경로로 이어지는 장애·접근 관계를 확인했다.
@evidence principles/design/spaces.md#space-boundary-authority 후벽 안쪽 면과 차고 바닥 datum은 원래 owner에서 받고 상부 문 이동 예약 뒤에 수납을 둔다.
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 선반 Z 시작을 attached-garage-extent의 후벽 안쪽 면, 높이를 차고 바닥 datum에서 받고 선반/작업대를 garage-front-opening 상부 문 이동 예약 뒤에 두어 두 owner 값을 재측정하지 않음을 확인했다.
...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 머드룸 하부 대기에서 선반·작업대·측면 창·닫힌 전면문 안쪽으로 가는 서쪽 세로·가운데 가로·오른쪽 세로 경로, 열린 문으로 차도에 나가는 경로, 추가 관찰이 적혀 누락이 없음을 확인했다.
...
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 서쪽 세로 X = [5.95, 6.95]·Z 수납 전면~-0.95 m, 가운데 가로 Z = [-4.75, -3.85], 오른쪽 세로 X = 10.35 m부터 안쪽 한계까지가 정해져 다음 층이 차고 경로를 발명할 필요가 없음을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 문 레일 점유를 garage-front-opening owner가 정한 실점유 값으로 받아 경로에서 빼고 이 H2가 레일 치수를 다시 적지 않음을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-verification-address #a143ab1 수납·서랍·작업자와 바구니 이동 평면, 차고 바닥 기준 수납/창/천장 단면, 닫힌/열린 패널과 레일, 머드룸 한 단 왕복을 spatial-observation-derivation에 더하고 결과를 unverified로 둔 것을 확인했다.
@evidence settings/00-production.md#use-profile 기존 사용체로 바구니 회전과 양방향 경로를 검사한다.
@evidenceReview settings/00-production.md#use-profile #6ef5afe 서쪽 세로 경로 X = [5.95, 6.95]의 1.00 m와 가로 경로 Z = [-4.75, -3.85]의 0.90 m를 use-profile 0.90 m 통로 하한에 대조하고 바구니 회전·양방향 왕복은 관찰 항목으로 둠을 확인했다.
@evidence settings/00-production.md#operative-subjects 가족을 사용 조건일 뿐 인물로 모델링하지 않고 차량을 전달 밖에 두므로 이 경로 검사는 캐릭터/차량 저작이 아니다.
@evidenceReview settings/00-production.md#operative-subjects #3380939 operative-subjects의 가족 사용 조건·차량 전달 밖 분류를 이 H2가 경로 검사를 캐릭터/차량 저작이 아닌 기존 사용체 통행 검사로 한정한 문장에 대조해 확인했다.
@evidence obligations/design/spaces.md#space-access-circulation 머드룸에서 차고의 수납·창·전면문까지 두 문 상태 모두의 입출입 경로를 배정한다.
@evidenceReview obligations/design/spaces.md#space-access-circulation #76c5e04 머드룸 차고 쪽 대기에서 수납·측면 창·닫힌 전면문 안쪽으로 가는 경로와 열린 전면문으로 차도에 나가는 경로가 두 문 상태 모두 배정되고 레일 실점유를 뺀 것을 연결·경로 요구에 대조했다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work #46f1b62 설정 garage의 닫힌 기준 상태·머드룸 관찰과 use-profile을 대조해 선반 앞 작업 Z = [-5.85, -4.80]과 겹치지 않는 가로 경로 Z = [-4.75, -3.85]가 나와 부모 수정이 필요 없음을 확인했다.
-->
