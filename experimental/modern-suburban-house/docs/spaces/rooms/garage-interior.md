...
@evidence settings/10-house.md#garage 선반·공구 수납을 다른 방과 같은 강도로 완성할 공간 예약을 둔다.
...
@evidence principles/core/inherited-units.md#derived-parent-differentiation 문을 닫은 기준 상태에서도 차고 내부 전체에 도달할 경로를 머드룸 기점으로 배정하고 기둥/수직 레일 관통을 금지한다.
@evidence principles/design/spaces.md#space-topology 세 경로와 문 앞 대기의 겹침을 연결로 넘기고 새 room이나 숨은 통로를 만들지 않는다.
