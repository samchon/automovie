...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 본문이 뒤쪽 본체·왼쪽 부분 합집합, hall-primary-door, 옷방 문의 별도 개구부 소비, 두 창 void, #primary-furniture-use 가구 배정, 욕실 필수 통과실 금지를 한 H2에 모두 두었음을 확인했다.
...
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 뒤쪽 본체와 왼쪽 부분 두 직사각형이 Z = -6.06에서 맞붙고, 복도 벽 개구부 X = [-2.70, -1.70]의 1.00 m가 0.90 m 목표를 담아 주침실 외곽과 출입 위치가 수치로 확정됨을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 옷방 문은 primary-wardrobe-plan의 별도 개구부, 두 창은 rear·left 입면 void 링크로 소비하고 좌표를 직접 쓰는 개구부는 hall-primary-door 하나뿐이어서 경계 값의 중복 저작이 없음을 확인했다.
...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 본문 표의 침대·뒤쪽/앞쪽 협탁과 등·낮은 서랍장, 발치 X = [-2.50, -1.50] 띠, 옷방 문 앞 대기, 두 창 커튼 0.12 m가 모두 서술되어 주침실 가구 사용 범위에 빈 owner가 없음을 확인했다.
...
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 침대 X = [-1.50, 0.65]·Z = [-8.65, -7.05]와 머리 +X, 두 협탁 Z 범위, 서랍장 Z = [-6.50, -5.10]와 서랍 0.40 m, 발치 띠와 서랍장 앞 X = [-4.60, -4.00]가 있어 배치가 수치로 확정됨을 확인했다.
@evidence principles/core/common.md#declared-basis 침대 머리를 창 없는 욕실 공유 벽 쪽에 둔 근거를 두 외벽 창과 옷방 문을 가리지 않기 위한 선택으로 밝힌다.
@evidenceReview principles/core/common.md#declared-basis #7ccd1cb 침대 머리를 오른쪽 창 없는 욕실 공유 벽에 둔 이유를 두 외벽 창과 옷방 문을 머리판으로 가리지 않기 위한 공간층 선택으로 밝히고 높이 기준을 storey-datums 상층 완성 바닥으로 명시했음을 확인했다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation #0632226 설정의 '침대만 키우는 대신 양옆과 발치의 사용 여유 및 수납'에 본문이 발치 X = [-2.50, -1.50] 띠에서 앞쪽·뒤쪽 침대 옆과 옷방으로 분기하는 배치 결정을 더했음을 확인했다.
@evidence principles/design/spaces.md#space-topology 모든 가구와 사용을 primary-bedroom 안에 두고 넓은 왼쪽 부분을 통과실이나 두 번째 침실로 나누지 않는다.
@evidenceReview principles/design/spaces.md#space-topology #3f8d925 표의 서랍장 Z = [-6.50, -5.10]가 뒤쪽 본체와 왼쪽 부분에 걸쳐 방 안에 들고, 서랍장 앞 작업을 왼쪽 부분에서 접근시키며 그 부분을 통과실·두 번째 침실로 나누지 않는 관계를 확인했다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 옷방 문의 경첩·열림·돌아설 공간은 primary-wardrobe-plan에서 받고, 반경 1.00 m 회전 상한 X = [-0.25, 0.75]에 0.30 m를 더한 대기만 이 H2가 비우며 창대/손잡이는 06 링크로 받음을 확인했다.
...
두 창의 커튼은 [후면 개구부](../envelope/rear.md#primary-rear-window)와 [왼쪽 개구부](../envelope/left.md#primary-left-window)를 host로 삼는 별도 예약이다. 각 예약의 가로 폭은 host 거친 폭 W에 양끝 0.10 m씩 더한 W + 0.20 m, 높이는 개구부 아래변보다 0.75 m 낮은 곳부터 위변보다 0.12 m 높은 곳까지, 깊이는 실내 마감 벽면에서 방 안쪽 0.12 m까지다. 이 띠 안에 봉·받침·천·손잡이 간섭을 모두 담고 [창대/손잡이](../06-openings.md#external-opening-interface)와 함께 접근 여유에서 뺀다. 넓은 왼쪽 부분을 통과실이나 두 번째 침실로 나누지 않는다. 회베이지 침구와 가구/등·커튼의 실제 원형·반복 배치·재료는 후속 저작이고 완결 면은 기존 room owner가 유지한다. 문 개방/침대 양옆·발치/두 창/열린 서랍/옷방 진입의 평면·단면과 02·05 및 방 전체 시야를 [관찰 owner](../04-observations.md#spatial-observation-derivation)가 검사하며 실제 점유·회전·프레임은 unverified다.
