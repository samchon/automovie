...
전면은 레퍼런스 01처럼 열린 포치로 택하고 외부 난간은 두지 않는다. 이는 [실내 계단 난간](02-stair.md#stair-clearance)을 생략하는 선택이 아니다. 낮은 화분·발판은 진입 폭 밖에 놓는다. 대지 보행길과의 정확한 연결/지반 마감은 미완료다. `src/spaces/porch.ts`가 바닥·챌판·디딤·접근을 소유한다. 현관문/세 단/아래 대기의 축, 위에서 아래로 돌아 나오는 길, 포치 가장자리와 접지·그림자는 unverified다.
...
이 지붕은 거실창과 현관문을 함께 덮고 차고 정면을 넘지 않는다. 지붕 벽 접합은 아직 정하지 않은 상층 창 아래에 있어야 하며, 출입문 head와 포치 보 아래의 높이는 같은 단면에서 검사한다. 순높이의 최저 구조점은 보/머리/받침을 포함하여 다시 읽는다. 폭이 긴 지붕을 두께 없는 판으로 대체하지 않는다. 구조 단면의 용량·재료 적층과 접합 상세는 후속 단계의 미완료다.
