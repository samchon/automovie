...
천장 바탕은 안쪽 벽 면에서 끝나고 외벽 두께 구역은 입면/공유 벽 owner가 지붕 아래면까지 닫는다. 벽에 받치는 단부는 그 벽과 하나의 접촉 경계를 공유하며 두 owner가 같은 접합 몸체를 포개지 않는다. 지붕 단차·박공 교차·굴뚝 주변의 실제 부재는 각 지붕/입면 owner를 소비하고, 천장 예약 바깥으로 내려오는 부재가 생기면 단면과 해당 실의 순높이를 다시 검사한다. 지지 부재의 의도된 접합은 이 평탄 예약 사이의 분리 여유와 구별해 명시하며 관통/겹침을 임의 접합으로 인정하지 않는다.
