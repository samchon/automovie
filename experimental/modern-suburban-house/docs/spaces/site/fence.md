...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 본문이 F·L·R·B 네 중심선, 다섯 행 구간 표, 공유 끝점 꺾임, 본채 왼쪽 벽·차고 오른쪽 앞 모서리 접점, 왼쪽 앞 구간 뒤 숨은 틈 금지, fence.ts owner를 모두 두는지 대조해 누락이 없음을 확인했다.
...
@evidenceReview settings/10-house.md#site-identity #452f15e site-identity의 우측 목재 울타리를 오른쪽 앞 닫힘·오른쪽 긴 면·뒤쪽 긴 면 등 다섯 구간 표와 '앞마당 전체를 횡단하는 울타리를 덧붙이지 않는다'에 대조해 정원 쪽 경계로 이어짐을 확인했다.
@evidence obligations/design/spaces.md#space-access-circulation 정원의 닫힌 경계를 울타리와 건물 외피로 배정하고 관리문 외의 숨은 통행 틈을 남기지 않는다.
@evidenceReview obligations/design/spaces.md#space-access-circulation #76c5e04 정원 외부 경로를 기존 관리문 하나로 두고 왼쪽 앞 구간 뒤 숨은 틈을 금하며 패널 밑 시공 간격을 통행 개구부로 세지 않는 본문을 대조해 닫힌 경계와 유일한 출입이 배정됨을 확인했다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work #46f1b62 site-identity 우측 울타리와 maps 소유 경계를 굴뚝·본채·관리길 외곽에서 도출한 L·R·B와 map-handoff-inputs 수용 확인에 대조해 설정 수정 없이 울타리 선이 정해짐을 확인했다.
-->
...
완결 울타리의 양면·끝·모서리·문 개구부·기둥 owner는 `src/spaces/site/fence.ts`, 움직이는 문짝·철물 원형 owner는 [옆마당 대문](../../models/02-exterior-doors.md#side-yard-gate)이며 담당은 이 production의 단일 저작자다. 같은 선의 패널·반복 판재·마감은 후속 단계에서도 이 완결 면 책임을 유지한다. 이 문서는 source 예약이며 실제 길이·부재 수·닫힌 경계·필지 포함을 측정한 결과가 아니다. 전체 평면과 양쪽 벽 접점, 모든 꺾임의 안팎 시야를 [전체 관찰](../04-observations.md#spatial-observation-derivation)에 추가하고 실제 폐합은 unverified로 둔다.
...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 본문이 side-yard-gate X 구간 비우기, 0.12 m 문기둥, 두 짧은 잔여 패널, 양 건물 접점 끝기둥·막음 부재, 닫힘/열림 별도 검사를 각각 문단으로 두는지 대조해 접합 범위에 누락이 없음을 확인했다.
...
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 문기둥 예약 폭 0.12 m를 개구부 안쪽 경계 바깥에 두고 좌우 안쪽 면을 개구부 양 끝에 맞추며 지상 받침도 X 구간 밖에 두는 본문을 대조해 문기둥 배치가 다음 층 추정 없이 정해짐을 확인했다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation #0632226 fence-enclosure-plan의 오른쪽 앞 구간에 없는 결정, 즉 패널 연속 생성 후 문짝 부착을 배제하고 개구부 X 구간을 먼저 비우는 접합 방식을 본문이 더하는지 대조해 부모 반복이 아님을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 끝기둥/기초 중심을 벽 밖으로 물리고 외벽을 두 번째 기둥으로 두껍게 하지 않으며 left-openings의 창·굴뚝을 지지체로 쓰지 않는 본문을 대조해 건물 외피 값이 울타리에서 재저작되지 않음을 확인했다.
@evidence principles/design/spaces.md#space-verification-address 닫힌 문에서 패널/문짝 경계의 연속과 열린 문의 앞뒤 통행·대기·회전을 별도로 검사한다.
@evidenceReview principles/design/spaces.md#space-verification-address #a143ab1 닫힌 문에서 패널/문짝 경계 연속과 열린 문에서 side-gate-interface 두 구역 통행·대기·회전을 따로 검사하고 걸쇠·회전·바구니 순폭을 unverified로 둔 본문을 대조해 반증 지점을 확인했다.
@evidence settings/10-house.md#site-identity 우측 목재 울타리가 관리문까지 실제로 닫히게 한다.
@evidenceReview settings/10-house.md#site-identity #452f15e site-identity 우측 목재 울타리를 두 잔여 패널이 문기둥까지 닫고 끝기둥·막음 부재가 벽 외피에 닿는 본문 배치에 대조해 울타리가 관리문 양쪽에서 끊김 없이 닫히도록 예약됨을 확인했다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work #46f1b62 site-identity 우측 목재 울타리를 side-yard-gate 개구부 비우기, 0.12 m 문기둥, 두 잔여 패널 배치에 대조해 설정 변경 없이 울타리가 문까지 닫혀 부모 수정이 없음을 확인했다.
-->
...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 본문이 패널 상단·g + 0.05 m 아래끝, 문 앞뒤 S 접촉, 일반 기둥 0.12 m·패널 ±0.05 m·기초 0.24 m 점유, 관목·나무와의 관계를 각각 두는지 대조해 높이·점유 범위에 누락이 없음을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-topology #3f8d925 문 앞뒤 보행면·문기둥 접촉을 관리길 상면 S에 맞추고 일반 기초는 포장에 침투하지 않으며 문기둥만 기존 예외를 쓰는 본문을 대조해 울타리와 포장의 접촉 관계를 확인했다.
@evidence principles/design/spaces.md#space-boundary-authority 문짝 상단은 side-gate-interface, 제외 여유는 side-walk-plan에서 받는다.
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 패널 상단을 side-gate-interface 문짝 상단 world 높이, 제외 여유를 side-walk-plan, 끝기둥 후퇴를 fence-gate-junction에서 받는 본문 링크를 대조해 높이·여유 값이 이중 저작되지 않음을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-verification-address #a143ab1 연속 지표 접촉선과 기둥/패널 아래 그림자를 각 긴 면 양쪽 시야·높이 변화 단면에서 보고 순폭·지표/기초 간섭을 unverified로 둔 본문을 대조해 g 규칙의 반증 지점을 확인했다.
@evidence settings/10-house.md#site-identity 우측 목재 울타리와 관목이 함께 놓여도 관목이 울타리 아래 틈을 가리지 않게 하고 지표 경사는 maps의 선택으로 받아 g로 둔다.
@evidenceReview settings/10-house.md#site-identity #452f15e site-identity의 관목·지표 경사를 '관목·나무를 울타리 아래 틈이나 접점 결함을 가리는 용도로 배치하지 않는다'와 maps 지표 g 정의에 대조해 식재가 결함 은폐에 쓰이지 않음을 확인했다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work #46f1b62 site-identity 울타리·관목·maps 지표를 g 변수 높이 규칙과 0.12/0.24 m 점유 예약에 대조해 지표가 없어도 규칙이 정해지고 설정 수정이 필요 없음을 확인했다.
-->
