...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 본문이 차도 쪽 앞 연결로·차고 옆 세로 길·뒤 가로 길의 세 띠, 앞쪽 t 보간, 바깥 0.20 m 여유, maps 필지 포함 조건, `src/spaces/site/side-walk.ts` owner를 모두 두는지 대조해 누락이 없음을 확인했다.
...
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 세로 길 왼쪽 끝 = 차고 바깥 오른쪽 면 +0.60 m·폭 1.20 m, 앞 연결로 = 포장 끝에서 Z 1.40~0.20 m 작은 1.20 m 띠, `(1 - t) × D(Z) + t × S` 식을 대조해 평면·높이가 확정됨을 확인했다.
@evidence principles/core/common.md#declared-basis S는 정원 아래 대기의 높이, D(Z)는 차도 owner의 상면에서 받고 이 식이 배수 성능이나 법규 판정이 아니라고 밝힌다.
@evidenceReview principles/core/common.md#declared-basis #7ccd1cb S를 garden-lower-landing 높이, D(Z)를 driveway owner 상면에서 받고 보간식을 배수 성능·법규 적합성 판정이 아닌 높이 예약으로 적은 본문을 대조해 관리길 높이 근거가 추적됨을 확인했다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation #0632226 site-identity의 우측 목재 울타리·관목 서술에 없는 차도→차고 옆→정원 아래 대기의 연속 관리 보행면과 세 띠 합집합 구성을 본문이 더하는지 대조해 부모 반복이 아님을 확인했다.
@evidence principles/design/spaces.md#space-topology 세 띠의 합집합이 하나의 완결 보행면이며 세 번째 도로 포트나 차고 옆문을 추가하지 않고 아래 대기와 끝선에서만 맞닿는다.
@evidenceReview principles/design/spaces.md#space-topology #3f8d925 세 띠 합집합이 하나의 완결 보행면이고 외부 node·세 번째 도로 포트·차고 옆문·두 번째 계단을 만들지 않으며 아래 대기와 끝선에서만 맞닿는 본문을 대조해 접속 그래프를 확인했다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 앞 연결로 Z를 site-access-interface 포장 끝, 세로 길 X를 attached-garage-extent, 뒤 가로 길을 아래 대기 바깥 끝에서 받는 본문 링크를 대조해 관리길이 새 기준선을 만들지 않음을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-verification-address #a143ab1 두 꺾임·경사 연결 두 끝·아래 대기 경계에서 사람/바구니 양방향 이동을 평면/단면과 전체 관찰에 넣고 지표 중복·포장 아래 빈틈·머리 공간을 미검증으로 둔 본문을 대조해 반증 지점을 확인했다.
@evidence settings/10-house.md#site-identity 우측의 목재 울타리와 관목 사이로 차도와 후면 정원을 잇는 대지 보행면을 둔다.
@evidenceReview settings/10-house.md#site-identity #452f15e site-identity의 관목·우측 목재 울타리 기초를 보행면 바깥 0.20 m 여유 밖에 두고 그 여유를 통로 폭에 더하지 않는 본문을 대조해 울타리·관목 사이로 차도와 정원이 이어짐을 확인했다.
@evidence settings/00-production.md#use-profile 폭 1.20 m 보행면에서 사람/바구니의 양방향 이동을 검사한다.
@evidenceReview settings/00-production.md#use-profile #6ef5afe use-profile의 0.60 m 점유체·0.75 m 바구니를 관리길 폭 1.20 m와 두 꺾임·경사 끝·대기 경계 양방향 이동 검사에 대조해 테스트 도형이 관리길 전 구간에 적용됨을 확인했다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work #46f1b62 site-identity 우측 울타리·관목과 use-profile 바구니 폭을 1.20 m 세 띠와 0.20 m 여유, maps 필지 포함 요구에 대조해 maps 없이도 내부 보행면이 정해져 부모 수정이 없음을 확인했다.
-->
...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 본문이 `side-front-access`·`side-rear-access` 구역, side-yard-gate의 Z 평면·X 구간·+X 경첩·문짝 높이·회전 반경, 두 대기와 걸쇠→통과 순서, fence.ts 인계를 모두 두는지 대조해 누락이 없음을 확인했다.
...
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 1.05 m 목표, S 위 0.05–1.70 m, 반경 1.20 m 이내, 앞 +Z 0.10–1.60 m·뒤 -Z 1.30–2.80 m를 대조해 뒤 대기가 1.20 m 회전 반경 밖에서 시작하고 두 대기가 각 1.50 m 깊이로 정해짐을 확인했다.
@evidence principles/core/common.md#declared-basis 문 평면 Z는 차고 전면 바깥 면, 개구부 폭은 세로 보행면 폭에서 받고 무장애 인증이 아니라고 밝힌다.
@evidenceReview principles/core/common.md#declared-basis #7ccd1cb gate 평면 Z를 차고 전면 바깥 면, 문기둥 안쪽 X 구간을 세로 보행면 폭에서 받고 '차량 진입이나 무장애 인증을 뜻하지 않는다'로 한정한 본문을 대조해 개구부 치수 근거가 추적됨을 확인했다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work #46f1b62 site-identity 우측 목재 울타리·관목을 1.05 m 관리문과 두 대기, '기본 상태는 닫힘' 문장에 대조해 설정에 없는 개구부 결정이 spaces 선택으로 성립하고 부모 수정이 필요 없음을 확인했다.
-->
