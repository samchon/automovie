...
공유 2층 바닥과 1층 천장에는 같은 계단 통행 구멍이 필요하다. 구멍의 평면은 [계단 구멍 owner](02-stair.md#stair-floor-opening)를 소비하며 양쪽 마감이 독립적으로 다른 구멍을 만들지 않는다. 구멍 아래의 계단 유효 머리 공간은 2.00 m 목표에 대해 실제 단면에서 검사한다. 빈 공간을 지붕까지 뚫어 복층 보이드로 바꾸는 방법은 허용하지 않는다.
