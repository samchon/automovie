...
<!--
@evidence principles/core/common.md#scope-preservation 왼쪽 완결 입면 전체와 주 지붕 끝 삼각 벽, 굴뚝·거실·주침실·올리브 침실의 벽 바인딩을 맡는다.
...
@evidence principles/core/common.md#declared-basis 외벽 평면은 main-building-extent, 아래면은 roof-profile-datums, 모서리 몸체는 exterior-boundary-junctions에서 받는다.
@evidence principles/core/inherited-units.md#derived-parent-differentiation 주 용마루가 좌우 방향이라는 설정에서 왼쪽 끝이 삼각 벽이 되고 전면 박공은 같은 교차선을 소비한다는 폐합 방식을 더한다.
@evidence principles/design/spaces.md#space-topology 왼쪽 벽이 거실·주침실·올리브 침실의 외부 경계이며 굴뚝 접면을 품는 관계를 정한다.
@evidence principles/design/spaces.md#space-boundary-authority 처마 돌출은 지붕 owner, 직선 벽 끝의 공통 몸체는 07이 소유하고 이 owner는 노출 마감만 연속해 닫는다.
@evidence principles/design/spaces.md#space-verification-address 왼쪽 전체 입면·두 모서리·삼각 벽과 지붕 아래면 접촉·각 방 창의 실내외 일치를 반증 주소로 둔다.
@evidence settings/10-house.md#main-mass 본채의 좌우 방향 주 용마루 끝을 실제 삼각 벽과 처마 밑면으로 닫는다.
...
@evidence principles/core/inherited-units.md#derived-parent-differentiation 입면을 창으로 채우는 대신 굴뚝·조리 수납·방 사용을 기준으로 왼쪽 벽의 창 유무를 방별로 결정한다.
...
<!--
@evidence principles/core/common.md#scope-preservation 거실의 두 번째 외벽 창 하나와 벽난로와의 분리를 맡는다.
...
@evidence settings/10-house.md#living 왼쪽 외벽의 벽난로와 굴뚝을 유지하면서 거실에 측면 채광을 더하는 위치를 정한다.
...
@evidence principles/core/common.md#declared-basis 창의 소속은 primary-plan의 뒤쪽 본체, 벽 구간은 left-openings의 굴뚝 회피 배치에서 받는다.
...
@evidence settings/10-house.md#primary-bedroom 주침실의 창과 얇은 커튼을 측면 두 칸 창으로 실현한다.
@evidenceExclude upstream/design/spaces.md#settings-and-map-revision-from-space-work primary-bedroom의 창·커튼 요구와 2층 순높이 범위를 대조했고 Y = [3.91, 5.31] m 창이 들어가 부모 수정이 없었다.
-->
...
@evidence settings/10-house.md#living 검은 화구·벽돌 본체·목재 선반을 가진 꺼진 벽난로가 왼쪽 외벽 굴뚝과 이어지도록 앞면 예약을 거실 문 앞 대기와 떨어뜨린다.
@evidence settings/20-verification.md#visual-grammar 벽난로에서 지붕 위로 솟는 벽돌 몸체와 마감 cap을 몸통 높이와 덮개 내밈으로 공간에 예약한다.
@evidenceExclude upstream/design/spaces.md#settings-and-map-revision-from-space-work living의 벽난로-굴뚝 연속, visual-grammar의 벽돌 몸체·cap, main-mass의 처마 범위를 대조했고 몸통이 지붕 돌출 끝 밖이라 notch로 해결돼 부모 수정이 없었다.
-->
