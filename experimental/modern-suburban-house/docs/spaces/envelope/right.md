...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 본문이 X 분할면 단차 벽, 본채·차고의 오른쪽 삼각 벽, 공유 벽 위 0.15 m 후레싱 접합 띠, 청회색 침실·욕조 욕실의 창 방향을 모두 다뤄 오른쪽 폐합 범위에 빠진 owner가 없음을 확인했다.
...
@evidence principles/design/spaces.md#space-boundary-authority 본채/차고 공유 벽체는 exterior-surface-handoff가 배정한 garage.ts가 소유하고 단차 벽과 앞뒤 외벽의 공통 몸체는 07의 높이별 점유 배정을 따르며 이 파일은 둘 다 다시 만들지 않는다.
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 본문이 공유 벽체를 exterior-surface-handoff의 garage.ts 소유로, 단차 벽과 앞뒤 외벽 공통 몸체를 exterior-boundary-junctions 높이별 점유로 넘기고 다시 만들지 않는다고 적어 이중 저작이 없음을 확인했다.
...
본채 오른쪽 외벽의 박공 삼각 벽은 [낮은 지붕의 앞/뒤 아래면](../roof/00-junctions.md#roof-profile-datums)에 맞춰 닫는다. 차고 오른쪽 외벽도 자기 Gfront/Gback 아래면에서 별도 삼각 벽을 만든다. 각 삼각 벽은 기존 외벽 두께 안에 있고 겹친 삼각 장식판으로 대체하지 않는다. 본채/차고 공유 벽체는 [차고 외곽](../00-building.md#attached-garage-extent)의 X = [5.50, 5.75] m 벽이며 [표면 배정](../03-surface-owners.md#exterior-surface-handoff)대로 `src/spaces/garage.ts`가 소유하고 이 파일이 다시 만들지 않는다.
...
@evidenceReview principles/design/spaces.md#space-topology #3f8d925 본문의 '그 구간을 통과하는 가짜 외부 창은 없다'와 차고 뒤 노출 구간·차고 오른쪽 벽에만 둔 세 창 바인딩을 대조해 오른쪽 창의 안팎 연결 관계가 복원됨을 확인했다.
@evidence principles/design/spaces.md#space-boundary-authority 본채 X = [5.50, 5.75] m와 차고 X = [11.45, 11.70] m 외벽 구간을 각 외곽에서 받고 두 외곽이 공유하는 벽체는 exterior-surface-handoff가 garage.ts에 배정한 것이라 그 구간에 창을 두지 않는다.
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 본문이 X = [5.50, 5.75] m·X = [11.45, 11.70] m를 각 외곽 링크로 받고 공유 벽체를 exterior-surface-handoff의 garage.ts 몫으로 인용해 오른쪽 창이 재측정 없이 한 owner 값을 소비함을 확인했다.
@evidence principles/design/spaces.md#space-verification-address 외벽/방 binding과 지붕/창/모서리 전체의 내외부 프레임을 반증 관찰로 둔다.
@evidenceReview principles/design/spaces.md#space-verification-address #a143ab1 본문의 '실제 외벽/방 binding과 지붕/창/모서리 전체의 내외부 프레임은 unverified' 문장이 세 오른쪽 창의 벽 배정 주장을 반증할 관찰을 지정하는지 대조해 성립함을 확인했다.
@evidence settings/10-house.md#openings 창을 배정한 방과 그 창을 품은 외벽이 같아야 한다는 조건으로 공유 벽 뒤 서비스실의 창을 배제한다.
@evidenceReview settings/10-house.md#openings #5663f6c 설정 openings의 '창을 배정한 방과 그 창을 품은 외벽이 같아야 한다'를 본문의 서비스실 무창과 세 창의 방별 외벽 바인딩에 대조해 공유 벽 뒤 방에 창이 배정되지 않음을 확인했다.
...
@evidenceReview obligations/design/spaces.md#space-envelope-interface #4b397de 본문의 공유 벽 구간 무창과 차고 뒤 노출 구간의 family-right-window·tub-right-window 배정을 common-room-plan·tub-bath-plan의 실내 방과 대조해 외부 창과 실내 방이 같은 벽을 가리킴을 확인했다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work #46f1b62 설정 openings의 방-외벽 일치와 service-band의 우측 서비스 띠를 본문의 서비스실 무창 결정에 대입해 service-band가 측면 창을 요구하지 않아 부모 수정 없이 성립함을 확인했다.
-->

[공통 개구부 인계](../06-openings.md#external-opening-interface)에 따라 본채 오른쪽 창은 [본채 외곽](../00-building.md#main-building-extent)의 외벽 X = [5.50, 5.75] m, 차고 오른쪽 창은 [차고 외곽](../00-building.md#attached-garage-extent)의 외벽 X = [11.45, 11.70] m에 바인딩한다. 두 외곽이 공유하는 벽체는 [표면 배정](../03-surface-owners.md#exterior-surface-handoff)대로 `src/spaces/garage.ts`의 것이므로 그 구간을 통과하는 가짜 외부 창은 없다. 본채 오른쪽 벽의 차고 뒤 노출 구간에는 ground-storey [kitchen-dining-family](../rooms/common.md#common-room-plan)의 [family-right-window](#family-right-window)와 upper-storey [tub-bathroom](../rooms/tub-bath.md#tub-bath-plan)의 [tub-right-window](#tub-right-window)를, 차고 오른쪽 벽에는 ground-storey [garage](../rooms/garage-interior.md#garage-interior-plan)의 [garage-right-window](#garage-right-window)를 바인딩한다.

[본채 우측 서비스 띠](../../settings/10-house.md#service-band)의 서비스실은 공유 벽 때문에 측면 창을 두지 않고 청회색 침실은 자기 전면 창을 사용한다. 실제 외벽/방 binding과 지붕/창/모서리 전체의 내외부 프레임은 unverified다.
...
@evidenceReview principles/design/spaces.md#space-topology #3f8d925 family-right-window가 ground-storey kitchen-dining-family에 속하고 차고 외곽 후벽보다 뒤에서 노출되는 본채 오른쪽 외벽에만 놓인다는 본문 관계로 방·벽·차고 인접이 복원됨을 확인했다.
@evidence principles/design/spaces.md#space-boundary-authority 차고 후벽 Z와 본채 외벽 두께를 외곽 owner에서 소비한다.
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 본문의 노출 구간이 attached-garage-extent 후벽, 외벽이 main-building-extent 링크로만 정의돼 family-right-window가 차고 후벽 Z나 외벽 두께를 스스로 재저작하지 않음을 확인했다.
...
`family-right-window`는 [본채 오른쪽 벽](#right-openings)의 Z = [-9.95, -8.25], Y = [0.75, 2.30] m 개구부로 ground-storey의 [kitchen-dining-family](../rooms/common.md#common-room-plan)에 속한다. [공통 인계](../06-openings.md#external-opening-interface)의 수직 창 두 칸이다. [차고 깊이 범위](../../settings/10-house.md#garage) 안에서 고른 [차고 외곽](../00-building.md#attached-garage-extent)의 후벽보다 뒤에서 노출되는 [본채 외곽](../00-building.md#main-building-extent)의 오른쪽 외벽에 바인딩하며 후면 창과 같은 가족실을 비춘다. 소파/커튼 앞 접근, 뒤 모서리와 두 창의 일치·실제 내외부 프레임은 unverified다.
...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 본문이 tub-right-window 한 칸, 앞쪽 trim 한계 Z = -7.40 m와 차고 뒤 지붕 돌출 끝의 분리, 바깥 +X 0.25 m sash 예약을 모두 적어 욕실 측면 창 범위에 빠진 owner가 없음을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 본문이 돌출 끝은 roof-profile-datums 링크로만 받고 trim 한계 Z = -7.40 m와 +X 0.25 m 이내 sash 점유만 자기 값으로 둬 tub-right-window 주변 값이 두 번 저작되지 않음을 확인했다.
@evidence principles/design/spaces.md#space-verification-address 지붕 간섭, 기구 배치 뒤 양방향 눈높이의 프라이버시, 창 조작 접근을 반증 관찰로 둔다.
@evidenceReview principles/design/spaces.md#space-verification-address #a143ab1 본문의 기구 배치 뒤 양방향 눈높이 검토와 지붕 간섭·접근·프라이버시·채광 unverified 목록이 tub-right-window의 높이·위치 주장을 반증할 관찰을 지정하는지 대조해 성립함을 확인했다.
@evidence settings/10-house.md#openings 욕실 창을 그 방을 품은 오른쪽 외벽에 두어 창을 배정한 방과 외벽이 같다는 조건을 따른다.
@evidenceReview settings/10-house.md#openings #5663f6c 설정 openings의 방-외벽 일치 조건을 본문의 tub-right-window가 tub-bathroom을 품은 본채 오른쪽 벽 Z = [-8.40, -7.50] m에 바인딩된 진술과 대조해 충족함을 확인했다.
@evidenceExclude upstream/design/spaces.md#settings-and-map-revision-from-space-work openings의 방-외벽 일치를 대조했고 욕조 욕실의 높은 흐린 창을 차고 뒤 지붕 돌출 끝과 분리한 본채 오른쪽 벽에 둘 수 있어 부모 수정이 없었다.
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work #46f1b62 설정 openings의 방-외벽 일치를 tub-right-window의 Z = [-8.40, -7.50] m 배치와 차고 뒤 돌출 끝 분리에 대입해 설정이 욕실 창 자리를 막지 않아 부모 수정이 필요 없음을 확인했다.
-->
...
@evidenceReview principles/design/spaces.md#space-topology #3f8d925 garage-right-window가 ground-storey garage에 속하고 상부 문 이동 예약보다 뒤쪽 차고 오른쪽 벽에 놓인다는 본문 관계로 방·벽·문 레일 인접이 형상 전에 정해짐을 확인했다.
@evidence principles/design/spaces.md#space-boundary-authority 상부 문 이동 예약은 garage-front-opening, 후벽 선반/작업대 예약은 garage-storage-use에서 소비하고 이 H2는 창 좌표와 높은 고정창 두 칸을 저작한다.
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 본문이 이동 예약은 garage-front-opening, 선반/작업대는 garage-storage-use 링크로만 받고 스스로는 Z = [-5.85, -4.25], Y = [1.40, 2.20] m와 두 칸만 저작해 garage-right-window 값의 owner가 하나임을 확인했다.
