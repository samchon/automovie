...
@evidence principles/design/spaces.md#space-boundary-authority 본채/차고 공유 벽은 차고 owner가 소유하고 단차 벽과 앞뒤 외벽의 공통 몸체는 07의 높이별 점유 배정을 따르며 이 파일은 둘 다 다시 만들지 않는다.
...
@evidence principles/design/spaces.md#space-boundary-authority 본채 X = [5.50, 5.75] m와 차고 X = [11.45, 11.70] m 외벽 구간을 각 외곽에서 받고 두 외곽이 공유하는 벽은 차고 외곽 owner의 것이라 그 구간에 창을 두지 않는다.
