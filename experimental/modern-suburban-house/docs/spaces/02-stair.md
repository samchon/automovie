...
[위 경로](#stair-reservation)와 하부 대기 면적에 한정하여, 층판 구멍은 X = [-1.80, -0.65]·Z = [-4.56, -0.25]의 세로 부분과 X = [-0.65, 1.87]·Z = [-4.56, -3.41]의 가로 부분을 합친 L형이다. 계단 전면 끝이 본채 안쪽 전면에 닿으므로 [상층 전면의 작은 계단 창](../settings/10-house.md#openings)은 실제 계단 공간으로 열린다. 창의 정확한 void·높이·살대는 입면 단계에서 이 경계에 바인딩한다. 창 뒤를 침실로 바꾸거나 전면에 가짜 창을 붙이지 않는다.
