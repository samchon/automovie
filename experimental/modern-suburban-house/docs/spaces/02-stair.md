...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 현관 하부 대기에서 상층 복도 도착면까지의 유일한 `main-stair`, 아래 flight·중간참·위 flight의 평면 예약 표와 높이·접속, `src/spaces/stair.ts` owner가 모두 본문에 있어 계단 경로 몫이 빠지지 않음을 확인했다.
...
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 3.06 m = 18 × 0.17 m, 7·9 디딤 × 0.28 m = 1.96·2.52 m 진행 길이, 8 × 0.17 = 1.36 m 중간참 높이를 검산하고 표의 Z = -1.45 → -3.41 구간이 1.96 m와 맞아 경로 결정이 완결됨을 확인했다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work #46f1b62 stair 설정의 ‘두 직선 flight와 중간참’·층고 분할 요구와 storey 3.06 m를 18 × 0.17 m 분할과 X = [-1.80, -0.65] 경로에 대조해 settings 수정 없이 L형 유일 연결이 성립함을 확인했다.
-->
...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 `main-stair-connection` 하나의 kind·from·to·bidirectional, route 양 끝과 꺾임, 중간 landing, steps 생략의 한계, 연결 검사와 단별 검사의 구별이 네 문단에 나뉘어 connector 인계 몫이 빠지지 않음을 확인했다.
...
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 kind stair, from front-entry, to upper-hall, bidirectional true와 `landings.at` = 중간참 중심까지의 3D polyline 길이 ÷ 전체 route 길이가 정해져 source가 connector 필드를 새로 고를 필요가 없음을 확인했다.
...
@evidenceReview principles/core/common.md#declared-basis #7ccd1cb steps 생략 근거를 공개 타입·`validateBuiltEnvironment`와 `test_architecture_built_connector.ts`의 spiral stair 사례로 대고 테스트 실행·계단 검증이 아니라고 밝힘을 확인했으며 그 테스트 파일도 저장소에 있다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation #0632226 stair 설정의 계단 하나를 두 flight 별도 등록 없는 connector 하나와 elements 묶음으로 넘기고 `landings.at`을 중간 index나 0.5로 고정하지 않는 결정이 설정에 없는 이 H2의 몫임을 대조했다.
...
@evidenceReview principles/design/spaces.md#space-topology #3f8d925 route가 하부 대기 내부 → 아래 flight → 중간참 한 번 꺾임 → 위 flight → 상부 도착면 내부 순서이고 `landings.space`가 main-stair를 가리키며 중간참을 별도 방·층으로 만들지 않음을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 단별 치수는 계단 공간과 단별 치수 링크에서 유지하고 균일 steps 요약이나 평균 run으로 바꾸지 않으며 참의 physical boundary와 지지 면을 stair owner에 남긴다는 문장으로 값의 단일 owner를 확인했다.
...
@evidenceReview principles/design/spaces.md#space-verification-address #a143ab1 마지막 문단의 세 공간 connector 조회 일치·landing의 같은 참 위 위치와, `builtConnectorGeometry`가 재지 않는 챌판·순폭·머리 공간을 실제 부재에서 읽는다는 구분이 반증 주소임을 확인했다.
@evidence settings/10-house.md#stair 두 flight를 별도 계단으로 등록하지 않고 같은 connector 안의 참과 부재로 묶는다.
@evidenceReview settings/10-house.md#stair #170ce55 stair 설정의 ‘별도 계단·사다리·복층 보이드는 없다’를, 두 flight를 별도 층간 계단으로 등록하지 않고 같은 connector의 elements로 묶으며 계단실 밖 직선 연결을 금하는 첫 문단에 대조했다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work #46f1b62 stair 설정의 ‘두 직선 flight와 중간참’과 ‘단면으로 검토’를 connector 형태에 대조해, steps를 생략해도 단 수·치수 검사를 실제 부재에서 요구하는 셋째 문단으로 settings 수정 없이 성립함을 확인했다.
-->
...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 L형 층판 구멍, 계단 창이 계단실로 열리는 관계, upper.ts와 방별 마감의 같은 구멍 소비, 네 0.15 m 보호/분리 띠, 계단실 위 높은 천장 폐합이 네 문단에 있어 구멍 둘레 몫이 빠지지 않음을 확인했다.
...
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 세로 X = [-1.80, -0.65]·Z = [-4.56, -0.25]와 가로 X = [-0.65, 1.87]·Z = [-4.56, -3.41]의 L형, X = [-1.95, -1.80] 등 네 띠가 각 0.15 m로 정해져 구멍 경계가 좌표로 확정됨을 확인했다.
@evidence principles/core/common.md#declared-basis 구멍은 위 경로와 하부 대기 면적에 한정해 산출하고 창 void와 높이는 stair-front-window에서 받는다고 밝힌다.
@evidenceReview principles/core/common.md#declared-basis #7ccd1cb 구멍을 ‘위 경로와 하부 대기 면적에 한정’해 산출하고 창의 void·높이는 stair-front-window 링크에서 소비하며 실제 창호는 후속 부재라고 밝혀 좌표마다 근거가 추적됨을 확인했다.
...
@evidenceReview settings/10-house.md#openings #5663f6c openings의 계단/복도용 작은 창과 ‘창을 배정한 방과 그 창을 품은 외벽이 같아야’를, 창이 실제 계단 공간으로 열리고 창 뒤를 침실로 바꾸거나 가짜 창을 붙이지 않는 첫 문단에 대조했다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work #46f1b62 openings의 계단/복도용 작은 창과 stair의 ‘복층 보이드는 없다’·‘실제 통행 개구부’를 L형 구멍의 계단실 한정과 지붕 미연장에 대조해 세 조건이 settings 수정 없이 함께 성립함을 확인했다.
-->
...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 두 flight·중간참 순폭, 손잡이·난간 0.075 m 점유 예약, 하부/상부 대기의 소속, 위 flight 아래 외투장의 비통행, 디딤·참별 머리 공간 2.00 m 검사가 본문에 있어 순폭 몫이 빠지지 않음을 확인했다.
...
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 1.15 m 경로 폭에서 양쪽 0.075 m를 뺀 1.00 m와 중간참 두 방향 각 1.00 m 통행 영역이 정해지고 예약선 초과 시 부재를 고치는 규칙이 있어 후속 모듈이 폭을 새로 정할 필요가 없음을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 경로 폭은 경로 링크, 두 대기는 entry-plan·upper-hall-plan 링크, 외투장은 entry-coat-storage 링크에서 받아 이 H2가 그 경계를 다시 정하지 않고 점유 예약만 더함을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-verification-address #a143ab1 `src/spaces/stair.ts`에서 파생할 단별 단면·90° 회전 경로·참 양쪽 시야에 0.60 × 0.45 × 1.90 m 사람과 0.75 m 바구니를 줄이지 않고 적용하고 디딤별 2.00 m 머리 공간을 보는 검사 주소를 확인했다.
@evidence settings/00-production.md#use-profile 0.95 m 계단 목표와 2.00 m 머리 공간 목표를 사람·바구니를 줄이지 않고 검사한다.
@evidenceReview settings/00-production.md#use-profile #6ef5afe use-profile의 계단 유효폭 0.95 m·머리 공간 2.00 m와 점유체 비축소 규칙을 계산 순폭 1.00 m, 각 디딤·참의 2.00 m 검사, 0.60 × 0.45 × 1.90 m 사람·0.75 m 바구니 적용 문장에 대조했다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work #46f1b62 use-profile의 0.60 × 0.45 × 1.90 m 사람과 0.75 m 바구니를 1.15 m L형 경로의 1.00 m 계산 순폭에 대조해 점유체를 줄이거나 프로필을 바꾸지 않고 예약이 성립함을 확인했다.
-->
...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 표의 여섯 경계 높이 역할, 경사 손잡이 0.90 m와 복도 보호 1.05 m, 난간살 간격 0.10 m 상한, 외투장 opening만 소비하는 계단 아래 막음이 본문에 있어 경계 높이 몫이 빠지지 않음을 확인했다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work #46f1b62 stair 설정의 참 통로 비차단 조건과 난간 재료를 표의 여섯 경계에 적용해, +X 열린 도착과 복도 1.05 m 보호가 함께 성립하므로 settings 수정이 필요 없음을 확인했다.
-->
