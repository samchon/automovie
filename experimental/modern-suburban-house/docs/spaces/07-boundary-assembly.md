...
@evidenceReview principles/core/common.md#scope-preservation #24155e1 끝점·L·T·십자 접합, 문 절단과 문틀·문짝, 수건걸이·거울·선반 지지 접면, 문 아래 중앙면 전환, 10-ground-floor 출입구 인계 문단을 대조해 접합 범위의 각 항목에 owner가 있음을 확인했다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 문틀·문짝을 원래 문 owner가 한 번 만들고 반대 방은 반대 면만 관찰하며 외벽·차고 공유 벽·계단 구조 접합은 기존 구조 owner가 받는 문장을 대조해 문 부재와 접합이 두 번 저작되지 않음을 확인했다.
...
@evidenceReview settings/10-house.md#openings #5663f6c settings openings의 문틀·문턱 구별 요구를 같은 절단 경계의 공유, 문턱/전환 부재의 원래 문 owner 통합, 닫힌 문을 벽 void 부재로 오해하면 실패라는 문장에 대조해 실내 문이 성립함을 확인했다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work #46f1b62 settings openings의 문짝·문틀·문턱 구별을 실내 문 절단 경계 공유와 중앙면 문턱 전환에 대조해 원래 문 owner 하나가 부재를 통합하므로 부모 수정 없이 성립함을 확인했다.
-->
...
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 boundaries.ts가 구역·높이 경계만 전달하고 front가 받는 모서리 몸체의 옆면 마감이 left/right의 연속 면으로 남는다는 문단을 대조해 몸체와 03-surface-owners 외부 마감이 겹쳐 저작되지 않음을 확인했다.
...
@evidenceReview settings/20-verification.md#surface-allocation #aae54d5 표의 구조 몸체 배정과 03-surface-owners#exterior-surface-handoff의 완결 시각 표면 구별을 surface-allocation의 입면 owner 요구에 대조해 모서리 트림·방 마감이 몸체 배정으로 쪼개지지 않음을 확인했다.
