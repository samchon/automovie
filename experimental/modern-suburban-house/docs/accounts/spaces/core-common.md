...
@evidence obligations/core/common.md#layer-boundary 47개 파일은 포함·인접·치수·통행·점유 예약만 정하고 부재 원형은 models, 표면 표현은 materials, 반복 배치는 instances, 조명은 systems, 필지·지표·외부 도로는 maps로 넘긴다고 각 owner에 적었다. 04의 engine-render-handoff는 공개 API가 공간 표현에 요구하는 조건만 기록하며 source를 저작하지 않는다.
-->
...
층 경계는 각 owner가 스스로 밝힌다. 가구·기기·선반의 표는 점유 상한 예약이며 원형·다리·손잡이 형상은 models, 세 스툴·여섯 의자의 배치는 instances, 재료와 광학값은 materials, 방별 광원은 systems가 소비한다. 필지·외부 보도·지표·식재는 [maps 인계](../../spaces/site/00-access.md#map-handoff-inputs)가 받을 입력으로만 적고 spaces가 대신 정하지 않는다. [렌더 인계](../../spaces/04-observations.md#engine-render-handoff)는 설치된 공개 API의 동작을 읽어 공간 표현이 갖춰야 할 조건만 남기며 서버나 뷰어 코드를 저작하지 않는다. 이 배분이 실제 source에서 지켜지는지는 source가 없는 현재 unverified다.
...
<!--
@evidence obligations/core/common.md#proportionate-development 주석·제목·공백을 뺀 본문은 공통 11파일 28 H2 / 48801자, 입면 4 / 24 / 10971, 지붕 9 / 12 / 7820, 포치 1 / 2 / 1678, 방 15 / 38 / 33725, 대지 7 / 16 / 17337, 합계 47 / 120 / 120332자다. 재생성 전 spaces는 house.md 1파일 7 H2 2938자였다. 계단(5 H2 / 5413자)과 공용부(6 / 5719)가 서비스 통로(1 / 941)와 옷방(2 / 1224)보다 깊고, 지붕 경사면은 roof/00(4383자)의 교차 계산을 소비해 각 375–495자에 머문다.
-->
