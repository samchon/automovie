 ...
 * @evidenceReview spaces/003-surface-ownership.md #0a92d68 envelope가 front·rear·left·right·roof 다섯 owner만 호출하고, 각 facade()가 자기 wall body·stone panel·창호 단면·reveal·drip·spandrel cassette·louvre·roller·corner를 함께 만드는 것을 facade.ts·curtainwall.ts에서 읽었다. 003의 H2들이 요구하는 면·개구·접합이 이 다섯 호출 안에 있고 viewer 전용 대체 외피는 없다.
 ...
 * @evidence spaces/003-surface-ownership.md#whole-surface-owners front/rear/left/right/roof 모듈이 각 표면 전체를 소유하며 full facade boundary는 house에 귀속하고 그 face는 외측 O에 둔다. 구조 내벽·실내 수평 마감과 입면 아래 plinth는 storey 모듈, 계단 void 앞 외벽 실내 면은 전면 facade에 남는다.
 * @evidenceReview spaces/003-surface-ownership.md#whole-surface-owners #251102d facade()가 C에서 벽을 절삭한 뒤 boundary.face.origin을 C+(d/2)N으로 옮기는 코드를 읽었고, builtEnvironmentEnvelopeFaces가 네 면의 법선을 front (0,0,-1)·rear (0,0,1)·left (1,0,0)·right (-1,0,0), centroid를 z=±6.00·x=±5.50으로 내는 것을 실행해 확인했다. 입면 아래 plinth는 ground.ts, 위 ring은 roof.ts가 만들어 표의 owner 배정과 맞는다.
 ...
 * @evidenceReview spaces/003-surface-ownership.md#right-face #5e40d3e right.ts의 두 Glazing(z=3.60..5.40, 1층 common과 2층 bath)과 face centroid x=-5.50·법선 (-1,0,0)을 읽었다. z=0의 배수관·clip·점검 덮개는 창 cut과 3.5m 떨어진 불투명 구간에 있다.
 ...
 * @evidenceReview spaces/003-surface-ownership.md#roof-face #a1ef28b v-076은 경사·거터·배수와 cassette70·girder3·rail44·post9를 roof/right 및 오류 없는 native audit에 대조했고, 설계에 보완한 나무4·관목48그룹·풀23그룹의 보존 배치와 garden 실행 연결까지 통과시켰다. 이 export의 지붕·관 연결과 buildHouse의 별도 garden 호출을 구분하여 읽었고 식재를 roof가 생성한다는 중복 소유는 없다. 제공된 WebGL2 외관·절개·하부 캡처를 열어 셀 사이 하늘, 프레임·레일 리듬과 지붕 위 지지의 분리를 확인했다. r3의 설계 추적과 캐노피 충실도·시각 판정은 닫혔으며 실제 배수 용량이나 장비 운용은 이 결과가 인증하지 않는다.
 * @evidence spaces/003-surface-ownership.md#glazing-interface 각 창의 jamb·mullion은 member 중심에서 body·web·압착판·덮개 네 부재, head·sill은 bay마다 인접 수직 부재 안쪽 면 사이에서 같은 단면으로 만든다. 유리는 seat 안으로 0.006 들어가고 gasket과 setting block이 받친다. cut 안쪽 reveal, 층간 cassette, 작업실·침실 하부 루버를 같은 owner가 만든다.
 * @evidenceReview spaces/003-surface-ownership.md#glazing-interface #364e2f7 curtainwall.ts의 SECTION 표(body s±0.020 n-0.070..-0.015, web ±0.010, 압착판 ±0.020 0.015..0.035, 덮개 ±0.018 0.035..0.070), 유리 L-0.006..R+0.006, gasket abs(s) 0.014..0.020, reveal 0.004/0.006, cassette 2.844..3.246·n 0.138..0.140, 루버 k=15·p=0.078·40% 날개와 현관 접근 bay 제외를 본문 수치와 대조했다. compiled scene에서 수직 부재 53×4, 수평 82×4, cassette 17장, 날개 285·지지대 25·arm 50을 셌다. 모따기와 루버 관 내부·수용 구멍은 box·rod로 단순화되어 있다.
 ...
 * @evidence spaces/003-surface-ownership.md#front-flex-glazing x=3.06..5.22의 ground opening이 작업실 clear face 안에 frame까지 들어간다. 낮에는 하부 반투명, 상부 clear이며 사적·야간 상태는 같은 opening의 shade를 내리고 하부 띠 앞에 고정 루버를 둔다.
 * @evidenceReview spaces/003-surface-ownership.md#front-flex-glazing #74f735e front.ts의 a=3.06·b=5.22·privacy lower와 두 bay, 하부 frosted 0.12..1.37, 두 bay의 날개 30장을 compiled scene에서 읽었다. roller drop은 facade.ts 상태식대로 사적·야간 100%다.
 * @evidence spaces/003-surface-ownership.md#front-bedroom-glazing child-one의 x=1.80..5.22 span에 하층 작업실 서측 jamb 중심 x=3.04를 우선 분할선으로 넣는다. 상층 sill/head와 privacy band를 따로 적용하고, 현관 접근 구역 위 첫 bay를 뺀 두 bay에만 루버를 둔다.
 * @evidenceReview spaces/003-surface-ownership.md#front-bedroom-glazing #8007264 front.ts의 breaks [3.04]로 edge가 1.80·3.04·4.13·5.22가 되는 것과, 첫 bay(날개 x=1.792..3.028)가 현관 접근 x=1.30..2.90과 겹쳐 louvre()에서 빠지고 나머지 두 bay에 날개 30장이 생기는 것을 compiled scene에서 확인했다.
 ...
 * @evidence spaces/003-surface-ownership.md#rear-bedroom-glazing primary의 x=-2.80..5.22 span을 upper floor에 맞춰 나누며 -X 욕실과 shared wall의 끝을 관통하지 않고, 일곱 bay의 하부 띠 앞에 루버를 둔다.
 * @evidenceReview spaces/003-surface-ownership.md#rear-bedroom-glazing #5c225a6 rear.ts의 a=-2.80·b=5.22·privacy lower와 일곱 bay, 날개 105장을 compiled scene에서 읽었다. 서측 jamb 바깥 cut -2.84는 욕실·주침실 벽(-3.02..-2.84)의 끝과 맞닿고 넘지 않는다.
 ...
 * @evidenceReview spaces/003-surface-ownership.md#left-bedroom-glazing #6319d4b left.ts의 상층 z=-5.40..-2.30·sill 3.32·head 6.00, 세 bay와 날개 45장, 두 창 사이 층간 cassette(Z=-5.44..-2.26)를 compiled scene에서 읽었다.
 ...
 * @evidenceReview spaces/003-surface-ownership.md#right-bath-glazing #49514f7 right.ts의 2층 z=3.60..5.40·sill 3.32·head 6.00·privacy all과 전 상태 frosted, 1층 창과의 층간 cassette(Z=3.56..5.44)를 읽었다.
