 ...
 * @evidenceReview spaces/003-surface-ownership.md#whole-surface-owners #0cbefcb front·rear·left·right·roof를 각 파일에서 조립하고 room lining은 실내 owner에 남겨 완결 표면을 가구 파일에 분산시키지 않는다.
 ...
 * @evidenceReview spaces/003-surface-ownership.md#front-face #f877f75 front owner가 문·계단 유리·방 유리를 자신의 벽에 절단하고 -X 코어는 불투명 실체로 남긴다.
 ...
 * @evidenceReview spaces/003-surface-ownership.md#rear-face #8b132df rear owner가 common·primary·bath 개구와 층 band를 한 면에 조립하여 실내 room 경계를 외관에서 보존한다.
 ...
 * @evidenceReview spaces/003-surface-ownership.md#left-face #333b56d left facade는 flex·child-one·child-two의 cut만 받고 나머지 측면은 stone 부재로 남긴다.
 ...
 * @evidenceReview spaces/003-surface-ownership.md#front-stair-glazing #1c3e22d 계단 전면 유리는 두 층을 각각 만들고 사이 floor band를 남겨 무지지 통유리 한 장으로 대체하지 않는다.
 ...
 * @evidenceReview spaces/003-surface-ownership.md#front-flex-glazing #74f735e flex 전면 cut과 frame을 방의 clear X 범위에 넣고 하부 반투명 구간을 생성하여 실내 lining 밖으로 개구를 밀지 않는다.
 ...
 * @evidenceReview spaces/003-surface-ownership.md#front-bedroom-glazing #8007264 상층 전면 child-one 유리는 하층 flex jamb 중심선을 분할 기준으로 사용하여 floor 위아래 베이가 어긋나지 않는다.
 ...
 * @evidenceReview spaces/003-surface-ownership.md#rear-common-glazing #8601cc9 rear-common은 한 room 전체의 고정 유리로 조립되며 설계에 없는 뒤 출입 connector를 만들지 않는다.
 ...
 * @evidenceReview spaces/003-surface-ownership.md#rear-bedroom-glazing #5c225a6 rear-primary의 서측 끝은 bathroom shared wall에서 멈춰 침실 창이 욕실 안까지 이어지지 않는다.
 ...
 * @evidenceReview spaces/003-surface-ownership.md#rear-bath-glazing #bca1863 욕실 후면 pane은 높은 sill과 frosted 재료를 사용하여 다른 rear 유리와 같은 clear 상태로 풀리지 않는다.
 ...
 * @evidenceReview spaces/003-surface-ownership.md#left-flex-glazing #2ca6488 left-flex glazing을 모서리에서 물려 전면 창과 사이에 실제 불투명 return을 유지한다.
 ...
 * @evidenceReview spaces/003-surface-ownership.md#left-bedroom-glazing #6319d4b left-bedroom glazing이 하층 flex의 Z 범위와 반복 간격을 이어 상하층 측면 창호의 정렬을 유지한다.
 ...
 * @evidenceReview spaces/003-surface-ownership.md#left-child-two-glazing #48f2284 left-child-two의 짧은 높은 창은 중간 침실 cell 안에만 생성되어 앞 침실의 긴 창을 복제하지 않는다.
 ...
 * @evidenceReview spaces/003-surface-ownership.md#right-common-glazing #914052e right-common의 유리 구간을 후면 z=3.60..5.40에 한정해 전면 설비 코어를 투명화하지 않는다.
 ...
 * @evidenceReview spaces/003-surface-ownership.md#right-bath-glazing #49514f7 right-bath glazing은 하층 common과 Z span을 맞추되 frosted 상태로 만들어 코어의 프라이버시를 유지한다.
 ...
 * @evidenceReview spaces/003-surface-ownership.md#envelope-corners #187c93e 외벽 모서리를 두 삼각 기둥으로 나누어 각 facade의 온전한 return 소유권과 외곽 폐합을 함께 유지한다.
 */
