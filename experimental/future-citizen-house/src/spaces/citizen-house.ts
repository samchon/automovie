 ...
 * @evidenceReview spaces/001-citizen-house.md #db00e07 citizenHouseSpaceSource가 buildHouse 하나로 topology·storey·envelope·garden·rooms를 조립하고 그 environment를 반환하는 것을 읽었다. 001의 집·대지·관찰 세 H2가 요구하는 산출물이 이 한 호출에서 나온다.
 ...
 * @evidence spaces/001-citizen-house.md#site-access buildHouse의 garden 호출이 외곽 밖에서 끝나는 지면·보도·두 디딤판·y=0 landing과 조경 전체를 만들고, 지면 위 부재는 지면에서 시작한다. site-access가 소비하는 roof-face의 정비 재배치에 따라 tree-0..3과 hedge의 기존 side/i ID를 후면에 보존하고 front-grass-3..7을 cassette 예약면 밖으로 옮긴다. 같은 호출의 canopy audit가 식물 bounds와 예약대의 겹침을 검사한다.
 * @evidenceReview spaces/001-citizen-house.md#site-access #c103499 garden.ts가 site-ground에서 본채 외곽을 빼고, approach tread 두 개(상단 -0.30·-0.15)와 landing(0)을 지면 -0.45부터 채우며 entryApproach x=1.30..2.90을 쓰는 것을 읽었다. compiled scene에서 두 tread와 landing, rear-paving이 모두 y=-0.45에서 시작한다.
