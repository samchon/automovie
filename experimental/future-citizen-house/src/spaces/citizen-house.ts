 ...
 * @evidenceReview spaces/001-citizen-house.md#citizen-house-space #b6afc3e compiled environment에서 citizen-site 아래 house, 그 아래 ground-storey·upper-storey와 각 room이 있고 building unit citizen-house의 element가 house-root인 것을 읽었다. 별동이나 추가 계단 connector는 없으며 건축 형상은 현재 buildHouse에서 컴파일된다.
 ...
 * @evidenceReview spaces/001-citizen-house.md#site-access #c02f28e garden.ts가 site-ground에서 본채 외곽을 빼고, approach tread 두 개(상단 -0.30·-0.15)와 landing(0)을 지면 -0.45부터 채우며 entryApproach x=1.30..2.90을 쓰는 것을 읽었다. compiled scene에서 두 tread와 landing, rear-paving이 모두 y=-0.45에서 시작하고, service-band 네 조각·cassette-staging-pad·site-sidewalk 두 조각이 흙 site-ground와 같은 y=-0.852..-0.45를 채우고, catch-bottom 하면 -0.852와 출구 원(y=-0.85..-0.75)이 흙 깊이 안에 있다. 본문에 없던 site-curb는 더 이상 생성되지 않아 보도·예약면과 겹치는 site 부재가 없다. roof-face의 식재 이동은 기존 형상·임시 material 값의 보존 조건이며 최종 식물 finish 소유권을 이 garden 호출에 부여하지 않는다.
 ...
 * @evidenceReview spaces/001-citizen-house.md#spatial-observation #a4c6c2d observations.ts가 environment의 cell·surface·connector·boundary.face·opening.profile에서 station을 만들고 위치가 없는 station도 이유와 함께 남기는 것을 읽었다. 외피 face가 O로 옮겨져 외부 관찰이 각 면의 바깥에서 선다. 2026-09-22 서버 409와 v-109의 후속 GPU 캡처는 그때의 관찰 기록으로 읽고 현재 트리의 전수 검증으로 승격하지 않는다.
