 ...
 * @evidenceReview spaces/001-citizen-house.md 등록값의 build가 집·대지·관찰을 포함한 환경을 반환하며 viewer도 동일 생산자를 호출하므로 001의 전체 범위가 실행된다.
 ...
 * @evidenceReview spaces/001-citizen-house.md#citizen-house-space buildHouse가 citizen-site 아래 한 house와 두 storey를 조립하므로 library 등록에서 별도 매스가 생기지 않는다.
 ...
 * @evidenceReview spaces/001-citizen-house.md#site-access garden의 두 디딤판 뒤 y=0 landing을 front-entry가 소비하여 대지 접근이 실내 그래프와 만난다.
 ...
 * @evidenceReview spaces/001-citizen-house.md#spatial-observation observations는 현재 cell과 opening을 열거하고 배치 불가능한 시점도 null로 남겨 대표 카메라만으로 분모를 축소하지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-scope-preservation 이 export는 001의 library 등록만 담당하고 방 경계나 외피 치수를 viewer 전용 형상으로 다시 정의하지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion 빈 등록 객체가 아니라 buildHouse의 실제 environment를 반환하고 lowering까지 소비되므로 타입 선언만으로 완료를 대신하지 않는다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work 001의 site→house→storey와 landing을 그대로 구현할 수 있어 이 등록에서 부모 대지나 건물 수를 수정할 필요는 발견되지 않았다.
 ...
 * @evidenceReview obligations/design/space-sources.md#space-source-invalid-topology 잘못된 parent나 도달성은 buildHouse가 오류로 거부하며, 원통 통행의 blocked와 미계측 계단 상승을 성공으로 바꾸지 않는다.
 ...
 * @evidenceReview obligations/design/space-sources.md#space-source-stable-identities room·boundary 참조와 반복 인덱스로 ID를 구성해 같은 입력에서 시간이나 난수 때문에 공간 주소가 변하지 않는다.
 */
