 ...
 * @evidenceReview spaces/002-spatial-graph.md 전체 002의 room·wall·portal·stair를 plan과 topology로 소비하며 내부 fit-out도 같은 cell 범위에 조립한다.
 ...
 * @evidenceReview principles/core/source-units.md#source-scope-preservation 바닥·분할·계단과 내부 가구까지만 이 조립이 맡고 외부 커튼월 표면은 envelope owner에 남긴다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion rooms·walls·stair가 실제 geometry와 connector를 반환하므로 설계 배열을 복사한 메타데이터만으로 소스 실현을 주장하지 않는다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work 002가 정한 L형 seam·계단 hole·문 열림을 구현할 입력이 모두 있어 구조 export에서 부모 방 수나 corridor 형식을 바꿀 필요는 없었다.
 ...
 * @evidenceReview obligations/design/space-sources.md#space-source-design-ownership 구조 조립은 designPlan의 방·층·개구 주소를 받아 사용하며 검증 편의를 위한 독립 공간 그래프를 만들지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#mass-and-storeys 11×12m 외곽과 3.20m 상층 datum을 하나의 house에 조립하여 면적 목표를 별동이나 추가 층으로 보충하지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#ground-level ground-level이 1층 room들의 부모이고 y=0 바닥이 외부 landing 및 첫 계단 단과 같은 높이를 소비한다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#upper-level upper-level의 y=3.20 바닥과 y=6.10 천장이 침실·욕실·복도에 공통으로 적용된다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#ground-partition 1층 방의 대향 clear face에서 shared wall을 산출하여 수납·위생실 사이 잔여 틈을 임의의 복도로 만들지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#entry 현관 room 안에서 계단 동측 보행 띠와 세 목적지 개구가 연결되어 계단만 있고 통행 공간이 없는 구성이 아니다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#flex-workroom flex는 전면 동측의 2.24m 폭 cell과 entry 출입으로 생성되며 common 쪽 폐쇄 벽을 지름길로 사용하지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#common-room 후면 common의 거실·식당·주방을 한 room과 연속 바닥으로 조립하여 가구 배치가 추가 분할 벽을 만들지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#powder-utility powder-utility는 -X 전면 cell과 현관 문을 갖고 급배수 가구가 common 쪽으로 위생실 경계를 넓히지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#storage-1f 1층 수납은 common 문으로만 접근하고 powder·entry 면을 닫아 별도의 숨은 연결이 생기지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#door-interface clear opening 밖 jamb·head와 두께 있는 leaf를 조립하고 열린 leaf도 장애물로 남겨 사각 구멍만 제공하지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#front-entry front-entry의 x=2.22 문과 외부 landing이 만나 계단 유리 frame과 겹치지 않는 전면 진입이 된다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#entry-flex entry-flex의 pocket leaf가 -Z로 clear width만큼 이동하여 짧은 반대쪽 벽 끝에 문짝을 밀어 넣지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#entry-common x=2.05의 leaf 없는 통로가 후면 common과 현관의 계단 옆 보행 띠를 직접 잇는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#entry-powder powder 안쪽으로 열리는 z=-3.20 방문이 현관에서 위생실로 직접 닿고 entry 보행 띠를 차지하지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#common-storage common-storage 문을 x=-4.14의 공용부 경계에 내어 수납 접근이 폐쇄된 현관 측 벽을 관통하지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#single-stair 9+9 riser와 중간 landing으로 3.20m를 올라 마지막 tread와 층 바닥을 중복된 한 단으로 세지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#stair-opening 상층 slab에서 명시된 stair-opening만 빼므로 계단 구멍을 거실 복층 보이드로 확대하지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#upper-partition 상층 cell 접촉면과 여섯 문으로 분할을 구성해 설비실 L형 내부 seam에는 벽을 추가하지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#upper-corridor 4.20m 일자 corridor가 계단 도착에서 시작해 여섯 목적지 문을 받아 분기 복도를 만들지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#primary-bedroom 후면 주침실의 clear cell과 corridor 문이 일치하여 bath와 공유하는 서측 벽 밖으로 침실 범위를 늘리지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#child-bedroom-1 L형 child-one의 두 cell 사이 벽을 만들지 않고 corridor 문을 확장 cell에 연결하여 침대 영역까지 같은 방으로 남긴다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#child-bedroom-2 child-two를 동측 중간 cell로 만들고 전용 corridor 문을 두어 다른 침실을 통과해야 들어가는 방이 되지 않는다.
 * @evidence spaces/002-spatial-graph.md#upper-bathroom x=-5.26..-3.02,z=1.28..5.76의 코어 cell을 만들고 z=1.86 방문으로 복도에 직접 잇는다.
 * @evidenceReview spaces/002-spatial-graph.md#upper-bathroom upper-bathroom의 서측 후면 cell 안에 위생 가구를 조립하고 문은 corridor에 직접 연결한다.
 * @evidence spaces/002-spatial-graph.md#upper-storage x=-5.26..-3.02,z=-0.14..1.10의 cell에 복도 직결 문을 둔다. 욕실·설비 경계는 닫힌 wall이다.
 * @evidenceReview spaces/002-spatial-graph.md#upper-storage upper-storage는 z=-0.14..1.10의 독립 cell이고 corridor 문이 있어 욕실 벽장으로 대체되지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#upper-service upper-service 두 cell의 합집합과 서측 corridor 문을 유지하여 계단 서쪽 부분이 고립된 별실이 되지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#corridor-primary 주침실 문은 북쪽 x=-0.58에서 상층 바닥에 연결되어 후면 방으로 직접 들어간다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#corridor-child-one z=-0.95의 동측 문이 child-one 확장 cell에 닿아 계단 뒤의 주 cell까지 L형 내부 연결을 유지한다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#corridor-child-two child-two 출입은 z=1.05의 동측 문으로 실현되어 child-one 출입과 별개의 corridor 연결이다.
 * @evidence spaces/002-spatial-graph.md#corridor-bathroom z=1.86,폭0.90의 방문을 복도 서측 욕실 경계에 두고 leaf는 욕실 안으로 연다.
 * @evidenceReview spaces/002-spatial-graph.md#corridor-bathroom 상층 서측 z=1.86 개구와 욕실 안으로 열린 leaf가 corridor-bathroom의 목적 방에 도착한다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#corridor-storage z=0.48 서측 문이 upper-storage 내부로 열려 욕실이나 설비실을 경유하는 수납 동선이 생기지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#corridor-service 설비실 문을 서측 z=-0.95 구간에 배치하므로 corridor 전면의 폐쇄 경계를 출입구로 오인하지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#wall-entry-flex entry-flex X 벽이 pocket 문과 그 이동 구간을 받아 facade 유리와 다른 내부 출입 경계를 만든다.
 * @evidence spaces/002-spatial-graph.md#wall-entry-common entry maxZ와 common minZ 사이 평균면에 wall을 만들며 x=2.05 개구 외에는 닫는다.
 * @evidenceReview spaces/002-spatial-graph.md#wall-entry-common entry-common Z 벽에 동측 통로를 두어 계단의 북쪽 도착과 1층 passage를 같은 구멍으로 섞지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#wall-entry-powder entry-powder 서측 벽의 개구가 위생실 안쪽 leaf에 맞아 현관을 가로지르는 문짝을 만들지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#wall-common-storage common-storage 벽의 단일 opening이 공용부에서 수납으로 연결되며 다른 구간은 실제 벽체다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#wall-corridor-primary 복도 북쪽 Z 벽에서 primary 문만 제거하여 그 뒤 주침실과 corridor가 하나의 room으로 합쳐지지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#wall-corridor-child-one child-one 확장 cell과 corridor 사이의 짧은 X 벽에 전용 문을 내어 침실 주 cell 경계를 잘못 자르지 않는다.
 * @evidence spaces/002-spatial-graph.md#wall-corridor-child-two 복도 동측과 child-two 서측의 z 교집합을 wall로 만들고 그 범위 안에 방문을 절삭한다.
 * @evidenceReview spaces/002-spatial-graph.md#wall-corridor-child-two corridor-child-two X 벽의 z=1.05 문과 나머지 실체 구간이 child-one 벽과 별도로 생성된다.
 * @evidence spaces/002-spatial-graph.md#wall-corridor-bathroom 욕실·복도의 겹치는 z segment에 shared wall 한 개와 욕실 문 하나를 생성한다.
 * @evidenceReview spaces/002-spatial-graph.md#wall-corridor-bathroom corridor-bathroom 벽에서 z=1.86 개구만 잘라 bathroom 문의 jamb와 shared wall이 대응한다.
 * @evidence spaces/002-spatial-graph.md#wall-corridor-storage 상층 수납·복도의 x edge 평균면과 z 교집합으로 벽을 생성하고 수납 문을 소비한다.
 * @evidenceReview spaces/002-spatial-graph.md#wall-corridor-storage upper-storage와 corridor 사이 X 벽에 수납 문을 생성하며 bath 문은 이 벽의 opening으로 붙지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#wall-corridor-service service 접촉 X 벽의 실제 z 구간만 사용하므로 corridor 전면 폐쇄 벽까지 서비스 문을 연장하지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#wall-powder-storage powder-storage의 Z 분리벽을 닫아 1층 설비 코어가 통과 동선으로 변하지 않는다.
 * @evidence spaces/002-spatial-graph.md#wall-flex-common 작업실 뒤와 common 앞 사이의 전체 x 교집합은 폐쇄 wall이다. 작업실의 출입은 현관 문으로만 유지된다.
 * @evidenceReview spaces/002-spatial-graph.md#wall-flex-common flex-common Z 벽은 닫혀 있어 작업실 손님 동선이 후면 공용부로 임의 관통하지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#wall-child-one-two 두 작은 침실 사이 Z 벽은 각 cell의 접촉 구간만 덮어 L형 child-one 내부를 나누지 않는다.
 * @evidence spaces/002-spatial-graph.md#wall-child-two-primary child-two 북측과 primary 남측의 x 겹침에 문 없는 벽을 두어 침실을 통과실로 쓰지 않는다.
 * @evidenceReview spaces/002-spatial-graph.md#wall-child-two-primary child-two와 primary 사이 Z 벽이 두 침실의 독립성을 유지하고 corridor 이외 문을 만들지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#wall-service-storage service-storage Z 벽에는 문을 만들지 않아 각 실의 corridor 직접 연결을 바꾸지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#wall-storage-bath storage-bath Z 벽이 linen 수납과 위생실을 분리하므로 한쪽 문으로 두 room을 통과하지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#wall-bath-primary bath-primary의 X 경계는 양 room 사이를 폐쇄하여 주침실에서 욕실로 임의 출입을 만들지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#wall-service-corridor-front service-corridor-front Z 벽은 폐쇄된 채 유지되고 서비스 출입은 서측 X 벽에만 남는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#stair-enclosure 계단 서측 설비실과 동측 child-one 벽을 hole 밖에 두어 열린 상층 도착을 벽체로 가로막지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#wall-entry-storage entry-storage X 벽은 개구 없이 생성되어 수납 접근은 common-storage 연결 하나로 유지된다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#wall-junctions 벽 끝 접합의 남은 실체만 별도 ID로 채워 room cell·opening·계단 hole 안에 보강 덩어리를 추가하지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#envelope-interface 층·방 datum을 외피 개구 입력에 전달하므로 facade가 실내와 무관한 두 번째 층선을 만들지 않는다.
 ...
 * @evidenceReview spaces/002-spatial-graph.md#stage-one-verification native audit가 storey 포함과 entry 도달을 검사하지만 연속 인체 통행·성능 인증을 그 결과로 주장하지 않는다.
 */
