...
export function garden(a: Assembly): void {
  ...
  for (const cut of cuts)
    ground = ground.flatMap((piece) => subtract(piece, cut));
  // Grade stops at the house outline and meets the plinth; it never runs under the house.
  ground = ground.flatMap((piece) =>
    subtract(piece, [datum.minX, datum.maxX, datum.minZ, datum.maxZ]),
  );
  // Every flush paving piece is filled down to the same soil bottom.
  for (const [i, p] of ground.entries())
    a.box(
      "site-ground-" + i,
      r,
      "soil",
  ...
  for (const side of [-1, 1]) {
    const strips = subtract(
      side < 0 ? [-7.8, -5.8, -7.3, 7.3] : [5.8, 7.8, -7.3, 7.3],
      [-6.1, -5.55, -0.3, 0.3],
    );
  ...
  catchPit(a);
  for (const [i, p] of subtract([-7.8, 7.8, -8.5, -7.7], cuts[3]).entries())
    a.box(
      "site-sidewalk-" + i,
      r,
      "paving",
  ...
  for (const [id, x0, x1, z0, z1] of [
    ...
    ["rear", -7.8, 7.8, datum.maxZ, 8.5],
    ["left", datum.maxX, 7.8, datum.minZ, datum.maxZ],
    ["right", -7.8, datum.minX, datum.minZ, datum.maxZ],
  ] as const)
    a.environment.surfaces.push({
      ...
      surface: {
        id: "site-walk-" + id,
        kind: "floor",
        polygon: [v(x0, 0, z0), v(x1, 0, z0), v(x1, 0, z1), v(x0, 0, z1)],
        height: { kind: "constant", value: -0.45 },
      },
    });
  a.environment.surfaces.push({
    space: r,
    surface: {
      ...
      height: { kind: "constant", value: datum.floors[0] },
    },
  });
  for (let side = -1; side <= 1; side += 2)
    for (let i = 0; i < 24; i++) {
      const n = (side < 0 ? 0 : 24) + i;
      const z = 7.44 + Math.floor(n / 16) * 0.26,
        x = -5.1 + ((n % 16) * 10.2) / 15;
      for (let k = 0; k < 5; k++)
        a.ellipsoid(
          "hedge-" + side + "-" + i + "-" + k,
          r,
          "leaf",
          x + Math.sin(k * 2.4) * 0.2,
          -0.11 + (k % 2) * 0.1,
          z + Math.cos(k * 2.4) * 0.2,
          0.62,
          0.7,
          0.6,
        );
  ...
  for (const [i, pos] of [
    ...
    for (let j = 0; j < 11; j++) {
      const angle = j * 2.399,
        y = top - 1.8 + j * 0.16,
        radius = 0.55 + 0.35 * Math.sin(j * 0.8);
      const end = v(
      ...
      for (let k = 0; k < 5; k++)
        a.ellipsoid(
          ...
          r,
          "leaf",
          end.x + Math.cos(k * 2.4) * 0.3,
          end.y + (k % 3) * 0.13,
          end.z + Math.sin(k * 2.4) * 0.3,
          0.6,
          0.3,
          0.6,
        );
    ...
    for (const e of a.environment.elements.slice(start)) {
      ...
      e.transform.rotation = Quaternion.multiply(
        rotation,
  ...
  for (let i = 0; i < 29; i++) {
    const x = -5.5 + i * 0.39;
    if (x > 1.05 && x < 3.1) continue;
    const z = x > -4.49 && x < -2.61 ? 7.6 : -6.6;
    for (let j = 0; j < 7; j++)
      a.rod(
        "front-grass-" + i + "-" + j,
        r,
...
function catchPit(a: Assembly): void {
  const r = "citizen-site";
  a.box("catch-bottom", r, "metal", -5.825, -0.851, 0, 0.55, 0.002, 0.6);
  for (const z of [-0.299, 0.299])
    a.box(
      "catch-side-z-" + z,
      r,
      "metal",
  ...
  a.box("catch-side-inner", r, "metal", -5.551, -0.66, 0, 0.002, 0.38, 0.6);
  const wall = extrudeAutoMovieRegion({
    outer: rectangle(-0.3, 0.3, -0.852, -0.47),
    holes: [circle(0, -0.8, 0.05)],
    depth: 0.002,
  });
  putMesh(
    a,
    ...
    r,
    "metal",
    transformAutoMovieMesh(wall, {
      rotation: yaw(-Math.PI / 2),
      translation: v(-6.099, 0, 0),
    }),
  );
  const holes = Array.from({ length: 12 }, (_, i) =>
    rectangle(-0.251 + i * 0.042, -0.223 + i * 0.042, -0.278, 0.278),
  );
  const model = a.model("catch-grate-model", "steel", {
    type: "mesh",
    mesh: heightRegion(
      rectangle(-0.275, 0.275, -0.3, 0.3),
      () => -0.02,
      () => 0,
      holes,
    ),
