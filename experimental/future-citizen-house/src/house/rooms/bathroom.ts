...
export function bathroom(a: Assembly): void {
  ...
  basin(new Item(a, "bath-vanity", r, -4.99, datum.floors[1], 2.16, Math.PI / 2), 1.0);
  // The cistern backs onto the lining face x=-datum.innerX and stays clear of the
  // right-face window cut (z>=3.56).
  toilet(new Item(a, "bath-toilet", r, -4.935, datum.floors[1], 3.33, Math.PI / 2));
  const shower = new Item(a, "bath-shower", r, -4.14, datum.floors[1], 4.94);
