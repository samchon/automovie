...
export function right(a: Assembly): void {
  ...
  for (const [i, range] of [[-0.30, 0.10], [0.30, 6.10]].entries())
    putMesh(a, "right-downpipe-" + i, "house", "canopy-metal", tubeMesh(-5.68, 0, range[0], range[1], 0.055, 0.05), "drainage");
  const half = Math.asin(0.04 / 0.055);
  putMesh(a, "right-inspection-body", "house", "canopy-metal", pipeSector(-5.68, 0, 0.10, 0.30, Math.PI + half, 3 * Math.PI - half), "drainage");
  ...
  for (const y of [5.80, 4.00, 2.20, 0.40]) {
    putMesh(a, "right-downpipe-collar-" + y, "house", "canopy-metal", tubeMesh(-5.68, 0, y - 0.01, y + 0.01, 0.059, 0.055), "pipe-clip");
    a.box("right-downpipe-clip-" + y, "house", "canopy-metal", (-5.621 - 5.50) / 2, y, 0, 0.121, 0.02, 0.02);
  }
}
