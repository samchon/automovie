...
/** Clear cell limits remain the source of each opening's measured edge. */
export const roomEdge = (
  id: string,
  axis: "x" | "z",
  end: "min" | "max",
  cell = 0,
): number =>
  roomById(id).cells[cell][(axis === "x" ? 0 : 2) + (end === "max" ? 1 : 0)];
export const glazingInset = 0.04;
export const standardWindowHeight = (level: 0 | 1) => ({
...
export function exteriorFrame(
  ...
  const along = face === "front" || face === "rear" ? "x" : "z";
  const normal: 1 | -1 = face === "rear" || face === "left" ? 1 : -1;
  const edge =
    face === "front"
      ? datum.minZ
      : face === "rear"
...
export function facade(
  ...
  f: Frame,
  windows: Glazing[],
  extraCuts: ReturnType<typeof localCut>[] = [],
): void {
  // A narrow solid return remains at the frame end even when glazing follows
  // a moving room clear edge; this also avoids a zero-width end wall cell.
  const cuts = [
    ...windows.map((w) =>
      localCut(
        f,
        w.id,
        Math.max(f.a + 0.002, w.a - 0.04),
        Math.min(f.b - 0.002, w.b + 0.04),
        w.sill - 0.04,
        w.head + 0.04,
      ),
    ),
    ...
    for (let j = 0; j < ny; j++) {
      let rects: Rect[] = [
        [
          ...
          ((i + 1) * (f.b - f.a)) / nx,
          (j * (f.top - f.floor)) / ny,
          ((j + 1) * (f.top - f.floor)) / ny,
        ],
      ];
      for (const c of cuts)
        rects = rects.flatMap((r) =>
          subtract(r, [c.x, c.x + c.width, c.y, c.y + c.height]),
        );
      for (const [k, r] of rects.entries()) {
        ...
        const u =
          (f.a + f.b) / 2 +
          ((r[0] + r[1]) / 2 - (f.b - f.a) / 2) *
            (f.along === "x" ? f.normal : -f.normal);
        // Population transforms are world metrics, not a second facade model.
        panels.push({
          id: i + "-" + j + "-" + k,
          translation: position(f, u, f.floor + (r[2] + r[3]) / 2, outer),
          scale:
            f.along === "x"
              ? v(r[1] - r[0] - 0.004, r[3] - r[2] - 0.004, 0.016)
              : v(0.016, r[3] - r[2] - 0.004, r[1] - r[0] - 0.004),
...
function glazing(a: Assembly, f: Frame, w: Glazing): void {
  ...
  const d0 = Math.max(w.a - 0.06, f.a),
    d1 = Math.min(w.b + 0.06, f.b);
  bar(
    a,
    f,
    w.id + "-drip",
  ...
  for (let i = 0; i < centres.length - 1; i++) {
    const left = centres[i] + 0.02,
      right = centres[i + 1] - 0.02;
    ...
    const drop =
      w.privacy === "all"
        ...
        : a.state.privacy === "day"
          ? 0.045
          : a.state.privacy === "night" || w.privacy === "lower"
            ? 1
            : 0.6;
    bar(
      a,
  ...
  a.environment.openings.push({
    ...
    profile: {
      outline: rectangle(
        Math.min(localU(f, w.a), localU(f, w.b)),
        Math.max(localU(f, w.a), localU(f, w.b)),
        w.sill - (f.floor + f.top) / 2,
        w.head - (f.floor + f.top) / 2,
      ),
    },
  });
}
...
export function corners(
  ...
  const signs =
    face === "front" || face === "rear"
      ? [-1, 1].map((x) => [x, face === "front" ? -1 : 1])
      : [-1, 1].map((z) => [face === "left" ? 1 : -1, z]);
  for (const [sx, sz] of signs) {
    const ix = sx * datum.innerX,
      iz = sz * datum.innerZ;
    const ox = sx < 0 ? datum.minX : datum.maxX,
      oz = sz < 0 ? datum.minZ : datum.maxZ;
    const inner = { x: ix, y: iz },
      outer = { x: ox, y: oz };
    const edge =
      face === "front" || face === "rear" ? { x: ix, y: oz } : { x: ox, y: iz };
    const profile = [inner, edge, outer];
    const height = datum.ceilings[1] - datum.floors[0];
    const mesh = extrudeAutoMovieProfile({ profile, depth: height });
    const element = a.place(
      name + "-corner-" + sx + "-" + sz,
      "corner",
      "house",
      a.model(name + "-corner-mesh-" + sx + "-" + sz, "stone", {
        type: "mesh",
        mesh,
      }),
      v(0, (datum.floors[0] + datum.ceilings[1]) / 2, 0),
      v(1, 1, 1),
      { x: Math.SQRT1_2, y: 0, z: 0, w: Math.SQRT1_2 },
    );
    const boundary = a.environment.boundaries.find(
      (b) => b.id === name + "-face",
    )!;
    boundary.elements.push(element);
    if (boundary.face) {
      const low = face === "front" || face === "rear" ? datum.minX : datum.minZ;
      const high =
        face === "front" || face === "rear" ? datum.maxX : datum.maxZ;
      boundary.face.outline = rectangle(low, high, -height / 2, height / 2);
    }
  }
}
