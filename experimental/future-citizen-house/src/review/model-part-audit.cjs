...
function parse(lines, anchor) {
  ...
  for (const line of lines) {
    ...
    const minimum = /^@cavity-min\s+([^:]+):\s*([^,]+),\s*(\d+)\/(\d+),\s*(\d+)\/(\d+)$/.exec(line);
    if (minimum) {
      ...
      if (cavityMinima.has(key) || !Number.isFinite(floor) || !Number.isFinite(wall) ||
        floor <= 0 || wall <= 0 || floor >= 1 || wall >= 0.5)
        throw Error(`${anchor}/${key}: invalid cavity minimum`);
      cavityMinima.set(key, { floor, wall });
    }
    const sole = /^@sole-grid\s+([^:]+):\s*([^,]+),\s*(\d+),\s*(\d+),\s*(\d+),\s*(\d+)$/.exec(line);
    if (sole) {
      const key = `${sole[1].trim()}/${sole[2].trim()}`;
      const divisors = sole.slice(3).map(Number);
      if (soleGrids.has(key) || divisors.some((value) => !Number.isFinite(value) || value <= 0))
        throw Error(`${anchor}/${key}: invalid sole grid`);
      soleGrids.set(key, { widthDivisor: divisors[0], depthDivisor: divisors[1],
        xInsetDivisor: divisors[2], zInsetDivisor: divisors[3] });
    }
  ...
  return { envelopes, parts, inventory, compositions, supportBindings, pinFaces, emitterFaces, miterJoints, voids, cavityMinima, soleGrids, pieces, shearsZ, radial, radialZ, bores, cavityProfiles, boresZ, boresX, ellipses, tangents, flatContacts, capContacts, cavityContacts, curveLayers, linearCurves, grids };
}
    ...
    for (const [state, envelope] of envelopes) {
      ...
      for (const [key, patch] of flatContacts) {
        ...
        if (guest && soleGrid && patch.axis === "-Y") {
          const width = guest.x[1] - guest.x[0], depth = guest.z[1] - guest.z[0];
          ...
          valid &&= [-1, 1].some((sx) => [-1, 1].some((sz) => {
            const x = centerX + sx * (width / 2 - width / soleGrid.xInsetDivisor);
            const z = centerZ + sz * (depth / 2 - depth / soleGrid.zInsetDivisor);
            return inRange(patch.u, [x - halfX, x + halfX]) &&
      ...
      for (const [key, minimum] of cavityMinima) {
        ...
        if (!host || host.shape !== "hollow" || cavities.length !== 1) {
          errors.push(`${anchor}/${key}: sealed cavity needs one hollow host and one void`);
          continue;
        }
        const cavity = cavities[0];
        const floor = cavity.y[0] - host.y[0];
        const walls = [cavity.x[0] - host.x[0], host.x[1] - cavity.x[1],
          cavity.z[0] - host.z[0], host.z[1] - cavity.z[1]];
        const minWall = Math.min(host.x[1] - host.x[0], host.z[1] - host.z[0]) * minimum.wall;
        if (Math.abs(cavity.y[1] - host.y[1]) > epsilon ||
          floor + epsilon < (host.y[1] - host.y[0]) * minimum.floor ||
          walls.some((width) => width + epsilon < minWall))
          errors.push(`${anchor}/${key}: sealed cavity floor or wall below declared minimum`);
      }
...
if (require.main !== module) {
  ...
  for (const [anchor, before, after] of [
    ["rear-counter-sink", "@void default: bowl, 0.915..1.285, 0.82..0.925", "@void default: bowl, 0.915..1.285, 0.80..0.925"],
    ["bath-accessories", "@void laundry-basket: body, -0.22..0.22, 0.033..0.4", "@void laundry-basket: body, -0.22..0.22, 0.001..0.4"]
  ]) {
    const lines = sections().get(anchor);
    if (!lines || !lines.join("\n").includes(before)) throw Error(`${anchor}: cavity fixture source absent`);
  ...
  if (!exterior || !exterior.join("\n").includes(oldPatch)) throw Error("outdoor sole fixture source absent");
  const movedErrors = audit(new Map([["exterior-furnishings",
    exterior.join("\n").replace(oldPatch, movedPatch).split("\n")]])).errors;
  if (!movedErrors.some((error) => error.includes("flat contact has no finite common patch")))
