...
function sections() {
  ...
  for (const name of names) {
    const lines = fs
      .readFileSync(path.join(root, "docs/models", `${name}.md`), "utf8")
      .split(/\r?\n/);
    let anchor;
...
function parse(lines, anchor) {
  ...
  const voids = new Map();
  /** @type {Map<string,{floor:number,wall:number}>} */
  const cavityMinima = new Map();
  ...
  /** @type {Map<string,{widthDivisor:number,depthDivisor:number,xInsetDivisor:number,zInsetDivisor:number}>} */
  const soleGrids = new Map();
  ...
  for (const line of lines) {
    const cap =
      /^@cap-contact\s+([^:]+):\s*([^,]+),\s*([^,]+),\s*([XYZ]),\s*([+-])$/.exec(
        line,
      );
    if (cap) {
      const state = cap[1].trim(),
        first = cap[2].trim(),
        second = cap[3].trim();
      const key = `${state}/${[first, second].sort((a, b) => a.localeCompare(b)).join("/")}`;
      if (capContacts.has(key))
        throw Error(`${anchor}/${key}: duplicate cap contact`);
      capContacts.set(key, {
        first,
        second,
        axis: cap[4].toLowerCase(),
        side: cap[5],
      });
    }
    const cavityContact =
      /^@cavity-contact\s+([^:]+):\s*([^,]+),\s*([^,]+),\s*([XYZ])$/.exec(line);
    if (cavityContact) {
      const state = cavityContact[1].trim(),
        host = cavityContact[2].trim(),
        guest = cavityContact[3].trim();
      const key = `${state}/${[host, guest].sort((a, b) => a.localeCompare(b)).join("/")}`;
      if (cavityContacts.has(key))
        throw Error(`${anchor}/${key}: duplicate cavity contact`);
      cavityContacts.set(key, {
        host,
        guest,
        axis: cavityContact[4].toLowerCase(),
      });
    }
    const support =
      /^@support\s+([^:]+):\s*([^,]+),\s*([^,]+),\s*([^,]+),\s*([−-]?[\d.]+),\s*([−-]?[\d.]+),\s*([−-]?[\d.]+)$/.exec(
        line,
      );
    if (support) {
      const state = support[1].trim();
      if (supportBindings.has(state))
        throw Error(`${anchor}/${state}: duplicate support binding`);
      const offset = /** @type {[number,number,number]} */ support
        .slice(5, 8)
        .map((value) => Number(value.replace("−", "-")));
      if (!offset.every(Number.isFinite))
        throw Error(`${anchor}/${state}: invalid support offset`);
      supportBindings.set(state, {
        anchor: support[2].trim(),
        state: support[3].trim(),
        part: support[4].trim(),
        offset,
      });
    }
    ...
    if (pin) {
      ...
      if (pinFaces.has(key))
        throw Error(`${anchor}/${key}: duplicate pin face`);
      pinFaces.set(key, pin[3].trim());
    }
    const emitter = /^@emitter-face\s+([^:]+):\s*([^,]+),\s*(-Y)$/.exec(line);
    if (emitter) {
      const state = emitter[1].trim();
      if (emitterFaces.has(state))
        throw Error(`${anchor}/${state}: duplicate emitter face`);
      emitterFaces.set(state, emitter[2].trim());
    }
    const composed =
      /^@compose\s+([^:]+):\s*([^,]+),\s*([^,]+),\s*([−-]?[\d.]+),\s*([−-]?[\d.]+),\s*([−-]?[\d.]+)$/.exec(
        line,
      );
    if (composed) {
      const state = composed[1].trim();
      if (compositions.has(state))
        throw Error(`${anchor}: duplicate composition ${state}`);
      const offset = /** @type {[number,number,number]} */ composed
        .slice(4, 7)
        .map((value) => Number(value.replace("−", "-")));
      if (!offset.every(Number.isFinite))
        throw Error(`${anchor}/${state}: invalid composition offset`);
      compositions.set(state, {
        anchor: composed[2].trim(),
        state: composed[3].trim(),
        offset,
      });
    }
    ...
    if (list) {
      ...
      if (inventory.has(state))
        throw Error(`${anchor}: duplicate inventory ${state}`);
      const names = list[2]
        .split(",")
        ...
        .flatMap((name) => {
          ...
          if (end < start || end - start > 999)
            throw Error(`${anchor}: invalid inventory range ${name}`);
          return Array.from(
            { length: end - start + 1 },
            (_, i) => `${range[1]}-${start + i}`,
          );
    ...
    const joint =
      /^@joint\s+([^:]+):\s*([^,]+),\s*([^,]+),\s*(miter45-xz)$/.exec(line);
    if (joint)
      miterJoints.add(
        `${joint[1].trim()}/${[joint[2].trim(), joint[3].trim()].sort((a, b) => a.localeCompare(b)).join("/")}`,
      );
    const cut =
      /^@void\s+([^:]+):\s*([^,]+),\s*([^,]+),\s*([^,]+),\s*([^,]+)$/.exec(
        line,
    ...
    if (cut) {
      const state = cut[1].trim(),
        host = cut[2].trim(),
        key = `${state}/${host}`;
      const region = {
        x: interval(cut[3], `${anchor}/${key}/void-x`),
        y: interval(cut[4], `${anchor}/${key}/void-y`),
        z: interval(cut[5], `${anchor}/${key}/void-z`),
      };
      voids.set(key, [...(voids.get(key) || []), region]);
    }
    const minimum =
      /^@cavity-min\s+([^:]+):\s*([^,]+),\s*(\d+)\/(\d+),\s*(\d+)\/(\d+)$/.exec(
        line,
      );
    if (minimum) {
      const key = `${minimum[1].trim()}/${minimum[2].trim()}`;
      const floor = Number(minimum[3]) / Number(minimum[4]);
      const wall = Number(minimum[5]) / Number(minimum[6]);
      if (
        cavityMinima.has(key) ||
        ...
        throw Error(`${anchor}/${key}: invalid cavity minimum`);
      cavityMinima.set(key, { floor, wall });
    }
    const sole =
      /^@sole-grid\s+([^:]+):\s*([^,]+),\s*(\d+),\s*(\d+),\s*(\d+),\s*(\d+)$/.exec(
        line,
      );
    if (sole) {
      const key = `${sole[1].trim()}/${sole[2].trim()}`;
      const divisors = sole.slice(3).map(Number);
      if (
        soleGrids.has(key) ||
        divisors.some((value) => !Number.isFinite(value) || value <= 0)
      )
        throw Error(`${anchor}/${key}: invalid sole grid`);
      soleGrids.set(key, {
        widthDivisor: divisors[0],
        depthDivisor: divisors[1],
        xInsetDivisor: divisors[2],
        zInsetDivisor: divisors[3],
      });
    }
    const segment =
      /^@piece\s+([^:]+):\s*([^,]+),\s*([^,]+),\s*([^,]+),\s*([^,]+)$/.exec(
        line,
      );
    if (segment) {
      const state = segment[1].trim(),
        host = segment[2].trim(),
        key = `${state}/${host}`;
      const region = {
        x: interval(segment[3], `${anchor}/${key}/piece-x`),
        y: interval(segment[4], `${anchor}/${key}/piece-y`),
        z: interval(segment[5], `${anchor}/${key}/piece-z`),
      };
      pieces.set(key, [...(pieces.get(key) || []), region]);
    }
    const shear =
      /^@shear-z\s+([^:]+):\s*([^,]+),\s*([^,]+),\s*([^,]+),\s*([\d.]+)$/.exec(
        line,
      );
    if (shear) {
      ...
      shearsZ.set(key, {
        y: interval(shear[3], `${anchor}/${key}/shear-y`),
        z: interval(shear[4], `${anchor}/${key}/shear-z`),
        halfDepth: Number(shear[5]),
      });
    }
    const ring = /^@radial\s+([^:]+):\s*([^,]+),\s*([\d.]+),\s*([\d.]+)$/.exec(
      line,
    );
    if (ring) {
      const key = `${ring[1].trim()}/${ring[2].trim()}`;
      if (radial.has(key))
        throw Error(`${anchor}/${key}: duplicate radial declaration`);
      const inner = Number(ring[3]),
        outer = Number(ring[4]);
      if (!(inner >= 0 && outer > inner))
        throw Error(`${anchor}/${key}: invalid radial interval`);
      radial.set(key, { inner, outer, centerX: 0, centerZ: 0 });
    }
    const offsetRing =
      /^@radial-at\s+([^:]+):\s*([^,]+),\s*([−-]?[\d.]+),\s*([−-]?[\d.]+),\s*([\d.]+),\s*([\d.]+)$/.exec(
        line,
      );
    if (offsetRing) {
      ...
      if (radial.has(key))
        throw Error(`${anchor}/${key}: duplicate radial declaration`);
      const centerX = Number(offsetRing[3].replace("−", "-")),
        centerZ = Number(offsetRing[4].replace("−", "-"));
      const inner = Number(offsetRing[5]),
        outer = Number(offsetRing[6]);
      if (!(inner >= 0 && outer > inner))
        throw Error(`${anchor}/${key}: invalid offset radial interval`);
      radial.set(key, { inner, outer, centerX, centerZ });
    }
    const faceRing =
      /^@radial-z\s+([^:]+):\s*([^,]+),\s*([−-]?[\d.]+),\s*([−-]?[\d.]+),\s*([\d.]+),\s*([\d.]+)$/.exec(
        line,
      );
    if (faceRing) {
      const key = `${faceRing[1].trim()}/${faceRing[2].trim()}`;
      if (radialZ.has(key))
        throw Error(`${anchor}/${key}: duplicate radial-z declaration`);
      const [centerX, centerY, inner, outer] = faceRing
        .slice(3)
        .map((value) => Number(value.replace("−", "-")));
      if (!(inner >= 0 && outer > inner))
        throw Error(`${anchor}/${key}: invalid radial-z interval`);
      radialZ.set(key, { inner, outer, centerX, centerY });
    }
    const bore = /^@bore\s+([^:]+):\s*([^,]+),\s*([\d.]+),\s*(.+)$/.exec(line);
    if (bore)
      bores.set(`${bore[1].trim()}/${bore[2].trim()}`, {
        radius: Number(bore[3]),
        y: interval(bore[4], `${anchor}/bore-y`),
      });
    const profile =
      /^@cavity-profile\s+([^:]+):\s*([^,]+),\s*(round|ellipse),\s*(\d+),\s*(\d+)\/(\d+),\s*(\d+),\s*([\d.]+)$/.exec(
        line,
      );
    if (profile) {
      const key = `${profile[1].trim()}/${profile[2].trim()}`;
      if (cavityProfiles.has(key))
        throw Error(`${anchor}/${key}: duplicate cavity profile`);
      cavityProfiles.set(key, {
        section: /** @type {"round"|"ellipse"} */ profile[3],
        bottomDivisor: Number(profile[4]),
        shoulderNumerator: Number(profile[5]),
        shoulderDenominator: Number(profile[6]),
        wallDivisor: Number(profile[7]),
        mouthRadius: Number(profile[8]),
      });
    }
    const boreZ =
      /^@bore-z\s+([^:]+):\s*([^,]+),\s*([−-]?[\d.]+),\s*([−-]?[\d.]+),\s*([\d.]+),\s*(.+)$/.exec(
        line,
      );
    if (boreZ) {
      ...
      boresZ.set(key, {
        centerX: Number(boreZ[3].replace("−", "-")),
        centerY: Number(boreZ[4].replace("−", "-")),
        radius: Number(boreZ[5]),
        z: interval(boreZ[6], `${anchor}/${key}/bore-z`),
      });
    }
    const boreX =
      /^@bore-x\s+([^:]+):\s*([^,]+),\s*([^,]+),\s*([−-]?[\d.]+),\s*([−-]?[\d.]+),\s*([\d.]+)$/.exec(
        line,
      );
    if (boreX) {
      ...
      boresX.set(key, {
        x: interval(boreX[3], `${anchor}/${key}/bore-x`),
        centerY: Number(boreX[4].replace("−", "-")),
        centerZ: Number(boreX[5].replace("−", "-")),
        radius: Number(boreX[6]),
      });
    }
    const ellipse =
      /^@ellipse\s+([^:]+):\s*([^,]+),\s*([\d.]+),\s*([\d.]+),\s*([\d.]+),\s*([\d.]+)(?:,\s*([−-]?[\d.]+),\s*([−-]?[\d.]+))?$/.exec(
        line,
      );
    if (ellipse)
      ellipses.set(`${ellipse[1].trim()}/${ellipse[2].trim()}`, {
        innerX: Number(ellipse[3]),
        innerZ: Number(ellipse[4]),
        outerX: Number(ellipse[5]),
        outerZ: Number(ellipse[6]),
        centerX: Number((ellipse[7] || "0").replace("−", "-")),
        centerZ: Number((ellipse[8] || "0").replace("−", "-")),
      });
    const tangent =
      /^@tangent\s+([^:]+):\s*([^,]+),\s*([^,]+),\s*([\d.]+),\s*([\d.]+)$/.exec(
        line,
      );
    if (tangent) {
      const state = tangent[1].trim(),
        host = tangent[2].trim(),
        guest = tangent[3].trim();
      const key = `${state}/${[host, guest].sort((a, b) => a.localeCompare(b)).join("/")}`;
      if (tangents.has(key))
        throw Error(`${anchor}/${key}: duplicate tangent proof`);
      tangents.set(key, {
        host,
        guest,
        radius: Number(tangent[4]),
        halfWidth: Number(tangent[5]),
      });
    }
    const flat =
      /^@flat-contact\s+([^:]+):\s*([^,]+),\s*([^,]+),\s*(-Y|-Z),\s*([−-]?[\d.]+),\s*([^,]+),\s*([^,]+)$/.exec(
        line,
      );
    if (flat) {
      const state = flat[1].trim(),
        guest = flat[2].trim(),
        host = flat[3].trim();
      const key = `${state}/${guest}/${host}`;
      if (flatContacts.has(key))
        throw Error(`${anchor}/${key}: duplicate flat contact`);
      flatContacts.set(key, {
        guest,
        host,
        axis: flat[4],
        plane: Number(flat[5].replace("−", "-")),
        u: interval(flat[6], `${anchor}/${key}/flat-u`),
        v: interval(flat[7], `${anchor}/${key}/flat-v`),
      });
    }
    const curve =
      /^@curve-layer\s+([^:]+):\s*([^,]+),\s*([^,]+),\s*([−-]?[\d.]+),\s*([−-]?[\d.]+),\s*([−-]?[\d.]+),\s*([\d.]+),\s*([\d.]+),\s*([\d.]+)$/.exec(
        line,
      );
    if (curve) {
      const [state, shell, cover] = curve
        .slice(1, 4)
        .map((value) => value.trim());
      const key = `${state}/${[shell, cover].sort((a, b) => a.localeCompare(b)).join("/")}`;
      if (curveLayers.has(key))
        throw Error(`${anchor}/${key}: duplicate curve layer`);
      const [c0, c1, c2, shellDepth, coverDepth, seatGap] = curve
        .slice(4)
        .map((value) => Number(value.replace("−", "-")));
      curveLayers.set(key, {
        shell,
    ...
    const linear =
      /^@curve-linear\s+([^:]+):\s*([^,]+),\s*([^,]+),\s*([−-]?[\d.]+),\s*([\d.]+),\s*([−-]?[\d.]+),\s*([−-]?[\d.]+),\s*([\d.]+),\s*([\d.]+),\s*([\d.]+)$/.exec(
        line,
      );
    if (linear) {
      const [state, host, guest] = linear
        .slice(1, 4)
        .map((value) => value.trim());
      const key = `${state}/${[host, guest].sort((a, b) => a.localeCompare(b)).join("/")}`;
      if (linearCurves.has(key))
        throw Error(`${anchor}/${key}: duplicate linear curve`);
      const [originY, spanY, c0, c1, hostDepth, gap, guestDepth] = linear
        .slice(4)
        .map((value) => Number(value.replace("−", "-")));
      linearCurves.set(key, {
        host,
        guest,
        originY,
        spanY,
    ...
    if (grid) {
      ...
      if (tokens.length !== 9)
        throw Error(`${anchor}: grid requires nine values`);
      const [prefix, cols, rows, pitchX, pitchZ, width, depth, ys, contact] =
        tokens;
      ...
      if (
        ...
        dimensions[0] < 1 ||
        dimensions[1] < 1 ||
        dimensions[0] * dimensions[1] > 999
      )
        throw Error(`${anchor}: invalid grid dimensions`);
      if (grids.has(grid[1].trim()))
        throw Error(`${anchor}: duplicate grid state`);
      grids.set(grid[1].trim(), {
        prefix,
        cols: dimensions[0],
        rows: dimensions[1],
        pitchX: dimensions[2],
  ...
  for (const line of rows) {
    const cells = line
      ...
      .map((cell) => cell.trim());
    if (cells.length !== 8)
      throw Error(`${anchor}: eight cells required in ${line}`);
    const [kind, state, id, shape, xs, ys, zs, contact] = cells;
    const entry = {
      state,
      id,
      shape,
      x: interval(xs, `${anchor}/${state}/${id}/x`),
      y: interval(ys, `${anchor}/${state}/${id}/y`),
      z: interval(zs, `${anchor}/${state}/${id}/z`),
      contact:
        contact === "-" ? [] : contact.split(",").map((value) => value.trim()),
    };
    const key = `${state}/${id}`;
    if (kind === "@envelope") {
      ...
      if (
        !["box", "cylinder", "curved", "hollow", "mitered-box"].includes(
          shape,
        ) ||
        parts.has(key)
      )
    ...
    for (let row = 0; row < grid.rows; row++)
      for (let col = 0; col < grid.cols; col++) {
        ...
        const entry = {
          ...
          y: grid.y,
          z: /** @type {[number,number]} */ [
            centerZ - grid.depth / 2,
            centerZ + grid.depth / 2,
          ],
          contact: [grid.contact],
        };
        if (parts.has(`${state}/${id}`))
          throw Error(`${anchor}: grid part duplicate ${state}/${id}`);
...
function visibleEmitterFraction(emitter, blockers, radial, state) {
  ...
  for (let ix = 0; ix < steps; ix++)
    for (let iz = 0; iz < steps; iz++) {
      const x =
        emitter.x[0] + ((ix + 0.5) * (emitter.x[1] - emitter.x[0])) / steps;
      const z =
        emitter.z[0] + ((iz + 0.5) * (emitter.z[1] - emitter.z[0])) / steps;
      if (radius) {
...
function pinFaceArea(pin, target) {
  const endMeetsFace =
    Math.abs(pin.x[1] - target.x[0]) <= epsilon ||
  ...
  for (let i = 0; i < slices; i++) {
    ...
    area +=
      Math.max(
        0,
        Math.min(target.y[1], centerY + reach) -
          Math.max(target.y[0], centerY - reach),
      ) * dz;
  }
  return area;
}
...
function overlap(a, b) {
  return /** @type {const} */ ["x", "y", "z"].map(
    (axis) =>
      Math.min(a[axis][1], b[axis][1]) - Math.max(a[axis][0], b[axis][0]),
  );
}
...
function surfaceContact(a, b) {
  ...
  return (
    separation.every((size) => size >= -epsilon) &&
...
function subtractBox(box, cavity) {
  ...
  if (
    /** @type {const} */ ["x", "y", "z"].some(
      (axis) => cut[axis][1] - cut[axis][0] <= epsilon,
    )
  )
    return [box];
  /** @type {Bounds[]} */
  ...
  const add = (x, y, z) => {
    if ([x, y, z].every(([lo, hi]) => hi - lo > epsilon))
      result.push({ x, y, z });
  };
 ...
 */
function exactRectangularOccupancy(
  part,
  voids,
  pieces,
  radial,
...
function cylinderAxis(part) {
  ...
  const possibilities = /** @type {const} */ ["x", "y", "z"].filter((axis) => {
    const others = /** @type {const} */ ["x", "y", "z"].filter(
      (candidate) => candidate !== axis,
    );
    return (
      Math.abs(lengths[others[0]] - lengths[others[1]]) <= epsilon &&
...
function cylinderCapArea(cylinder, targetBox, axis) {
  const other = /** @type {const} */ ["x", "y", "z"].filter(
    (candidate) => candidate !== axis,
  );
  const u = other[0],
    v = other[1];
  if (
    !(
      Math.abs(cylinder[axis][0] - targetBox[axis][1]) <= epsilon ||
      Math.abs(cylinder[axis][1] - targetBox[axis][0]) <= epsilon
    )
  )
    return 0;
  ...
  for (let i = 0; i < steps; i++) {
    ...
    area +=
      Math.max(
        0,
        Math.min(targetBox[v][1], centerV + reach) -
          Math.max(targetBox[v][0], centerV - reach),
      ) * du;
  }
  return area;
}
...
function capContactArea(
  ...
  const transverse = /** @type {const} */ ["x", "y", "z"].filter(
    (candidate) => candidate !== axis,
  );
  ...
  const filled = (part, a, b) => {
    ...
    if (axis === "y") {
      ...
      if (ellipse) {
        const x = a - ellipse.centerX,
          z = b - ellipse.centerZ;
        if (
          (x / ellipse.outerX) ** 2 + (z / ellipse.outerZ) ** 2 > 1 ||
          (x / ellipse.innerX) ** 2 + (z / ellipse.innerZ) ** 2 < 1
        )
          return false;
      }
      ...
      if (
        bore &&
        plane >= bore.y[0] - epsilon &&
        plane <= bore.y[1] + epsilon &&
        Math.hypot(a, b) < bore.radius
      )
        return false;
    }
    if (axis === "z") {
      ...
      if (
        bore &&
        plane >= bore.z[0] - epsilon &&
        plane <= bore.z[1] + epsilon &&
...
function boredPinContact(host, pin, bore) {
  ...
  return {
    contact: arcFacets > 0 && Math.abs(radius - bore.radius) <= epsilon,
...
function planarContactOutsideBores(a, b, boresZ, voids, pieces) {
  if (
    ![a, b].some((part) => boresZ.has(`${part.state}/${part.id}`)) ||
    [a, b].some((part) => !["box", "hollow"].includes(part.shape))
  )
    return false;
  for (const first of occupiedBoxes(a, voids, pieces))
    for (const second of occupiedBoxes(b, voids, pieces)) {
      ...
      const normal = /** @type {const} */ ["x", "y", "z"].find(
        (axis) =>
          Math.abs(
            Math.min(first[axis][1], second[axis][1]) -
              Math.max(first[axis][0], second[axis][0]),
          ) <= epsilon,
      );
      if (!normal) continue;
      const [u, v] = /** @type {const} */ ["x", "y", "z"].filter(
        (axis) => axis !== normal,
      );
      ...
      for (let i = 0; i < samples; i++)
        for (let j = 0; j < samples; j++) {
          ...
          const cut = [a, b].some((part) => {
            ...
            return (
              bore &&
              point.z >= bore.z[0] - epsilon &&
              point.z <= bore.z[1] + epsilon &&
              Math.hypot(point.x - bore.centerX, point.y - bore.centerY) <
                bore.radius
            );
          });
          if (!cut) uncut++;
...
function actualContact(
  ...
  for (const [host, guest] of [
    ...
    if (
      bore &&
      ring &&
      Math.abs(bore.centerX - ring.centerX) <= epsilon &&
      Math.abs(bore.centerY - ring.centerY) <= epsilon &&
      Math.min(bore.z[1], guest.z[1]) - Math.max(bore.z[0], guest.z[0]) >
        epsilon
    )
      return Math.abs(ring.outer - bore.radius) <= epsilon;
  }
  const ar = radial.get(`${a.state}/${a.id}`),
    br = radial.get(`${b.state}/${b.id}`);
  if (
    ar &&
    br &&
    Math.abs(ar.centerX - br.centerX) <= epsilon &&
    Math.abs(ar.centerZ - br.centerZ) <= epsilon
  ) {
    ...
    return (
      (Math.abs(vertical) <= epsilon && radius > epsilon) ||
      (vertical > epsilon &&
        (Math.abs(ar.outer - br.inner) <= epsilon ||
          Math.abs(br.outer - ar.inner) <= epsilon))
    );
  }
  const az = radialZ.get(`${a.state}/${a.id}`),
    bz = radialZ.get(`${b.state}/${b.id}`);
  if (
    az &&
    bz &&
    Math.abs(az.centerX - bz.centerX) <= epsilon &&
    Math.abs(az.centerY - bz.centerY) <= epsilon
  ) {
    const depth = Math.min(a.z[1], b.z[1]) - Math.max(a.z[0], b.z[0]);
    const radius = Math.min(az.outer, bz.outer) - Math.max(az.inner, bz.inner);
    return (
      (Math.abs(depth) <= epsilon && radius > epsilon) ||
      (depth > epsilon &&
        (Math.abs(az.outer - bz.inner) <= epsilon ||
          Math.abs(bz.outer - az.inner) <= epsilon))
    );
  }
  const contact = occupiedBoxes(a, voids, pieces).some((aa) =>
    occupiedBoxes(b, voids, pieces).some((bb) => surfaceContact(aa, bb)),
  );
  const exactA = exactRectangularOccupancy(
    a,
    voids,
    pieces,
    radial,
  ...
  const exactB = exactRectangularOccupancy(
    b,
    voids,
    pieces,
    radial,
  ...
  const finiteCap =
    (capA &&
      exactB &&
      occupiedBoxes(b, voids, pieces).some(
        (box) => cylinderCapArea(a, box, capA) > 1e-8,
      )) ||
    (capB &&
      exactA &&
      occupiedBoxes(a, voids, pieces).some(
        (box) => cylinderCapArea(b, box, capB) > 1e-8,
      ));
  if (
    contact &&
    [a.shape, b.shape].some((shape) =>
      ["curved", "cylinder", "hollow"].includes(shape),
    ) &&
    !(exactA && exactB) &&
    !finiteCap &&
...
function actualOverlap(
  ...
  if (provedTangents.has(key) || provedCurves.has(key) || provedLinear.has(key))
    return false;
  for (const [host, guest] of [
    [a, b],
    [b, a],
  ...
  if (
    ...
    return (
      Math.min(a.y[1], b.y[1]) - Math.max(a.y[0], b.y[0]) > epsilon &&
  ...
  if (
    ...
    return (
      Math.min(a.z[1], b.z[1]) - Math.max(a.z[0], b.z[0]) > epsilon &&
      Math.min(az.outer, bz.outer) - Math.max(az.inner, bz.inner) > epsilon
    );
  return occupiedBoxes(a, voids, pieces).some((aa) =>
    occupiedBoxes(b, voids, pieces).some((bb) =>
      overlap(aa, bb).every((size) => size > epsilon),
    ),
  );
}
...
function towelFormula(lines, envelopes, parts) {
  ...
  if (!heights || !rule || !gap || !footprint)
    return ["folded-towels: prose formula or variant list absent"];
  const errors = [];
  const expectedStates = heights
    .slice(1)
    .map((value) => String(Math.round(Number(value) * 1000)));
  if (expectedStates.join(",") !== [...envelopes.keys()].join(","))
    errors.push(
      "folded-towels: table states differ from prose variant heights",
    );
  ...
  const near = (state, id, axis, observed, expected) => {
    if (Math.abs(observed - expected) > 0.0000001)
      errors.push(
        `folded-towels/${state}/${id}/${axis}: table ${observed} differs from prose formula ${expected}`,
      );
  };
...
function chargerFormula(lines, envelopes, voids) {
  ...
  const interfaceCut =
    /본체 위쪽 x=±([\d.]+),z=±([\d.]+),y=([\d.]+)\.\.([\d.]+)m/.exec(prose);
  const portCut =
    /앞쪽 edge z=\+([\d.]+)\.\.\+([\d.]+),x=±([\d.]+),y=([\d.]+)\.\.([\d.]+)m/.exec(
      prose,
    );
  if (!outer || !interfaceCut || !portCut)
    return ["entry-charger: prose dimensions absent"];
  const envelope = envelopes.get("default"),
    cuts = voids.get("default/body");
  if (!envelope || !cuts || cuts.length !== 2)
    return ["entry-charger: measured envelope/voids absent"];
  /** @type {[number[],number[]][]} */
  const expected = [
  ...
  return expected.flatMap(([actual, target], index) =>
    Math.abs(actual[0] - target[0]) > epsilon ||
    Math.abs(actual[1] - target[1]) > epsilon
...
function rugFormula(lines, envelopes, parts, voids, radial) {
  const prose = lines.join("\n");
  const dimensions =
    /폭 ([\d.]+), 깊이 ([\d.]+), 높이 ([\d.]+)m, `bedroom-rug\/1600x2200`은 폭 ([\d.]+), 깊이 ([\d.]+), 높이 ([\d.]+)m/.exec(
      prose,
    );
  const bases =
    /`living` base의 Y 높이 ([\d.]+)m·`bedroom1600x2200` base의 Y 높이 ([\d.]+)m, pile 높이는 두 변종 모두 ([\d.]+)m/.exec(
      prose,
    );
  const round = /지름 ([\d.]+)m·높이 ([\d.]+)m 변종/.exec(prose);
  const edge = /안쪽으로 ([\d.]+)m 폭의 bound-edge/.exec(prose);
  if (!dimensions || !bases || !round || !edge)
    return ["rugs: prose dimensions absent"];
  const errors = [];
  ...
  const check = (state, width, depth, height, baseHeight) => {
    const envelope = envelopes.get(state),
      base = parts.get(`${state}/base`),
      pile = parts.get(`${state}/pile`),
      border = parts.get(`${state}/bound-edge`);
    if (!envelope || !base || !pile || !border) {
      errors.push(`rugs/${state}: table state absent`);
      return;
    }
    const expected = [
      [envelope.x[0], -width / 2],
      [envelope.x[1], width / 2],
      [envelope.z[0], -depth / 2],
      [envelope.z[1], depth / 2],
      [envelope.y[1], height],
      [base.y[1], baseHeight],
      [pile.y[0], baseHeight],
      [pile.y[1], height],
      [border.y[0], baseHeight],
      [border.y[1], height],
    ];
    for (const [actual, target] of expected)
  ...
  /** @type {[string,number,number][]} */
  const rectangularStates = [
    ["living", Number(dimensions[1]) / 2, Number(dimensions[2]) / 2],
    ["bedroom1600x2200", Number(dimensions[4]) / 2, Number(dimensions[5]) / 2],
  ];
  for (const [state, halfX, halfZ] of rectangularStates) {
    const cut = voids.get(`${state}/bound-edge`)?.[0],
      pile = parts.get(`${state}/pile`);
    if (
      !cut ||
      !pile ||
      Math.abs(cut.x[1] - (halfX - borderWidth)) > epsilon ||
      Math.abs(cut.z[1] - (halfZ - borderWidth)) > epsilon ||
      Math.abs(pile.x[1] - cut.x[1]) > epsilon ||
      Math.abs(pile.z[1] - cut.z[1]) > epsilon
    )
      errors.push(`rugs/${state}: prose border width differs from table`);
  }
  const ring = radial.get("round1200/bound-edge"),
    circle = radial.get("round1200/pile");
  if (
    !ring ||
    !circle ||
    Math.abs(ring.inner - (Number(round[1]) / 2 - borderWidth)) > epsilon ||
...
function simpleProseEnvelope(anchor, lines, envelopes) {
  ...
  const rules = {
    "dining-chair": {
      pattern: /`dining-chair`는 폭 ([\d.]+), 깊이 ([\d.]+), 높이 ([\d.]+)m/,
      order: ["x", "z", "y"],
    },
    "storage-basket": {
      pattern: /폭 ([\d.]+), 깊이 ([\d.]+), 높이 ([\d.]+)m/,
      order: ["x", "z", "y"],
    },
    "wall-art": {
      pattern: /폭 ([\d.]+), 높이 ([\d.]+), 전체 깊이 ([\d.]+)m/,
      order: ["x", "y", "z"],
    },
    "living-display": {
      pattern: /폭 ([\d.]+), 높이 ([\d.]+), Z 깊이 ([\d.]+)m/,
      order: ["x", "y", "z"],
    },
  };
  ...
  const envelope = envelopes.get("default");
  if (!values || !envelope)
    return [`${anchor}: prose envelope dimensions absent`];
  const errors = [];
...
function pendantFormula(lines, envelopes, radial) {
  ...
  if (!length || !cord || !shade || !diffuser || !canopy)
    return ["dining-pendant: prose diameter/length absent"];
  const observed = [
    envelopes.get("default")?.y[0],
    radial.get("default/cord")?.outer,
    radial.get("default/shade-wall")?.outer,
    radial.get("default/diffuser")?.outer,
  ...
  return observed.flatMap((value, i) =>
    value === undefined || Math.abs(value - expected[i]) > epsilon
...
function bookFormula(lines, envelopes, parts) {
  ...
  if (!allowed || !cover || !spine)
    return ["books: prose variant/formula absent"];
  const C = Number(cover[1]),
    S = Number(spine[1]);
  const states = [...allowed[1].matchAll(/`(\d+x\d+x\d+)`/g)].map(
    (match) => match[1],
  );
  if (
    states.length !== 3 ||
    states.join(",") !== [...envelopes.keys()].join(",")
  )
  ...
  for (const state of states) {
    ...
    for (const [id, bounds] of Object.entries(expected)) {
      const observed =
        id === "*" ? envelopes.get(state) : parts.get(`${state}/${id}`);
      if (!observed) {
        errors.push(`books/${state}/${id}: table part absent`);
        continue;
      }
      for (const axis of /** @type {const} */ ["x", "y", "z"])
        if (
          Math.abs(observed[axis][0] - bounds[axis][0]) > epsilon ||
          Math.abs(observed[axis][1] - bounds[axis][1]) > epsilon
        )
          errors.push(
            `books/${state}/${id}/${axis}: prose formula differs from table`,
          );
    }
  }
  return errors;
...
function propFormula(lines, envelopes, bores, ellipses, tangents) {
  const prose = lines.join("\n");
  const bowl =
    /`decor-bowl`은 외경 ([\d.]+), 높이 ([\d.]+)m, 벽 두께 ([\d.]+)m/.exec(
      prose,
    );
  const tray =
    /`decor-tray`는 전체 폭 ([\d.]+)·깊이 ([\d.]+)·높이 ([\d.]+)m/.exec(prose);
  const cup = /`decor-cup`은 몸체 외경 2R, 높이 ([\d.]+), 벽 두께 t인/.exec(
    prose,
  );
  if (!bowl || !tray || !cup)
    return ["tabletop-props: prose dimensions absent"];
  const errors = [];
  ...
  const check = (state, width, height, depth) => {
    ...
    if (
      !box ||
      Math.abs(box.x[1] - box.x[0] - width) > epsilon ||
  ...
  const bowlBore = bores.get("bowl/shell"),
    cupBore = bores.get("cup/body"),
    ellipse = ellipses.get("tray/rim"),
    tangent = tangents.get("cup/body/handle");
  const cupDiameter = tangent ? 2 * tangent.radius : NaN;
  check("cup", cupDiameter, Number(cup[1]), cupDiameter);
  if (
    !bowlBore ||
    Math.abs(bowlBore.radius - (Number(bowl[1]) / 2 - Number(bowl[3]))) >
      epsilon ||
    !cupBore ||
    !(cupBore.radius > 0 && cupBore.radius < cupDiameter / 2) ||
    !ellipse ||
    Math.abs(ellipse.outerX - Number(tray[1]) / 2) > epsilon ||
    Math.abs(ellipse.outerZ - Number(tray[2]) / 2) > epsilon ||
    !tangent ||
    Math.abs(tangent.radius - cupDiameter / 2) > epsilon ||
...
function tubFormula(lines, envelopes, parts, voids) {
  ...
  const inner =
    /내벽 간 치수는 길이 ([\d.]+)×폭 ([\d.]+)m, rim 안쪽의 실제 위쪽 개구는 ([\d.]+)×([\d.]+)m/.exec(
      prose,
    );
  const underside = /내부 바닥판의 아래면 y=([\d.]+)m/.exec(prose);
  if (!outer || !inner || !underside)
    return ["bathtub: prose dimensional source absent"];
  const shell = voids.get("default/shell")?.[0],
    rim = voids.get("default/rim")?.[0];
  const box = envelopes.get("default"),
    floor = parts.get("default/floor");
  if (!shell || !rim || !box || !floor) return ["bathtub: table source absent"];
  const pairs = [
    [box.z[1] - box.z[0], Number(outer[1])],
    [box.x[1] - box.x[0], Number(outer[2])],
    [box.y[1], Number(outer[3])],
    [shell.z[1] - shell.z[0], Number(inner[1])],
    [shell.x[1] - shell.x[0], Number(inner[2])],
    [rim.z[1] - rim.z[0], Number(inner[3])],
    [rim.x[1] - rim.x[0], Number(inner[4])],
    [floor.y[0], Number(underside[1])],
  ];
  return pairs.flatMap(([actual, expected], i) =>
...
function equipmentFormula(lines, envelopes, parts, grids) {
  const prose = lines.join("\n");
  const display =
    /전체 화면판은 폭 ([\d.]+), 높이 ([\d.]+), 깊이 ([\d.]+)m/.exec(prose);
  const keyboard =
    /`work-keyboard`는 폭 ([\d.]+), 깊이 ([\d.]+), 높이 ([\d.]+)m/.exec(prose);
  const keys =
    /(\d+)열×(\d+)행의 낮은 key cap은 각각 X 폭 ([\d.]+)m·Z 깊이 ([\d.]+)m·Y 높이 ([\d.]+)m로 y=([\d.]+)\.\.([\d.]+)/.exec(
      prose,
    );
  const pitchX = /x=\(c−5\.5\)×([\d.]+)m/.exec(prose);
  const pitchZ = /z=\(r−1\.5\)×([\d.]+)m/.exec(prose);
  if (!display || !keyboard || !keys || !pitchX || !pitchZ)
    return ["work-equipment: prose grid or dimensions absent"];
  const d = envelopes.get("display"),
    k = envelopes.get("keyboard");
  const housing = parts.get("display/housing"),
    bezel = parts.get("display/display-bezel"),
    grid = grids.get("keyboard");
  if (!d || !k || !housing || !bezel || !grid)
    return ["work-equipment: table parts absent"];
  const pairs = [
    [housing.x[1] - housing.x[0], Number(display[1])],
    [housing.y[1] - housing.y[0], Number(display[2])],
    [housing.z[1] - housing.z[0] + bezel.z[1] - bezel.z[0], Number(display[3])],
    [k.x[1] - k.x[0], Number(keyboard[1])],
    [k.z[1] - k.z[0], Number(keyboard[2])],
    [k.y[1] - k.y[0], Number(keyboard[3])],
    [grid.cols, Number(keys[1])],
    [grid.rows, Number(keys[2])],
    [grid.width, Number(keys[3])],
    [grid.depth, Number(keys[4])],
    [grid.y[1] - grid.y[0], Number(keys[5])],
    [grid.y[0], Number(keys[6])],
    [grid.y[1], Number(keys[7])],
    [grid.pitchX, Number(pitchX[1])],
    [grid.pitchZ, Number(pitchZ[1])],
  ];
  return pairs.flatMap(([actual, expected], i) =>
    Math.abs(actual - expected) > epsilon
...
function plantProof(parsed) {
  ...
  const source = fs.readFileSync(
    path.join(root, "docs/models/004-decor-and-fixtures.md"),
    "utf8",
  );
  let input;
  try {
    input = plantProducer.check(source);
  } catch (error) {
    return {
      errors: [`potted-plant: ${String(error)}`],
      contacts,
      nonOverlaps,
    };
  }
  const joins = new Map(
    [...source.matchAll(/^@plant-join\s+(\d+):\s+(branch-\d+),\s+(.+)$/gm)].map(
      (match) => [`${match[1]}/${match[2]}`, match[3].split(",").map(Number)],
    ),
  );
  const apices = new Map(
    [...source.matchAll(/^@plant-apex\s+(\d+):\s+(leaf-\d+),\s+(.+)$/gm)].map(
      (match) => [`${match[1]}/${match[2]}`, match[3].split(",").map(Number)],
    ),
  );
  if (
    joins.size !== input.heights.length * 5 ||
    apices.size !== input.heights.length * 15
  )
  ...
  if (
    !(
      2 * branchBase * Math.sin(Math.PI / 5) > 2 * input.branchRadius &&
      2 * maxBladeAngle < (2 * Math.PI) / 5 &&
      lateral > input.leafWidth &&
      input.leafThickness > 0
    )
  )
  ...
  const pair = (state, left, right) =>
    `${state}/${[left, right].sort((a, b) => a.localeCompare(b)).join("/")}`;
  let expectedCount = 0;
  for (const millimetres of input.heights) {
    ...
    const actual = [...parsed.parts.values()].filter(
    ...
    if (
      !envelope ||
      !inventory ||
      actual.length !== expected.parts.length ||
      inventory.join(",") !== expected.parts.map((part) => part.id).join(",")
    ) {
      errors.push(
        `potted-plant/${state}: generated part population differs from formula`,
      );
      continue;
    }
    for (const axis of /** @type {const} */ ["x", "y", "z"])
      if (
        envelope[axis].some(
          (value, i) => Math.abs(value - expected.envelope[axis][i]) > epsilon,
        )
      )
        errors.push(
          `potted-plant/${state}: ${axis} envelope differs from formula`,
        );
    ...
    for (const part of expected.parts) {
      ...
      if (
        !found ||
        found.shape !== part.shape ||
        found.contact.join(",") !== part.contact ||
        /** @type {const} */ ["x", "y", "z"].some((axis) =>
          found[axis].some(
            (value, i) => Math.abs(value - part[axis][i]) > epsilon,
          ),
        )
      )
        errors.push(
          `potted-plant/${state}/${part.id}: bounds, shape or contact differs from formula`,
        );
    }
    /** @param {string} left @param {string} right */
    const attach = (left, right) => {
      const key = pair(state, left, right);
      contacts.add(key);
      nonOverlaps.add(key);
    };
    attach("pot", "soil");
    for (let i = 0; i < 5; i++) {
      ...
      const branchBounds = {
        x: [
          Math.min(baseX, tipX) - branchRadius,
          Math.max(baseX, tipX) + branchRadius,
        ],
        y: [baseY - branchRadius, baseY + branchRadius],
        z: [
          Math.min(baseZ, tipZ) - branchRadius,
          Math.max(baseZ, tipZ) + branchRadius,
        ],
      };
      if (
        Math.abs(baseY - tipY) > epsilon ||
        /** @type {const} */ ["x", "y", "z"].some((axis) =>
          branch[axis].some(
            (value, end) =>
              Math.abs(value - branchBounds[axis][end]) > 2 * epsilon,
          ),
        )
      )
        errors.push(
          `potted-plant/${state}/branch-${i}: measured capsule differs from part bounds`,
        );
      if (
        Math.abs(Math.hypot(baseX, baseZ) - branchRadius - stemSurface) >
        2 * epsilon
      )
        errors.push(
          `potted-plant/${state}/branch-${i}: stem tangent distance differs`,
        );
      ...
      for (let j = 0; j < 3; j++) {
        ...
        const distance = apex
          ? Math.hypot(apex[0] - tipX, apex[2] - tipZ)
          : Infinity;
        if (
          !leaf ||
          !apex ||
          apex.length !== 3 ||
          !apex.every(Number.isFinite) ||
          Math.abs(distance - branchRadius) > 2 * epsilon ||
          apex[0] < leaf.x[0] - epsilon ||
          apex[0] > leaf.x[1] + epsilon ||
          apex[2] < leaf.z[0] - epsilon ||
          apex[2] > leaf.z[1] + epsilon ||
          Math.abs(leaf.y[0] - apex[1]) > epsilon ||
          Math.abs(apex[1] - tipY) > epsilon
        )
          errors.push(
            `potted-plant/${state}/branch-${i}/leaf-${3 * i + j}: tangent point differs`,
          );
      ...
      for (let a = 0; a < 3; a++)
        for (let b = a + 1; b < 3; b++)
          nonOverlaps.add(
            pair(state, `leaf-${3 * i + a}`, `leaf-${3 * i + b}`),
          );
    }
  }
  if (
    parsed.parts.size !== expectedCount ||
    parsed.envelopes.size !== input.heights.length
  )
...
function cavityProfileErrors(part, profile) {
  ...
  const shoulder =
    (height * profile.shoulderNumerator) / profile.shoulderDenominator;
  const bodyInner = diameter / 2 - wall;
  const mouthOuter = profile.mouthRadius + wall;
  /** @type {string[]} */ const errors = [];
  if (part.shape !== "hollow")
    errors.push("cavity profile requires a hollow part");
  if (
    ![
      width,
      height,
  ...
  if (
    !(
      wall > 0 &&
      profile.mouthRadius > 0 &&
      mouthOuter < diameter / 2 &&
...
function positiveFraction(value) {
  ...
  if (!match || Number(match[2]) === 0)
    throw Error(`invalid positive fraction ${value}`);
...
function vesselAttachmentProof(lines, anchor, design) {
  ...
  const seen = new Set();
  if (
    lines.some((line) => line.includes("`@vessel-attachments`")) &&
    !lines.some((line) => line.startsWith("@vessel-attachments"))
  )
    errors.push(
      `${anchor}: prose names vessel attachments but no row declares them`,
    );
  for (const line of lines.filter((candidate) =>
    candidate.startsWith("@vessel-attachments"),
  )) {
    rows++;
    const match = /^@vessel-attachments\s+([^:]+):\s*(.+)$/.exec(line);
    if (!match) {
      errors.push(`${anchor}: malformed vessel attachment row`);
      continue;
    }
    const cells = match[2].split(",").map((cell) => cell.trim());
    if (cells.length !== 7) {
      errors.push(`${anchor}: vessel attachment needs seven fields`);
      continue;
    }
    let spoutY, channel, gripY, holeX, holeY, gripZ;
    try {
      spoutY = positiveFraction(cells[1]);
      channel = positiveFraction(cells[2]);
      gripY = fractionInterval(cells[3]);
      holeX = fractionInterval(cells[4]);
      holeY = fractionInterval(cells[5]);
      gripZ = positiveFraction(cells[6]);
    } catch (error) {
      errors.push(`${anchor}: ${String(error)}`);
      continue;
    }
    for (const state of match[1].split(",").map((value) => value.trim())) {
      ...
      if (seen.has(key)) {
        errors.push(`${anchor}/${key}: repeated vessel attachment`);
        continue;
      }
      seen.add(key);
      const part = design.parts.get(key),
        profile = design.cavityProfiles.get(key);
      if (!part || !profile || profile.section !== "round") {
        errors.push(
          `${anchor}/${key}: attachment needs a measured round cavity profile`,
        );
        continue;
      }
      const W = part.x[1] - part.x[0],
        H = part.y[1] - part.y[0],
        D = part.z[1] - part.z[0];
      const R = Math.min(W, D) / 2,
        wall = Math.min(W, D) / profile.wallDivisor;
      const shoulder = profile.shoulderNumerator / profile.shoulderDenominator;
      const mouth = profile.mouthRadius,
        centerZ = part.z[0] + R;
      /** @param {number} y */
      const outerAt = (y) =>
        R + ((mouth + wall - R) * (y - shoulder)) / (1 - shoulder);
      /** @param {number} y */
      const innerAt = (y) =>
        R - wall + ((mouth - (R - wall)) * (y - shoulder)) / (1 - shoulder);
      const spoutInner = channel * mouth,
        spoutOuter = spoutInner + wall;
      const problem = [];
      if (
        !(W > 0 && H > 0 && D > 0 && wall > 0 && shoulder > 0 && shoulder < 1)
      )
      ...
      if (
        !(
          ...
          spoutInner > 0 &&
          spoutInner < mouth &&
          spoutInner < innerAt(spoutY) &&
          spoutOuter < R &&
          spoutY * H - spoutOuter > 0 &&
          spoutY * H + spoutOuter < H &&
          part.z[1] - (centerZ + R) > 0
        )
      )
        problem.push("spout leaves cavity or measured bounds");
      if (
        !(
          shoulder <= gripY[0] &&
          gripY[0] < holeY[0] &&
          ...
          gripZ > 0 &&
          centerZ - gripZ * W > part.z[0] &&
          centerZ + gripZ * W < part.z[1] &&
          holeX[0] * R > spoutOuter
        )
      )
        problem.push("grip hole or finite root leaves measured bounds");
      if (problem.length)
        errors.push(`${anchor}/${key}: ${problem.join("; ")}`);
      else proved++;
    }
  }
  for (const [key, profile] of design.cavityProfiles) {
    const part = design.parts.get(key);
    if (
      profile.section === "round" &&
      part &&
      part.z[1] - part.z[0] > part.x[1] - part.x[0] + epsilon &&
      !seen.has(key)
    )
      errors.push(
        `${anchor}/${key}: round cavity has extra depth but no vessel attachment`,
      );
  }
...
function vesselClosureProof(lines, anchor, design) {
  ...
  let rows = 0,
    ...
    proved = 0;
  const seen = new Set();
  const declarations = lines.filter((candidate) =>
    candidate.startsWith("@vessel-closure"),
  );
  ...
  for (const line of declarations) {
    rows++;
    const match = /^@vessel-closure\s+([^:]+):\s*([^,]+),\s*(\d+\/\d+)$/.exec(
      line,
    );
    if (!match) {
      errors.push(`${anchor}: malformed vessel closure row`);
      continue;
    }
    ...
    for (const state of match[1].split(",").map((value) => value.trim())) {
      ...
      if (seen.has(key)) {
        errors.push(`${anchor}/${key}: repeated vessel closure`);
        continue;
      }
      seen.add(key);
      const part = design.parts.get(key),
      ...
      if (!part || !profile || part.shape !== "hollow") {
        errors.push(
          `${anchor}/${key}: closure needs a hollow part and cavity profile`,
        );
        continue;
      }
      const W = part.x[1] - part.x[0],
      ...
      if (
        !(
          ...
          profile.mouthRadius > 0 &&
          capHeight > 0 &&
          capHeight < 1 - shoulder &&
          bottomOuter > bottomInner &&
          bottomOuter <= bodyRadius &&
          neckOuter > profile.mouthRadius &&
          neckOuter < bodyRadius
        )
      )
        errors.push(
          `${anchor}/${key}: cap cannot close the neck within measured bounds`,
        );
      else proved++;
    }
  }
...
function audit(allSections, mutate, onlyState) {
  ...
  for (const [anchor, lines] of allSections) {
    ...
    for (const [key, profile] of cavityProfiles) {
      ...
      if (!part) {
        errors.push(`${anchor}/${key}: cavity profile has no part`);
        continue;
      }
      if (bores.has(key))
        errors.push(`${anchor}/${key}: cavity profile duplicates a bore`);
    ...
    const plant =
      anchor === "potted-plant"
        ? plantProof(parsed)
        : { errors: [], contacts: new Set(), nonOverlaps: new Set() };
    errors.push(...plant.errors);
    if (anchor === "cabinet-and-shelf") {
      try {
        const source = fs.readFileSync(
          path.join(root, "docs/models/002-storage-and-sleep.md"),
          "utf8",
        );
        cabinetProducer.check(source);
      } catch (error) {
        errors.push(`cabinet-and-shelf: ${String(error)}`);
      }
    }
    if (!envelopes.size) {
      errors.push(`${anchor}: no @envelope rows`);
      continue;
    }
    if (
      anchor === "entry-bench" &&
      (compositions.size !== 1 || !compositions.has("default"))
    )
    ...
    for (const [state, envelope] of envelopes) {
      ...
      const stateParts = [...parts.values()].filter(
        (part) => part.state === state,
      );
      ...
      for (const part of stateParts)
        if (part.shape === "hollow") {
          ...
          if (
            ...
            bores.has(key) ||
            boresX.has(key) ||
            cavityProfiles.has(key) ||
            boresZ.has(key) ||
            ellipses.has(key) ||
            radial.has(key) ||
            radialZ.has(key) ||
            lines.some((line) => line.startsWith("@plant-spec:"))
          )
            provedHollowParts++;
          else
            errors.push(`${anchor}/${key}: hollow part has no authored cavity`);
        }
      for (const row of lines.filter((line) =>
        line.startsWith(`@component-count ${state}:`),
      )) {
        const declaration =
          /^@component-count ([^:]+): ([a-z][a-z0-9-]*), (\d+)$/.exec(row);
        if (!declaration) {
          errors.push(`${anchor}/${state}: malformed component count`);
          continue;
        }
        const part = parts.get(`${state}/${declaration[2]}`);
        if (!part) {
          errors.push(
            `${anchor}/${state}/${declaration[2]}: component host absent`,
          );
          continue;
        }
        ...
        if (actual !== Number(declaration[3]))
          errors.push(
            `${anchor}/${state}/${part.id}: ${actual} occupied components contradict declared ${declaration[3]}`,
          );
      }
      const composition = compositions.get(state);
      const supported = stateParts.filter((part) =>
        part.contact.some((target) => /^support@[\d.]+$/.test(target)),
      );
      ...
      if (binding) {
        ...
        if (!childPart)
          errors.push(`${anchor}/${state}: support child part absent`);
        else
          for (const part of supported) {
            const declared = Number(
              part.contact
                .find((target) => /^support@[\d.]+$/.test(target))
                ?.slice(8),
            );
            const sharedX =
              Math.min(part.x[1], childPart.x[1] + binding.offset[0]) -
              Math.max(part.x[0], childPart.x[0] + binding.offset[0]);
            const sharedZ =
              Math.min(part.z[1], childPart.z[1] + binding.offset[2]) -
              Math.max(part.z[0], childPart.z[0] + binding.offset[2]);
            const childVoids =
              child?.voids.get(`${binding.state}/${binding.part}`) || [];
            const cavityFloor = childVoids.find(
              (voidBox) =>
            ...
            const top =
              (cavityFloor?.y[0] ?? childPart.y[1]) + binding.offset[1];
            if (
              Math.abs(declared - top) > epsilon ||
              Math.abs(part.y[0] - top) > epsilon ||
              sharedX <= epsilon ||
              sharedZ <= epsilon
            )
              errors.push(
                `${anchor}/${state}/${part.id}: support face differs from ${binding.anchor}/${binding.state}/${binding.part}`,
              );
          }
        if (child)
          for (const own of stateParts) {
            ...
            for (const referent of child.parts.values()) {
              ...
              const childBoxes = occupiedBoxes(
                referent,
                child.voids,
                child.pieces,
              ).map((box) => ({
                x: /** @type {[number,number]} */ [
                  box.x[0] + binding.offset[0],
                  box.x[1] + binding.offset[0],
                ],
                y: /** @type {[number,number]} */ [
                  box.y[0] + binding.offset[1],
                  box.y[1] + binding.offset[1],
                ],
                z: /** @type {[number,number]} */ [
                  box.z[0] + binding.offset[2],
                  box.z[1] + binding.offset[2],
                ],
              }));
              if (
                ownBoxes.some((box) =>
                  childBoxes.some((other) =>
                    overlap(box, other).every((depth) => depth > epsilon),
                  ),
                )
              )
                errors.push(
                  `${anchor}/${state}/${own.id}: penetrates support child ${binding.anchor}/${binding.state}/${referent.id}`,
                );
            }
          }
      }
      if (composition) {
        ...
        const childParts = [...(child?.parts.values() || [])].filter(
          (part) => part.state === composition.state,
        );
        if (!childEnvelope || !childParts.length)
          errors.push(`${anchor}/${state}: composition child state absent`);
        else {
          const pieces = [
            ...stateParts,
            ...childParts.map((part) => ({
              ...part,
              x: /** @type {[number,number]} */ part.x.map(
                (value) => value + composition.offset[0],
              ),
              y: /** @type {[number,number]} */ part.y.map(
                (value) => value + composition.offset[1],
              ),
              z: /** @type {[number,number]} */ part.z.map(
                (value) => value + composition.offset[2],
              ),
            })),
          ];
          for (const axis of /** @type {const} */ ["x", "y", "z"]) {
            const bounds = [
              Math.min(...pieces.map((part) => part[axis][0])),
              Math.max(...pieces.map((part) => part[axis][1])),
            ];
            if (
              bounds.some(
                (value, i) => Math.abs(value - envelope[axis][i]) > epsilon,
              )
            )
              errors.push(
                `${anchor}/${state}: composite ${axis} envelope differs from child and own parts`,
              );
          }
          const attached = stateParts.some((own) =>
            pieces
              .slice(stateParts.length)
              .some(
                (referent) =>
                  Math.abs(own.y[0] - referent.y[1]) <= epsilon &&
                  Math.min(own.x[1], referent.x[1]) -
                    Math.max(own.x[0], referent.x[0]) >
                    epsilon &&
                  Math.min(own.z[1], referent.z[1]) -
                    Math.max(own.z[0], referent.z[0]) >
                    epsilon,
              ),
          );
          if (!attached)
            errors.push(
              `${anchor}/${state}: composite child has no finite face contact`,
            );
        }
      }
      ...
      for (const [key, patch] of flatContacts) {
        ...
        const guest = byId.get(patch.guest),
          host = byId.get(patch.host);
        const axis = patch.axis === "-Y" ? "y" : "z";
        const transverse = patch.axis === "-Y" ? "z" : "y";
        /** @param {[number,number]} interval @param {[number,number]} bounds */
        const inRange = (interval, bounds) =>
          interval[0] >= bounds[0] - epsilon &&
          interval[1] <= bounds[1] + epsilon;
        let valid =
          !!guest &&
          (guest.contact.includes(patch.host) ||
            (!!host && host.contact.includes(patch.guest))) &&
          occupiedBoxes(guest, voids, pieces).some(
            (box) =>
              Math.abs(box[axis][0] - patch.plane) <= epsilon &&
              inRange(patch.u, box.x) &&
              inRange(patch.v, box[transverse]),
          ) &&
          patch.u[1] - patch.u[0] > epsilon &&
          patch.v[1] - patch.v[0] > epsilon;
        const soleGrid = soleGrids.get(`${state}/${patch.guest}`);
        if (guest && soleGrid && patch.axis === "-Y") {
          const width = guest.x[1] - guest.x[0],
            depth = guest.z[1] - guest.z[0];
          const centerX = (guest.x[0] + guest.x[1]) / 2;
          const centerZ = (guest.z[0] + guest.z[1]) / 2;
          const halfX = width / (2 * soleGrid.widthDivisor);
          const halfZ = depth / (2 * soleGrid.depthDivisor);
          valid &&= [-1, 1].some((sx) =>
            [-1, 1].some((sz) => {
              const x =
                centerX + sx * (width / 2 - width / soleGrid.xInsetDivisor);
              const z =
                centerZ + sz * (depth / 2 - depth / soleGrid.zInsetDivisor);
              return (
                inRange(patch.u, [x - halfX, x + halfX]) &&
                inRange(patch.v, [z - halfZ, z + halfZ])
              );
            }),
          );
        }
        if (patch.host === "wall")
          ...
          patch.host === "ground" ||
          patch.host === "support" ||
          /^support@[\d.]+$/.test(patch.host)
        ) {
          const height = patch.host.startsWith("support@")
            ? Number(patch.host.slice(8))
            : 0;
          valid &&=
            patch.axis === "-Y" && Math.abs(patch.plane - height) <= epsilon;
        } else if (host) {
          valid &&= occupiedBoxes(host, voids, pieces).some(
            (box) =>
          ...
          if (
            patch.axis === "-Y" &&
            shear &&
            Math.abs(shear.y[1] - patch.plane) <= epsilon
          )
            valid &&= inRange(patch.v, [
          ...
          if (patch.axis === "-Y" && host.shape === "cylinder") {
            ...
            valid &&=
              Math.abs(radiusX - radiusZ) <= epsilon &&
              patch.u.every((x) =>
                patch.v.every(
                  (z) =>
        ...
        if (!valid)
          errors.push(
            `${anchor}/${key}: flat contact has no finite common patch`,
          );
        else if (host)
          provedCurves.add(
            `${state}/${[patch.guest, patch.host].sort((a, b) => a.localeCompare(b)).join("/")}`,
          );
        else provedExternal.add(`${patch.guest}/${patch.host}`);
      }
      for (const [key, cap] of capContacts) {
        if (!key.startsWith(`${state}/`)) continue;
        const first = byId.get(cap.first),
          second = byId.get(cap.second);
        ...
        const valid =
          !!first &&
          !!second &&
          (first.contact.includes(cap.second) ||
            second.contact.includes(cap.first)) &&
          Math.abs(first[axis][end] - second[axis][1 - end]) <= epsilon &&
          transverse.every(
            (other) =>
              Math.min(first[other][1], second[other][1]) -
                Math.max(first[other][0], second[other][0]) >
              epsilon,
          ) &&
          capContactArea(
            first,
        ...
        if (!valid)
          errors.push(
            `${anchor}/${key}: cap contact has no finite common face`,
          );
        else provedCurves.add(key);
      }
      for (const [key, cavityContact] of cavityContacts) {
        if (!key.startsWith(`${state}/`)) continue;
        const host = byId.get(cavityContact.host),
          guest = byId.get(cavityContact.guest);
        ...
        const valid =
          !!host &&
          !!guest &&
          (host.contact.includes(guest.id) ||
            guest.contact.includes(host.id)) &&
                ...
                transverse.every(
                  (other) =>
                    ...
                    Math.min(guest[other][1], hole[other][1]) -
                      Math.max(guest[other][0], hole[other][0]) >
                      epsilon,
                ),
            ),
          );
        if (!valid)
          errors.push(
            `${anchor}/${key}: cavity side has no finite contact face`,
          );
        else provedCurves.add(key);
      }
      for (const [key, bore] of boresX) {
        ...
        const cylinder = stateParts.filter(
          (part) =>
            part.contact.includes(host?.id || "") &&
            part.shape === "cylinder" &&
            host?.contact.includes(part.id),
        );
        if (
          !host ||
          cylinder.length !== 1 ||
          ...
          bore.centerZ - bore.radius < host.z[0] - epsilon ||
          bore.centerZ + bore.radius > host.z[1] + epsilon ||
          bore.x[0] < host.x[0] - epsilon ||
          bore.x[1] > host.x[1] + epsilon ||
          Math.abs((cylinder[0].y[0] + cylinder[0].y[1]) / 2 - bore.centerY) >
            epsilon ||
          Math.abs((cylinder[0].z[0] + cylinder[0].z[1]) / 2 - bore.centerZ) >
            epsilon ||
          Math.abs((cylinder[0].y[1] - cylinder[0].y[0]) / 2 - bore.radius) >
            epsilon ||
          Math.abs((cylinder[0].z[1] - cylinder[0].z[0]) / 2 - bore.radius) >
            epsilon ||
          Math.min(cylinder[0].x[1], bore.x[1]) -
            Math.max(cylinder[0].x[0], bore.x[0]) <=
            epsilon
        )
          errors.push(
            `${anchor}/${key}: axial bore lacks a fitted cylinder contact`,
          );
        else
          provedCurves.add(
            `${state}/${[host.id, cylinder[0].id].sort((a, b) => a.localeCompare(b)).join("/")}`,
          );
      }
      if (
        (anchor === "ceiling-surface-light" ||
          anchor === "dining-pendant" ||
      ...
      if (luminousId) {
        ...
        if (
          !luminous ||
          visibleEmitterFraction(luminous, stateParts, radial, state) < 0.95
        )
          errors.push(
            `${anchor}/${state}: emissive face is occluded from the room`,
          );
      }
      for (const part of stateParts)
        if (
          ...
          /^hinge-/.test(part.id) &&
          part.contact.some(
            (target) => target === "closed-panel" || target === "bed-frame",
          ) &&
          !pinFaces.has(`${state}/${part.id}`)
        )
          errors.push(
            `${anchor}/${state}/${part.id}: missing pin-face declaration`,
          );
      for (const [key, targetId] of pinFaces) {
        ...
        if (
          ...
          !target ||
          pin.shape !== "cylinder" ||
          !pin.contact.includes(targetId) ||
          pinFaceArea(pin, target) <= 1e-8
        )
          errors.push(
            `${anchor}/${key}: pin end cap has no finite contact face with ${targetId}`,
          );
        else
          provedCurves.add(
            `${state}/${[pin.id, targetId].sort((a, b) => a.localeCompare(b)).join("/")}`,
          );
      }
      for (const [key, cavities] of voids) {
        ...
        const host = byId.get(key.slice(state.length + 1));
        if (!host) {
          errors.push(`${anchor}/${key}: void host absent`);
          continue;
        }
        for (const cavity of cavities) {
          for (const axis of /** @type {const} */ ["x", "y", "z"])
            if (
              cavity[axis][0] < host[axis][0] - epsilon ||
              cavity[axis][1] > host[axis][1] + epsilon
            )
              errors.push(`${anchor}/${key}: void ${axis} exits host`);
        }
        if (!occupiedBoxes(host, voids, pieces).length)
          errors.push(`${anchor}/${key}: void removes entire host`);
      }
      for (const [key, minimum] of cavityMinima) {
        if (!key.startsWith(`${state}/`)) continue;
        const host = byId.get(key.slice(state.length + 1));
        const cavities = voids.get(key) || [];
        if (!host || host.shape !== "hollow" || cavities.length !== 1) {
          errors.push(
            `${anchor}/${key}: sealed cavity needs one hollow host and one void`,
          );
          continue;
        }
        const cavity = cavities[0];
        const floor = cavity.y[0] - host.y[0];
        const walls = [
          cavity.x[0] - host.x[0],
          host.x[1] - cavity.x[1],
          cavity.z[0] - host.z[0],
        ...
        const minWall =
          Math.min(host.x[1] - host.x[0], host.z[1] - host.z[0]) * minimum.wall;
        if (
          Math.abs(cavity.y[1] - host.y[1]) > epsilon ||
          floor + epsilon < (host.y[1] - host.y[0]) * minimum.floor ||
          walls.some((width) => width + epsilon < minWall)
        )
          errors.push(
            `${anchor}/${key}: sealed cavity floor or wall below declared minimum`,
          );
      }
      for (const [key, regions] of pieces) {
        ...
        const host = byId.get(key.slice(state.length + 1));
        if (!host) {
          errors.push(`${anchor}/${key}: piece host absent`);
          continue;
        }
        for (const axis of /** @type {const} */ ["x", "y", "z"]) {
          ...
          if (
            Math.abs(min - host[axis][0]) > epsilon ||
            Math.abs(max - host[axis][1]) > epsilon
          )
            errors.push(
              `${anchor}/${key}: piece union ${axis} differs from part bounds`,
            );
        }
        ...
        while (changed) {
          ...
          for (let i = 0; i < regions.length; i++)
            if (
              !visited.has(i) &&
              [...visited].some((j) => surfaceContact(regions[i], regions[j]))
            ) {
              visited.add(i);
              changed = true;
            }
        ...
        if (visited.size !== regions.length)
          errors.push(`${anchor}/${key}: piece union disconnected`);
      }
      for (const [key, shear] of shearsZ) {
        if (!key.startsWith(`${state}/`)) continue;
        const host = byId.get(key.slice(state.length + 1));
        const regions = pieces.get(key) || [];
        const upper = regions.find(
          (piece) =>
            Math.abs(piece.y[0] - shear.y[0]) <= epsilon &&
            Math.abs(piece.y[1] - shear.y[1]) <= epsilon,
        );
        if (
          !host ||
          host.shape !== "curved" ||
          !upper ||
          !(shear.halfDepth > epsilon) ||
          Math.abs(upper.z[0] - (shear.z[0] - shear.halfDepth)) > epsilon ||
          Math.abs(upper.z[1] - (shear.z[1] + shear.halfDepth)) > epsilon ||
          !regions.some(
            (piece) =>
              Math.abs(piece.y[1] - shear.y[0]) <= epsilon &&
              Math.min(piece.x[1], upper?.x[1] || 0) -
                Math.max(piece.x[0], upper?.x[0] || 0) >
                epsilon &&
              Math.min(piece.z[1], shear.z[0] + shear.halfDepth) -
                Math.max(piece.z[0], shear.z[0] - shear.halfDepth) >
                epsilon,
          )
        )
          errors.push(
            `${anchor}/${key}: shear-z bounds or lower finite contact differ`,
          );
      }
      for (const [key, radii] of radial) {
        ...
        const host = byId.get(key.slice(state.length + 1));
        if (!host) {
          errors.push(`${anchor}/${key}: radial host absent`);
          continue;
        }
        if (
          Math.abs(host.x[0] - (radii.centerX - radii.outer)) > epsilon ||
      ...
      for (const [key, radii] of radialZ) {
        ...
        const host = byId.get(key.slice(state.length + 1));
        if (!host) {
          errors.push(`${anchor}/${key}: radial-z host absent`);
          continue;
        }
        if (
          Math.abs(host.x[0] - (radii.centerX - radii.outer)) > epsilon ||
      ...
      for (const [key, bore] of bores) {
        ...
        if (
          ...
          bore.radius <= 0 ||
          bore.radius >=
            Math.min(host.x[1] - host.x[0], host.z[1] - host.z[0]) / 2 -
              epsilon ||
          bore.y[0] < host.y[0] - epsilon ||
          bore.y[1] > host.y[1] + epsilon
        )
      ...
      for (const [key, bore] of boresZ) {
        ...
        const host = byId.get(key.slice(state.length + 1));
        const nearX = host
          ? Math.max(host.x[0] - bore.centerX, 0, bore.centerX - host.x[1])
          : Infinity;
        const nearY = host
          ? Math.max(host.y[0] - bore.centerY, 0, bore.centerY - host.y[1])
          : Infinity;
        const farX = host
          ? Math.max(
              Math.abs(host.x[0] - bore.centerX),
              Math.abs(host.x[1] - bore.centerX),
            )
          : 0;
        const farY = host
          ? Math.max(
              Math.abs(host.y[0] - bore.centerY),
              Math.abs(host.y[1] - bore.centerY),
            )
          : 0;
        if (
          !host ||
          ...
          bore.z[0] < host.z[0] - epsilon ||
          bore.z[1] > host.z[1] + epsilon
        )
          errors.push(
            `${anchor}/${key}: bore-z cuts no circular face or consumes its host`,
          );
      }
      for (const [key, ellipse] of ellipses) {
        ...
        if (
          ...
          ellipse.innerX <= 0 ||
          ellipse.innerZ <= 0 ||
          ellipse.innerX >= ellipse.outerX ||
          ellipse.innerZ >= ellipse.outerZ ||
          ...
          errors.push(
            `${anchor}/${key}: elliptic ring differs from part bounds`,
          );
      }
      for (const [key, tangent] of tangents) {
        if (!key.startsWith(`${state}/`)) continue;
        const host = byId.get(tangent.host),
          guest = byId.get(tangent.guest);
        const zBack = Math.sqrt(tangent.radius ** 2 - tangent.halfWidth ** 2);
        if (
          !host ||
          !guest ||
          !(tangent.radius > tangent.halfWidth && tangent.halfWidth > 0) ||
          host.shape !== "hollow" ||
          guest.shape !== "curved" ||
          Math.abs(host.x[0] + tangent.radius) > epsilon ||
          Math.abs(host.x[1] - tangent.radius) > epsilon ||
          Math.abs(host.z[1] - tangent.radius) > epsilon ||
          Math.abs(guest.x[0] + tangent.halfWidth) > epsilon ||
          Math.abs(guest.x[1] - tangent.halfWidth) > epsilon ||
          Math.abs(guest.z[0] - zBack) > epsilon ||
          guest.y[0] < host.y[0] ||
          guest.y[1] > host.y[1] ||
      ...
      for (const [key, curve] of curveLayers) {
        if (!key.startsWith(`${state}/`)) continue;
        const shell = byId.get(curve.shell),
          cover = byId.get(curve.cover);
        const base = curve.c0,
          end = curve.c0 + curve.c1 + curve.c2;
        const monotone =
          curve.c1 < -epsilon && curve.c1 + 2 * curve.c2 < -epsilon;
        const segments = [
          ...(pieces.get(`${state}/${curve.cover}`) || []),
        ...
        const shapedPieces =
          ...
          segments.every(
            (segment) =>
              matches(segment.x[0], cover.x[0]) &&
              matches(segment.x[1], cover.x[1]),
          ) &&
          matches(segments[0].y[0], shell.y[0]) &&
          matches(segments[0].y[1], segments[1].y[0]) &&
          matches(segments[1].y[1], shell.y[1]) &&
          matches(
            segments[0].z[0],
            at(segments[0].y[1]) + curve.shellDepth + curve.seatGap,
          ) &&
          matches(segments[0].z[1], cover.z[1]) &&
          matches(segments[1].z[0], at(shell.y[1]) + curve.shellDepth) &&
          matches(
            segments[1].z[1],
            at(segments[1].y[0]) + curve.shellDepth + curve.coverDepth,
          );
        if (
          !shell ||
          !cover ||
          !monotone ||
          !(curve.shellDepth > 0 && curve.coverDepth > 0) ||
          !(curve.seatGap >= 0) ||
          !shapedPieces ||
          shell.shape !== "curved" ||
          cover.shape !== "curved" ||
          Math.abs(shell.z[0] - end) > epsilon ||
          Math.abs(shell.z[1] - (base + curve.shellDepth)) > epsilon ||
          Math.abs(cover.z[0] - (end + curve.shellDepth)) > epsilon ||
          Math.abs(shell.y[1] - cover.y[1]) > epsilon ||
          !lines.join("\n").includes("z(y)=−0.10−0.17t−0.02×4t(1−t)")
        )
          errors.push(
            `${anchor}/${key}: curved layer polynomial or contact is inconsistent`,
          );
        else provedCurves.add(key);
      }
      for (const [key, curve] of linearCurves) {
        if (!key.startsWith(`${state}/`)) continue;
        const host = byId.get(curve.host),
          guest = byId.get(curve.guest);
        /** @param {number} y */
        const at = (y) =>
          curve.c0 + (curve.c1 * (y - curve.originY)) / curve.spanY;
        /** @param {number} value @param {number} expected */
        const approx = (value, expected) =>
          Math.abs(value - expected) <= 0.00001;
        if (
          !host ||
          !guest ||
          host.shape !== "curved" ||
          ...
          host.y[0] < curve.originY - epsilon ||
          host.y[1] > curve.originY + curve.spanY + epsilon ||
          guest.y[0] < curve.originY - epsilon ||
          guest.y[1] > curve.originY + curve.spanY + epsilon ||
          Math.max(host.y[0], guest.y[0]) >= Math.min(host.y[1], guest.y[1]) ||
          host.x[0] > guest.x[0] + epsilon ||
          host.x[1] < guest.x[1] - epsilon ||
          !approx(host.z[0], at(host.y[1])) ||
          !approx(host.z[1], at(host.y[0]) + curve.hostDepth) ||
          !approx(guest.z[0], at(guest.y[1]) + curve.hostDepth + curve.gap) ||
          !approx(
            guest.z[1],
            at(guest.y[0]) + curve.hostDepth + curve.gap + curve.guestDepth,
          )
        )
          errors.push(
            `${anchor}/${key}: linear curved surfaces do not match part bounds`,
          );
      ...
      if (!names) errors.push(`${anchor}/${state}: no @inventory row`);
        ...
        for (const name of names)
          if (!byId.has(name))
            errors.push(`${anchor}/${state}: inventoried part ${name} absent`);
        for (const part of stateParts)
          if (!names.includes(part.id))
            errors.push(
              `${anchor}/${state}: part ${part.id} missing from inventory`,
            );
        if (new Set(names).size !== names.length)
          errors.push(`${anchor}/${state}: duplicate inventory name`);
      }
      for (const part of stateParts) {
        examined++;
        for (const axis of /** @type {const} */ ["x", "y", "z"])
          if (
            part[axis][0] < envelope[axis][0] - epsilon ||
            part[axis][1] > envelope[axis][1] + epsilon
          )
            errors.push(
              `${anchor}/${state}/${part.id}: ${axis} exits declared envelope`,
            );
        if (!part.contact.length)
          errors.push(`${anchor}/${state}/${part.id}: contact path absent`);
        for (const target of part.contact) {
          ...
          if (adjacent) {
            if (
              ...
              errors.push(
                `${anchor}/${state}/${part.id}: no face contact with ${target}`,
              );
          } else if (
            target === "ground" ||
            target === "support" ||
            /^support@[\d.]+$/.test(target)
          ) {
            const height = target.startsWith("support@")
              ? Number(target.slice(8))
              : 0;
            if (
              !occupiedBoxes(part, voids, pieces).some(
                (box) =>
                  Math.abs(box.y[0] - height) <= epsilon &&
                  box.x[1] - box.x[0] > epsilon &&
                  box.z[1] - box.z[0] > epsilon,
              )
            )
              errors.push(`${anchor}/${state}/${part.id}: misses ${target}`);
            if (
              part.shape === "curved" &&
              !shearsZ.has(`${state}/${part.id}`) &&
              !provedExternal.has(`${part.id}/${target}`)
            )
              errors.push(
                `${anchor}/${state}/${part.id}: curved ${target} lacks finite contact patch`,
              );
          } else if (target === "wall") {
            if (
              !occupiedBoxes(part, voids, pieces).some(
                (box) => Math.abs(box.z[0] - envelope.z[0]) <= epsilon,
              )
            )
              errors.push(`${anchor}/${state}/${part.id}: misses wall datum`);
            if (
              (part.shape === "curved" || part.shape === "hollow") &&
              !provedExternal.has(`${part.id}/wall`)
            )
              errors.push(
                `${anchor}/${state}/${part.id}: curved wall lacks finite contact patch`,
              );
          } else if (
            target === "ceiling" ||
            target === "suspension" ||
            ...
            if (
              !occupiedBoxes(part, voids, pieces).some(
                (box) => Math.abs(box.y[1]) <= epsilon,
              )
            )
              errors.push(
                `${anchor}/${state}/${part.id}: misses ${target} datum`,
              );
          } else
            errors.push(
              `${anchor}/${state}/${part.id}: contact target ${target} absent`,
            );
        }
      }
      for (let i = 0; i < stateParts.length; i++)
        for (let j = i + 1; j < stateParts.length; j++) {
          const a = stateParts[i],
            b = stateParts[j],
            extent = overlap(a, b);
          if (
            actualOverlap(
            ...
            if (a.shape === "box" && b.shape === "box")
              errors.push(
                `${anchor}/${state}: ${a.id} intersects ${b.id} (${extent.join(" x ")})`,
              );
            else {
              ...
              const miter =
                ...
                a.shape === "mitered-box" &&
                b.shape === "mitered-box" &&
                Math.abs(extent[0] - 0.009) <= epsilon &&
                Math.abs(extent[1] - 0.018) <= epsilon &&
              ...
              if (miter) provedMiterJoints.add(key);
                ...
                errors.push(
                  `${anchor}/${state}: ${a.id} / ${b.id} needs shape intersection proof (${a.shape}/${b.shape})`,
                );
            }
          }
        }
      const reached = new Set(
        stateParts
          .filter(
            (part) =>
              part.contact.includes("ground") ||
              part.contact.some(
                (target) =>
                  target === "support" || /^support@[\d.]+$/.test(target),
              ) ||
              part.contact.includes("ceiling") ||
              part.contact.includes("suspension") ||
              part.contact.includes("underside") ||
              (part.contact.includes("wall") && !byId.has("wall")),
          )
          .map((part) => part.id),
      );
      let changed = true;
      while (changed) {
        ...
        for (const part of stateParts)
          if (
            !reached.has(part.id) &&
            part.contact.some((target) => reached.has(target))
          ) {
            reached.add(part.id);
            changed = true;
          }
      ...
      for (const part of stateParts)
        if (!reached.has(part.id))
          errors.push(
            `${anchor}/${state}/${part.id}: disconnected from ground or wall`,
          );
    }
    for (const part of parts.values())
      if (!envelopes.has(part.state))
        errors.push(`${anchor}/${part.state}/${part.id}: undeclared state`);
    for (const key of voids.keys())
      if (!parts.has(key)) errors.push(`${anchor}/${key}: void has no part`);
    for (const key of pieces.keys())
      if (!parts.has(key)) errors.push(`${anchor}/${key}: piece has no part`);
    for (const key of radial.keys())
      if (!parts.has(key))
        errors.push(`${anchor}/${key}: radial declaration has no part`);
    for (const key of radialZ.keys())
      if (!parts.has(key))
        errors.push(`${anchor}/${key}: radial-z declaration has no part`);
    for (const key of bores.keys())
      if (!parts.has(key)) errors.push(`${anchor}/${key}: bore has no part`);
    for (const key of boresZ.keys())
      if (!parts.has(key)) errors.push(`${anchor}/${key}: bore-z has no part`);
    for (const key of ellipses.keys())
      if (!parts.has(key)) errors.push(`${anchor}/${key}: ellipse has no part`);
    for (const key of tangents.keys())
      if (!provedTangents.has(key))
        errors.push(`${anchor}/${key}: tangent proof was not discharged`);
    for (const key of curveLayers.keys())
      if (!provedCurves.has(key))
        errors.push(`${anchor}/${key}: curved layer proof was not discharged`);
    for (const key of linearCurves.keys())
      if (!provedLinear.has(key))
        errors.push(`${anchor}/${key}: linear curve proof was not discharged`);
    for (const state of inventory.keys())
      if (!envelopes.has(state))
        errors.push(`${anchor}/${state}: inventory for undeclared state`);
    for (const key of miterJoints)
      if (!provedMiterJoints.has(key))
        errors.push(`${anchor}/${key}: miter proof has no qualifying joint`);
    if (anchor === "folded-towels")
      errors.push(...towelFormula(lines, envelopes, parts));
    if (anchor === "entry-charger")
      errors.push(...chargerFormula(lines, envelopes, voids));
    if (anchor === "rugs")
      errors.push(...rugFormula(lines, envelopes, parts, voids, radial));
    errors.push(...simpleProseEnvelope(anchor, lines, envelopes));
    if (anchor === "dining-pendant")
      errors.push(...pendantFormula(lines, envelopes, radial));
    if (anchor === "books")
      errors.push(...bookFormula(lines, envelopes, parts));
    if (anchor === "tabletop-props")
      errors.push(...propFormula(lines, envelopes, bores, ellipses, tangents));
    if (anchor === "bathtub")
      errors.push(...tubFormula(lines, envelopes, parts, voids));
    if (anchor === "work-equipment")
      errors.push(...equipmentFormula(lines, envelopes, parts, grids));
  }
  return {
    prototypes: allSections.size,
    measuredPrototypes,
    parts: examined,
    hollowParts,
    provedHollowParts,
    cavityProfileParts,
    provedCavityProfiles,
    vesselRows,
    vesselStates,
    provedVesselStates,
    closureRows,
    closureStates,
    provedClosureStates,
    unprovedCurvedContacts: [...unprovedCurved],
    errors,
  };
}
...
function randomFixture() {
  ...
  for (const [owner, lines] of baseline)
    for (let index = 0; index < lines.length; index++) {
      ...
      if (cells.length === 8 && cells[0] === "@part")
        candidates.push({ owner, index, cells });
    }
  ...
  for (const kind of ["nudge", "lift", "delete", "overlap"]) {
    const eligible =
      kind === "overlap"
        ? candidates.filter((entry) =>
            candidates.some(
              (peer) =>
                peer.owner === entry.owner &&
                peer.cells[1] === entry.cells[1] &&
                peer.cells[2] !== entry.cells[2],
            ),
          )
        : candidates;
    ...
    if (kind === "delete") lines.splice(picked.index, 1);
      ...
      const peers = eligible.filter(
        (peer) =>
          peer.owner === picked.owner &&
          peer.cells[1] === cells[1] &&
          peer.cells[2] !== cells[2],
      );
      ...
      if (!envelope)
        throw Error(`missing model envelope ${picked.owner}/${cells[1]}`);
  ...
  for (const [owner, lines] of baseline) {
    ...
    for (let index = 0; index < lines.length; index++) {
      const match =
        /^@(bore|bore-z|cavity-profile|void|ellipse|radial|radial-z)\s+([^:]+):\s*([^,]+)/.exec(
          lines[index],
        );
      if (!match) continue;
    ...
    for (const [key, indices] of grouped)
      if (indices.length === 1 && parsed.parts.get(key)?.shape === "hollow")
        cavityRows.push({ owner, key, index: indices[0] });
  }
  if (!cavityRows.length)
    throw Error("empty hollow cavity mutation population");
  ...
  const cavityError = audit(cavityChanged).errors.find((error) =>
    error.includes(
      `${cavity.owner}/${cavity.key}: hollow part has no authored cavity`,
    ),
  );
  results.push({
    kind: "cavity-delete",
    owner: cavity.owner,
    state: cavity.key.split("/")[0],
    part: cavity.key.split("/")[1],
    red: !!cavityError,
    first: cavityError || null,
  });
  const profiles = [];
  for (const [owner, lines] of baseline)
    for (let index = 0; index < lines.length; index++) {
  ...
  if (!profileSource)
    throw Error(`missing model section ${selectedProfile.owner}`);
  const profileLines = [...profileSource];
  const profilePart = parse(profileLines, selectedProfile.owner).parts.get(
    `${selectedProfile.state}/${selectedProfile.part}`,
  );
  if (!profilePart)
    throw Error(
      `missing cavity profile part ${selectedProfile.owner}/${selectedProfile.state}`,
    );
  profileLines[selectedProfile.index] = profileLines[
    selectedProfile.index
  ].replace(/[\d.]+$/, String(profilePart.x[1] - profilePart.x[0]));
  ...
  const profileError = audit(profileChanged).errors.find((error) =>
    error.includes(
      `${selectedProfile.owner}/${selectedProfile.state}/${selectedProfile.part}: cavity profile mouth`,
    ),
  );
  results.push({
    kind: "profile-mouth",
    ...
    part: selectedProfile.part,
    red: !!profileError,
    first: profileError || null,
  });
  const ellipseProfiles = profiles.filter((entry) => {
    const lines = baseline.get(entry.owner);
    if (
      !lines ||
      !/^@cavity-profile\s+[^:]+:\s*[^,]+,\s*ellipse,/.test(lines[entry.index])
    )
      return false;
    const part = parse(lines, entry.owner).parts.get(
      `${entry.state}/${entry.part}`,
    );
    return part && part.x[1] - part.x[0] > part.z[1] - part.z[0] + epsilon;
  });
  if (!ellipseProfiles.length)
    throw Error("empty elliptical cavity profile population");
  ...
  if (!ellipseSource)
    throw Error(`missing model section ${selectedEllipse.owner}`);
  const ellipseLines = [...ellipseSource];
  ellipseLines[selectedEllipse.index] = ellipseLines[
    selectedEllipse.index
  ].replace(", ellipse,", ", round,");
  ...
  const ellipseError = audit(ellipseChanged).errors.find((error) =>
    error.includes(
      `${selectedEllipse.owner}/${selectedEllipse.state}/${selectedEllipse.part}: round cavity body`,
    ),
  );
  results.push({
    kind: "profile-section",
    owner: selectedEllipse.owner,
    state: selectedEllipse.state,
    part: selectedEllipse.part,
    red: !!ellipseError,
    first: ellipseError || null,
  });
  const red = results.filter((result) => result.red).length;
  if (red !== results.length)
    throw Error(`random model mutation red ${red}/${results.length}`);
  return {
    population: candidates.length,
    cavityPopulation: cavityRows.length,
    profilePopulation: profiles.length,
    ellipseProfilePopulation: ellipseProfiles.length,
...
function randomVesselFixture() {
  ...
  results.push({
    ...
    red: missing.some((error) =>
      error.includes("round cavity has extra depth"),
    ),
    first: missing[0] || null,
  });
  const declaration = /^@vessel-attachments\s+([^:]+):\s*([^,]+)/.exec(
    original[picked.index],
  );
  ...
  const part = parse(profileLines, picked.owner).parts.get(
    `${state}/${declaration[2].trim()}`,
  );
  if (profileAt < 0 || !part) throw Error("selected vessel profile vanished");
  profileLines[profileAt] = profileLines[profileAt].replace(
    /[\d.]+$/,
  ...
  const findings = audit(changed).errors;
  results.push({
    field: "random profile mouth",
    state,
    red: findings.some((error) =>
      error.startsWith(`${picked.owner}/${state}/`),
    ),
    first: findings[0] || null,
  });
  const red = results.filter((result) => result.red).length;
  if (red !== results.length)
    throw Error(`vessel attachment mutations red ${red}/${results.length}`);
  return {
    population: candidates.length,
    statePopulation: stateOptions.length,
    mutations: results.length,
    red,
    results,
  };
...
function randomClosureFixture() {
  ...
  for (const [owner, lines] of baseline)
    for (let index = 0; index < lines.length; index++)
      if (lines[index].startsWith("@vessel-closure"))
        candidates.push({ owner, index });
  if (!candidates.length)
    throw Error("empty vessel closure mutation population");
  const picked = candidates[randomInt(candidates.length)];
  const source = baseline.get(picked.owner) || [];
  const results = [];
  const invalid = [...source];
  invalid[picked.index] = invalid[picked.index].replace(/\d+\/\d+$/, "2/1");
  if (invalid[picked.index] === source[picked.index])
    throw Error("selected vessel closure declaration unchanged");
  const withInvalid = new Map(baseline);
  withInvalid.set(picked.owner, invalid);
  const invalidErrors = audit(withInvalid).errors;
  results.push({
    kind: "cap height",
    red: invalidErrors.some((error) =>
      error.includes("cap cannot close the neck within measured bounds"),
    ),
    first: invalidErrors[0] || null,
  });
  ...
  results.push({
    kind: "closure deleted",
    red: removedErrors.some(
      (error) =>
        error.startsWith(`${picked.owner}: prose names vessel closure`) ||
        (error.startsWith(`${picked.owner}/`) &&
          error.includes("lacks a closure")),
    ),
    first: removedErrors[0] || null,
  });
  const declaration = /^@vessel-closure\s+([^:]+):\s*([^,]+),/.exec(
    source[picked.index],
  );
  ...
  if (profileAt < 0 || !part)
    throw Error("selected closure profile disappeared");
  profileLines[profileAt] = profileLines[profileAt].replace(
    /[\d.]+$/,
    String(part.x[1] - part.x[0]),
  );
  const withWideMouth = new Map(baseline);
  withWideMouth.set(picked.owner, profileLines);
  const profileErrors = audit(withWideMouth).errors;
  results.push({
    kind: "random mouth",
    state,
    red: profileErrors.some((error) =>
      error.startsWith(`${picked.owner}/${state}/`),
    ),
    first: profileErrors[0] || null,
  });
  const red = results.filter((result) => result.red).length;
  if (red !== results.length)
    throw Error(`vessel closure mutations red ${red}/${results.length}`);
  return {
    population: candidates.length,
    statePopulation: states.length,
    mutations: results.length,
    red,
    results,
  };
...
if (require.main !== module) {
  ...
  if (baseline.errors.length || baseline.parts !== 5)
    throw Error(`dining-table baseline failed: ${baseline.errors}`);
  const mutations = [
    [
      "out of bounds",
      "-0.90..0.90 | 0.696..0.74",
      "-0.90..1.10 | 0.696..0.74",
      "exits declared envelope",
    ],
    [
      "lifted leg",
      "-0.8325..-0.7875 | 0..0.696",
      "-0.8325..-0.7875 | 0.05..0.696",
      "misses ground",
    ],
    [
      "missing part",
  ...
  for (const [anchor, before, after] of [
    [
      "rear-counter-sink",
      "@void default: bowl, 0.915..1.285, 0.82..0.925",
      "@void default: bowl, 0.915..1.285, 0.80..0.925",
    ],
    [
      "bath-accessories",
      "@void laundry-basket: body, -0.22..0.22, 0.033..0.4",
      "@void laundry-basket: body, -0.22..0.22, 0.001..0.4",
    ],
  ]) {
    const lines = sections().get(anchor);
    if (!lines || !lines.join("\n").includes(before))
      throw Error(`${anchor}: cavity fixture source absent`);
    const measured = audit(
      new Map([[anchor, lines.join("\n").replace(before, after).split("\n")]]),
    );
    if (
      !measured.errors.some((error) =>
        error.includes("sealed cavity floor or wall below declared minimum"),
      )
    )
      throw Error(
        `${anchor}: cavity floor mutation escaped: ${measured.errors}`,
      );
  ...
  const movedPatch =
    "@flat-contact outdoor-chair: body, support, -Y, 0, -0.28..-0.25, -0.28..-0.25";
  if (!exterior || !exterior.join("\n").includes(oldPatch))
    throw Error("outdoor sole fixture source absent");
  const movedErrors = audit(
    new Map([
      [
        "exterior-furnishings",
        exterior.join("\n").replace(oldPatch, movedPatch).split("\n"),
      ],
    ]),
  ).errors;
  if (
    !movedErrors.some((error) =>
      error.includes("flat contact has no finite common patch"),
    )
  )
    throw Error(`outdoor sole contact mutation escaped: ${movedErrors}`);
  results.push({ label: "outdoor sole contains contact", caught: true });
  ...
  if (stoolBaseline.errors.length)
    throw Error(`island-stool baseline failed: ${stoolBaseline.errors}`);
  const movedMiter = stool
    .join("\n")
    .replace(
      "-0.1065..-0.0885 | leg-0,leg-2",
      "-0.1065..-0.0875 | leg-0,leg-2",
    );
  if (movedMiter === stool.join("\n"))
    throw Error("island-stool miter mutation source absent");
  const miterErrors = audit(
    new Map([["island-stool", movedMiter.split("\n")]]),
  ).errors;
  if (
    !miterErrors.some((error) =>
      error.includes("needs shape intersection proof"),
    )
  )
  ...
  const removedDeck = bedSource.replace(
    "| @part | 1000 | support-deck | box | -0.515..0.515 | 0.26..0.28 | -1.03..1.03 | frame-side-left,frame-side-right,mattress |",
    "",
  );
  if (
    removedDeck === bedSource ||
    !audit(new Map([["fixed-bed", removedDeck.split("\n")]])).errors.some(
      (error) => error.includes("inventoried part support-deck absent"),
    )
  )
  ...
  const changedFormula = towelSource.replace(
    "두 0.004m 음영 틈",
    "두 0.005m 음영 틈",
  );
  if (
    changedFormula === towelSource ||
    !audit(
      new Map([["folded-towels", changedFormula.split("\n")]]),
  ...
  const chargerMutations = [
    [
      ...
      "",
      "needs shape intersection proof",
    ],
    [
      "recess narrowed",
      "@void default: body, -0.026..0.026, 0.013..0.015, -0.0375..0.0375",
      "@void default: body, -0.020..0.020, 0.013..0.015, -0.0375..0.0375",
      "needs shape intersection proof",
    ],
    [
      "cavity exits host",
      "@void default: body, -0.006..0.006, 0.0045..0.0105, 0.05..0.06",
      "@void default: body, -0.006..0.006, 0.0045..0.0105, 0.05..0.07",
      "void z exits host",
    ],
    [
      "prose width changed",
  ...
  const undersideCandidates = [...undersideSections].flatMap(([owner, lines]) =>
    lines.flatMap((line, index) => {
      ...
      return cells.length === 8 &&
        cells[0] === "@part" &&
        cells[7].split(",").includes("underside")
        ? [{ owner, index, cells }]
        : [];
    }),
  );
  if (!undersideCandidates.length)
    throw Error("empty underside contact mutation population");
  ...
  if (
    !audit(undersideSections).errors.some((error) =>
      error.includes("misses underside datum"),
    )
  )
    throw Error(
      `${underside.owner}/${underside.cells[1]}/${underside.cells[2]}: underside contact mutation did not fail`,
    );
  ...
  if (audit(new Map([["rugs", rugs]])).errors.length)
    throw Error("rugs baseline failed");
  const rugMutations = [
    [
      "rectangular cavity deleted",
      "@void living: bound-edge, -1.375..1.375, 0.013..0.016, -1.8..1.8",
      "",
      "needs shape intersection proof",
    ],
    [
      "radial ring widened inward",
      "@radial round1200: bound-edge, 0.575, 0.6",
      "@radial round1200: bound-edge, 0.57, 0.6",
      "needs shape intersection proof",
    ],
    [
      "round pile radius changed",
      "@radial round1200: pile, 0, 0.575",
      "@radial round1200: pile, 0, 0.57",
      "prose radial border width differs",
    ],
    [
      "prose living width changed",
      "폭 2.80, 깊이 3.65",
      "폭 2.90, 깊이 3.65",
      "prose/table interval differs",
    ],
  ];
  for (const [label, before, after, expected] of rugMutations) {
    if (!rugsSource.includes(before))
      throw Error(`rugs ${label}: mutation source absent`);
    const errors = audit(
      new Map([["rugs", rugsSource.replace(before, after).split("\n")]]),
    ).errors;
    if (!errors.some((error) => error.includes(expected)))
  ...
  for (const [anchor, before, after] of [
    ...
    if (!source || !source.includes(before))
      throw Error(`${anchor}: prose mutation source absent`);
    const errors = audit(
      new Map([[anchor, source.replace(before, after).split("\n")]]),
    ).errors;
    if (
      !errors.some((error) => error.includes("prose/table x envelope differs"))
    )
      throw Error(
        `${anchor}: prose mutation missed envelope mismatch: ${errors}`,
      );
    results.push({ label: `${anchor} prose width changed`, caught: true });
  }
  const pendantSource = sections().get("dining-pendant")?.join("\n");
  if (!pendantSource || !pendantSource.includes("지름 0.045m"))
    throw Error("pendant mutation source absent");
  const pendantErrors = audit(
    new Map([
      [
        "dining-pendant",
        pendantSource.replace("지름 0.045m", "지름 0.050m").split("\n"),
      ],
    ]),
  ).errors;
  ...
  results.push({
    label: "dining-pendant prose shade diameter changed",
    caught: true,
  });
  const bookSource = sections().get("books")?.join("\n");
  if (!bookSource || !bookSource.includes("두께 0.004m"))
    throw Error("books mutation source absent");
  const bookErrors = audit(
    new Map([
      ["books", bookSource.replace("두께 0.004m", "두께 0.005m").split("\n")],
    ]),
  ).errors;
  if (
    !bookErrors.some((error) =>
      error.includes("prose formula differs from table"),
    )
  )
  ...
  for (const [label, before, after, expected] of [
    [
      "cup pad moved into body",
      "0.042074..0.0825 | body",
      "0.040000..0.0825 | body",
      "tangent proof invalid",
    ],
    [
      "cup bore widened",
      "@bore cup: body, 0.0365",
      "@bore cup: body, 0.0430",
      "bore is not a contained open cavity",
    ],
    [
      "tray inner radius enlarged",
      "@ellipse tray: rim, 0.174",
      "@ellipse tray: rim, 0.181",
      "elliptic ring differs",
    ],
    [
      "bowl prose diameter changed",
      "외경 0.22, 높이",
      "외경 0.23, 높이",
      "prose/table envelope differs",
    ],
  ]) {
    if (!propsSource.includes(before))
      throw Error(`${label}: mutation source absent`);
    const errors = audit(
      new Map([
        ["tabletop-props", propsSource.replace(before, after).split("\n")],
      ]),
    ).errors;
    if (!errors.some((error) => error.includes(expected)))
  ...
  if (!tubSource || !tubSource.includes("길이 1.53×폭 0.68m"))
    throw Error("bathtub prose mutation source absent");
  const tubErrors = audit(
    new Map([
      [
        "bathtub",
        tubSource
          .replace("길이 1.53×폭 0.68m", "길이 1.50×폭 0.68m")
          .split("\n"),
      ],
    ]),
  ).errors;
  ...
  for (const [label, before, after, expected] of [
    [
      "grid loses four keys",
      "@grid keyboard: keys, 12, 4",
      "@grid keyboard: keys, 11, 4",
      "inventoried part keys-44 absent",
    ],
    [
      "grid pitch overlaps keys",
      "@grid keyboard: keys, 12, 4, 0.025",
      "@grid keyboard: keys, 12, 4, 0.010",
      "intersects keys-1",
    ],
    [
      "prose pitch changed",
      "×0.025m, 행",
      "×0.026m, 행",
      "prose/table dimension",
    ],
  ]) {
    if (!equipmentSource.includes(before))
      throw Error(`${label}: mutation source absent`);
    const errors = audit(
      new Map([
        ["work-equipment", equipmentSource.replace(before, after).split("\n")],
      ]),
    ).errors;
    if (!errors.some((error) => error.includes(expected)))
      throw Error(`${label}: expected ${expected}, got ${errors}`);
  ...
  for (const [label, before, after] of [
    [
      "cabinet prose island door clearance changed",
      "y=0.101..0.849",
      "y=0.101..0.850",
    ],
    [
    ...
    if (!cabinetSource.includes(before))
      throw Error(`${label}: mutation source absent`);
    let caught = false;
    try {
      cabinetProducer.check(cabinetSource.replace(before, after));
    } catch {
      caught = true;
    }
  ...
  if (!benchSource?.includes(composeLine) || !cabinetSection)
    throw Error("entry-bench composition fixture absent");
  for (const [label, after, expected] of [
    ["child removed", "", "cabinet child composition absent"],
    [
      "child state changed",
      composeLine.replace("/closed", "/unknown"),
      "composition child state absent",
    ],
    [
      "child lifted",
      composeLine.replace(", 0, 0, 0", ", 0, 0.01, 0"),
      "composite y envelope differs",
    ],
  ]) {
    const errors = audit(
      new Map([
  ...
  const curvedMutations = [
    [
      ...
      "1800",
      (parsed) => {
        const rail = parsed.parts.get("1800/frame-head");
        if (!rail) throw Error("head rail absent");
        rail.z[1] = -1.06;
      },
      "leg-0: no face contact with frame-head",
    ],
    [
      "murphy-bed",
      "guest",
      (parsed) => {
        const part = parsed.parts.get("guest/bed-frame");
        if (!part) throw Error("guest frame absent");
        part.z[0] = 0.23;
      },
      "pin end cap has no finite contact face",
    ],
    [
      "murphy-bed",
      "guest",
      (parsed) => {
        parsed.boresX.delete("guest/case-side-left");
      },
      "hollow part has no authored cavity",
    ],
    [
      "murphy-bed",
      "guest",
      (parsed) => {
        const bore = parsed.boresX.get("guest/case-side-left");
        if (!bore) throw Error("side bore absent");
        bore.radius = 0.03;
      },
      "axial bore lacks a fitted cylinder contact",
    ],
    [
      "murphy-bed",
      "guest",
      (parsed) => {
        parsed.flatContacts.delete("guest/pillow/mattress");
      },
      "murphy-bed/guest/mattress/pillow",
    ],
    [
      "ceiling-surface-light",
      "default",
      (parsed) => {
        const part = parsed.parts.get("default/diffuser");
        if (!part) throw Error("diffuser absent");
        part.y = [-0.015, -0.012];
      },
      "emissive face is occluded",
    ],
    [
      "dining-pendant",
      "default",
      (parsed) => {
        parsed.emitterFaces.delete("default");
      },
      "emitter face declaration absent",
    ],
    [
      "portable-lamps",
      "reading",
      (parsed) => {
        parsed.emitterFaces.delete("reading");
      },
      "emitter face declaration absent",
    ],
    [
      ...
      "desk-task",
      (parsed) => {
        parsed.emitterFaces.delete("desk-task");
      },
      "emitter face declaration absent",
    ],
    [
      ...
      "reading",
      (parsed) => {
        const part = parsed.parts.get("reading/diffuser");
        if (!part) throw Error("reading diffuser absent");
        part.y = [1.13, 1.134];
      },
      "emissive face is occluded",
    ],
    [
      ...
      "desk-task",
      (parsed) => {
        const part = parsed.parts.get("desk-task/diffuser");
        if (!part) throw Error("task diffuser absent");
        part.y = [0.38, 0.384];
      },
      "emissive face is occluded",
    ],
    [
      ...
      "guest",
      (parsed) => {
        parsed.pinFaces.delete("guest/hinge-right");
      },
      "missing pin-face declaration",
    ],
    [
      "toilet",
      "lid-open",
      (parsed) => {
        const ring = parsed.ellipses.get("lid-open/seat");
        if (!ring) throw Error("seat ring absent");
        ring.centerZ = 0.1;
      },
      "elliptic ring differs",
    ],
    [
      ...
      "oven",
      (parsed) => {
        const binding = parsed.supportBindings.get("oven");
        if (!binding) throw Error("oven support absent");
        binding.state = "unknown";
      },
      "support child part absent",
    ],
    [
      "kitchen-island",
      "default",
      (parsed) => {
        const binding = parsed.supportBindings.get("default");
        if (!binding) throw Error("island support absent");
        binding.offset[0] = -0.3;
      },
      "penetrates support child",
    ],
  ];
  for (const [anchor, state, mutate, expected] of curvedMutations) {
    ...
    if (
      (anchor === "cooking-appliances" || anchor === "kitchen-island") &&
      !cabinetLines
    )
      throw Error("cabinet fixture child absent");
    const sample = new Map([[anchor, lines]]);
    if (
      (anchor === "cooking-appliances" || anchor === "kitchen-island") &&
      cabinetLines
    )
      sample.set("cabinet-and-shelf", cabinetLines);
    const check = audit(
      sample,
      (owner, parsed) => {
        if (owner === anchor) mutate(parsed);
      },
      state,
    );
    ...
    if (!findings.some((error) => error.includes(expected)))
      throw Error(
        `${anchor}/${state}: mutation did not reach ${expected}: ${findings}`,
      );
  ...
  const changedComponents = cabinetComponentLines.map((line) =>
    line === "@component-count media/2000x440x350/closed: fixed-front, 3"
      ? "@component-count media/2000x440x350/closed: fixed-front, 2"
      : line,
  );
  if (
    changedComponents.every(
      (line, index) => line === cabinetComponentLines[index],
    ) ||
    !audit(
      new Map([["cabinet-and-shelf", changedComponents]]),
      undefined,
      "media/2000x440x350/closed",
    ).errors.some((error) =>
      error.includes("occupied components contradict declared"),
    )
  )
    throw Error("cabinet disconnected-front component count mutation escaped");
  results.push({
    label: "cabinet disconnected-front component count",
    caught: true,
  });
  ...
  if (!laundryBoreSource.includes(circularBore))
    throw Error("laundry circular bore declaration absent");
  for (const [label, replacement, expected] of [
    [
      "laundry circular bore narrowed",
      "@bore-z washer: body, 0, 0.38, 0.17, 0.246..0.30",
      "needs shape intersection proof",
    ],
    [
      "laundry circular bore widened",
      "@bore-z washer: body, 0, 0.38, 0.21, 0.246..0.30",
      "no face contact",
    ],
    ...
    const errors = audit(
      new Map([
        [
          "laundry-appliances",
          laundryBoreSource.replace(circularBore, replacement).split("\n"),
        ],
      ]),
    ).errors;
    if (!errors.some((error) => error.includes(expected)))
  ...
  const flatMutations = [
    [
      "basin",
      "800",
      "800/tap-spout/tap-body",
      (patch) => {
        patch.u = [0.14, 0.17];
      },
      "no finite common patch",
    ],
    [
      "kitchen-island",
      "default",
      "default/tap-spout/tap-body",
      (patch) => {
        patch.plane += 0.001;
      },
      "no finite common patch",
    ...
    [
      "portable-lamps",
      "bedside-globe",
      "bedside-globe/globe/stem-short",
      (patch) => {
        patch.u = [-0.009, 0.009];
      },
      "no finite common patch",
  ...
  for (const [anchor, state, key, change, expected] of flatMutations) {
    ...
    const errors = audit(
      ...
      (owner, parsed) => {
        ...
        if (!patch)
          throw Error(`${anchor}/${key}: flat-contact fixture absent`);
        change(patch);
      },
      state,
    ).errors;
    ...
    results.push({
      label: `${anchor}/${key} finite contact mutation`,
      caught: true,
    });
  }
  {
    const anchor = "dining-chair",
      state = "default",
      lines = sections().get(anchor);
    ...
    results.push({
      label: `${anchor} shear-z endpoint mutation`,
      caught: true,
    });
  }
  const plantSource = fs.readFileSync(
    path.join(root, "docs/models/004-decor-and-fixtures.md"),
    "utf8",
  );
  const plantMutations = [
    [
      "height array changed",
      '"heights":[180,280,600,800,1100]',
      '"heights":[180,280,610,800,1100]',
    ],
    ["soil surface changed", '"soilSurface":0.34', '"soilSurface":0.30'],
  ];
  for (const [label, before, after] of plantMutations) {
    if (!plantSource.includes(before))
      throw Error(`plant ${label} mutation source absent`);
    let caught = false;
    try {
      plantProducer.check(plantSource.replace(before, after));
    } catch {
      caught = true;
    }
  ...
  for (const [anchor, lines] of completeSections) {
    ...
    const fixtureSections = () => {
      ...
      while (pending.length) {
        ...
        if (!source)
          throw Error(`${anchor}: missing fixture dependency ${current}`);
        const parsed = parse(source, current);
        for (const child of [
          ...parsed.compositions.values(),
          ...parsed.supportBindings.values(),
        ]) {
          if (selected.has(child.anchor)) continue;
          const childLines = completeSections.get(child.anchor);
          if (!childLines)
            throw Error(`${anchor}: fixture child ${child.anchor} absent`);
    ...
    if (ownBaseline.errors.length)
      throw Error(`${anchor}: fixture baseline failed: ${ownBaseline.errors}`);
    for (const part of parts.values()) {
      measuredParts++;
      ...
      const check = (change, expected) => {
        const found = audit(
          ...
          (owner, parsed) => {
            ...
            if (!entry)
              throw Error(
                `${anchor}/${part.state}/${part.id}: mutation target absent`,
              );
            change(entry, parsed);
          },
          part.state,
        ...
        if (!found.some((error) => error.includes(expected)))
          throw Error(
            `${anchor}/${part.state}/${part.id}: mutation missed ${expected}: ${found}`,
          );
        mutationChecks++;
      };
      check((entry) => {
      ...
      check((_entry, parsed) => {
        parsed.parts.delete(`${part.state}/${part.id}`);
      }, `inventoried part ${part.id} absent`);
      check((entry) => {
        entry.contact = [];
      }, "contact path absent");
    }
  ...
  for (const axis of /** @type {const} */ ["x", "y", "z"]) {
    ...
    if (
      ...
      cylinderCapArea(cylinder, broad, axis) <= 1e-8 ||
      cylinderCapArea(cylinder, tangent, axis) > 1e-8
    )
      throw Error(
        `${axis}-axis cylinder cap accepted a tangent or rejected a finite patch`,
      );
  ...
  console.log(
    JSON.stringify(
      {
        baselineParts: baseline.parts,
        measuredParts,
        mutationChecks,
        mutations: results,
        unselected: randomFixture(),
        vesselAttachments: randomVesselFixture(),
        vesselClosures: randomClosureFixture(),
      },
      null,
      2,
    ),
  ...
  if (result.unprovedCurvedContacts.length)
    result.errors.push(
