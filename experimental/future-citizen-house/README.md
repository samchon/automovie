...
불합격한 Canvas2D 구현을 제거하고 production 내부 CJS 서버 → 공개 engine → WebGL 클라이언트 경로를 작성했다. 새 집 형상을 입면·층·계단·12개 방·대지의 완결 owner로 제자리 재작성했다. 뷰어는 같은 typed producer를 CommonJS로 실행하며 evidence 검증은 별도의 canonical lint가 맡는다. tsx는 package.json에 선언했으며 조정자의 설치와 그 뒤의 HTTP 검증이 아직 필요하다. 첫 시작 시도는 실행기 미설치로 exit 1이었다. 현재 GPU 관찰은 미지급이다. three와 @types/three는 package.json에 선언했고 조정자가 설치했다. 현재 RENDERER와 다섯 reference의 화면 일치는 unverified다.
