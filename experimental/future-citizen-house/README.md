...
[Assembly.material](src/house/assembly.ts)은 표면 owner가 지정한 재료 이름으로 native material을 생성한다. 현재 `pv`만 [셀 색상·alpha 텍스처](src/house/canopy-finish.ts)를 사용하며 그 밖의 재료 이름은 명시된 단색과 roughness로 그린다. 절차 생성 texture PNG와 [생성 코드](src/materials/generate-textures.mjs)는 보존하지만 재료 바인딩이 은퇴한 동안 화면의 비-PV 표면에는 결합하지 않는다. 재료 바인딩은 materials review 뒤 materialSources draft에서 재개한다. 복구 지점: `36eec2d161f9438ef04414073a207fda855d1bdc`의 `src/materials/bindings.ts`와 `src/house/assembly.ts`.
...
[재료 설계](docs/materials/001-binding-and-scale.md)는 불투명 외피·유리/PV·목재·직물/도장·습식 마감을 분해한다. [관찰 집합](docs/materials/007-observation.md)은 현재 topology 전수 관찰과 다섯 reference에 거리·접합·상태 표본을 더한다. v-082의 설계 PASS 뒤 2026-09-24에 `materials: evidence`로 전진했고 [공통](docs/accounts/materials/core-common.md)·[재료](docs/accounts/materials/design-materials.md) 의무 account를 더했다. [선행 관계 gate](.agents/skills/evidence-graph/staging.md#transitions)에 필요한 `materials: review`가 아직 아니므로 materialSources 선언은 비활성이다. 바인딩 복구와 실제 재료 source 검증은 그 뒤 단계로 남긴다. 이전 결합과 전후 캡처를 정식 재료 source 또는 전수 시각 PASS로 기록하지 않는다. 새 창호 깊이·차양·가구 형상은 이 설계에서 승인하지 않는다.
