...
[재료 초안](docs/materials/001-binding-and-scale.md)은 현재 전달 한계 안에서 불투명 외피·유리/PV·목재·직물/도장·습식 마감을 분해한다. [관찰 집합](docs/materials/007-observation.md)은 현재 topology 전수 관찰과 다섯 reference에 거리·접합·상태 표본을 더한다. `materials: draft`이며 소스 구현이나 재료 시각 PASS를 뜻하지 않는다. 새 창호 깊이·차양·가구 형상은 이 초안에서 승인하지 않는다.
