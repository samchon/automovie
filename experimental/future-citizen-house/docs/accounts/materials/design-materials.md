...
<!--
@evidence obligations/design/materials.md#addressable-material-decisions 새 마감 25종과 보존·retained 역할, texture family 9종, 관찰 4역할을 29 H2에 배정한다. 석재 입자, 층간 띠 경계, 목재 끝면, 도기와 좌판의 광택처럼 따로 바뀌는 결정은 각자 H2를 갖고, 같은 구성·좌표·표본을 공유하는 변형(직물 네 색, steel과 거울, 두 joinery 도장, 유리 두 상태, PV와 canopy-metal, 기준 목재 마감별 끝면)은 한 H2 안의 명시 항목으로 둔다.
@evidence obligations/design/materials.md#material-identity-assembly 각 마감 H2가 기준색을 정하고 표면층이 있는 마감은 명목 두께(실내 도장 .15mm, 층간 띠·coated-metal 도막 .08mm, joinery 도막 .1mm, 바닥·계단 투명 .06mm, 문·수납 베니어 .6mm+투명 .06mm, 가구 베니어 .6mm, 유약 .5mm, 직물 .001m, 섬유 .003m)를 이름으로 정하며, 층이 없는 석재·유리·금속·tile·좌판·screen 직물은 geometry의 재료 자체로 두고, 허용 대체는 retained/<현재 material id> 하나뿐이라고 배정한다.
@evidence obligations/design/materials.md#material-surface-assignment 001의 역할 주소 위에서 002~006이 표면군을 나눠 받고 wood-end·floor-band·retained로 끝면·부분 띠·보존 면까지 배정한다. oak cabinet은 back 판까지, junction 노출면은 향한 방의 벽 마감을, slab는 계단 구멍 쪽 절단면과 전면 strip 윗면만, 전면 외벽 body의 실내 면은 계단 void 띠만 도장을 받는다. 층간 띠 끝을 가로지르는 두 panel member와 wall body 외향 면 안의 띠 경계를 빼면 두 마감은 실제 벽·문턱·부재 모서리에서만 바뀐다.
...
| --- | --- | --- |
| limestone-honed | 002#limestone-panels | 네 입면의 stone panel·wall body·corner prism·drip(층간 띠 외향 면과 계단 void에 드러나는 면 제외), roof-slab과 roof-bearing ring 전체 |
...
두 마감이 만나는 곳은 층간 띠 끝을 가로지르는 두 panel member(`front-face-stone-panels:1-4-0`, `rear-face-stone-panels:2-3-0`)와 wall body 외향 면 안의 띠 경계를 빼면 실제 형상의 경계다. 명목 표면층은 실내 도장 .15mm, 층간 띠·coated-metal 도막 .08mm, joinery 도막 .1mm, 바닥·계단 투명 .06mm, 문·수납 베니어 .6mm+투명 .06mm, 가구 베니어 .6mm, 유약 .5mm, 직물 .001m, 섬유 .003m이고 석재·유리·금속·tile·좌판·screen 직물은 층 없이 geometry의 재료 자체다. 공유 벽은 양쪽 방 lining이 각자 마감을 받고(예: 현관 plaster/작업실 felt, 복도 plaster/욕실 tile), 문턱은 host 벽 두께를 채운 oak 문턱판이 tile 방 쪽 벽면 선에서 tile 바닥과 만난다. 층간 띠는 쌍 창의 frame 바깥선에서 끝나는 석재 위 도장이고, junction 노출면은 향한 방의 벽 마감(공용부 전면벽은 도장, 욕실 동측 벽은 tile)을 따른다. 같은 면에 서로 다른 두 사후 칠하기 owner를 두지 않는다.
