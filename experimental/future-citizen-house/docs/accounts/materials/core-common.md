...
@evidence obligations/core/common.md#proportionate-development 7파일 29 H2의 주석 제외 본문 36,506자와 draft(929170b2)의 29,656자, H2별 438~5,783자 분포를 기록했다. 채널·좌표와 관찰, 여러 부재의 경계를 가르는 층간 띠와 실내 도장에 긴 본문을, 단일 마감에는 짧은 본문을 두었고 증가분은 v-084·v-086·v-088이 요구한 범위·부류·면 분할·색 배정, 교차 member·관찰 station 결정, 현재 소비 경로·texture 공유의 사실 서술, 좌표·석재·census의 근거와 저작 선택 표시, census가 대조할 prototype·자원·viewer·목재 끝면 항목이다.
-->
...
2026-09-24 v-084·v-086·v-088 판정의 수리와 그 뒤 요청 전 전수 검사의 수리 뒤 일곱 파일의 evidence 주석을 제거하고 연속 빈 줄을 한 빈 줄로 정규화한 문자열을 셌다. 비교 기준은 draft를 커밋한 929170b2의 같은 파일이다. 문자 수는 heading을 포함하며 시각 품질의 점수가 아니다.

| 파일 | H2 | 본문 문자 수 | draft 본문 | H2별 문자 범위 |
| --- | ---: | ---: | ---: | ---: |
| 001-binding-and-scale.md | 3 | 8397 | 7164 | 1337–4013 |
| 002-exterior-solids.md | 4 | 5380 | 3706 | 694–2804 |
...
| 007-observation.md | 4 | 9074 | 7934 | 626–5783 |
| 합계 | 29 | 36506 | 29656 | 438–5783 |

가장 긴 H2는 007#scale-and-junction-samples와 001#surface-bindings다. 전자는 native validateTextureScale의 입력·반환·경고 조건과 production wrapper schema, 후자는 prototype 레코드와 instanceSlot 소비 경로를 소유해 모든 textured 표면이 그 계약을 공유한다. 단일 마감 H2는 대부분 438~1,146자로 색·응답·texture·좌표·표본의 같은 항목을 각자 채운다. 층간 띠(2,804자)는 네 입면 창 쌍의 frame 포함 범위, 띠 끝을 가로지르는 두 panel member와 slab 가장자리의 결합 조건을, 실내 도장(2,145자)은 junction 12개·노출면 16개를 향한 공간의 벽 마감에 맞추는 규칙과 slab 면 분할을, 직물(1,434자)은 네 색의 항목별 배정과 곡면 pillow 좌표를 담아 길다. draft 대비 +6,850자는 001이 현재 payload의 prototype 소비 경로, viewer의 texture source 공유·three cache key·정적 batch, 좌표 H2의 저작 선택과 validateTextureScale 원본, 표면 분해·완결 표면·좌표·납품 기준 링크를 사실대로 적고 낡은 단계 표현을 정리한 1,233자, 이 두 H2와 wet-tile의 junction 1면·그 면을 담는 관찰 station·retained 역할 명시와 그에 맞춘 exposed-steel 대상 목록, 직물 색 배정, census·접합 표본의 면별 목록, 석재가 받는 근거와 저작 선택의 표시, census의 member별 prototype·modelRecipe와 variant·texture 자원·RGBA bytes·viewer 대조·목재 끝면 수 항목, 낡은 단계 표현 정리이며 walnut 제거로 004는 22자 줄었다. 파일 수와 H2 수는 draft와 같다.
