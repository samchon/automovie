...
한국어로 판단과 수치의 근거를 쓰고 finish id, texture id, `*-floor-boards` 같은 역할 주소, 공개 API 이름은 source에서 쓰는 표기 그대로 둔다. roughness·metallic·moiré 같은 렌더 용어도 원어로 둔다. cassette·seal의 texture asset은 보존되지만 현 단계에는 native binding이 없다. 본문의 양이나 evidence 단계가 전체 재료 구현이나 시각 합격을 뜻하지 않는다. 완성된 materialSources, 실제 native census, GPU 프레임과 reference 대조는 이후 단계의 미완료로 남아 있다.
