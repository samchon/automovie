...
@evidenceReview obligations/core/common.md#purpose-fit #7b32c66 공간 문서의 76개 H2는 집의 포함, cell·문·계단, 외피와 관찰 주소를 나눈다. 이 배분은 건축 source의 입력이지 실내 물체 형상이나 전체 시각 판정의 대체물이 아니다.
...
@evidenceReview obligations/core/common.md#layer-boundary #5271f94 001의 집·대지·관찰, 002의 내부 구획, 003의 외피 건축 결정을 spaces에 두고 source 실행과 GPU 결과는 각 소비 경로에서 따로 읽는다. 현재 방 source의 가구 메시가 공간 문서의 영구 물체 소유권을 만들지 않는다.
...
@evidenceReview obligations/core/common.md#production-language #3ef4142 집·구획·외피의 판단은 한국어 본문으로 적고 room·storey·portal ID와 축은 source와 대응하는 주소로 남긴다. 코드 이름만으로 공간 결정이나 미검증 성과를 선언하지 않는다.
...
@evidenceReview obligations/core/common.md#proportionate-development #78feb28 집·내부 구획·외피의 H2 분포는 3·54·19개이며 과거 단일 H2의 혼합 역할과 구별한다. 2026-09-21 문자 수 표는 당시 snapshot이고 현재 source 구현이나 전수 시각 판정의 수치가 아니다.
-->
...
철회 전 worklog에 보존한 공간 본문은1 H2·5,674자였고, 한 H2가 상태 보고·집 계층·방 그래프·외피·fit-out을 함께 담았다. 현재는 포함·도착·관찰, 내부 구획·접속, 외피의 공간 결정을 나누고 검토 이력과 작성자 배정을 각각 worklog와 settings owner로 옮겼다. 이전 버전 전체의 파일별 본문 snapshot은 남아 있지 않아 그 전체 인구 비교는 unverified이며 이 단일 보존본으로 과거 모든 파일의 양을 추정하지 않는다. 건축 source는 구현됐고 부분 컴파일·GPU 관찰 이력이 있지만 현재 트리의 전체 관찰 분모 판정은 unverified다. 개별 room·opening·wall의 분리는 수정 주소를 만들 뿐 그 수로 방의 읽힘이나 요구 관찰 통과를 주장하지 않는다.
