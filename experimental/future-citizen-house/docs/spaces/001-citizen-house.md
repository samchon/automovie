...
**Status:** latest independent read-only reviewer가 다섯 reference를 실제로 모두 열어 확인한 source 결손을 반영해 stair guard/rail, 식재 형상, common kitchen fit-out와 warm practical light, 열린 flex sliding-door를 보강했다. 이 보강 이후 source revision에 대한 current-source 시각 review gate와 다섯 reference 재비교는 아직 독립 재판정 전이며, 측정하지 않은 건물 성능이나 인체 사용성 인증은 여전히 주장하지 않는다.
