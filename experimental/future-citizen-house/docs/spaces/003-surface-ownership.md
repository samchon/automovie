...
@evidenceReview settings/001-production.md#production-visual-grammar 입면·층·방의 geometry와 material을 같은 완결 면의 owner에 귀속하여 공통 재료 언어를 서로 덧칠하는 구현으로 쪼개지 않는다.
-->

<!--
@evidence principles/core/common.md#declared-basis 본채와 표면 분해 선언을 받아 외피를 네 입면과 지붕으로 나누고 datum에서 두께와 층선을 받는다.
@evidenceReview principles/core/common.md#declared-basis settings의 surface-decomposition 배정을 공간 source 파일 책임으로 소비한다.
@evidence principles/core/common.md#scope-preservation 외벽·개구·틀·shade·return을 각 완결 표면의 범위에 남기고 실내 floor/ceiling 책임도 연결한다.
@evidenceReview principles/core/common.md#scope-preservation 같은 입면을 stair·flex 조각의 별도 소유권으로 나누지 않는다.
...
@evidenceReview principles/core/common.md#substantive-completion 실내 lining과 floor·ceiling 및 외부 return의 생성 책임까지 구분한다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation 부모의 단독 소유 규칙에 실제 접합과 조립 소비 관계를 더한다.
...
@evidenceReview principles/design/spaces.md#space-topology 외피·room lining·storey 면이 각각 같은 공간 경계를 공유한다.
@evidence principles/design/spaces.md#space-boundary-authority 외곽·층선·두께는 mass datum 하나를 쓰며 입면이 room별 창폭을 따로 복제하지 않는다.
@evidenceReview principles/design/spaces.md#space-boundary-authority portal은 한 번 생성하고 양면 요구만 전달하여 동일 문짝을 중복 저작하지 않는다.
...
@evidenceReview principles/design/spaces.md#space-verification-address seam 변경은 이웃 표면도 재검증하게 하여 파일별 완료로 틈을 숨기지 않는다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work 부모 배정으로 실제 외측·내측·수평면 책임이 구분되어 소유권 규칙을 수정할 필요가 없다.
...
@evidenceReview contracts/citizen-house-spatial-requirements.md#citizen-house-spatial-requirements 네 입면·지붕·각 층·각 방을 온전한 표면 단위로 배정한다.
...
@evidenceReview settings/003-spatial-basis.md#surface-decomposition 표면 하나의 geometry와 material을 한 owner가 맡고 helper는 배치를 소유하지 않는다.
-->
...
@evidenceReview principles/core/common.md#declared-basis front=-Z와 화면 우측=-X 관례를 써 reference 좌우를 뒤집지 않는다.
@evidence principles/core/common.md#scope-preservation 목재 현관문·계단 유리·작업실 유리·코어와 층간 spandrel을 같은 전면의 실제 부재 범위로 남긴다.
@evidenceReview principles/core/common.md#scope-preservation 전면 전체를 한 owner가 맡으며 현관이나 stair 유리만 따로 분할하지 않는다.
@evidence principles/core/common.md#substantive-completion outward -Z, plane 도출, 전체 폭과 네 opening owner 및 나머지 벽 폐쇄를 정했다.
@evidenceReview principles/core/common.md#substantive-completion 문·네 유리 구간·floor band와 corner 접합까지 면 전체를 지정한다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation 부모의 전면 커튼월 요구에 room별 절단과 불투명 구간을 배정한다.
@evidence principles/design/spaces.md#space-topology 전면 외주는 house의 -Z 경계이고 실제 cut만 외부/실내를 잇거나 고정 유리로 구분한다.
@evidenceReview principles/design/spaces.md#space-topology 계단·flex·child-one 내부 범위와 전면 개구가 대응한다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority front face는 외벽 실체를 소유하고 opening 폭은 개별 profile에서 받는다.
...
@evidenceReview principles/design/spaces.md#space-verification-address 전면 sill·jamb·층 band를 opening과 floor 원본에 대조하도록 한다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work 기존 전면 room 범위로 문과 유리를 배정할 수 있어 부모 공간 그래프를 수정하지 않는다.
...
@evidenceReview contracts/citizen-house-spatial-requirements.md#citizen-house-spatial-requirements 전면은 계단 유리와 작업실·상층 일부 유리를 두고 -X 코어는 불투명하게 남긴다.
...
@evidenceReview settings/003-spatial-basis.md#envelope-and-privacy 현관문과 계단 유리의 화면 관계를 실제 전면 배치로 유지한다.
-->
...
@evidenceReview principles/core/common.md#declared-basis rear=+Z와 두 floor datum을 소비해 후면 위치를 정한다.
@evidence principles/core/common.md#scope-preservation 공용부와 주침실의 넓은 유리 및 욕실 privacy 개구와 층선 폐쇄를 모두 남긴다.
@evidenceReview principles/core/common.md#scope-preservation 후면 유리를 뒤 출입구로 바꿔 추가 connector를 만들지 않는다.
...
@evidenceReview principles/core/common.md#substantive-completion 하층 common·상층 primary·bath와 층 band가 함께 지정됐다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation 후면 대면적 유리 요구에 bath 접촉 jamb와 두 층 분할을 더한다.
@evidence principles/design/spaces.md#space-topology 하나의 house 후면 경계에 두 층의 room별 opening을 붙이고 욕실과 침실을 한 opening으로 합치지 않는다.
@evidenceReview principles/design/spaces.md#space-topology 주침실과 bath의 경계가 상층 후면 유리 끝으로 이어진다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority rear owner가 외측 면을 맡고 실내 room 끝을 창 분할에 사용한다.
...
@evidenceReview principles/design/spaces.md#space-verification-address 공용부 유리와 상층 두 room의 구분을 후면 전체·개구 관찰에서 묻는다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work room 경계와 floor line만으로 후면 분할이 가능해 부모에 다른 매스를 요구하지 않는다.
...
@evidenceReview contracts/citizen-house-spatial-requirements.md#citizen-house-spatial-requirements 후면 대부분을 common·primary 유리로 쓰고 bath는 privacy 개구로 구분한다.
-->
...
@evidenceReview principles/core/common.md#declared-basis 전면 관찰의 좌측인 +X 면을 사용한다.
@evidence principles/core/common.md#scope-preservation 각 방의 창과 나머지 불투명 벽 및 앞뒤 return을 남겨 전 측면을 무차별 유리로 바꾸지 않는다.
@evidenceReview principles/core/common.md#scope-preservation 측면 창을 코너 전체 유리로 확장하지 않는다.
...
@evidenceReview principles/core/common.md#substantive-completion 세 opening과 나머지 실체·roof 접합이 지정됐다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation 부모의 단순 상자에 방별 측면 채광 범위를 구체화한다.
@evidence principles/design/spaces.md#space-topology 동측 house 외주에서 각 창은 자기 room에 면하며 한 opening이 서로 다른 침실을 잇지 않는다.
@evidenceReview principles/design/spaces.md#space-topology flex·child-one의 상하 창과 child-two의 중간 짧은 창이 각 cell에 대응한다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority left face는 전체 외측 면을 맡고 room 내부 lining을 중복 생성하지 않는다.
...
@evidenceReview principles/design/spaces.md#space-verification-address 측면 코너 return과 창 끝이 실내 범위를 넘는지 검사한다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work 외주 내 각 방에 측면 창을 배정할 수 있어 부모 직사각형을 바꿀 필요가 없다.
...
@evidenceReview contracts/citizen-house-spatial-requirements.md#citizen-house-spatial-requirements 좌측은 작업실·두 자녀실의 설계 창만 열고 불투명 return을 남긴다.
-->
...
@evidenceReview principles/core/common.md#declared-basis right=-X 관례와 코어 cell을 근거로 창 없는 전면을 정한다.
@evidence principles/core/common.md#scope-preservation 앞쪽 위생·수납·설비의 불투명 벽을 유지하고 후단의 두 층 창과 corner return도 생략하지 않는다.
@evidenceReview principles/core/common.md#scope-preservation 후면 창을 전면 위생·수납 구간까지 연장하지 않는다.
@evidence principles/core/common.md#substantive-completion -X normal·plane 입력과 두 창 owner, 공유 z span 및 floor 대응을 결정했다.
@evidenceReview principles/core/common.md#substantive-completion common·bath의 후면 opening과 전면 실체를 면 전체에 배정한다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation 코어 불투명 요구에 후단 두 채광 구간을 한정한다.
@evidence principles/design/spaces.md#space-topology 서측 house boundary는 코어를 닫고 후단의 각 room에만 지정된 opening이 면한다.
@evidenceReview principles/design/spaces.md#space-topology 하층 common·상층 bathroom의 같은 Z span이 층별로 대응한다.
@evidence principles/design/spaces.md#space-boundary-authority minX의 외측/내측 평균에서 plane을 구하고 창의 z span은 opening owner를 소비한다.
@evidenceReview principles/design/spaces.md#space-boundary-authority right owner가 외벽과 panel joint를 맡고 방 끝을 소비한다.
...
@evidenceReview principles/design/spaces.md#space-verification-address 전면 코어 폐쇄와 후면 privacy 창을 우측 전체 관찰에서 대조한다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work 후단 채광만 열어 코어 기능을 유지할 수 있어 부모의 우측 불투명 요구를 고칠 필요가 없다.
...
@evidenceReview contracts/citizen-house-spatial-requirements.md#citizen-house-spatial-requirements 화면 우측 -X의 설비 코어를 불투명 면으로 유지한다.
-->
...
@evidenceReview principles/core/common.md#declared-basis roof top 6.40m와 캐노피 범위는 저작 datum이며 reference 픽셀 측량이 아니다.
@evidence principles/core/common.md#scope-preservation roof 전체와 PV·지지·상면·노출 하부를 남기며 박공·굴뚝·옥상 출입을 새로 발명하지 않는다.
@evidenceReview principles/core/common.md#scope-preservation 지붕·실외 처마만 소유하고 실내 천장 마감을 중복 생성하지 않는다.
@evidence principles/core/common.md#substantive-completion roof datum과 canopy의 평면·underside·깊이·짧은 지지 및 반복 인터페이스를 지정한다.
@evidenceReview principles/core/common.md#substantive-completion edge·캐노피 하부·짧은 지지와 최대 모듈 간격이 지정됐다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation 부모의 PV 캐노피에 지지 높이와 반복 범위를 배정한다.
@evidence principles/design/spaces.md#space-topology 지붕은 본채 전체를 덮고 캐노피는 짧은 지지 위의 부재이며 별도 room이나 접근 connector가 아니다.
@evidenceReview principles/design/spaces.md#space-topology 캐노피 지지가 본채 지붕에 놓이며 독립된 별동을 만들지 않는다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority roof owner가 캐노피 상하면을 맡고 upper owner가 실내 ceiling을 맡는다.
...
@evidenceReview principles/design/spaces.md#space-verification-address 노출 roof 상하와 지지를 관찰 대상으로 두어 위에서만 보이는 판으로 검사를 축소하지 않는다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work 본채 지붕 위에서 캐노피를 지지할 수 있어 부모의 단순 직사각형 매스를 수정할 필요가 없다.
...
@evidenceReview contracts/citizen-house-spatial-requirements.md#citizen-house-spatial-requirements 평지붕 위 얇은 PV 캐노피를 두고 박공·굴뚝·본채 캔틸레버를 추가하지 않는다.
...
@evidenceReview settings/003-spatial-basis.md#envelope-and-privacy 평지붕과 짧은 지지를 갖춘 PV 부재로 외피 canon을 소비하되 v-071의 단순한 표현은 개선 대상으로 남는다.
-->
...
@evidenceReview principles/core/common.md#declared-basis 최대 bay 1.25m와 frame 치수는 저작 입력으로 선언한다.
...
@evidenceReview principles/core/common.md#scope-preservation glazing은 채광·privacy 개구이며 임의의 출입 connector를 추가하지 않는다.
...
@evidenceReview principles/core/common.md#substantive-completion jamb·head·sill·mullion 차감과 structural cut 확장이 정해졌다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation 부모의 창호 실물 요구에 glass clear를 산출할 반복 규칙을 더한다.
@evidence principles/design/spaces.md#space-topology 창의 cut은 host 두께를 관통하지만 고정 유리로 채워지며 방 사이 경계를 가로지르는 panel을 허용하지 않는다.
@evidenceReview principles/design/spaces.md#space-topology 각 pane은 소속 room 외피에 붙고 floor band를 관통하지 않는다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority room·floor 경계를 먼저 고정하고 남은 span에서 mullion을 나눈다.
...
@evidenceReview principles/design/spaces.md#space-verification-address frame 깊이·cut 포함과 bay 정렬을 실제 부재 bounds로 검사한다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work 부모가 정한 room·floor 기준으로 반복을 나눌 수 있어 공간 그래프 수정이 필요하지 않다.
...
@evidenceReview contracts/citizen-house-spatial-requirements.md#citizen-house-spatial-requirements frame·pane·shade를 분리하여 커튼월을 벽의 사각 구멍으로 대신하지 않는다.
-->
...
@evidenceReview principles/core/common.md#declared-basis 폭은 stair-hole 양끝과 frame 여유에서 유도한다.
@evidence principles/core/common.md#scope-preservation 두 층 계단 유리와 그 사이 spandrel을 모두 남겨 계단실을 가짜 복층 거실로 바꾸지 않는다.
@evidenceReview principles/core/common.md#scope-preservation 계단 유리를 entry 문이나 flex 창까지 확장하지 않는다.
...
@evidenceReview principles/core/common.md#substantive-completion 층별 sill·head와 중간 band가 지정되어 단일 통유리 판으로 끝나지 않는다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation 계단실 커튼월 요구에 hole과 정렬된 상하층 span을 부여한다.
@evidence principles/design/spaces.md#space-topology 하층은 entry, 상층은 계단 구멍에 면한 upper-storey이며 유리가 새로운 통행 connector를 만들지 않는다.
@evidenceReview principles/design/spaces.md#space-topology 두 pane 층이 각 floor에 대응하며 계단 공간 외부 면에 붙는다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority 계단 범위가 원본이고 front-stair profile은 그 끝에서 파생한다.
...
@evidenceReview principles/design/spaces.md#space-verification-address front-entry frame과 stair jamb의 간섭을 같은 전면에서 검사한다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work 계단 hole과 층선으로 유리 범위를 얻어 부모의 단일 계단을 이동할 필요가 없다.
...
@evidenceReview contracts/citizen-house-spatial-requirements.md#citizen-house-spatial-requirements 전면 계단실 유리가 두 층을 드러내되 floor band를 남긴다.
...
@evidenceReview settings/003-spatial-basis.md#privacy-states 낮의 투명·사적 60% shade·야간 전폐를 계단 유리에 적용한다.
-->
...
@evidenceReview principles/core/common.md#declared-basis X=3.06..5.22는 flex clear face와 frame 여유를 따른다.
...
@evidenceReview principles/core/common.md#scope-preservation 개구를 entry나 외벽 모서리까지 넓히지 않는다.
...
@evidenceReview principles/core/common.md#substantive-completion ground sill·head와 반투명 하부를 창 범위 안에 지정한다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation 가변 작업실 채광 요구에 고정 하부 privacy 층을 더한다.
@evidence principles/design/spaces.md#space-topology 창은 flex-workroom과 외부의 경계이며 현관 직결 문을 대신하는 출입이 아니다.
@evidenceReview principles/design/spaces.md#space-topology 창은 flex 외피이며 common으로 연결되는 통로가 아니다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority flex cell이 창 끝의 원본이며 interior lining은 같은 cut을 소비한다.
...
@evidenceReview principles/design/spaces.md#space-verification-address frame 포함 structural cut이 실내 lining 안에 드는지 검사한다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work flex 전면 cell 안에 창과 privacy를 배정할 수 있어 부모의 작업실 접근을 고칠 필요가 없다.
...
@evidenceReview contracts/citizen-house-spatial-requirements.md#citizen-house-spatial-requirements 전면 작업실은 창을 갖고 하부 시선대의 프라이버시를 유지한다.
...
@evidenceReview settings/003-spatial-basis.md#privacy-states 하부 반투명은 유지하고 사적·야간에는 shade를 전폐한다.
-->
...
@evidenceReview principles/core/common.md#declared-basis X=1.80..5.22와 하층 flex jamb 중심이 분할의 근거다.
@evidence principles/core/common.md#scope-preservation 상층 침실과 계단 사이의 경계를 보존하고 하층 방 폭에 맞추려고 침실 cell을 잘라내지 않는다.
@evidenceReview principles/core/common.md#scope-preservation child-one 창을 stair void나 service 코어까지 연장하지 않는다.
...
@evidenceReview principles/core/common.md#substantive-completion 상층 sill·head와 의무 분할선·하부 privacy가 지정됐다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation 부모의 상층 전면 유리에 하층 jamb를 잇는 추가 분할을 부여한다.
...
@evidenceReview principles/design/spaces.md#space-topology 전면 child-one cell과 창이 대응하고 floor band가 아래 flex와 나눈다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority 상층 room 끝과 하층 jamb 중심을 우선하고 나머지 bay만 반복한다.
...
@evidenceReview principles/design/spaces.md#space-verification-address 층을 사이에 둔 jamb 정렬과 cell 밖 유리 돌출을 검사한다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work 하층 jamb 기준을 상층 span에 넣을 수 있어 부모 room 경계를 바꿀 필요가 없다.
...
@evidenceReview contracts/citizen-house-spatial-requirements.md#citizen-house-spatial-requirements 상층 전면 일부를 child-one 창으로 쓰며 하층 bay와 맞춘다.
...
@evidenceReview settings/003-spatial-basis.md#privacy-states 침실 하부는 반투명이고 사적·야간 shade는 전체를 가린다.
-->
...
@evidenceReview principles/core/common.md#declared-basis X=-5.22..5.22는 common 외측 clear 범위에서 파생한다.
@evidence principles/core/common.md#scope-preservation 거실·식당·주방에 면한 후면 유리와 창틀을 모두 남기되 그래프에 없는 rear 출입을 추가하지 않는다.
@evidenceReview principles/core/common.md#scope-preservation 고정 유리를 새 후면 출입구로 해석하지 않는다.
...
@evidenceReview principles/core/common.md#substantive-completion 하층 sill·head와 반복 bay 및 shade 상태를 지정한다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation 후면 대부분 유리 요구를 단일 common span으로 구체화한다.
@evidence principles/design/spaces.md#space-topology 공용부와 정원을 나누는 고정창이며 외부 정원으로의 새 통행 route는 없다.
@evidenceReview principles/design/spaces.md#space-topology 거실·식당·주방의 한 room 외부 면에 연속 창이 붙는다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority common 양끝이 원본이며 mullion은 남은 폭에서 나눈다.
...
@evidenceReview principles/design/spaces.md#space-verification-address 긴 span의 모든 bay와 corner return을 개구 관찰에서 확인한다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work 한 common span으로 후면 유리가 가능해 부모 공용부에 칸막이를 추가할 이유가 없다.
...
@evidenceReview contracts/citizen-house-spatial-requirements.md#citizen-house-spatial-requirements 후면 common 전체에 유리를 두어 연속 생활 공간을 드러낸다.
...
@evidenceReview settings/003-spatial-basis.md#privacy-states 낮은 투명·shade 상승, 사적은 60%, 야간은 전폐로 구분한다.
-->
...
@evidenceReview principles/core/common.md#declared-basis X=-2.80..5.22는 primary clear cell과 frame 여유를 따른다.
@evidence principles/core/common.md#scope-preservation 주침실의 후면 유리를 남기면서 욕실과의 shared wall을 관통하는 유리판은 만들지 않는다.
@evidenceReview principles/core/common.md#scope-preservation 후면 전체를 침실 창으로 덮어 욕실 개구를 흡수하지 않는다.
...
@evidenceReview principles/core/common.md#substantive-completion 상층 sill·head와 하부 반투명 구간을 지정한다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation 후면 침실 채광 요구에 코어 접촉 jamb를 부여한다.
@evidence principles/design/spaces.md#space-topology opening은 primary-bedroom에만 면하고 욕실과 침실을 하나의 room binding으로 합치지 않는다.
@evidenceReview principles/design/spaces.md#space-topology primary의 후면 외부 면에만 pane이 붙고 bath와 분리된다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority bath-primary shared wall이 창 서측 끝의 우선 기준이다.
...
@evidenceReview principles/design/spaces.md#space-verification-address 욕실과 만나는 jamb 및 floor transom 정렬을 검사한다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work 주침실 cell 안에서 privacy 창을 만들 수 있어 부모 코어 경계를 변경하지 않는다.
...
@evidenceReview contracts/citizen-house-spatial-requirements.md#citizen-house-spatial-requirements 주침실 후면 유리는 bath 경계에서 멈춰 사적 room 구분을 유지한다.
@evidence settings/003-spatial-basis.md#privacy-states 주침실의 day 사적 유리와 private/night 전체 shade 상태를 이 후면 창에 배정한다.
@evidenceReview settings/003-spatial-basis.md#privacy-states 침실 하부 privacy를 유지하며 사적·야간 shade는 전체를 덮는다.
-->
...
@evidenceReview principles/core/common.md#declared-basis X=-4.86..-3.42와 높이 4.20..5.65는 bathroom 내부에서 정한다.
...
@evidenceReview principles/core/common.md#scope-preservation 작은 privacy 창을 primary의 대면적 유리로 합치지 않는다.
@evidence principles/core/common.md#substantive-completion host·room·유효 폭·높이 offset과 반투명 상태를 정해 후면 코어의 제한된 개구를 확정한다.
@evidenceReview principles/core/common.md#substantive-completion 높은 sill·낮춘 head·전체 frost 상태가 정해졌다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation 욕실 privacy 요구에 후면의 제한된 높은 opening을 부여한다.
@evidence principles/design/spaces.md#space-topology 후면 house boundary 안의 opening은 upper-bathroom만 향하며 외부 출입을 만들지 않는다.
@evidenceReview principles/design/spaces.md#space-topology 후면 욕실 면 안에 놓여 주침실 채광 범위를 침범하지 않는다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority bath cell이 외곽 한계이며 해당 profile이 이 창 clear를 소유한다.
...
@evidenceReview principles/design/spaces.md#space-verification-address 높은 opening의 room 포함과 frost 재료 바인딩을 검사한다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work 코어 cell에 높은 privacy 창을 배정할 수 있어 부모 불투명 코어 원칙을 수정하지 않는다.
...
@evidenceReview contracts/citizen-house-spatial-requirements.md#citizen-house-spatial-requirements 후면 욕실 창은 높은 sill과 고정 반투명으로 코어 프라이버시를 지킨다.
@evidence settings/003-spatial-basis.md#privacy-states 욕실 유리는 모든 허용 상태에서 반투명을 유지하고 shade 변화가 이를 clear로 바꾸지 않는 입력을 소비한다.
@evidenceReview settings/003-spatial-basis.md#privacy-states 세 상태 모두 전체 반투명을 유지해 낮 상태에서도 욕실이 투명해지지 않는다.
-->
...
@evidenceReview principles/core/common.md#declared-basis Z=-5.40..-2.30은 flex 측면 안에서 정한 저작 span이다.
@evidence principles/core/common.md#scope-preservation 전면 창과 직교하는 측면 유리를 남기면서 corner의 불투명 return을 지우거나 틀을 겹치지 않는다.
@evidenceReview principles/core/common.md#scope-preservation 측창을 전면 전체와 합친 wraparound 면으로 확대하지 않는다.
...
@evidenceReview principles/core/common.md#substantive-completion 하층 높이·corner return·privacy 하부를 함께 지정한다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation 가변실 채광 요구에 측면의 제한된 긴 span을 추가한다.
...
@evidenceReview principles/design/spaces.md#space-topology 같은 flex room의 두 외부 면이 실체 corner에서 만난다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority left-flex profile이 clear를 소유하고 front face와는 return으로 분리된다.
...
@evidenceReview principles/design/spaces.md#space-verification-address 측창 양끝과 전면 corner 실체가 겹치거나 벌어지는지 검사한다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work corner return을 남기고도 flex 측창이 들어가므로 부모 직사각형 외곽을 고칠 필요가 없다.
...
@evidenceReview contracts/citizen-house-spatial-requirements.md#citizen-house-spatial-requirements 작업실 측창은 모서리 실체를 남겨 무리한 유리 코너를 만들지 않는다.
@evidence settings/003-spatial-basis.md#privacy-states 전면과 같은 작업실 사적 상태를 측면 창에도 배정하여 어느 한 면으로 프라이버시가 빠지지 않는지 검사한다.
@evidenceReview settings/003-spatial-basis.md#privacy-states 하부 반투명에 사적·야간 전폐 shade를 더해 전면과 같은 방 상태를 유지한다.
-->
...
@evidenceReview principles/core/common.md#declared-basis left-flex와 같은 Z=-5.40..-2.30 범위를 상층 datum에 올린다.
@evidence principles/core/common.md#scope-preservation 상하 jamb 대응과 첫 침실의 독립 범위를 함께 보존하며 L자 연장부까지 창을 임의 확장하지 않는다.
@evidenceReview principles/core/common.md#scope-preservation 창을 child-one의 내부 L형 확장부까지 임의로 늘리지 않는다.
...
@evidenceReview principles/core/common.md#substantive-completion 상층 sill·head와 하층 공통 반복 기준을 지정한다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation 침실 측면 채광에 층 사이 jamb 연속성을 더한다.
@evidence principles/design/spaces.md#space-topology child-bedroom-1의 동측 전면부가 면하는 고정창이며 하층 flex와는 층 band로 분리된다.
@evidenceReview principles/design/spaces.md#space-topology 하층 flex 위의 child-one 외피로 대응하며 floor band로 층을 구분한다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority 공통 Z span과 room 끝을 우선하므로 각 층이 독립 간격을 발명하지 않는다.
...
@evidenceReview principles/design/spaces.md#space-verification-address 상하 동일 jamb와 child-one cell 포함을 측면 관찰에서 검사한다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work 하층 span이 상층 child-one 안에도 들어가 부모의 침실 배치를 바꿀 필요가 없다.
...
@evidenceReview contracts/citizen-house-spatial-requirements.md#citizen-house-spatial-requirements 첫 자녀실 측창을 하층 작업실 bay와 정렬한다.
@evidence settings/003-spatial-basis.md#privacy-states 첫 침실의 사적 상태와 shade는 동측 창에서도 유지되며 전면 상태만 바꿔 전체 방을 보호했다고 판정하지 않는다.
@evidenceReview settings/003-spatial-basis.md#privacy-states 침실 하부 반투명과 사적·야간 전폐를 이 측창에도 적용한다.
-->
...
@evidenceReview principles/core/common.md#declared-basis Z=0.35..1.90과 상층 sill+0.70은 child-two cell 안에서 정한다.
@evidence principles/core/common.md#scope-preservation 작은 침실2에도 실제 창과 privacy 제어를 남기며 room 밖으로 frame을 늘려 창을 크게 보이게 하지 않는다.
@evidenceReview principles/core/common.md#scope-preservation 앞 자녀실의 긴 창을 복제해 이 방 범위를 벗어나지 않는다.
...
@evidenceReview principles/core/common.md#substantive-completion 짧은 span·높은 sill·낮춘 head를 확정한다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation 작은 침실 채광 요구에 중간 cell용 높은 창을 추가한다.
@evidence principles/design/spaces.md#space-topology opening은 child-bedroom-2만 향하고 첫 침실이나 주침실 외주와 합쳐진 통합창이 아니다.
@evidenceReview principles/design/spaces.md#space-topology 동측 중간 침실의 외벽에만 pane이 붙는다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority child-two profile이 높이를 소유하며 left-bedroom 값을 묵시적으로 재사용하지 않는다.
...
@evidenceReview principles/design/spaces.md#space-verification-address 짧은 opening과 앞뒤 침실 벽 사이의 간격을 검사한다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work 중간 cell 안에 채광 창을 둘 수 있어 부모에 다른 facade나 추가 방을 요구하지 않는다.
...
@evidenceReview contracts/citizen-house-spatial-requirements.md#citizen-house-spatial-requirements 두 번째 침실에도 독립 측창을 주되 인접 침실 경계를 넘지 않는다.
@evidence settings/003-spatial-basis.md#privacy-states 둘째 침실의 day 사적 유리와 private/night shade를 이 창에도 배정해 같은 방의 상태 누락을 검사한다.
@evidenceReview settings/003-spatial-basis.md#privacy-states 높은 창에도 침실 privacy 상태를 적용해 사적·야간 노출을 남기지 않는다.
-->
...
@evidenceReview principles/core/common.md#declared-basis Z=3.60..5.40은 common의 후면 측벽 범위에서 정한다.
@evidence principles/core/common.md#scope-preservation 앞쪽 코어는 닫힌 벽으로 남기고 후방 공용부에만 고정창과 실제 frame을 둔다.
@evidenceReview principles/core/common.md#scope-preservation 측창을 powder·storage 전면까지 연장하지 않는다.
@evidence principles/core/common.md#substantive-completion room·host·z span·ground sill/head를 정해 불투명 서비스 면의 제한된 개구를 지정한다.
@evidenceReview principles/core/common.md#substantive-completion 하층 sill·head와 후면 corner 여유를 지정한다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation 공용부 채광에 코어 뒤의 제한된 측면 span을 부여한다.
@evidence principles/design/spaces.md#space-topology 창은 common-room 서측 후단과 외부의 경계이고 앞쪽 위생·수납 room을 노출하는 opening이 아니다.
@evidenceReview principles/design/spaces.md#space-topology pane은 common room에 붙으며 서비스 공간의 창이 아니다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority right-common profile이 clear를 정하고 right face가 동일 cut을 소비한다.
...
@evidenceReview principles/design/spaces.md#space-verification-address 전면 코어 실체와 후단 창 구분을 우측 면 전체에서 검사한다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work 코어 뒤 common 면을 사용할 수 있어 부모의 우측 불투명 프로그램을 고칠 필요가 없다.
...
@evidenceReview contracts/citizen-house-spatial-requirements.md#citizen-house-spatial-requirements 우측 유리는 공용부 후단에 한정해 전면 설비 코어를 불투명하게 남긴다.
@evidence settings/003-spatial-basis.md#privacy-states 공용부의 허용 유리·shade 상태를 후면 큰 창뿐 아니라 서측 작은 창에도 적용할 입력으로 받는다.
@evidenceReview settings/003-spatial-basis.md#privacy-states 공용부의 낮 투명·사적 60%·야간 전폐 상태를 후면 창과 함께 적용한다.
-->
...
@evidenceReview principles/core/common.md#declared-basis 하층 right-common과 같은 Z=3.60..5.40을 상층에 적용한다.
@evidence principles/core/common.md#scope-preservation 상하 창틀 정렬과 욕실의 고정 반투명을 함께 남겨 공용 유리와 같은 clear 창으로 만들지 않는다.
@evidenceReview principles/core/common.md#scope-preservation 욕실 측창을 service 전면의 투명 띠로 확대하지 않는다.
...
@evidenceReview principles/core/common.md#substantive-completion 상층 창 높이와 전체 frost 및 하층 bay 대응을 지정한다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation 욕실 채광에 공용부 위 정렬된 privacy span을 더한다.
...
@evidenceReview principles/design/spaces.md#space-topology 하층 common과 상층 bath가 같은 외부 span에서 층 band로 분리된다.
@evidence principles/design/spaces.md#space-boundary-authority 하층 창이 z span/module의 원본이며 욕실 창은 upper 높이와 상태만 자기 입력으로 소비한다.
@evidenceReview principles/design/spaces.md#space-boundary-authority 공통 Z 범위는 층간 정렬을 맡고 bath profile은 상층 높이를 소유한다.
...
@evidenceReview principles/design/spaces.md#space-verification-address 층별 material binding과 상하 jamb 위치를 함께 검사한다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work 동일 span이 상층 bath에도 들어가 부모 코어 room의 경계를 바꿀 이유가 없다.
...
@evidenceReview contracts/citizen-house-spatial-requirements.md#citizen-house-spatial-requirements 상층 우측 bath 창은 반투명으로 코어의 사적 사용을 유지한다.
@evidence settings/003-spatial-basis.md#privacy-states 욕실은 day·private·night 모두 반투명이라는 조건을 하층 공용 창의 다른 상태와 구별하여 적용한다.
@evidenceReview settings/003-spatial-basis.md#privacy-states 욕실은 낮·사적·야간 모두 반투명으로 남겨 공용부 tint 상태와 구분한다.
-->
...
@evidenceReview principles/core/common.md#declared-basis 0.24m 외벽의 내외측 끝에서 corner 사각 범위를 파생한다.
@evidence principles/core/common.md#scope-preservation 네 외부 corner를 실제 volume으로 닫고 window frame을 감추려 corner 두께를 없애지 않는다.
@evidenceReview principles/core/common.md#scope-preservation 실내 shared-wall junction과 외부 corner 책임을 합치지 않는다.
...
@evidenceReview principles/core/common.md#substantive-completion 사각을 두 삼각 기둥으로 나누는 접합과 소유를 지정한다.
...
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation 온전한 입면 소유 규칙에 실제 모서리에서 만나는 절반 실체를 부여한다.
@evidence principles/design/spaces.md#space-topology 두 인접 외벽은 닫힌90° corner에서 만나며 그 부피 안에 출입이나 room 간 통행을 두지 않는다.
@evidenceReview principles/design/spaces.md#space-topology 인접 두 외벽이 만나 외곽을 닫되 room이나 opening을 추가하지 않는다.
...
@evidenceReview principles/design/spaces.md#space-boundary-authority 각 facade가 자기 return 삼각형을 맡아 같은 corner를 두 번 생성하지 않는다.
...
@evidenceReview principles/design/spaces.md#space-verification-address 중복·틈과 corner를 관통하는 cut을 상하 모서리 관찰로 검사한다.
...
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work 기존 외곽 두께 안에서 모서리를 닫을 수 있어 부모 매스나 room 끝을 수정하지 않는다.
...
@evidenceReview contracts/citizen-house-spatial-requirements.md#citizen-house-spatial-requirements 직사각형 외벽 모서리를 실체로 닫고 유리 효과를 위해 빈 코너를 만들지 않는다.
-->
