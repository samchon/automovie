...
**상태: review 선언을 유지한 제자리 재작성. 현재 수정안의 컴파일 결과와 독립 판정 전이다.** [좌표 기준](../settings/003-spatial-basis.md#coordinate-datum)을 따른다. 이 문서의 치수는 reference 픽셀의 역산값이 아닌 저작 결정이다. 고정 그래프의 노드와 연결 의미를 유지하면서 기존의 무소유 틈, 벽 관통, 계단 막힘을 제거하기 위한 입력이다. 기존 source는 이 값을 아직 구현하지 않는다.
