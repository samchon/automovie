...
`sanitary-ceramic`은 basin rim/bowl, toilet pedestal/bowl/cistern, 샤워 트레이의 실제 면이다. 기구 목록은 [상층 프로그램](../settings/002-household.md#upper-program)과 방 owner의 형상에서, 배관·작동이 범위 밖이라는 한계는 [프로그램 경계](../settings/002-household.md#program-boundary)에서 받는다. 이름이 white/tile이던 용도도 방 owner가 이 역할로 명시한다. texture 없음, 색 #e7e6df, roughness=.19, metallic=0, clearcoat=.12다. 명목 .5mm 유약은 기존 기하 안에 포함되며 벽 타일, 침구, 세탁기·keyboard 같은 가전 흰색에 적용하지 않는다.

곡면은 현재 mesh 그대로 반사하고 부재 두께/배관은 방 owner에 남긴다. [습식 방 관찰](007-observation.md#reference-material-samples)에서 도기가 직물처럼 보이지 않는지, basin 내부가 바닥 tile과 같은 재료로 읽히지 않는지 확인한다. 기기의 단순한 형상이나 실제 배관 작동은 이 glossy 마감으로 해결했다고 하지 않는다.
...
`sanitary-seat`는 toilet의 기존 `*-toilet-seat` 면만 받는 흰 성형 수지 마감이다. 방 owner의 타원형 좌판과 두께를 유지하며 도기 유약이나 직물 피복을 선언하지 않는다. 색 #e7e6df, roughness=.30, metallic=0, clearcoat=0, texture 없음이다. 기존 linen이라는 색 이름을 실제 직물로 해석하지 않는다. 면 방향은 현재 mesh 법선을 따르고 광택의 차이만 도기 bowl과 구분한다.
...
@evidence principles/core/common.md#declared-basis 낮은 채도의 녹색 가구는 시각 문법, murphy panel은 flex-states, 문짝 분할과 손잡이는 방 owner 형상에서 받고 색·광택은 이 층의 선택이다.
...
@evidenceExclude upstream/design/materials.md#parent-revision-from-material-work common-room의 kitchen-wall-bank·island·overhead cabinet, flex-workroom의 murphy closed-panel과 flex-states를 대조했다. panel이 이미 분리된 element라 부모 수리 없이 역할을 나눌 수 있었다.
...
@evidence spaces/002-spatial-graph.md#common-room 주방 wall bank·island의 녹색 cabinet과 overhead의 밝은 cabinet을 공용부 owner가 두 도장으로 배정한다.
@evidence spaces/002-spatial-graph.md#flex-workroom 작업실 murphy closed-panel을 방 owner가 joinery-light로 배정한다.
-->

`joinery-green`은 kitchen-island·kitchen-wall-bank cabinet의 green back·door·shelf·side 판에 색 #626b59·roughness=.44를 배정한다. `joinery-light`는 kitchen-overhead 및 flex murphy closed-panel의 plaster색 판에 #c9c3b7·roughness=.48을 쓴다. 명목 .1mm 도막, texture 없음, metallic=0이다. murphy closed-panel은 작업 상태에만 있고 손님 상태에서는 방 owner가 그 자리에 guest bed를 내리므로 이 마감이 결합할 panel이 없다. 실제 문짝 분할·문손잡이·틈·작동은 방 owner의 현재 형상 그대로다.
...
@evidence principles/core/inherited-units.md#derived-parent-differentiation 부모는 대지·기기를 형상으로 정하고 식재는 roof-face가 ID·형상·재료 보존까지 정한다. 이 H2는 재료 층 전체 교체에서 이들을 보존 역할로 명시하고 retained 역할에 모르는 이름을 몰아넣지 않는 규칙을 더한다.
...
@evidenceExclude upstream/design/materials.md#parent-revision-from-material-work site-access의 포장·landing·집수구, roof-face의 나무4·관목48그룹·풀23그룹 보존과 roof 방수 면·canopy pedestal·support anchor·거름망·cassette 체결구, right-face의 점검 덮개, ground-level·upper-level의 기초·bearing ring·upper slab 면, 방 owner의 기기와 소품을 대조했다. 보존에 필요한 주소와 값이 부모에 모두 있었다.
