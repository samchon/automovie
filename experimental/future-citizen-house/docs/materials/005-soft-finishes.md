...
@evidence spaces/003-surface-ownership.md#front-face 전면 입면 owner가 소유한 외벽 body에서 계단 void로 드러나는 실내 면 두 띠(창 cut 아래 3.184..3.25, head 위 6.04..6.10)와 그 사이 위층 drip의 실내 쪽 끝면을 그 owner가 이 도장으로 배정한다.
@evidence spaces/003-surface-ownership.md#roof-face roof owner의 roof-bearing-front가 계단 void 위 z=-5.766..-5.76에 드러내는 6mm 아래 면을 그 owner가 이 도장으로 배정해 void 벽의 위 끝이 ceiling과 같은 도장으로 이어진다.
...
ref03의 sofa/rug, ref02의 침구, ref04의 chair를 [방 검사](007-observation.md#reference-material-samples)로, woven-grain의 1m·3m·12m 읽힘을 [거리 검사](007-observation.md#scale-and-junction-samples)로 비교한다. 원거리에서 작은 weave가 평균색으로 사라지는 것은 정상이며 격자 moiré가 생기면 실패다. 원단 미세 그림자·실의 단면·주름 추가는 현재 renderer/형상 범위 밖이다.
