...
검사 입력은 현재 상태의 `buildHouse()` 반환과 공개 lowering/instanceSlot이 만든 실제 모델·부재·material·UV다. [바인딩 인터페이스](001-binding-and-scale.md#surface-bindings)의 역할별로 source owner, element 또는 set/member, part/면, material ID, texture ID, physical tile, UV 근거를 열거한다. 면 단위로 나뉜 부재는 면별로 적는다: junction solid 12개의 노출면 16개와 각 면이 향한 공간·마감, 숨은 면 수, upper slab 네 piece의 계단 구멍 쪽 절단면·전면 strip 윗면과 piece 사이 맞닿은 면을 포함한 나머지 면, 전면 외벽 body가 계단 void에 드러내는 실내 띠 두 개와 나머지 body 면, 다섯 층간 band의 cassette plate·return·slot strip·seal, 별도 metal clip·steel anchor와 그 뒤 limestone panel·wall body, clear opening마다(entry-common 개구 포함 11개) host 벽 두께를 채운 문턱판, roof-slab·roof-bearing의 외곽 면과 나머지 면, 계단 void의 위층 drip 끝면과 roof-bearing-front 아래 면. 목재는 texture를 받은 면 수와 [끝면](004-wood.md#wood-end-faces)의 무texture 면 수를 따로 센다. retained 역할도 [보존 H2](006-wet-and-joinery.md#retained-surfaces)가 이름으로 든 역할별로 적고 catch-all 한 줄로 묶지 않는다. 적용 없는 역할도 0을 남기고, missing/중복 role·지원하지 않는 channel·잘못된 색 공간은 실패로 보고한다. 미정 배정을 plaster나 retained로 대신하지 않는다.
...
필수 접합은 front/right 모서리의 석재 corner prism, 입면 위 끝과 지붕 slab·bearing ring, 입면 아래 끝과 기초 bearing ring, 다섯 층간 cassette band의 위아래·frame 바깥선·실제 panel 이음·slot·seal·석재 drip 아래 접합과 배후 stone panel과의 깊이 분리, slab 외곽이 외장 평면에 나오지 않는지, 공용부 전면벽 두 junction 도장 면과 욕실 동측 tile 벽 junction 면(`upper-bathroom/corner-1`에서 보이는 x=-3.02, z=2.40..2.58의 -X면), 계단 구멍의 하층 ceiling 끝·slab 절단면·stair-lining·가장자리 junction의 연속과 전면 strip 윗면·외벽 실내 두 띠·위층 drip 끝면·roof-bearing 아래 면, door leaf/jamb/head, 첫 계단/꺾임참/마지막 단과 층 바닥의 이어짐 및 난간·stringer와 목재의 광택 차이, 계단 옆 끝·참 끝·복도 도착의 통판 끝면, oak 문턱판과 tile 바닥, 식탁 top edge와 다리 끝, worktop/edge/sink, sofa의 평면/곡면 seam과 pillow의 아래 pole·뒤쪽 seam, 각 texture의 두 반복 주기 경계다. 각 접합의 부재 ID와 표면 정상 방향을 기록한다. grain 늘어남, 뒤집힌 face, 이중 tint, 눈에 띄는 타일 경계, moiré와 가짜 geometry 읽힘이 실패다.
...
추가 다섯 장면은 reference01 외관의 석재 패널성·1층/2층 이음·front와 right의 같은 재료 읽힘, 짙은 frame, 금속 cassette·seal, 유리, PV, 02 절개의 바닥/침구/습식면 구분, 03 공용부의 oak floor/식탁/직물/초록 cabinet/밝은 worktop/금속/투명 유리, 04 flex의 felt/desk/직물/접이식 전면·열린 책장의 선반 가로결·back 판·손잡이 경계·하부 frosted 유리의 사생활 읽힘, 05 상층의 door/jamb 세로결/floor/유리 반사·상층 창 frame·하부 frosted 유리다. powder-utility와 upper-bathroom의 방 관찰(`upper-bathroom/corner-1`·`corner-3` 포함)은 습식 방 관찰로서 도기가 직물처럼 보이지 않는지, basin 내부가 바닥 tile과 다르게 읽히는지, toilet seat/bowl 경계, 수전·거울과 거울 반사의 한계, 문턱의 oak/tile 경계, 기구 아래 남은 바닥 면을 본다. 세 침실의 방 관찰은 침대 base/head·desk의 결을, 모든 방 관찰은 벽·천장의 이음과 개구부 반환면을 포함한다. reference02의 절개는 검사 수단이며 전달 프레임이 아니다. ref01·03·05는 기존 GPU 캡처와 나란히 비교해 같은 재료로 읽히는지 명시한다. 사진의 픽셀로 roughness나 치수를 역산하지 않는다.
