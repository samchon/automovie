...
필수 접합은 front/right 모서리의 석재 corner prism, 입면 위 끝과 지붕 slab·bearing ring, 입면 아래 끝과 기초 bearing ring, 네 입면 floor-band의 위아래와 frame 바깥선의 띠 끝, 띠 위 석재 drip과 띠의 경계와 띠의 외향/반환면 배정, 띠 끝을 가로지르는 `front-face-stone-panels:1-4-0`·`rear-face-stone-panels:2-3-0`의 panel 안 도장 경계, slab 외곽이 외장 평면에 나오지 않는지, 공용부 전면벽 두 junction 도장 면과 욕실 동측 tile 벽 junction 면(`upper-bathroom/corner-1`에서 보이는 x=-3.02, z=2.40..2.58의 -X면), 계단 구멍의 하층 ceiling 끝·slab 절단면·stair-lining·가장자리 junction의 연속과 전면 strip 윗면·외벽 실내 두 띠·위층 drip 끝면·roof-bearing 아래 면, door leaf/jamb/head, 첫 계단/꺾임참/마지막 단과 층 바닥의 이어짐 및 난간·stringer와 목재의 광택 차이, 계단 옆 끝·참 끝·복도 도착의 통판 끝면, oak 문턱판과 tile 바닥, 식탁 top edge와 다리 끝, worktop/edge/sink, sofa의 평면/곡면 seam과 pillow의 아래 pole·뒤쪽 seam, 각 texture의 두 반복 주기 경계다. 각 접합의 부재 ID와 표면 정상 방향을 기록한다. grain 늘어남, 뒤집힌 face, 이중 tint, 눈에 띄는 타일 경계, moiré와 가짜 geometry 읽힘이 실패다.
