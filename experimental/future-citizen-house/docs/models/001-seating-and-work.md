...
`living-sofa/straight`는 폭 2.70, 높이 0.88, 깊이 0.96m의 세 좌석이다. `living-sofa/chaise-right`는 오른쪽 좌석 앞에 길이 0.82m의 연장 쿠션을 더하여 전체 깊이 1.78m로 만든다. 원점은 바닥의 외곽 폭 중심과 본체 깊이 중심, +Z가 착석 앞이고 +X가 오른쪽이다. 본체는 x=±1.35, z=±0.48이다. 네 노출 다리는 단면 0.06×0.06m, 중심 x=±1.21,z=±0.34, y=0..0.10이다. 프레임은 x=±1.19,z=±0.48,y=0.10..0.31이고 양팔은 x=±(1.19..1.35), z=±0.48, y=0.10..0.64다. 좌판 세 개의 각 폭은 (2.70−2×0.16−2×0.025−2×0.018)/3m, 중심 x=−0.789666..,0,+0.789666.., z=−0.32..+0.48, y=0.31..0.45다. 두 좌판 사이 0.025m, 팔과 좌판 사이 0.018m 빈 틈을 남긴다. 등받이는 y=0.31..0.88에서 두께 0.09m이고 뒷면은 하단 z=−0.40, 상단 z=−0.48이고 앞면은 각 높이에서 0.09m 앞쪽이므로 위로 갈수록 0.08m 기울며, 폭 0.764666..m의 등 쿠션 셋은 각 좌판 중심 x에 y=0.46..0.82, 깊이 0.12m로 독립한다. 베개 셋은 같은 x 중심, 폭 0.48·높이 0.22·깊이 0.16m로 중심 y=0.66,z=−0.14에 둔다. 긴 의자 변종의 연장 프레임과 좌판은 오른쪽 좌판과 같은 x 폭·중심으로 z=0.48..1.30, y=0.10..0.31과 y=0.31..0.45를 각각 차지한다. 앞다리 둘은 중심 x=0.789666..±0.32,z=1.20, 단면 0.06×0.06m,y=0..0.10이다. 접합선은 z=0.48에만 있고 발치·프레임 아래는 빈다.
...
`dining-chair`는 폭 0.48, 깊이 0.52, 높이 0.84m이며 바닥 네 발 중심을 원점, +Z를 앉는 앞쪽으로 둔다. 목재 좌판 구조는 y=0.38..0.415, 그 위 0.035m 두께의 분리된 천 좌판은 y=0.415..0.45다. 굽은 등판은 y=0.49..0.84, z=-0.22..-0.15이고 좌판과 등판 사이 높이 0.025m 이상의 틈을 남긴다. 다리 단면은 0.03×0.03m, 네 중심은 x=±0.19, z=±0.19이며 바닥에서 좌판 아래까지 이어진다. `seat-frame/upper/edge/underside`, `seat-pad/upper/edge/underside`, `back/front/rear/edge`, `leg-0..3/shaft/top/sole`이 안정 주소다. 좌판 직물과 구조 목재가 한 face를 공유하지 않는다. 정면·측면·45°에서 천 좌판의 두께와 등판의 분리·곡률을 확인한다. ref03에서 목재 의자의 별도 천 좌판을 채택하고 ref02에서 여섯 자리 반복만 받는다. ref04의 껍질형 책상 의자는 [책상 의자](#desk-chair)로 구분한다. ref01·05에는 식탁 의자가 없다. 실제 착석 강도는 `unverified`다.

## 고리 발받침 섬 스툴 {#island-stool}

`island-stool`은 좌판 지름 0.36, 상면 y=0.63, 전체 점유 0.40×0.40×0.63m다. 네 발 접촉 중심이 원점이고 +Z는 섬을 향한다. 좌판은 두께 0.045m, 다리 네 개는 0.025m 사각 단면에 x/z=±0.13 중심이며 y=0..0.585다. 발받침은 y=0.23에서 다리 중심 사이를 잇는 지름 0.018m의 네 수평 막대가 닫힌 사각 고리를 이룬다. 막대는 다리의 내부 면에 닿고 서로 이격된 장식 네 조각으로 보이지 않는다. `seat/upper/edge/underside`, `leg-0..3/shaft/top/sole`, `footrest-0..3/outer/contact`가 안정 주소다. 위·옆·아래 45°에서 고리의 연속성과 빈 중앙이 보여야 한다. ref03의 발받침 고리와 긴 섬 측면의 세 스툴 관계를 채택한다. ref02의 간단한 스툴은 배치 확인에만 쓰고, ref01·04·05에서 형상을 역산하지 않는다. 실제 하중은 `unverified`다.

## 벽 연결 작업 책상과 침실 변종 {#work-desk}

prototype은 `work-desk/flex-1400`, `work-desk/bed-1200`, `work-desk/bed-1240`, `work-desk/bed-1250` 네 개다. 숫자는 상판 X 폭 mm이며 모두 깊이 0.60, 기본 높이 0.74m다. 원점은 바닥에서 상판 폭·깊이의 중심, +Z가 앉는 자리다. flex 상판은 두께 0.04m로 y=0.70..0.74, 좌측 x=-0.61에 폭 0.18·깊이 0.46·높이 0.70m(y=0..0.70)의 서랍장을, 우측 x=+0.61에 외경 0.05m의 두 겹 사각 기둥을 둔다. 서랍장 전면의 얕은 세 줄은 y=0.16, 0.32, 0.48 중심의 높이 0.14m 서랍 face이며 손잡이 틈은 각 0.01m다. 벽측에는 길이 1.40m, 높이 0.12m의 고정 back rail을 상판 뒤 z=-0.29에 두되 벽 구멍이나 방 lining을 생성하지 않는다. 조절을 읽히게 하는 기둥의 외관은 y=0.05..0.44 하부 0.05m, y=0.39..0.70 상부 0.038m, 잠금 collar는 y=0.43..0.47에 폭 0.062m다. 기본 상태 하나만 표시하고 높이 이동이나 전동 성능은 주장하지 않는다. 책상 우측에 접힌 보조판은 0.30×0.42×0.025m, 경첩 축은 x=W/2-0.015, y=0.69, z=-0.10..+0.32이며 닫힌 판이 상판 아래에 수직으로 붙는다. 손잡이는 0.08×0.014×0.018m로 판 외측 중앙에 놓인다. flex의 back rail은 폭 1.40·깊이 0.025·높이 0.12m로 z=-0.30..-0.275, y=0.74..0.86에 놓는다. 보조판의 접힌 상태는 x=0.685..0.710, y=0.39..0.69, z=-0.10..+0.32이며 경첩 Y가 아닌 Z축을 중심으로 열린 검사 상태에서 상판 우측으로 수평 전개된다. 세 침실 변종은 같은 상판 두께, 앞쪽 z=+0.24·x=±(W/2−0.065)에 y=0..0.70인 단면 0.045m 고정 다리 둘, 뒤쪽 z=-0.30..-0.275·y=0.74..0.82의 폭 W back rail과 왼쪽 중심 x=−W/2+0.20, y=0.64, z=0인 폭 0.30·깊이 0.46·높이 0.12m 한 서랍을 사용하고 telescopic·collar·aux를 생성하지 않는다.

안정 part는 `top/upper/edge/underside`, `back-rail/front/back/top/underside/edge`; 침실에만 `support-left/right/shaft/top/sole`과 `drawer-one/front/back/side-left/side-right/top/underside/edge`; flex에만 `drawer-case/front/back/side-left/side-right/top/underside`, `drawer-0..2/front/back/side-left/side-right/top/underside/edge`, `telescopic-lower/outer/top/contact`, `telescopic-upper/outer/top/contact`, `collar/outer/contact`, `aux-panel/front/back/edge`, `aux-hinge/outer/contact`, `aux-pull/outer/contact`다. H2의 변종 키가 부품 집합을 고정한다. 정면·측면·상부·45°에서 무릎 공간과 벽 rail·서랍, flex의 기둥·접힌 판을 확인한다. ref04의 벽에 붙은 책상과 서랍선을 채택하되 사진의 숨은 지지 방식은 채택하지 않는다. settings의 조절식 단서를 보여야 하므로 ref04에 보이지 않는 기둥을 보이는 형태로 둔다. ref02의 침실 책상은 작은 고정 변종의 척도 관계로 채택하고 ref01·03·05에는 책상 세부가 없다. 실제 벽 앵커·높이 조절 작동·하중은 `unverified`다.

## 천 씌운 셸 책상 의자 {#desk-chair}

`desk-chair`는 폭 0.52, 깊이 0.55, 높이 0.83m다. 네 발의 접촉 중심이 원점, +Z가 앉는 앞쪽이다. 등받이와 좌면은 외관상 한 장의 곡면 셸이며 좌면 상단 y=0.45, 등 상단 y=0.83, 뒤 기울기는 하단 z=-0.10에서 상단 z=-0.27로 0.17m다. 셸 두께 0.025m와 그 앞의 0.035m 천층을 각각 닫힌 부품으로 만들고, 좌면 아래 네 다리는 중심 x=±0.18, z=±0.18에 지름 0.025m로 둔다. `shell/front/back/edge`, `upholstery/seat/back/edge`, `leg-0..3/shaft/top/sole`이 안정 주소다. 정면·측면·45°에서 좌면과 등판이 이어진 곡률을 확인하고 다리 사이를 막지 않는다. ref04의 초록 천 씌운 연결형 셸 의자를 채택하여 현재 `chair()`의 독립 직각 등판을 교체한다. ref02의 침실 책상 의자는 동일 prototype 반복의 근거이고 ref03의 목재 식탁 의자와 혼용하지 않는다. ref01·05에는 책상 의자를 판독할 형상이 없다. 인체 적합성은 `unverified`다.
