...
[공통 단위·주소·관찰](000-representation.md#model-address-and-scale)을 따른다. 방별 독립 물체 398개를 세는 작업 목록은 `.wiki/사물-목록.md`에 있으며 이 문서는 종류별 재사용 형상만 정한다. 아래 변종은 `state`로 판별하고 같은 state는 동일한 부품과 치수를 낸다. 각 표의 수치는 m 단위의 국소 점유다. 제작 전에는 재료·방별 배치·실제 작동을 주장하지 않는다.

## 독립 직물과 침구 {#household-textiles}

`household-textiles`는 벤치 쿠션, 소파 쿠션, 베개, 담요, 침구 세트, 접힌 시트, 식탁 매트, 행주, 욕실 매트와 현관 매트를 같은 재단 직물 계열의 상태 변종으로 만든다. 각 상태는 하나의 닫힌 직물 덩어리이며 침대 매트리스나 소파 본체에 합치지 않는다. 표의 원점은 놓이는 면의 중심이고 +Z는 물체를 보는 앞이다. 테두리 봉제선은 `body/edge`, 노출 윗면은 `body/upper`, 받침면은 `body/sole`로 나누며 얇은 매트는 뒷면도 별도 `body/underside`다. 접힌 직물은 위아래 층의 실루엣을 한 부품 안에서 접힌 자국으로 표현하되 풀림 동작은 만들지 않는다.

모든 직물은 표의 W×H×D 경계를 갖는 닫힌 모서리 둥근 직육면체다. XY·YZ 단면의 모서리 반경은 min(H/2,W/12,D/12), 상·하면 중심은 평평하고 모서리만 부풀린다. `folded-sheet`와 `blanket`은 Z 방향으로 접힌 선을 정중앙에 두고 그 선의 Y 좌표를 상면보다 H/8 낮추되 하부는 닫는다. `bedding-set`은 침대에 펼친 판, `pillow`와 두 cushion은 중앙이 가장 높은 독립 덩어리다. 판형 매트는 상·하 평면을 유지하고 가장자리만 닫아 바닥과 겹치지 않는다. 네 중립 관찰에서 접힌 선·봉제 edge·빈 주변 면이 판독되지 않으면 이 proxy는 실패다.

첫 변종 `bench-cushion`의 대표 국소 상면은 y=0.055이며 아래 표가 각 상태의 정확한 경계를 소유한다. 모든 상태의 `body/outer/sole`은 같은 주소 규칙을 따르고, 위에서 명시한 분할 face는 해당 상태에서만 낸다.
...
| @envelope | bedding-set | * | bounds | -0.98..0.98 | 0..0.08 | -1.03..1.03 | - |
| @part | bedding-set | body | curved | -0.98..0.98 | 0..0.08 | -1.03..1.03 | support |
...
`personal-articles`는 신발 한 짝, 걸린 코트와 일상 옷 한 벌, 옷걸이, 닫힌 우산과 우산꽂이를 독립 물체로 둔다. 신발 두 짝은 두 instance이고 옷 여덟 벌도 여덟 instance다. 원점은 전시 또는 걸림 면의 중심이고 +Z는 전면이다. 신발의 굽·발등·앞코는 닫힌 단일 `body`의 서로 다른 `sole/upper/toe` face, 옷과 우산은 접힌 외형의 `body/front/back/edge` face다. 옷걸이의 고리와 어깨는 같은 연속 부품의 `body/hook/shoulder`로 구별하고 우산꽂이는 열린 `body/outer/inner/rim/sole`의 두께 있는 통이다.
...
첫 변종 `shoe`의 대표 국소 상면은 y=0.13이며 아래 표가 각 상태의 정확한 경계를 소유한다. 모든 상태의 `body/outer/sole`은 같은 주소 규칙을 따르고, 위에서 명시한 분할 face는 해당 상태에서만 낸다.
...
| @envelope | coat | * | bounds | -0.31..0.31 | 0..1.12 | -0.075..0.075 | - |
| @part | coat | body | curved | -0.31..0.31 | 0..1.12 | -0.075..0.075 | support |
| @envelope | garment | * | bounds | -0.24..0.24 | 0..0.72 | -0.055..0.055 | - |
| @part | garment | body | curved | -0.24..0.24 | 0..0.72 | -0.055..0.055 | support |
| @envelope | hanger | * | bounds | -0.21..0.21 | 0..0.2 | -0.0175..0.0175 | - |
| @part | hanger | body | curved | -0.21..0.21 | 0..0.2 | -0.0175..0.0175 | support |
...
접시는 상부 반지름 W/2와 안쪽 평평한 바닥 반지름 W/3을 두 환형 띠로 이어 높이 H에서 rim을 닫는다. 병은 아래 2H/3의 몸통 폭 W, 위 1H/3의 목 폭 W/3이며 벽 두께 W/18, 바닥 두께 H/12를 가진 열린 회전체다. 포크는 Z 길이 D의 손잡이와 앞쪽 D/4에 네 이빨의 빈 틈 셋, 숟가락은 앞쪽 D/3의 오목한 타원 머리, 칼은 앞쪽 D/2의 한쪽 날 실루엣을 가진 닫힌 얇은 판이다. 폭·두께는 표의 W·H를 끝까지 사용하고 끝단의 날은 0두께가 아닌 H/4로 닫는다. 식기 간 독립 경계가 식탁 거리에서 사라질 수 있는 것은 proxy 한계이며 그 경우 상부 중립 view에서만 개수를 검증한다.

첫 변종 `plate`의 대표 국소 상면은 y=0.032이며 아래 표가 각 상태의 정확한 경계를 소유한다. 모든 상태의 `body/outer/sole`은 같은 주소 규칙을 따르고, 위에서 명시한 분할 face는 해당 상태에서만 낸다.
...
`pot`, `pan`, `utensil-crock`, `glass-jar`의 용기 몸통은 X 폭 W의 24각 회전체이고 열린 입구에 벽 두께 W/18, 바닥에 두께 H/12를 둔다. 팬의 +Z 손잡이는 몸통 뒤 경계부터 표의 전방 D/2까지 폭 W/9로 뻗고, 냄비는 양측 폭 W/8의 짧은 귀를 외곽 W 안에 넣는다. `kettle`과 `coffee-brewer`는 같은 중공 몸통 위에 전체 H의 상부 1/3에 좁은 출구와 전방 D/4 길이의 주둥이를 내며, 손잡이 고리는 안쪽을 관통하는 실제 빈 영역을 갖는다. 도마·칼 블록·토스터는 표의 박스 점유를 쓰되 도마 손잡이 구멍, 블록 상면 슬롯, 토스터 상면 슬롯을 두께를 남기는 절삭으로 낸다. `utensil`은 긴 막대와 넓은 끝, 건조대는 바닥 받침과 동일 간격의 다섯 세로 슬롯이 연결된 한 부품이다. 각각의 슬롯·구멍과 손잡이 아래 빈 공간이 상부·측면 view에서 보여야 하며 실제 도구 분리는 하지 않는다.

첫 변종 `pot`의 대표 국소 상면은 y=0.17이며 아래 표가 각 상태의 정확한 경계를 소유한다. 모든 상태의 `body/outer/sole`은 같은 주소 규칙을 따르고, 위에서 명시한 분할 face는 해당 상태에서만 낸다.
...
`bath-accessories`는 비누 용기, 휴지 걸이, 휴지 한 롤과 묶음, 칫솔 컵, 칫솔, 샴푸 병, 세제 병, 욕실 휴지통, 빨래 바구니를 공통 욕실·세탁 소품의 치수 상태로 둔다. 원점은 놓이는 면 중심, +Z는 사용자가 바라보는 전면이다. 걸이는 벽 접촉 `body/back/arm`, 병·컵·바구니는 `body/outer/inner/rim/sole`, 소모품은 `body/outer/end/sole`을 사용한다. 실제 충전물·배관·배출은 별도 system과 instance가 결정한다.

컵·병·휴지통·바구니는 폭 W와 깊이 D의 둥근 사각 또는 원형 외벽에서 벽 두께 min(W,D)/18을 빼고 상부를 열며 바닥 두께 H/12를 남긴다. 병 세 상태의 위쪽 1/5은 목으로 폭을 절반으로 줄이고 펌프 또는 뚜껑은 전체 H 안에 닫힌 돌출로 남긴다. 휴지 롤은 바깥 반지름 W/2, 안쪽 반지름 W/8의 관통 고리이고 묶음은 두 롤씩 쌓인 외곽을 한 덩어리의 네 원형 끝 face로 구별한다. 걸이는 벽쪽 판과 두 팔의 아래 빈 공간, 칫솔은 가는 손잡이와 윗부분 짧은 머리의 연속 판이다. 빈 용기 안쪽은 실제로 열리고 닫힌 바닥에서 끝나며 렌더러가 불투명 덩어리로 막아서는 안 된다.

첫 변종 `soap-dispenser`의 대표 국소 상면은 y=0.17이며 아래 표가 각 상태의 정확한 경계를 소유한다. 모든 상태의 `body/outer/sole`은 같은 주소 규칙을 따르고, 위에서 명시한 분할 face는 해당 상태에서만 낸다.
...
| @envelope | tissue-holder | * | bounds | -0.085..0.085 | 0..0.075 | -0.045..0.045 | - |
| @part | tissue-holder | body | box | -0.085..0.085 | 0..0.075 | -0.045..0.045 | support |
...
첫 변종 `file-box`의 대표 국소 상면은 y=0.31이며 아래 표가 각 상태의 정확한 경계를 소유한다. 모든 상태의 `body/outer/sole`은 같은 주소 규칙을 따르고, 위에서 명시한 분할 face는 해당 상태에서만 낸다.
...
공구 상자와 기계 외함은 표의 닫힌 박스이며 앞면 중앙 손잡이·점검 패널을 별도 face로 분리한다. 청소기와 두 길쭉한 도구는 바닥 몸통과 전체 H까지 올라가는 손잡이가 끊기지 않는 곡면이다. 접힌 사다리는 폭 W 양쪽 레일과 H/5 간격의 가로대 다섯 개가 이루는 닫힌 연결망으로, 레일 사이가 비어 있어야 한다. 여분 등기구는 24각 원통의 막힌 바닥과 드러난 앞 확산 face다. 호스 릴은 외경 W의 두 원형 측판을 깊이 D 양쪽에 두고 중심 원통과 감긴 외측 고리를 이어 가운데 구멍을 보인다. 실루엣이 막힌 판으로 보이면 해당 state는 실패다.

첫 변종 `tool-box`의 대표 국소 상면은 y=0.22이며 아래 표가 각 상태의 정확한 경계를 소유한다. 모든 상태의 `body/outer/sole`은 같은 주소 규칙을 따르고, 위에서 명시한 분할 face는 해당 상태에서만 낸다.
...
`exterior-furnishings`는 우편함, 외부 벤치·의자·탁자, 정원 조명, 빗물통, 자전거 거치대와 자전거를 실내 가구와 구별되는 외부형 상태로 둔다. 원점은 지면 접촉 중심이고 +Z가 진입로를 향한다. 우편함과 빗물통의 개구, 자전거의 두 바퀴와 프레임, 외부 좌석의 상·하부 틈은 하나의 `body` 안에서 닫힌 절삭 형상으로 만든다. 주소는 `body/front/back/side/top/sole`이며 자전거는 `body/wheel/front-frame/rear-frame`을 추가한다.

우편함은 폭 W의 상부 투입 틈과 앞면 분할선을 가진 중공 상자, 빗물통은 두께 W/24의 열린 원통이며 아래 H/18은 닫힌 바닥이다. 벤치와 의자는 상면 y=0.55H의 좌판, 뒷면 높이 H의 등판, 네 모서리 다리가 일체 연결된 형상이고 다리 사이·좌판 아래는 빈 공간이다. 외부 탁자는 높이 H의 얇은 상판과 네 다리가 연결된다. 정원 조명은 24각 기둥과 상단 H/8 길이의 확산 face다. 자전거 거치대는 표 폭 W의 뒤집힌 U형 원형봉으로 중앙이 열려 있다. 자전거는 X 방향으로 바퀴 둘을 놓되 중심 x=±0.545,y=0.34, 외반경 0.34m, 두께 0.035m이고 앞·뒤 바퀴 원의 위쪽을 프레임 삼각망으로 잇는다. 안장 상단은 y=0.93, 핸들 상단은 y=1.12이며 양쪽 Z 폭의 극단은 핸들에서만 나온다. 두 바퀴·프레임 사이의 공기가 정면과 45°에서 보이지 않으면 자전거 proxy는 실패다.

첫 변종 `mailbox`의 대표 국소 상면은 y=0.48이며 아래 표가 각 상태의 정확한 경계를 소유한다. 모든 상태의 `body/outer/sole`은 같은 주소 규칙을 따르고, 위에서 명시한 분할 face는 해당 상태에서만 낸다.
...
## 현관 거울과 벽등 {#wall-accessories}

`wall-accessories`는 독립 벽거울과 두 종류 벽등을 고정점 기준 상태로 둔다. 거울은 판 두께와 둘레 프레임을 하나의 닫힌 `body/front/back/edge`로 구분한다. 벽등은 벽쪽 받침, 팔, 발광 갓의 서로 다른 `body/back/arm/diffuser` face를 가진다. 원점은 벽 접촉면 중심이고 +Z가 실내 방향이므로 모든 상태에서 뒤쪽 평면이 벽 datum이다.

원점의 Y는 물체 아래선이고 X는 폭 중심이다. 거울은 전체 W×H 판의 바깥 둘레 W/30을 프레임 face로 남기고 앞 평면은 Z 최대보다 D/8 물러난 반사 face다. 벽등은 뒤쪽 원형 접촉판, 전방 D/2까지 나온 팔과 마지막 D/2의 원통형 확산 갓을 일체 연결하며 갓의 아래면을 별도 diffuser face로 둔다. 벽 접촉 뒤판은 두 상태 모두 Z 최소와 같은 평면이며 광량은 여기서 정하지 않는다.

첫 변종 `entry-mirror`의 대표 국소 상면은 y=1.22이며 아래 표가 각 상태의 정확한 경계를 소유한다. 모든 상태의 `body/outer/sole`은 같은 주소 규칙을 따르고, 위에서 명시한 분할 face는 해당 상태에서만 낸다.

ref01의 현관 온광과 ref05의 복도 실용 조명 규모를 참고해 정면·45°에서 거울과 빛 표면을 분리한다. ref02·03·04에서 보이지 않는 회로와 제품 형상은 만들지 않는다. 조도·배선은 `unverified`다.
...
첫 변종 `pointing-device`의 대표 국소 상면은 y=0.035이며 아래 표의 높이가 상태별 경계다. 두 변종의 `body/outer/sole`은 공통 주소 규칙을 따른다.
