...
<!--
@evidenceReview obligations/core/common.md#proportionate-development #78feb28 표의 27파일 값을 `npm run self-check`가 같은 방식으로 다시 출력해 일치하고(openings 5102, junctions 4652, observations 7545 포함), v-077 대비 늘어난 열여덟 파일 모두가 v-077→현재 값과 그 파일을 바꾼 변경을 가지며 짧아진 둘과 그대로인 여섯도 구별된다.
-->
