...
| openings·junctions·circulation | 3/8 | 4087·4288·1869 |
...
| ownership·observations | 2/4 | 3860·6778 |
| site | 1/6 | 5518 |
...
부모 settings는 허용 범위와 방/부재 정체성을 정하고 spaces는 실제 기준선·벽 두께·문 중심·roof 지지선·순환·대지 구획 및 소유를 더했다. 같은 settings 문장을 방 이름만 바꿔 늘린 분포가 아니다. v-077 판정 제출본 a4c1d46870fb186cbf29c5b112a5476a2bfeb0e4를 같은 방식으로 재면 26파일/37 H2였고 기존 26파일은 모두 남았으며 매스·치수·동선 결정도 빠지지 않았다. 기존 파일 중 roofs/colonnade(1583→1488)와 roofs/porch(704→650)만 짧아졌는데, 파라펫 뒤 외쪽 경사로 바꾸며 폐기한 최소 높이 제한·notch 문장이 빠진 결과이고 두 문서의 지지선·끝선·하부 소유 결정은 그대로다. 늘어난 파일은 그 뒤의 여러 변경이 겹쳐 있어 파일별 v-077→현재 값과 그 파일을 바꾼 변경만 적는다. junctions(1956→4288)는 파라펫·박공 폐쇄 표, 기단·코핑 단면 H2, 제실 처마·동측 박공 코핑 행과 외피 겹침 스캔 문장, observations(5778→6778)는 대지 관찰 묶음, 주랑 추가 모서리와 접합 관찰, 뷰어 실행 인계, ownership(2489→3860)은 대지 두 행과 외부 setting 문장, 하부 띠 H2, 내부 경계벽 reveal 행과 표면 역검사 문장, assembly·sanctuary·east roof(1164·675·535→1990·944·936)는 파라펫·안쪽 경사 재설계와 이번 제실 지지·동측 박공 경사 수정, openings(3635→4087)는 채광구 여덟 개, building·storey(3493·2755→3578·3143)는 대지 이관에 따른 접점·접지 문장에서 늘었다. 새 site(5518)는 대지 결정을 더했다. 새 account와 주석은 공간 수를 늘린 것으로 세지 않는다. 이 분포 비교는 실제 구현 깊이·통과·시각 품질의 완료 판정이 아니며 모두 후속 소스/프레임에서 별도로 검토해야 한다.
