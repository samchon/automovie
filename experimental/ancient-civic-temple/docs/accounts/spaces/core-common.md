...
<!--
@evidence obligations/core/common.md#purpose-fit 27개 파일을 기준 평면/높이·경계/접합·동선·입면·방·roof·표면 소유·관찰·대지 역할로 대조했다. 방 목록만 남기면 물리 접합과 외관이 비고, 외피만 남기면 각 방의 직접 문과 이용 영역이 비며, 대지가 없으면 두 접점이 이어질 외부 공간과 외벽 하단이 읽을 지면이 비는 서로 다른 후속 결정을 아래 지도에 기록한다.
-->
...
<!--
@evidence obligations/core/common.md#proportionate-development 현재 27파일/43 H2를 전체로 읽고 본문 분량을 역할별로 비교했다. 관찰·개구부·기준선·대지는 다수 경계/실패 경우를 다루어 길고 단일 날개/방은 상위 공유 결정을 소비해 짧다. v-077 제출본과 파일/단위 수 및 본문을 비교해 매스·동선 축소가 없음을 기록한다.
-->
...
| openings·junctions·circulation | 3/7 | 4087·2351·1869 |
