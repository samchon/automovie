...
<!--
@evidence obligations/core/common.md#proportionate-development 여덟 파일 36 H2, 주석 제외 본문 21,767자를 이전 판(파일 하나 17 H2, 14,799자)과 build-scope의 prototype 목록에 대조했다. 원주·기와·분수·정문 문짝·제단·트러스가 800자 안팎으로 가장 길고 스툴·작은 용기·풀이 300자 안팎으로 가장 짧아 결과의 무게에 따라 분량이 배분됐다.
-->

현재 모델 population은 여덟 파일, 36개 H2, 주석을 뺀 본문 21,767자다. 파일별로 fixtures 10 H2·5,420자, entablature 5 H2·3,624자, openings 4 H2·3,016자, wares 6 H2·2,436자, landscape 4 H2·2,118자, scale 3 H2·1,764자, columns 2 H2·1,732자, cladding 2 H2·1,657자다. 빈 scaffold에서 다시 만들기 전의 판은 파일 하나에 17개 H2, 14,799자였고 문짝·기둥·기와를 한 H2씩으로 묶었다. 지금은 문틀·양개·외개·창틀, 주랑·포치 원주, 평기와·용마루를 따로 두어 서로 다른 소비자와 변경 경로를 가진 결정이 각자 주소를 가지며 수와 분량이 함께 늘었다.

H2별로는 리뷰 거리에서 건물의 정체성을 가장 크게 좌우하는 주랑 원주(954자), 평기와(889자), 분수(850자), 양개 문짝(849자), 공통 축척(838자), 제단(809자), 제실 트러스(801자)가 가장 길다. 스툴(302자), 작은 탁상 용기(298자), 벽 밑 풀(322자), 얕은 그릇(324자)은 형상 결정이 적은 작은 부재라 가장 짧다. [build-scope](../../settings/00-delivery.md#build-scope)가 models에 배정한 기둥·문짝·문틀·기와·수반·제단·가구·용기·식생·이웃 외피 prototype은 모두 한 H2 이상을 가진다. 설계와 source 사이의 분량은 비교하지 않으며 모든 H2가 modelSources에서 실현되는지는 그 분기가 열린 뒤 따로 센다.
