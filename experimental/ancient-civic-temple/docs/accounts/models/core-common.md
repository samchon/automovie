...
[portable](../../models/portable.md)은 벤치·소형 등잔·항아리 받침·멜대·손수레·물동이·화분·봉헌판·쟁반·직물·첨필·필기판·끈 뭉치의 재사용 prototype을 소유한다. 각 방의 복제 수는 instances가 정한다. 작은 문서 상자는 [궤](../../models/fixtures.md#chest)의 s=0.55 변형이고 낮은 향 받침은 [그릇](../../models/wares.md#offering-bowl)과 쟁반의 조합이라 새 형상을 중복 저작하지 않는다.

## 모델과 다른 제작 분기의 경계 {#layer-routing}

<!--
@evidence obligations/core/common.md#layer-boundary 49개 H2 중 공통 기준 셋은 축척·UV0·물리 반복 길이·fallback·관절·검토 규칙을, 원형 46개는 형상·part 표면 ID·점유와 배치 기준점을 정한다. 실제 비트맵과 광학 반응은 materials, 복제와 접촉은 instances, 물의 흐름과 빛은 systems, 외피·지면은 spaces가 맡는다고 아래 본문이 구분한다.
-->

원형을 정의하는 46개 H2는 각각 자기 prototype의 형상, part와 표면 ID, [공통 UV0·결속 규칙](../../models/scale.md#reference-scale), 가려진 접촉면과 빈 공간, 점유 범위와 배치 기준점을 결정한다. 나머지 scale의 세 H2는 공통 축척·UV0·텍스처 반복 길이·fallback·관절·검토 판 규칙을 정한다. 반복 부재의 배치 수·간격·잘림은 instances가 판정된 공간과 합성 지붕에서 유도한다. 이 모델 문서의 fallback 색과 반복 길이는 텍스처 입력 계약이고, 실제 비트맵 선택·색과 거칠기의 최종 반응은 materials가 소유한다.
...
아래 표는 아홉 문서의 원형 H2에서는 part마다, 공통 규칙인 scale의 세 H2에서는 `공통 규칙` 요약 행으로 만드는 source 입력 색인이다. `공통 규칙`은 방출 part 이름이 아니다. 각 행은 원본 H2 링크·본문 문자 수·SHA-256을 보존하고, 값이 바뀌면 `self-check --sync-accounts`에서 기계적으로 바뀐다. self-check는 49개 H2와 121개 형상 part 주소의 정확한 일치를 검사한다. 이 색인은 빠진 설계 결정을 `0`으로 인증하지 않는다. 리뷰어와 저작자는 링크된 원본 H2의 위치·크기·단면·분할·접합을 판정하고 구현 중 빠진 선택을 찾으면 해당 H2를 먼저 고친다.
...
| [fixtures/niche](../../models/fixtures.md#niche) | `plinth` | 670 | `41463b12b73f2ddc23b8a12087a052da8b1834b887d2570a94968c79472b16b7` |
| [fixtures/niche](../../models/fixtures.md#niche) | `body` | 670 | `41463b12b73f2ddc23b8a12087a052da8b1834b887d2570a94968c79472b16b7` |
| [fixtures/niche](../../models/fixtures.md#niche) | `recess` | 670 | `41463b12b73f2ddc23b8a12087a052da8b1834b887d2570a94968c79472b16b7` |
| [fixtures/niche](../../models/fixtures.md#niche) | `cap` | 670 | `41463b12b73f2ddc23b8a12087a052da8b1834b887d2570a94968c79472b16b7` |
...
| [fixtures/scroll-shelf](../../models/fixtures.md#scroll-shelf) | `frame` | 926 | `32abe5257a0a20cd1085f67f55f4a6e7f5fcaeb9e7ec804d5f6ca62a5253cfae` |
| [fixtures/scroll-shelf](../../models/fixtures.md#scroll-shelf) | `divider` | 926 | `32abe5257a0a20cd1085f67f55f4a6e7f5fcaeb9e7ec804d5f6ca62a5253cfae` |
...
| [portable/carrying-yoke](../../models/portable.md#carrying-yoke) | `beam` | 596 | `71601e09cce24a79723bc89518415c9a9bb52e450bc8e5cabe5f994ea74a4fab` |
| [portable/carrying-yoke](../../models/portable.md#carrying-yoke) | `hook` | 596 | `71601e09cce24a79723bc89518415c9a9bb52e450bc8e5cabe5f994ea74a4fab` |
...
| [scale/reference-scale](../../models/scale.md#reference-scale) | `공통 규칙` | 10535 | `f919ec5f3cfa309a96815200a752299464b520a84b8288539eeb367091c2fb71` |
