...
<!--
@evidenceReview principles/core/common.md#scope-preservation #24155e1 쓰기·설치·기동·검증·출력 보관을 모두 배정해 구현 외 실행 과정의 책임도 누락하지 않는다.
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 production 저작과 조정자 설치/서버 기동을 나눠 다음 작업자가 누가 실행하는지 다시 정하지 않는다.
@evidenceReview principles/core/common.md#declared-basis #7ccd1cb 사용자 지시와 README ownership이 쓰기/검증 권위이고 커밋은 별도 절차 소유로 연결돼 있다.
@evidenceReview principles/core/settings.md#fact-status #93a284a 실행할 권한을 선언한 문서이며 의존성이 설치됐거나 서버가 켜졌다는 결과를 보고하지 않는다.
@evidenceReview principles/core/settings.md#source-support #430bca9 npm run lint의 소유를 README에 연결하고 우회 명령을 공식 검증처럼 제시하지 않았다.
@evidenceReview principles/core/settings.md#capability-boundary #83a6f2c 저작자는 의존성 선언까지 할 수 있으나 설치·viewer 기동 권한은 조정자에게 있어 자동 설치를 수행할 수 없다.
@evidenceReview principles/core/settings.md#constraint-sufficiency #20cf612 추가 flag·직접 binary·설정 파일을 금지하고 시작 명령·디렉터리·포트·경로를 인계하도록 구체화했다.
@evidenceReview principles/core/settings.md#observable-identity #4ccb62e 새 건물 형상을 정하지 않는 실행 단위이며 비교 출력은 외부 capture 경로에 두도록 보관 위치만 소유한다.
-->
...
<!--
@evidenceReview principles/core/common.md#scope-preservation #24155e1 stage부터 push 거부 보고까지 다뤄 커밋만 만들고 전달 성공으로 보고하는 누락을 막는다.
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 add→commit→push와 non-fast-forward 처리까지 정해 공유 checkout에서 동기화 수단을 새로 고르지 않는다.
@evidenceReview principles/core/common.md#declared-basis #7ccd1cb 2026-09-21 사용자 정정이 pull --rebase 금지의 근거이며 이전 절차를 현재 권위로 남기지 않았다.
@evidenceReview principles/core/settings.md#fact-status #93a284a branch와 명령은 수행 절차이고 커밋 해시·push 종료 코드는 실제 실행 후 보고할 결과로 나뉜다.
@evidenceReview principles/core/settings.md#source-support #430bca9 production 명시 경로와 지정 branch가 사용자 문자열을 유지하며 원격 성공을 추측하는 별도 명령을 보태지 않았다.
@evidenceReview principles/core/settings.md#capability-boundary #83a6f2c 다른 저작자의 stage/수정을 stash하거나 되돌릴 수 없고 master·force-push도 금지돼 공유 작업을 보존한다.
@evidenceReview principles/core/settings.md#constraint-sufficiency #20cf612 generated/cache/wiki 제외와 차단 오류 보고를 지정해 실행 불가를 무단 우회로 해결하지 못한다.
@evidenceReview principles/core/settings.md#observable-identity #4ccb62e viewer 작성도 커밋 시점으로 다룰 뿐 커밋 성공이 화면을 검증했다는 시각 조건을 추가하지 않는다.
-->
