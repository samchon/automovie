...
`basin-inner`의 닫힌 바닥 판은 Y=0.115~0.120m, `water` 원판은 Y=0.435~0.440m로 두께가 각각 0.005m이고 각각의 원주는 48분할이다. 파문 `ripple`은 수면 Y=0.44m에서 시작해 꼭대기 Y=0.446m에 닿고 바깥 반지름은 0.114m다.
...
몸체는 앞뒤 Z=0.02~0.38m이고, 머리판은 Z=0~0.42m다. `body`는 칸 좌우 X=−0.55~−0.30m와 +0.30~+0.55m의 전 높이 석판이다. `recess-frame`은 X=−0.30~+0.30m, Y=0.60~0.75m와 1.55~1.80m의 아래·위 석판이며 Z=0.02~0.38m다. `recess`는 X=−0.30~+0.30m, Y=0.75~1.55m, Z=0.02~0.16m의 뒤쪽 석판이고 앞면 Z=0.16m에서 칸 앞면 Z=0.38m까지 0.22m가 비어 있다. 이 뒤판이 Z=0.02~0.16m 부피를 소유하므로 몸체 내부에서 칸 뒤에 소유 없는 빈 공간은 없다. 벽과 뒤판 사이의 Z=0~0.02m 틈은 몸체 바깥에 남는다.
...
두 마디 원통은 줄기 원통과 동축으로 겹쳐 고정한다. 이 겹침은 별도 안쪽 빈 공간이 아니라 한 금속 부재의 결합이다.
...
네 모서리의 ㄱ자 띠도 `strap` part에 포함한다. 뒤 경첩 띠와 앞뒤 네 모서리 띠의 합집합이 이 part의 점유 범위이며 어느 띠도 별도 part나 몸체 목재에 속하지 않는다.
