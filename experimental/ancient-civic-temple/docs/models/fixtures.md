...
## 봉헌실 진열대 {#display-shelf}

[공동 봉헌실](../settings/30-interiors.md#offering-room)의 벽 쪽 목재 진열대다. [선반](../settings/35-objects.md#shelves)의 두꺼운 측판과 넓고 낮은 칸이 근거이며 벽에 매몰되지 않는다.
...
part와 표면은 `side`, `board`다. 칸 세 개는 앞뒤로 열린 빈 공간이며 선반 판 윗면에 그릇이 놓인다. 배치와 벽면은 instances가 정한다.
