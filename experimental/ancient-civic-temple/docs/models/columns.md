...
@evidence principles/core/common.md#substantive-completion 부재별 치수, 보 윗면 접선 식에서 유도한 전체 높이 h≈2.62554m와 몸통 높이 h−0.38m, 24분할·법선·배치 기준점이 있어 source가 기둥 형상을 다시 고르지 않는다.
...
@evidenceExclude upstream/design/models.md#settings-and-space-revision-from-model-work ring-volume의 0.35m 기둥 예산과 roofs/assembly의 주랑 처마 3.20m를 기둥 폭 0.34m·보 접선에서 유도한 높이 약 2.62554m에 대조했고 예산 안에 들어 부모를 고치지 않았다.
...
로컬 원점은 기단 바닥면의 중심이며 기둥 축은 로컬 +Y다. 부재는 아래부터 기단(정방 0.34×0.34m, 높이 0.08m), 받침(원통 반지름 0.155m, 높이 0.10m), 몸통(원뿔대, 아래 반지름 0.135m에서 위 0.115m), 목 띠(원통 반지름 0.125m, 높이 0.03m), 받침머리(원뿔대 반지름 0.125m→0.155m, 높이 0.10m), 주두 판(정방 0.34×0.34m, 높이 0.07m)이다. 전체 높이 h는 [주랑 목재 보](entablature.md#colonnade-beam)의 접촉 식으로 얻은 보 윗면에서 보 깊이 0.28m를 뺀 값 `h=2.905543738…−0.28=2.625543738…m`다. 몸통 높이는 `h−(0.08+0.10+0.03+0.10+0.07)=2.245543738…m`다. source는 반올림한 화면 표기가 아니라 앞의 접촉 식을 소비한다. 원형 부재는 24분할 다면체이고 몸통과 받침은 부드러운 법선, 기단과 주두 판은 평평한 면 법선을 쓴다. 점유 상자는 `0.34×h×0.34m`다.
