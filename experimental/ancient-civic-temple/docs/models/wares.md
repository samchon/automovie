...
로컬 원점은 바닥면 중심이다. 지름 0.40m·높이 0.32m의 약간 벌어지는 원통 껍질(아래 반지름 0.18m, 위 0.20m, 벽 두께 0.015m)이며 바닥 위 0.02m에 안쪽 바닥이 있다. 벽은 Y=0.02~0.305m를 열 칸으로 나눈 각 칸 중심에 연직 폭 0.016m인 가로 띠를 둔다. 아래에서 j=0..9로 세어 짝수 j 띠만 0.004m 바깥으로 나오고 홀수 j는 벽과 같은 반지름이다. 24개의 세로 살은 둘레를 따라 중심각 15°마다 놓이며 각 살의 접선 방향 폭은 0.012m, 바깥 돌출은 0.004m다. 테두리는 중심선 반지름 0.20m·중심 높이 Y=0.305m·관 반지름 0.015m인 원환(주환 24분할·관 8분할)으로 벽 상단과 접하고 맨 위가 Y=0.32m다. 돌출 살과 테두리의 최대 반지름 0.215m를 포함한 점유 상자는 0.43×0.32×0.43m다.
