...
@evidence upstream/design/models.md#settings-and-space-revision-from-model-work 높은 제실 등잔대와 별개인 이동식 비품의 settings 정체성 owner가 없어 35-objects#portable-lamp를 새로 만들고 30-interiors#offering-room의 방 목록에 넣었다. 이 부모 수리를 마친 뒤 형상 산문을 정했다.
...
@evidence principles/core/inherited-units.md#derived-parent-differentiation 35-objects#jar-rack의 두 항아리를 받는 보관실 가구을 로컬 원점·형상 치수·part와 표면·검토 가능한 점유 상자로 좁힌다.
...
@evidence upstream/design/models.md#settings-and-space-revision-from-model-work 바퀴가 멈춘 서비스 마당의 외부 소품의 settings 정체성 owner가 없어 35-objects#handcart를 새로 만들고 30-interiors#service-yard의 방 목록에 넣었다. 이 부모 수리를 마친 뒤 형상 산문을 정했다.
...
축 원통은 둘레 16분할이다. 판의 아랫면 Y=0.43m와 축의 윗면 Y=0.248m 사이는 중심 (X,Z)=(±0.24,0.10)m, X·Z 각 0.04m 단면의 연직 목재 지지재 두 개가 잇는다. 지지재의 Y 범위는 0.248~0.43m이며 윗면은 판에, 아랫면은 축에 닿는다. 두 손잡이는 X 폭 0.04m·연직 두께 0.04m 각재다. 중심선은 각각 X=±0.24m, (Y,Z)=(0.45,−0.65)m에서 (0.70,−1.20)m까지 직선이고 단면을 그 중심선에 따라 평행 이동한다. 시작면은 판의 뒤쪽 단면과 겹쳐 고정되고 끝의 점유 높이는 Y=0.72m다.
...
@evidence principles/core/inherited-units.md#derived-parent-differentiation 35-objects#bucket의 열린 입과 손잡이가 있는 마당 용기을 로컬 원점·형상 치수·part와 표면·검토 가능한 점유 상자로 좁힌다.
...
물동이는 고정된 빈 용기이고 물 표면·실제 담기는 양은 모델에 없다. 로컬 원점은 바닥 중심이다. 몸체 바깥은 Y=0에서 반지름 0.11m, Y=0.28m에서 반지름 0.15m로 벌어지는 16분할 원뿔대다. 바닥 두께 0.02m이며 안쪽은 Y=0.02m에서 반지름 0.095m부터 입 아래 반지름 0.135m까지 열려 있다. 손잡이는 XY 평면에서 X=±0.14m·Y=0.27m 두 접점과 꼭대기 (0,0.44)m를 잇는 반타원 중심선의 12구간 관이고 관 반지름 0.01m·둘레 8분할이다. 접점은 몸체 바깥면 안으로 약 0.0086m 들어가 끊기지 않으며 손잡이 가운데 아래는 빈 공간이다. 점유 상자는 0.30×0.45×0.30m다.

손잡이 중심선은 반타원으로 정한다. t=0..π를 12등분하고 각 점을 (X,Y,Z)=(0.14 cos t,0.27+0.17 sin t,0)m에 놓아 양끝 (±0.14,0.27)m과 꼭대기 (0,0.44)m를 정확히 지난다. 관의 바깥 X는 ±0.15m, 꼭대기 Y는 0.45m이며 접점은 몸체 안으로 약 0.0086m 들어간다. 닫힌 전체 점유 상자는 0.30×0.45×0.30m다.
...
화분은 외부에 놓는 넓은 도기다. 로컬 원점은 바닥 중심이다. 바깥 몸체는 바닥 반지름 0.14m에서 윗입 반지름 0.21m까지 Y=0~0.36m로 벌어지고, 안쪽은 반지름 0.12m에서 0.19m까지 바닥 Y=0.03m에서 열린다. 테두리는 반지름 방향 두께 0.02m다. 노출 흙면은 윗면 Y=0.270m·두께 0.001m인 반지름 0.165m 원판이며 테두리보다 0.09m 낮다. 그 위 식물은 [풀 prototype](landscape.md#grass-tuft)의 인스턴스이고 흙면 자체는 심을 수 있는 구멍을 만들지 않는다. 원형 면은 16분할이다. 점유 상자는 0.42×0.36×0.42m다.

Y=0.270m에서 안쪽 벽 반지름은 0.12+(0.19−0.12)(0.270−0.03)/(0.36−0.03)≈0.1709m다. 흙면 반지름은 0.165m로 두어 안쪽 벽과 약 0.0059m 간격을 남긴다.
...
앞뒤의 X 방향 림 둘은 X=−0.26~0.26m까지 이어지고 좌우의 Z 방향 림 둘은 그 사이 X=−0.245~0.245m에서 끝난다. 모서리는 면을 맞대되 림끼리 부피를 겹치지 않는다.
