...
@evidence principles/core/common.md#substantive-completion 선대·가로대·판 두께와 바닥 띄움, 짝 폭에서 유도한 손잡이 X와 양면 Z, 원환 16×8분할·받침판 핀 0.006m 반지름, 두 경첩 중심 높이·12분할을 확정해 source가 문짝 철물 위치와 구조를 새로 고르지 않는다.
...
@evidence principles/design/models.md#representation-contract frame·panel·ring·hinge part로 목재 두 면과 금속 두 면을 구별하고 손잡이·경첩을 짝 node의 자식으로 둔다.
...
각 짝의 로컬 원점은 경첩 축의 바닥 점이다. 축은 연직이며 문틀 안감의 안쪽 모서리 선 위, 여는 쪽 벽면에서 0.05m 안쪽에 있다. 닫힌 자세에서 짝은 로컬 X 0~(유효 폭/2), Z −0.05~0 범위를 차지하고 바닥에서 0.01m 띄워 높이는 유효 높이−0.01m다. 판은 두께 0.05m이며 선대 폭 0.10m, 윗 가로대 0.10m, 가운데 가로대 0.10m(아랫면 높이 1.00m), 아래 가로대 0.16m로 테두리를 짜고 그 사이 두 판은 양면에서 0.015m 들어가 두께 0.02m다. 맞닿는 선대 쪽 높이 1.05m에 청동 고리 손잡이(바깥 반지름 0.06m, 굵기 0.012m 원환)와 받침판(반지름 0.04m, 두께 0.006m)이 양면에 있고, 경첩 쪽 모서리 위·아래에 핀 경첩(반지름 0.02m, 높이 0.10m 원통) 두 개가 있다. 손잡이와 받침판 중심은 각 짝의 X=(유효 폭/2)−0.15m·Y=1.05m이고 받침판 중심은 양면 Z=0m/−0.05m, 고리 중심은 각 면 바깥으로 0.015m 이동한 Z=+0.015m/−0.065m다. 고리는 받침판 바깥면 중심에서 면 법선 방향으로 반지름 0.006m·길이 0.009m인 핀에 붙는 로컬 XY 평면 원환(주환 16분할·관 8분할)이다. 두 경첩 원통 중심은 X=0·Z=−0.025m, Y=0.25m와 (유효 높이−0.25m)이며 둘레 12분할이다. 점유 상자는 한 짝당 (유효 폭/2)×유효 높이×0.05m에 손잡이 돌출 0.03m를 더한 값이다.
...
@evidence principles/core/common.md#substantive-completion 가로 띠·쇠 띠·널 다섯 장의 치수에 손잡이 X=(유효 폭−0.12m)·Y=1.05m와 양면 Z·원환 16×8분할을 더해 source가 판문 철물 위치를 새로 고르지 않는다.
...
@evidence principles/design/models.md#representation-contract board·batten·strap·ring part로 목재와 금속 표면을 나누고 여는 쪽 면과 반대면의 형상 차이를 정한다.
...
로컬 원점은 경첩 축의 바닥 점이며 축과 닫힌 자세의 규칙은 양개 문짝과 같다. 짝은 폭 = 유효 폭, 높이 = 유효 높이−0.01m, 두께 0.05m이다. 여는 쪽 면에는 가로 띠 두 줄(폭 0.12m, 두께 0.025m, 아랫면 높이 0.25m와 유효 높이−0.40m)과 그 위의 쇠 띠 경첩(폭 0.04m, 두께 0.006m, 짝 폭의 0.6배 길이)이 있고, 반대면은 세로 널 다섯 장의 이음 홈(폭 0.01m, 깊이 0.004m)만 보인다. 경첩 반대쪽의 쇠 고리 손잡이(바깥 반지름 0.05m, 굵기 0.01m)는 양면에서 중심 X=(유효 폭−0.12m)·Y=1.05m, Z=+0.025m/−0.075m이고 로컬 XY 평면의 주환 16분할·관 8분할이다. 각 고리는 반지름 0.006m·길이 0.025m의 짧은 수평 핀으로 해당 판 면(Z=0m 또는 −0.05m)에 붙는다. 점유 상자는 유효 폭×유효 높이×0.05m에 띠·손잡이 돌출 0.04m를 더한 값이다.
