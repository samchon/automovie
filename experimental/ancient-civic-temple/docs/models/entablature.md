...
로컬 원점은 평보 아랫면의 중심이고 폭 방향은 로컬 X, 두께는 Z다. 평보는 길이 11.20m(제실 서·동 벽 안쪽 면 사이), 단면 0.22×0.28m이며 아랫면 Y=4.56m, 윗면 4.84m로 [제실 지붕](../spaces/roofs/sanctuary.md#sanctuary-roof)의 측벽 위치 하부(약 4.92m)보다 낮게 벽에 걸린다. 두 경사재는 단면 0.18×0.20m로 평보 양끝에서 지붕 하부를 따라 용마루 아래(하부 약 7.18m)까지 오르고, 가운데 기둥(0.18×0.18m)은 평보 윗면에서 용마루 아래까지, 두 버팀재(0.14×0.14m)는 가운데 기둥 아래쪽에서 경사재 가운데로 뻗는다. 점유 상자는 11.20×약 2.62×0.22m다. 측벽 채광구(창틀 윗끝 약 4.46m)는 평보 아래에 남는다.
