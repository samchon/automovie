...
@evidence principles/design/spaces.md#space-topology 북쪽 마당과 공유 벽을 뚫지 않고 마당 문→주랑→보관실 문으로 접근한다.
@evidence principles/design/spaces.md#space-boundary-authority storage는 방과 문턱·완결 내부 표면을 소유하며 항아리/바구니 형상과 배치 수량을 재저작하지 않는다.
@evidence principles/design/spaces.md#space-verification-address 마당에서의 양방향 운반·문턱·모서리·네 방위로 큰 항아리의 문 가림과 중앙 진입 축소를 찾는다.
@evidence principles/core/inherited-units.md#derived-parent-differentiation 벽을 따른 저장 요구를 북동남 물품 영역과 서쪽 빈 반입 축으로 구체화한다.
@evidenceExclude upstream/design/spaces.md#settings-and-map-revision-from-space-work storage의 용기/바구니와 use-profile의 작은 봉헌물 운반을 대조했다. 주랑을 경유하는 동고 바닥으로 접근하므로 마당 공유벽에 새 문이나 서비스 계단을 요구하지 않았다.
@evidence settings/30-interiors.md#storage 문서 선반 대신 큰 용기·상자·바구니를 벽 쪽에 두고 중앙 반입을 비운다.
@evidence settings/35-objects.md#vessels 큰 항아리가 문을 가리는 경우 후속 배치와 빈 진입 예약을 함께 수리하도록 한다.
@evidence settings/35-objects.md#baskets 바구니를 저장물로 받아 벽 쪽 영역에 두며 운반 통로를 점유하지 않게 한다.
-->
