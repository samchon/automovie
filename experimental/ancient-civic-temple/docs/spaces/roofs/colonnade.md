...
<!--
@evidenceReview principles/core/common.md#scope-preservation #24155e1 북동 구간과 마당 쪽 끝, 두 모서리 골까지 같은 외쪽 지붕의 범위로 남겨 북쪽 주랑 일부가 덮임에서 빠지지 않는다.
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 지지선 높이·12도·동쪽 east-room+돌출까지의 후보 영역이 있어 이전의 min(N,G) 높이 제한 없이도 덮개를 재현할 수 있다.
@evidenceReview principles/core/common.md#declared-basis #7ccd1cb 제실 남벽 앞 약 3.65m가 12도 산술로 표시되고 compiled 계측이라는 주장이 없다.
@evidenceReview principles/design/spaces.md#space-topology #3f8d925 마당 위에는 외쪽 지붕의 좁은 끝 돌출만 있고 마당 전체를 덮지 않아 서비스 마당이 실내가 되지 않는다.
...
@evidenceReview settings/20-envelope.md#colonnade #29bfe45 북동 모서리와 마당 접면까지 덮임이 이어져 한 주랑의 지붕 관계가 끊기지 않는다.
...
<!--
@evidenceReview principles/core/common.md#scope-preservation #24155e1 남쪽 주랑뿐 아니라 반환벽 옆 두 꼬리와 두 모서리 골, 파라펫 앞 끝까지 같은 덮개로 남겨 현관 좌우가 무지붕으로 빠지지 않는다.
@evidenceReview principles/core/common.md#substantive-completion #5b9d0e7 세 후보 영역의 좌표 경계와 12도가 적혀 있어 이전 X 방향 용마루와 notch 없이 덮개를 재현할 수 있다.
@evidenceReview principles/core/common.md#declared-basis #7ccd1cb 남측 파라펫 앞 약 3.95m가 12도 산술로 적히고 현관 몸체 경계는 기준선의 후퇴벽·반환벽 선을 소비한다.
@evidenceReview principles/design/spaces.md#space-topology #3f8d925 포치 박공이 덮는 곳은 현관 몸체뿐이고 양쪽 꼬리는 이 덮개가 맡아 포치가 뒤쪽 주랑을 덮는다는 가정이 없다.
@evidenceReview principles/design/spaces.md#space-boundary-authority #d114e35 덮개 외면과 주랑 하부, 반환벽·후퇴벽 외면이 각자 소유로 남아 같은 면이 두 번 저작되지 않는다.
@evidenceReview principles/design/spaces.md#space-verification-address #a143ab1 후퇴벽 뒤와 반환벽 바깥 접합을 관찰 대상으로 적어 정면 사진만으로 꼬리 구간의 틈을 통과시키지 않는다.
@evidenceReview principles/core/inherited-units.md#derived-parent-differentiation #0632226 낮은 주랑 덮개를 파라펫 뒤 외쪽 경사와 현관 제외 영역이라는 결정으로 좁혔다.
@evidenceExcludeReview upstream/design/spaces.md#settings-and-map-revision-from-space-work #46f1b62 물린 포치와 연속 덮임이 현관 몸체 제외만으로 양립해 이 단위로 settings를 고칠 이유가 없었다.
@evidenceReview settings/20-envelope.md#entrance-porch #e6180af 덮개가 현관 몸체를 넘지 않아 파라펫 사이 포치 박공이 정면에서 온전히 보인다.
@evidenceReview settings/20-envelope.md#colonnade #29bfe45 남쪽 주랑과 두 꼬리가 같은 덮개 아래 있어 포치와 무관하게 한 고리의 지붕이 이어진다.
-->
