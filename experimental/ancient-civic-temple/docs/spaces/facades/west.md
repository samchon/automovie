...
<!--
@evidence principles/core/common.md#scope-preservation 긴 봉헌실의 서측 파라펫 전 길이와 코핑·파라펫 뒷면, 양끝 모서리·기단, 지붕 위로 드러난 제실 서측벽 외면을 관찰 대상에서 빼지 않는다.
@evidence principles/core/common.md#substantive-completion 추가 창/출입구가 없는 서벽 범위, 코핑 상단 4.85m, 안쪽 면의 세 띠, 북남 모서리 절단·지면 접촉을 정한다.
@evidence principles/core/common.md#declared-basis west 기준선과 남측 입면의 코핑·기단 규칙, junctions의 대각 접합과 띠 분할을 가져와 별도 벽 높이를 발명하지 않는다.
@evidence principles/design/spaces.md#space-topology 안쪽은 봉헌실 하나이며 바깥에서 새 출입구를 뚫거나 파라펫 뒤에 통로를 만들어 주랑 직접 출입의 의미를 바꾸지 않는다.
@evidence principles/design/spaces.md#space-boundary-authority 서측 완결 외면·코핑·파라펫 뒷면·제실 서측벽 외면은 west, 지붕 아래 안쪽은 offering이며 모서리 절단이 외면을 다른 입면 owner에게 넘기지 않는다.
@evidence principles/design/spaces.md#space-verification-address 서측 정면·두 모서리·파라펫 뒤 지붕 만남·제실 서측벽에서 길이·접지·코핑 높이를 읽고 무창 실내 밝기는 후속 렌더에 남긴다.
@evidence principles/core/inherited-units.md#derived-parent-differentiation 연속 회벽 요구를 창 없는 봉헌실 서측 파라펫과 대각 끝 접합의 단독 면 소유로 확정한다.
@evidenceExclude upstream/design/spaces.md#settings-and-map-revision-from-space-work walls의 연속 회벽, offering-room의 주랑 출입, 수리된 roof-form의 파라펫을 대조했다. 추가 외부 창 없이도 요구된 연결과 파라펫이 유지되며 실제 주광 읽힘은 렌더 질문으로 남겼다.
@evidence settings/20-envelope.md#walls 외벽 전 길이의 회벽·기단을 실제 닫힌 벽 표면에 연속 결속한다.
@evidence settings/20-envelope.md#roof-form 서측 외곽을 코핑 파라펫으로 올려 안쪽으로 기우는 서측 외쪽 지붕의 높은 끝을 가린다.
-->
...
source `src/spaces/facades/west.ts`가 전 길이의 외측 회벽·기단·코핑 표면과 벽 실체를 소유한다. 안쪽 면은 [띠 분할](../junctions.md#gable-closures)로 지붕 아래 봉헌실 마감, 지붕 두께 안 접면, 지붕 위 파라펫 뒷면으로 나뉘며 앞의 것은 봉헌실, 뒤의 것은 서측 입면이 맡는다. 서측 spine의 제실 구간이 봉헌실 지붕 위로 드러낸 외면도 서측 입면 소유다. 북·남 입면과 만나는 두 모서리의 실체 범위는 [대각 접합](../junctions.md#wall-junctions)으로 자르며 서측 완결 외면은 그대로 유지한다. 석재·회벽의 연속을 마감 owner가 이 완결 면에 결속하며 임의 패치를 붙이지 않는다.

서측 정면 관찰과 두 끝 모서리, 파라펫 뒤 지붕 만남과 제실 서측벽 관찰로 길이·기단 접지·코핑 높이를 읽는다. 개구부가 없다고 관찰에서 제외하지 않는다. 창을 만들지 않고도 봉헌실 내부가 주랑 문과 실제 환경광에서 읽히는지는 후속 렌더에서 검증한다.
