...
const strip = (wall: string, face: number, axis: "x" | "z", from: number, to: number,
  observation: string, evidence: string, minY?: number): AddressException =>
  ({ wall, face, axis, from, to, observation, evidence, minY });
...
export const exposedAddressExceptions: readonly AddressException[] = [
  ...
  strip("wall.facade-east", 1, "z", p.southInner, p.southOuter, "exterior.corner.southeast", "spaces/observations.md#geometry-observations"),
