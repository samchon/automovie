...
void test("review audit runs every probe after local gates and sums failed exits", () => {
  ...
  const lines: string[] = [];
  const statuses = [0, 0, 0, 1, 0, 0, 0, null, 0, 0];
  ...
  assert.deepEqual(calls.slice(3).map((call) => call.args[0]), [
    "src-literal-duplication.cjs", "src-review-host.mjs", "docs-review-host.mjs",
    "doc-review-numbers.mjs", "doc-anchor-graph.cjs", "face-binding-owner.cjs",
  ...
  assert.equal(calls[9]?.command, "python");
  assert.match(lines.join(""), /benchmark audit: 10 gates, 2 failed/);
});
...
void test("review audit accepts a complete green run", () => {
  ...
  assert.match(lines.join(""), /benchmark audit: 10 gates, 0 failed/);
});
