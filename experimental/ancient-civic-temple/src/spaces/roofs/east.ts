 ...
 * @evidenceReview spaces/roofs/east.md #9840ccb # 동측 박공의 한 후보 polygon을 gablePlanes 양 경사면으로 내고 마당 북쪽 끝과 남측 파라펫 끝을 다르게 다룬다.
 * @evidenceReview spaces/roofs/east.md#east-roof #674b805 # eastCourt 지지 3.20과 동측 벽 중심 지지 3.55를 19도 gablePlanes에 넣어 비중앙 X 약 7.36 용마루와 남쪽 돌출 없는 끝을 얻는다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation #e4bc845 # 경사와 지지 높이는 rules를 받고 북쪽만 yardFront−overhang까지 가므로 마당 전체 위에 지붕을 새로 올리지 않는다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion #e9c974f # gablePlanes가 서로 다른 지지 높이를 가진 두 면의 polygon·height·thickness를 반환해 업무 날개 박공이 단일 평판으로 납작해지지 않는다.
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work #d9ad066 # 동측 원본의 두 지지선·19도·남단 southInner를 실제 인자에 대조했고 용마루 약 4.53m가 코핑 아랫면 4.69m보다 낮아 부모 수정이 필요하지 않았다.
 */
