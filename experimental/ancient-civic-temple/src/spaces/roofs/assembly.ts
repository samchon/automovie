 ...
 * @evidenceReview spaces/roofs/assembly.md #f89d644 # 지붕 합성 파일의 공통 높이·경사·두께·돌출을 RoofRules 한 값으로 제공하고 각 후보 owner가 이를 소비한다.
 * @evidenceReview spaces/roofs/assembly.md#roof-junctions #1812a91 # courtEave 3.2, eastWallSupport 3.55, sanctuarySupport 5.35, porchSupport 4와 경사 12/22/19도·두께 0.18·돌출 0.35가 원본 입력과 같다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation #e4bc845 # 경사·지지 값 객체만 내고 실제 patch polygon이나 roof surface는 별도 지붕 함수가 정한다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion #e9c974f # RoofRules 타입의 유한 literal이 다섯 roof 함수를 호출할 때 같은 인자로 들어가 개별 후보의 미정 경사가 없다.
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work #d9ad066 # 부모에서 이미 수리한 동측 19도·제실 지지 5.35m를 이 값 객체에 맞췄고 1cm 겹침 스캔에서 코핑 관통 0을 확인했다.
 */
 ...
 * @evidenceReview spaces/roofs/assembly.md #f89d644 # 서측·남북 주랑·동측 후보를 composeHighest로 한 날개에 합친 뒤 제실과 포치를 별도 patch로 더한다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation #e4bc845 # 날개 후보만 서로 높이를 비교하고 제실·포치는 차집합으로 자르지 않으며 분할 조각에 원본 owner·tier·두께를 보존한다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion #e9c974f # 후보 교차 polygon을 partitionPlan과 clipPlan으로 잘라 동고면은 선언 순서 하나만 남기는 RoofPatch 배열을 실제 반환한다.
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work #d9ad066 # 네 모서리 골과 떠 있는 제실 처마는 부모의 최대 상면 합성으로 구현되고 막음 뒷면은 junctions 수리 뒤 environment가 별도 face로 냈다.
 */
