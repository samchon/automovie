 ...
 * @evidenceReview spaces/roofs/colonnade.md #5e9d85f # 북쪽 한 polygon과 현관 몸체를 비운 남쪽 세 polygon을 모두 roof-colonnade 후보로 내어 한 주랑 지붕 소유를 유지한다.
 * @evidenceReview spaces/roofs/colonnade.md#north-canopy #a29fbe5 # courtBack에서 북쪽으로 오르는 12도 면은 northRing에서 끝나고 동쪽 마당 쪽 끝은 eastRoom+overhang까지 나가되 마당 전체를 덮지 않는다.
 * @evidenceReview spaces/roofs/colonnade.md#south-canopy #698df98 # courtFront에서 남쪽으로 오르는 면을 entranceBack 뒤에서 양쪽 꼬리로 나눠 후퇴벽·반환벽·포치 몸체를 지붕 후보에서 비운다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation #e4bc845 # leanSlope·courtEave·normalThickness·overhang을 rules로 받고 기준선 p 밖에서 새 지지 높이를 선택하지 않는다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion #e9c974f # leanToPlanes가 북쪽 한 조각과 남쪽 세 조각의 높이 평면·수직 두께를 만들어 두 고리 구간의 지붕 후보를 반환한다.
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work #d9ad066 # northRing과 southInner 끝선 및 현관 제외 세 영역을 두 parent H2와 비교해 지붕 연결을 위해 마당을 덮거나 주랑을 분할할 필요가 없었다.
 */
