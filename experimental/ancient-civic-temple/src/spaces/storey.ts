 ...
 * @evidenceReview spaces/storey.md # templeLevels 한 객체에 temple-ground와 네 접지 높이·slab·낮은 천장 예약이 있어 이 production의 층 높이 원천이 분산되지 않는다.
 * @evidence spaces/storey.md#ground-storey Y=0 완성면, 중정 -0.12, 도로 -0.24, 구조체 0.18, 낮은 천장 3.10과 예약 0.10·보 깊이 0.18을 설계 값 그대로 옮긴다.
 * @evidenceReview spaces/storey.md#ground-storey # floor 0, courtyard −0.12, publicRoad −0.24와 구조체·천장·보 예약 값을 지상층 표와 수치별로 대조했다.
 * @evidence spaces/storey.md#threshold-support 문턱 바닥이 쓰는 완성면과 구조체 두께를 방 바닥과 같은 값으로 제공해 벽 두께 안 문턱 예약이 같은 높이를 쓴다.
 * @evidenceReview spaces/storey.md#threshold-support # 모든 roomFloorInput이 templeLevels.floor와 slabThickness를 받아 문턱 slab과 방 slab이 같은 완성면·두께를 소비한다.
 * @evidence principles/core/source-units.md#source-scope-preservation 높이와 예약만 가지며 바닥·천장 실체는 방 소유 함수가 만든다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # 이 export에는 숫자와 storey ID만 있고 바닥·천장 mesh를 생성하는 함수는 각 방 파일에 남는다.
 * @evidence principles/core/source-units.md#source-substantive-completion as const 값으로 모든 방·벽·connector가 같은 층 기준을 받는다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # storey 객체가 공통 floor·slab·ceiling 값을 제공해 방 cell과 외벽 하단 계산이 독립 높이 리터럴에 의존하지 않는다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # temple-ground 한 층의 수치가 부모 표와 같고 이 객체가 새 층·계단 높이를 도입하지 않아 설계층 수리는 없었다.
 */
 ...
 * @evidenceReview spaces/storey.md # templeWallBottom은 대지·인접 바닥의 두 최저 입력을 받아 공통 외벽 하단 하나만 반환한다.
 * @evidence spaces/storey.md#wall-ground-contact 두 최저값 중 낮은 쪽에서 구조체 두께 0.18m를 뺀 값을 공통 외벽 하단으로 돌려준다.
 * @evidenceReview spaces/storey.md#wall-ground-contact # 반환식 Math.min(outsideContactMinimum, adjacentFloorMinimum)−slabThickness가 벽 하단의 부모 산식을 그대로 구현한다.
 * @evidence principles/core/source-units.md#source-scope-preservation 외부 지면을 여기서 만들지 않고 호출자가 site의 지면과 실제 바닥에서 읽은 값만 받는다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # 함수의 두 매개변수로 접지값을 받으며 site grade mesh나 새 바닥 좌표를 이 층 파일에 만들지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # 두 Number.isFinite guard가 비유한 접지값을 거부하고 성공 경로에서 하나의 m 단위 하단 수치를 반환한다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # wall-ground-contact가 정한 min−0.18m 식을 그대로 쓰고 대지 높이는 호출자가 공급해 부모 접지 규칙을 수정하지 않았다.
 */
