 ...
 * @evidenceReview spaces/site.md # SiteIds는 unit과 space를 temple-site로, 단독 뿌리 element를 site.root로 지정한다.
 * @evidence spaces/site.md#site-extent 대지를 건물과 다른 소유 단위 temple-site로 주소화한다.
 * @evidenceReview spaces/site.md#site-extent # building unit과 space ID가 temple-site라 건물 temple의 자식 방으로 대지를 넣지 않는다.
 * @evidence principles/core/source-units.md#source-scope-preservation ID 세 개만 가진다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # 상수의 키가 unit·space·root 셋뿐이라 새 대지 구조체나 다른 층 ID를 포함하지 않는다.
 * @evidence principles/core/source-units.md#source-substantive-completion as const 값으로 대지 조립·connector·관찰이 같은 ID를 쓴다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # 조립 함수와 site 관찰·connector가 같은 temple-site 문자열을 재사용할 단일 식별자 객체다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 부모의 temple-site 별도 unit 이름을 유지하고 대지를 temple-ground 아래로 옮기지 않았다.
 */
 ...
 * @evidenceReview spaces/site.md # TempleSite에는 별도 unit·space, 세 model·element와 보행 support·walkable 목록의 여섯 필드가 있다.
 * @evidence principles/core/source-units.md#source-scope-preservation 대지 단위가 내는 항목만 담는다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # 타입은 대지 조립 산출만 담고 건물 방이나 후속 이웃 집 instance를 속성으로 소유하지 않는다.
 * @evidence principles/core/source-units.md#source-substantive-completion environment가 대지를 건물 옆 단위로 합칠 때 필요한 여섯 필드를 정한다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # createTempleSite의 반환을 여섯 필드로 제한해 environment가 대지 단위와 표면·보행 입력을 빠짐없이 합칠 수 있다.
 * @evidenceExclude upstream/design/space-sources.md#design-revision-from-space-source-work site.md의 대지 산출 항목을 그대로 담는다.
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 별도 site unit의 기존 조립 항목만 타입화하며 대지를 건물 안 공간으로 바꾸지 않았다.
 */
export interface TempleSite {
  /**
   * @evidence spaces/site.md TempleSite.unit는 대지 building unit(temple-site, 뿌리 element, 공간)이다.
   * @evidenceReview spaces/site.md # unit 필드가 temple-site id·site.root element·temple-site space를 결합한 IAutoMovieBuildingUnit을 받는다.
   ...
   * @evidenceReview principles/core/source-units.md#source-scope-preservation # 이 속성은 site unit 하나의 타입이며 본 건물 temple unit을 여기에 재등록하지 않는다.
   ...
   * @evidenceReview principles/core/source-units.md#source-substantive-completion # unit 값에 별도 ID와 root·space가 있어 environment.buildings에 본 건물과 나란히 넣을 수 있다.
   ...
   * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 대지 unit ID가 부모의 temple-site 그대로라 새 소유 계층은 필요하지 않았다.
   */
  unit: IAutoMovieBuildingUnit;
  /**
   * @evidence spaces/site.md TempleSite.space는 네 볼록 cell을 가진 대지 공간이다.
   * @evidenceReview spaces/site.md # space 필드에 건물 직사각형 바깥 네 방향 levelCell을 가진 exact site 공간이 들어간다.
   * @evidence principles/core/source-units.md#source-scope-preservation TempleSite.space는 대지 공간 하나만 담고 건물 공간·층 아래에 넣지 않는다.
   * @evidenceReview principles/core/source-units.md#source-scope-preservation # site 공간의 parent는 null이고 id는 temple-site라 지상층 room tree의 자식이 아니다.
   ...
   * @evidenceReview principles/core/source-units.md#source-substantive-completion # siteCells 남·북·서·동 네 구획이 공간 필드에 묶여 건물 외곽 안쪽을 대지 cell로 채우지 않는다.
   ...
   * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # site extent에서 건물 footprint를 뺀 네 방향 cell만 받아 부모 대지 분해를 유지한다.
   */
  space: IAutoMovieBuiltSpace;
  /**
   * @evidence spaces/site.md TempleSite.models는 지면·경계석·먼 배경 세 model이다.
   * @evidenceReview spaces/site.md # models 배열은 site-ground·site-curbs·site-distant 세 surfaceModel의 결과다.
   ...
   * @evidenceReview principles/core/source-units.md#source-scope-preservation # site model 셋을 명시해 이웃 주택 원형이나 나무 수관을 대지 소스가 만들지 않는다.
   ...
   * @evidenceReview principles/core/source-units.md#source-substantive-completion # 세 model의 독립 ID가 대지 지면·닫힌 경계석·열린 먼 배경 표면을 각각 environment에 제공한다.
   ...
   * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # site-paving 두 owner와 distant-ridge 한 owner만 받았고 후속 배치용 이웃 model은 앞당기지 않았다.
   */
  models: IAutoMovieModel[];
  /**
   * @evidence spaces/site.md TempleSite.elements는 뿌리와 세 model element다.
   * @evidenceReview spaces/site.md # elements 배열에는 site.root 하나와 세 site model 각각을 가리키는 element가 있다.
   * @evidence principles/core/source-units.md#source-scope-preservation TempleSite.elements는 뿌리와 세 model element만 담는다.
   * @evidenceReview principles/core/source-units.md#source-scope-preservation # 한 root와 models.map 셋뿐이라 이웃 집이나 나무 instance element가 추가되지 않는다.
   ...
   * @evidenceReview principles/core/source-units.md#source-substantive-completion # 세 model element의 parent가 ids.root이고 model 값이 각 m.id라 대지 표면이 한 root에서 보인다.
   ...
   * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # site.root 아래 표면 셋만 부모-자식으로 놓아 건물 element 트리를 건드리지 않았다.
   */
  elements: IAutoMovieBuiltElement[];
  /**
   * @evidence spaces/site.md TempleSite.supports는 흙띠·포장·이웃 바닥의 보행 support다.
   * @evidenceReview spaces/site.md # supports 필드는 SitePieces의 지면 구획에서 생긴 floor support 배열을 받으며 경계석 prism의 top은 별도다.
   ...
   * @evidenceReview principles/core/source-units.md#source-scope-preservation # templeSiteSupports만 반환 필드로 쓰고 curb faces에서 support를 만들지 않아 경계석 위를 보행 면으로 세지 않는다.
   ...
   * @evidenceReview principles/core/source-units.md#source-substantive-completion # plane 높이의 built surface 목록이 site 관찰의 supportHeight와 접근 connector의 지면 높이 입력이 된다.
   ...
   * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 지면 경사에서 유도된 support만 사용해 부모의 세 구간 grade를 새로 선택하지 않았다.
   */
  supports: IAutoMovieBuiltSurface[];
  /**
   * @evidence spaces/site.md TempleSite.walkable는 이웃 바닥을 뺀 보행 가능 support ID 목록이다.
   * @evidenceReview spaces/site.md # walkable 필드는 templeSiteWalkable이 support에서 earth·paving ID만 뽑은 결과다.
   * @evidence principles/core/source-units.md#source-scope-preservation TempleSite.walkable는 보행 가능한 support ID만 담고 이웃 바닥을 뺀다.
   * @evidenceReview principles/core/source-units.md#source-scope-preservation # 이웃 ground.* support는 남아 있어도 walkable 목록에서는 제외돼 방문자가 이웃 필지로 걷지 않는다.
   ...
   * @evidenceReview principles/core/source-units.md#source-substantive-completion # support ID 문자열 목록이 environment.walkable으로 합쳐져 대지 접근의 허용 바닥을 특정한다.
   ...
   * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # earth와 paving만 보행 목록으로 고르고 neighbor ground를 빼 부모 대지 경계 밖 이동을 허용하지 않았다.
   */
  walkable: string[];
}
 ...
 * @evidenceReview spaces/site.md # createTempleSite가 별도 unit·네 cell space·세 surfaceModel·네 element·support 및 walkable ID를 한 결과에 조립한다.
 * @evidence principles/core/source-units.md#source-scope-preservation 건물 공간·층 아래에 넣지 않고 이웃·나무 개체는 만들지 않는다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # unit의 id와 space parent=null이 본 건물과 분리되고 models 배열도 지면·경계석·먼 배경으로 한정된다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # surfaceModel 셋, siteCells, siteSupports 및 해당 elements가 TempleSite 구조로 반환되어 environment 소비자가 추가 조립 결정을 하지 않는다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # temple-site 별도 unit과 네 방향 cell·세 owner를 판정된 site 설계대로 결합했으며 이웃 배치를 앞당기지 않았다.
 */
