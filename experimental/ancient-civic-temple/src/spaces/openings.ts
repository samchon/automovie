 ...
 * @evidenceReview spaces/openings.md #a3a99de # templeDoorPassages에는 정문·제실·봉헌실·동측 세 업무방·마당·서비스 출입의 여덟 ID와 각 host·room·치수·스윙 필드가 모두 있다.
 ...
 * @evidenceReview spaces/openings.md#doors #8a0716f # 여덟 객체의 width·height·room·adjacent·swing을 문 표 각 ID에 맞춰 대조했으며 두 양개문 외 나머지 여섯은 room 방향 스윙이다.
 * @evidence principles/core/source-units.md#source-scope-preservation 문 위치와 치수는 표의 값만 쓰고 문짝 형상은 만들지 않는다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation #e4bc845 # 배열은 passage 수치와 스윙만 정의하고 문짝 mesh·경첩 node는 후속 모델에 남긴다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion #e9c974f # DoorPassage 배열이 templeDoorVoid, roomFloorInput, circulation, opening 관찰의 공통 문 ID 입력이다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work #d9ad066 # 설계 문 표의 여덟 ID·중심·통과치수를 그대로 복사했고 문턱 및 connector가 같은 passage를 읽으므로 문 위치를 다시 열지 않았다.
 */
 ...
 * @evidenceReview spaces/openings.md #a3a99de # templeDoorProfile은 passage의 center·width·height·frame으로 네 꼭짓점의 host local XY 윤곽을 계산한다.
 * @evidence principles/core/source-units.md#source-scope-preservation 유효 폭 양옆과 head 위에 틀 0.06m만 더하고 다른 여유를 만들지 않는다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation #e4bc845 # half=width/2+frame이고 top=floor+height+frame이라 유효 문 둘레에 틀 한 겹 외 추가 여유가 없다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion #e9c974f # 반환 outline의 네 모서리가 door.center±half와 floor..head를 닫아 opening.profile에 그대로 제공된다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work #d9ad066 # boundary-ownership의 유효폭+양쪽 frame 및 head+frame을 그대로 계산해 설계 void 치수를 늘리지 않았다.
 */
 ...
 * @evidenceReview spaces/openings.md #a3a99de # clerestory 반환 배열은 북·남 박공 각 두 개와 서·동 spine 각 두 개로 여덟 ID이며 각 항목에 host 범위와 profile이 있다.
 ...
 * @evidenceReview spaces/openings.md#clerestories #5e3e79e # 두 c값과 두 z값의 네 map을 세어 8개를 확인했고 남·서·동 창은 .upper/-upper 외부 경계에만 결속된다.
 * @evidence principles/core/source-units.md#source-scope-preservation 창 위치·높이·결속 경계는 설계 값만 쓰며 유리나 추가 창을 만들지 않는다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation #e4bc845 # window helper는 0.4m 구멍과 0.06m 틀 윤곽만 내고 glazing·sash 또는 아홉 번째 채광구는 만들지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion #e9c974f # 각 창 객체의 profile 네 모서리는 center±0.26과 sill−0.06..sill+0.46으로 계산되고 벽 절단 함수가 이를 재사용한다.
 ...
 * @evidenceReview upstream/design/space-sources.md#design-revision-from-space-source-work #d9ad066 # 남·서·동 창 배열의 boundary가 선행 수리의 sanctuary-upper 경계를 사용해 옛 두 공간 경계에 창 threshold를 되살리지 않는다.
 */
 ...
 * @evidenceReview spaces/openings.md #a3a99de # templeDoorVoid는 문 ID에 대해 center±width/2±frame의 길이 구간과 slab 바닥부터 head+frame까지 수직 절단을 반환한다.
 * @evidence spaces/storey.md#threshold-support void 아랫면을 문턱 slab 아랫면(완성면−0.18m)으로 두어 벽이 문턱 예약을 비운다.
 * @evidenceReview spaces/storey.md#threshold-support #6354225 # bottom=floor−slabThickness라 문턱 아래 구조체도 문 host 벽에서 실제로 비워지는지 확인했다.
 * @evidence principles/core/source-units.md#source-scope-preservation 길이 방향은 유효 폭+양쪽 틀, 위는 head+틀만 비운다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation #e4bc845 # from·to·top의 계산에 door.width·height·frame 외 다른 opening allowance가 섞이지 않는다.
 * @evidence principles/core/source-units.md#source-substantive-completion WallVoid 하나를 돌려줘 wallColumns가 문턱 아래·인방 위만 남긴다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion #e9c974f # id와 from·to·bottom·top 네 절단값이 WallVoid 하나에 있어 벽 column 생성에서 문 아래·위 실체를 분리할 수 있다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work #d9ad066 # door void 바닥이 기존 slab 아랫면이고 윗선이 기존 head+frame이라 문턱 또는 host 치수를 새로 정하지 않았다.
 */
 ...
 * @evidenceReview spaces/openings.md #a3a99de # DoorVoidsOn은 passage의 boundary startsWith 입력을 거쳐 선택된 문만 WallVoid로 옮기므로 벽별 문 절단을 구분한다.
 * @evidence principles/core/source-units.md#source-scope-preservation 문 표를 걸러 void를 만들 뿐 새 문을 만들지 않는다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation #e4bc845 # 이 함수는 이미 선언된 templeDoorPassages를 filter/map할 뿐 새 opening ID나 profile을 조립하지 않는다.
 * @evidence principles/core/source-units.md#source-substantive-completion 해당 벽의 WallVoid 목록을 돌려준다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion #e9c974f # boundaryPrefix에 맞는 모든 passage가 templeDoorVoid로 바뀐 배열로 반환되어 wall host가 받을 문 목록이 완성된다.
 * @evidenceExclude upstream/design/space-sources.md#design-revision-from-space-source-work openings.md의 문-경계 결속을 그대로 소비한다.
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work #d9ad066 # host 선택은 기존 boundary 문자열에 대한 필터이며 문이 없는 경계에는 빈 배열이라 부모의 출입 배정을 바꾸지 않는다.
 */
 ...
 * @evidenceReview spaces/openings.md #a3a99de # ClerestoryVoidsOn은 한 host ID에 정확히 결속된 창의 실제 profile 좌표에서 벽 절단 구간을 읽는다.
 * @evidence principles/core/source-units.md#source-scope-preservation 결속 경계가 정확히 일치하는 창만 고른다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation #e4bc845 # 창 선택은 window.boundary===boundary라 접두어가 같은 다른 상부 host의 창을 끌어오지 않는다.
 * @evidence principles/core/source-units.md#source-substantive-completion profile의 x·y 범위와 같은 WallVoid 목록을 돌려준다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion #e9c974f # 네 profile 모서리의 X/Y 최소·최대를 각각 from/to/bottom/top에 넣어 창 frame 외곽과 벽 void가 일치한다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work #d9ad066 # 기존 여덟 window.boundary와 profile 범위만 절단 입력으로 소비하므로 창 위치나 외부 host를 새로 고를 이유가 없었다.
 */
