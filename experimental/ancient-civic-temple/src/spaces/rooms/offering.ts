 ...
 * @evidenceReview spaces/rooms/offering.md #48c9733 # OfferingPlan이 westInner·westRoom·northInner·southInner를 네 변으로 받아 19.3m 긴 봉헌실을 한 사각형으로 둔다.
 ...
 * @evidenceReview principles/core/source-units.md#source-scope-preservation #e4bc845 # 이 plan에는 네 기준선 참조 이외의 격벽선이 없어 긴 방 뒤편을 추가 공간으로 분리하지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion #e9c974f # Floor·levelCell·Ceiling이 같은 OfferingPlan을 소비해 길이 19.3m 공간의 바닥과 천장이 다른 구획을 갖지 않는다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work #d9ad066 # 봉헌실 plan의 네 끝선이 부모의 서측 긴 방 범위를 그대로 가리키고 문턱 위치는 여기서 다시 고르지 않아 부모 경계를 고치지 않았다.
 */
 ...
 * @evidenceReview spaces/rooms/offering.md #48c9733 # Floor의 roomFloorInput이 offering body와 door passage를 하나의 방 id로 받아 바닥과 동쪽 직접 문턱을 같은 owner 아래 방출한다.
 ...
 * @evidenceReview spaces/storey.md#threshold-support #6354225 # offering Floor가 passage 목록과 층 두께를 제공하므로 서측 spine의 door-offering 벽 두께 문턱을 주랑 쪽으로 넘기지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-scope-preservation #e4bc845 # floor·slab 두 높이는 storey에서 오고 roomFloorInput이 offering 문만 필터하므로 다른 방 문턱이 봉헌실 바닥에 붙지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion #e9c974f # 한 floor 입력에 body 구획과 passages가 같이 있어 문턱 slab/support를 방 바닥 생성과 분리해 누락하지 않는다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work #d9ad066 # 봉헌실 바닥은 기존 OfferingPlan과 door-offering passage만 받아 서측 spine void나 방 길이를 새로 정하지 않았다.
 */
 ...
 * @evidenceReview spaces/rooms/offering.md #48c9733 # 반환 레코드는 id·kind offering과 단층 parent를 갖고 한 body 및 문턱 cell로 긴 봉헌실 한 방을 표현한다.
 ...
 * @evidenceReview spaces/rooms/offering.md#offering-volume #bd0ef98 # levelCell의 긴 plan과 offering thresholdCells가 한 cells 배열에 있으며 추가 back-room 레코드가 없다.
 ...
 * @evidenceReview principles/core/source-units.md#source-scope-preservation #e4bc845 # y.lowCeiling과 OfferingPlan이 본체 cell을 한정하고 passage는 offering에 속한 문턱만 더해 후실이 생기지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion #e9c974f # 공간 반환값의 cells는 body와 문턱을 함께 담아 주랑 쪽 문턱 관찰이 offering 내부로 귀속될 근거를 제공한다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work #d9ad066 # 방 본체는 기존 긴 평면과 lowCeiling을 쓰고 문턱도 해당 passage만 추가하므로 봉헌실을 나누는 부모 변경이 필요하지 않았다.
 */
 ...
 * @evidenceReview spaces/rooms/offering.md #48c9733 # ceilingBoardFaces가 OfferingPlan과 y.lowCeiling·ceilingBoardReserve를 받아 긴 방 전체를 덮는 0.10m 널판을 낸다.
 * @evidence principles/core/source-units.md#source-scope-preservation 봉헌실 평면 전체에 층의 낮은 천장 높이와 널판 예약만 쓰고 노출 보는 만들지 않는다(후속 부재).
 * @evidenceReview principles/core/source-units.md#source-scope-preservation #e4bc845 # 천장 export는 방 평면과 층 높이만 받고 지붕 서까래나 보 형상을 방 소스에 조립하지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion #e9c974f # offering id의 ceilingBoardFaces 호출이 아랫면과 구조 뒷면을 분리한 닫힌 판을 긴 방 plan 전부에서 생성한다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work #d9ad066 # 봉헌실 천장 아랫면과 두께는 storey 예약 그대로이고 서측 지붕 하부와 별개 판이므로 부모 지붕 높이를 바꾸지 않았다.
 */
