 ...
 * @evidenceReview spaces/rooms/colonnade.md # regions 배열의 north·west·east·south·southwest·southeast 여섯 조각이 중정의 네 면과 현관 양옆에서 하나의 주랑 고리를 이룬다.
 * @evidence principles/core/source-units.md#source-scope-preservation 영역 끝선을 기준선으로만 정하며 영역 경계는 cell 분해선일 뿐 방·벽을 늘리지 않는다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # 여섯 사각형은 templePlan 기준선만 참조하고 wall export를 만들지 않으므로 셀 분해가 새 벽이나 방으로 변하지 않는다.
 * @evidence principles/core/source-units.md#source-substantive-completion id가 붙은 여섯 사각형 목록으로 바닥·cell·관찰의 영역 중심이 같은 분해를 쓴다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # 각 조각의 id와 네 끝선이 Floor·공간 cell·관찰의 공통 region 입력이 되는지 확인했다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 여섯 region의 안쪽 선이 courtFront·courtBack·westCourt·eastCourt를 따르고 남쪽 둘이 현관 몸체를 피하므로 부모의 고리 경계를 바꿀 이유가 없었다.
 */
 ...
 * @evidenceReview spaces/rooms/colonnade.md # Floor는 여섯 region 모두에 y.floor를 붙여 colonnade 한 owner의 바닥 입력으로 보내며 중정과 현관 바닥을 포함하지 않는다.
 * @evidence principles/core/source-units.md#source-scope-preservation 영역 경계를 물리 이음선으로 만들지 않고 문턱은 각 방이 소유하므로 주랑은 문턱을 만들지 않는다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # roomFloorInput의 passage 인수가 빈 배열이고 여섯 region의 공유선은 geometry 입력으로만 쓰여 주랑 소유 문턱이 늘지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # Floor가 여섯 구획 전체를 한 번의 roomFloorInput 호출로 넘겨 내부 cell 선 대신 연속 바닥 표면을 구성한다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 주랑 Floor가 door passage를 넣지 않아 방 쪽 threshold-support와 충돌하지 않고 고리 바닥의 기존 소유만 소비한다.
 */
 ...
 * @evidenceReview spaces/rooms/colonnade.md # 반환 레코드는 id와 kind가 colonnade인 하나의 공간이며 여섯 region의 roofedCells를 그 공간의 cells로 합친다.
 * @evidence spaces/rooms/colonnade.md#ring-volume 구멍 있는 하나의 고리를 영역과 합성 날개 지붕 하부의 교집합 cell로 표현하고 덮임이 부족하면 거부한다.
 * @evidenceReview spaces/rooms/colonnade.md#ring-volume # 각 영역을 tier wing 지붕 조각으로 roofedCells에 넣어 중정 구멍을 남기고 덮이지 않는 주랑 조각은 성공 값으로 돌리지 않는다.
 * @evidence principles/core/source-units.md#source-scope-preservation 상한은 실제 날개 지붕 하부이며 중정 위나 방 안으로 cell을 넓히지 않는다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # roof.filter의 wing 선택과 여섯 경계 region이 주랑 cell의 평면·상한을 제한해 sanctuary 지붕이나 중정 상공을 채우지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # flatMap 안의 roofedCells가 여섯 영역 각각의 wing patch 덮임을 요구하며 성공 때만 정확한 주랑 cell 배열을 반환한다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # colonnade 공간은 기존 여섯 영역과 실제 날개 지붕 하부만 교차시키므로 새 순환 루프나 상한 재설계를 요구하지 않았다.
 */
