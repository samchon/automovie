 ...
 * @evidenceReview spaces/rooms/storage.md #b8b7a08 # StoragePlan의 eastRoom·eastInner·storageBack·storageFront가 기록실 뒤 서비스 마당 앞 보관실의 네 끝선이다.
 ...
 * @evidenceReview principles/core/source-units.md#source-scope-preservation #e4bc845 # 이 plan은 네 datum 참조만 있어 보관실 내부에 새 수납방을 분리할 수평선을 더하지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion #e9c974f # Floor·Storage 공간·Ceiling이 공통 plan 한 개를 사용해 보관실의 바닥과 상한이 같은 중간 구획을 덮는다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work #d9ad066 # 보관실 네 경계는 부모의 storage-volume에서 왔고 이 객체는 door-storage 위치를 다시 정하지 않아 상위 수리가 없다.
 */
 ...
 * @evidenceReview spaces/rooms/storage.md #b8b7a08 # storage id의 roomFloorInput 하나가 body와 door-storage passage를 받아 본체·반입 문턱을 같은 floor owner로 둔다.
 ...
 * @evidenceReview spaces/storey.md#threshold-support #6354225 # door-storage를 포함한 passage 목록을 storage floor에 넘겨 벽 두께 slab과 유효폭 support의 소유가 주랑으로 넘어가지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-scope-preservation #e4bc845 # 바닥 높이와 구조 두께는 storey 변수이고 roomFloorInput은 storage id를 선택해 이웃 yard 외부 문턱을 만들지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion #e9c974f # body 평면과 passage 목록을 한 호출에 전달해 보관실 floor slab 및 유일한 주랑 반입 threshold를 함께 반환한다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work #d9ad066 # 보관실 바닥 한 구획과 storage 소유 door passage만 입력해 기록실·마당 구획을 재정의할 필요가 없었다.
 */
 ...
 * @evidenceReview spaces/rooms/storage.md #b8b7a08 # 반환 공간은 storage id·kind 및 단층 parent를 갖고 기록실이나 마당의 cell을 추가하지 않는다.
 ...
 * @evidenceReview spaces/rooms/storage.md#storage-volume #37f608a # storage.body 한 cell과 door-storage threshold만 합쳐 후면 yard와 맞댄 벽에 새 문턱 cell을 만들지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-scope-preservation #e4bc845 # StoragePlan과 y.lowCeiling이 본체 범위를 고정하며 thresholdCells도 storage 방 문만 고른다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion #e9c974f # 단층 exact 공간의 cells가 본체와 반입 문턱 모두를 포함해 주랑 connector의 storage 종점을 검사할 수 있다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work #d9ad066 # 부모 storage-volume의 평면·상한과 기존 door-storage를 그대로 합쳐 connector 끝점을 바꾸지 않았다.
 */
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion #e9c974f # storage id와 직사각 plan의 천장 생성은 노출면과 back 면을 분리한 닫힌 판을 보관실 위에 제공한다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work #d9ad066 # ceilingBoardFaces의 남북 끝이 StoragePlan에서 정해져 열린 서비스 마당 위로 천장판을 늘리는 부모 변경이 없다.
 */
