 ...
 * @evidenceReview spaces/rooms/sanctuary.md # SanctuaryPlan의 네 경계가 westRing·eastRing·northInner·sanctuaryFront를 사용해 후면 축 제실 한 칸을 정의한다.
 ...
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # 평면 객체에는 네 templePlan 선만 있으며 제단 국소 석단이나 감실 recess 치수는 제실 geometry로 끌어오지 않는다.
 * @evidence principles/core/source-units.md#source-substantive-completion as const 사각형 하나로 제실 바닥과 지붕 하부 cell이 같은 평면을 받는다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # Floor와 sanctuary roofedCells가 동일한 plan을 참조해 제실 바닥 외곽과 지붕 아래 부피가 다른 방으로 갈라지지 않는다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 부모의 제실 네 경계가 그대로 plan에 들어가고 제단 위치·채광구는 이 객체가 고르지 않아 평면 개정이 없다.
 */
 ...
 * @evidenceReview spaces/rooms/sanctuary.md # sanctuary Floor는 body와 남쪽 양개 문 passage를 한 방 id로 roomFloorInput에 넘겨 문턱 바닥을 제실 소유로 둔다.
 ...
 * @evidenceReview spaces/storey.md#threshold-support # door-sanctuary passage와 slabThickness를 제공해 제실 남벽 안의 넓은 slab 및 통과 폭 support를 제실 floor가 방출한다.
 ...
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # 바닥 높이와 구조 두께는 storey에서 오며 roomFloorInput이 sanctuary 문만 골라 제단의 국소 단을 바닥에 복제하지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # 한 floor 결과가 sanctuary body와 door-sanctuary 문턱을 함께 소유해 남측 경계 void와 실제 통과 바닥이 같은 passage에서 나온다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 제실 Floor가 판정된 본체 평바닥과 남벽 문턱만 낼 뿐 제단 단을 새 slab으로 만들지 않아 상위 바닥 설계가 유지된다.
 */
 ...
 * @evidenceReview spaces/rooms/sanctuary.md # sanctuary id·kind와 단층 parent를 가진 레코드 하나가 제실 본체 및 문턱 cell을 담는다.
 ...
 * @evidenceReview spaces/rooms/sanctuary.md#sanctuary-volume # tier sanctuary 두 roof patch 아래 body cell과 door-sanctuary threshold가 하나의 제실 공간에 합쳐져 내부 순환은 주랑 문 하나로 열린다.
 ...
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # roof.filter가 sanctuary tier만 골라 상한을 실제 경사 지붕 하부에 두며 평판 천장이나 방 밖 cell을 추가하지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # roofedCells가 제실 plan을 지붕 두 면으로 분해하고 thresholdCells가 남벽 양개문을 더해 경사 상한의 단일 제실 레코드를 완성한다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 제실 plan과 sanctuary roof patch·기존 door passage가 맞고 채광구 결속은 openings owner에서 수리됐으므로 방 부피 부모를 다시 열지 않았다.
 */
