 ...
 * @evidenceReview spaces/rooms/records.md # RecordsPlan의 네 변이 eastRoom·eastInner·recordsBack·recordsFront라 관리실과 보관실 사이의 중간 방에만 걸친다.
 ...
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # 네 끝선이 templePlan 참조이고 별도의 숫자나 중간 칸막이 값이 없어 기록실 경계를 재설계하지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # 하나의 RecordsPlan 객체를 Floor·body cell·Ceiling이 같이 사용해 기록실 전후 경계가 구성마다 달라지지 않는다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 기록실 plan은 부모의 네 기준선만 옮기고 문턱을 새 위치로 옮기지 않아 records-volume의 수정이 필요하지 않았다.
 */
 ...
 * @evidenceReview spaces/rooms/records.md # records id로 body와 문 passage를 같은 roomFloorInput에 주어 중간 기록실 바닥과 직접 문턱의 owner를 묶는다.
 ...
 * @evidenceReview spaces/storey.md#threshold-support # floor 입력이 기록실 id 및 templeDoorPassages를 받아 door-records의 wall-depth slab과 좁은 support를 기록실에 귀속할 수 있다.
 ...
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # 두 높이는 y.floor·y.slabThickness이고 door-records는 passage 소유에 따라 선택되어 이 함수 안의 독립 위치 리터럴이 없다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # roomFloorInput 한 결과 안에 records body slab과 문턱 slab/support가 있어 동측 spine void와 바닥 생성이 같은 passage를 쓴다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # records body 구획과 door-records 한 문턱만 바닥으로 보내 이웃 두 업무방의 바닥을 침범하는 새 구획이 없다.
 */
 ...
 * @evidenceReview spaces/rooms/records.md # 반환 레코드가 records id·kind와 temple-ground parent를 유지하며 별도 열람실 공간을 만들지 않는다.
 ...
 * @evidenceReview spaces/rooms/records.md#records-volume # records.body 한 levelCell과 기록실 passage 문턱만 같은 cells에 있어 북쪽·남쪽 공유벽에 추가 출입 cell이 없다.
 ...
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # RecordsPlan과 y.lowCeiling이 cell 평면·상한을 고정해 administration 또는 storage 부피로 넓히지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # levelCell 및 thresholdCells가 한 records 공간에 들어가 주랑 쪽 출입 관찰의 내부성 판정에 두 부피가 제공된다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 본체 plan과 lowCeiling, 기존 door-records 문턱만 사용해 기록실 공간 범위나 connector 종점을 수정하지 않았다.
 */
 ...
 * @evidenceReview spaces/rooms/records.md # Ceiling이 records plan을 3.10m lowCeiling과 0.10m boardReserve로 닫아 판정된 기록실 천장 판을 낸다.
 ...
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # 천장 호출은 records plan과 층 값만 쓰며 칸 선반·노출 장선 부재를 이 공간 소스가 직접 만들지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # records id의 ceilingBoardFaces가 본체 평면 전체에 노출 아랫면과 구조 뒷면을 가진 닫힌 판을 반환한다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 기록실 천장은 기존 기준선과 낮은 천장 예약만 사용하며 이웃 두 방의 천장 plan과 공유 칸막이 치수를 바꾸지 않았다.
 */
