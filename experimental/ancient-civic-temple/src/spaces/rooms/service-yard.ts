 ...
 * @evidenceReview spaces/rooms/service-yard.md # ServiceYardPlan은 eastRoom·eastInner·northInner·yardFront 네 선으로 후면 우측의 마당 한 칸만 정한다.
 ...
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # plan이 templePlan 네 선을 그대로 참조해 마당 내부에 별도 구조물이나 둘째 마당을 만들 기준선이 없다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # 바닥과 body cell이 ServiceYardPlan을 공통으로 쓰고 Ceiling export는 없어 열린 마당 평면을 완결한다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 서비스 마당 네 변이 부모 야드 범위를 그대로 가리키고 두 출입구 위치는 이 객체가 고르지 않아 상위 수정이 필요하지 않았다.
 */
 ...
 * @evidenceReview spaces/rooms/service-yard.md # yard Floor는 body와 templeDoorPassages를 service-yard id로 묶어 서쪽 주랑 문과 동쪽 외부 문의 바닥 owner를 하나로 유지한다.
 ...
 * @evidenceReview spaces/storey.md#threshold-support # 동일한 roomFloorInput에 두 passage를 공급해 서쪽 문턱의 주랑 면 끝과 동쪽 문턱의 외벽 바깥 끝을 마당이 받도록 한다.
 ...
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # y.floor·slabThickness를 그대로 소비하고 service-yard id가 문 표를 거르므로 보관실 문턱을 이 마당 바닥에 복제하지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # 야드 본체와 양방향 출입 passage가 한 Floor 반환에 있어 서·동 문턱의 slab 및 지지부가 빠지지 않는다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 두 문 passage를 기존 마당 floor 입력으로 처리하고 외부 서비스 문 끝을 새 지면으로 연장하지 않아 부모의 문턱 경계와 일치한다.
 */
 ...
 * @evidenceReview spaces/rooms/service-yard.md # service-yard id·kind 및 단층 parent의 공간 하나에 body와 두 threshold cell만 들어간다.
 ...
 * @evidenceReview spaces/rooms/service-yard.md#yard-volume # body levelCell과 두 thresholdCells가 같은 야드 공간에 들어가고 courtEave는 논리 상한일 뿐 이 export는 지붕 mesh를 반환하지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # 상한은 templeRoofRules.courtEave이고 plan은 ServiceYardPlan 하나라 보관실 영역이나 마당 위 실체 천장을 만들지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # 하나의 exact yard 공간이 body·west door·east exterior door cell을 담아 두 connector 종점의 내부성을 함께 판정하게 한다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 마당의 기존 네 기준선과 courtEave를 논리 cell에만 쓰고 두 문턱도 기존 passage여서 야드 상한을 고칠 이유가 없었다.
 */
