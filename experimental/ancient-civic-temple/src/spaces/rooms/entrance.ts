 ...
 * @evidenceReview spaces/rooms/entrance.md #17949b3 # EntranceSteps의 세 값 1.8·0.35·0.12m가 현관 두 단의 폭·디딤·상승을 그대로 지정한다.
 * @evidence principles/core/source-units.md#source-scope-preservation 계단 치수만 가지며 경로는 circulation이 소비한다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation #e4bc845 # 이 export는 width·tread·rise 세 치수만 가지고 입구 route나 connector를 직접 생성하지 않는다.
 * @evidence principles/core/source-units.md#source-substantive-completion as const 값으로 바닥 구획과 계단 connector가 같은 치수를 쓴다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion #e9c974f # 상수 객체가 entrance floors의 half·firstBack과 circulation의 step connector 입력을 함께 지탱한다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work #d9ad066 # 석단 치수 세 값을 판정된 현관 설계에서 그대로 받았고 source에서 계단 수나 위치를 바꾸지 않았다.
 */
 ...
 * @evidenceReview spaces/rooms/entrance.md #17949b3 # Floors의 first-tread·upper-landing·west-support·east-support 네 구획에서 둘째 디딤과 참은 별도 경계 없이 같은 상부참이다.
 * @evidence principles/core/source-units.md#source-scope-preservation 둘째 디딤과 참 사이에 추가 챌면을 만들지 않고 중앙 폭은 계단 폭의 절반을 축 양쪽에 적용한다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation #e4bc845 # half=steps.width/2를 양쪽 경계로 쓰고 상부참 한 구획만 반환해 중간 높이의 가짜 챌면이 생기지 않는다.
 * @evidence principles/core/source-units.md#source-substantive-completion id·범위·완성면을 가진 구획 목록으로 바닥 slab과 cell 하한이 같은 값을 쓴다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion #e9c974f # 네 반환 구획에 각각 id·네 평면 끝선·floor가 있어 바닥 생성과 roofedCells가 동일한 단높이를 소비한다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work #d9ad066 # 첫 디딤은 road+rise, 나머지 참과 받침은 y.floor라 설계의 −0.12/0m 두 완성면만 구현한다.
 */
 ...
 * @evidenceReview spaces/rooms/entrance.md #17949b3 # Floor가 네 현관 구획과 문 passage를 묶고 southOuter에 닿은 slab만 exteriorBottom으로 늘려 접지부를 연결한다.
 * @evidence principles/core/source-units.md#source-scope-preservation 외벽 하단은 호출자가 넘긴 값만 쓰고 지면을 여기서 만들지 않는다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation #e4bc845 # exteriorBottom 인수를 null일 때는 그대로 두고 제공됐을 때도 slab.bottom만 낮추며 site ground를 방 소스에 추가하지 않는다.
 * @evidence principles/core/source-units.md#source-substantive-completion null이면 기본 두께, 값이 있으면 앞쪽 구획 하단을 낮춘 입력을 돌려준다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion #e9c974f # null과 숫자 분기가 각각 기본 roomFloorInput 또는 southOuter 구획의 낮아진 bottom을 반환해 지면 접촉 입력이 누락되지 않는다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work #d9ad066 # 남면 접지 하단은 기존 storey 계열 호출자가 주고 계단 plan은 entrance 예약을 유지하므로 상위 접지선을 바꾸지 않았다.
 */
 ...
 * @evidenceReview spaces/rooms/entrance.md #17949b3 # 반환되는 entrance 공간 하나가 네 구획의 roofedCells와 후퇴벽 door passage의 thresholdCells를 함께 가진다.
 * @evidence spaces/rooms/entrance.md#entrance-volume 각 구획의 실제 완성면을 cell 하한으로, 합성 포치 지붕 하부를 상한으로 써서 석재 내부를 공간에 넣지 않는다.
 * @evidenceReview spaces/rooms/entrance.md#entrance-volume #8bd8ef3 # roofedCells마다 region.floor가 하한이고 porch tier patch가 상한이라 낮은 첫 디딤 높이를 옆 받침에 복사하지 않는다.
 * @evidence principles/core/source-units.md#source-scope-preservation 계단 최저 높이를 현관 전체에 복사하지 않는다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation #e4bc845 # 각 region 자체 floor를 roofedCells에 넘기므로 first-tread의 낮은 하한이 upper-landing·양쪽 support cell에 번지지 않는다.
 * @evidence principles/core/source-units.md#source-substantive-completion roofedCells가 포치 지붕 덮임을 검사하고 문턱 cell을 더한 공간을 돌려준다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion #e9c974f # 네 roofedCells 호출과 entrance thresholdCells 결과가 단층 exact 레코드에 합쳐져 현관 전 영역의 덮임과 문턱이 함께 제공된다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work #d9ad066 # 현관 네 완성면과 기존 porch patch·door passage를 교차했을 뿐 새 공간 구획이나 층 높이 결정이 드러나지 않았다.
 */
