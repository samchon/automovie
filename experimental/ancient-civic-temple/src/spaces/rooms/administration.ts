 ...
 * @evidenceReview spaces/rooms/administration.md # templeAdministrationPlan의 서·동·북·남 값이 관리실 본체에 선언된 네 기준선을 각각 가리키며 이웃 기록실 구간을 포함하지 않는지 확인했다.
 ...
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # 이 export에는 네 plan datum의 참조만 있고 치수 리터럴이나 기록실 경계를 바꾸는 분기가 없어 공간 설계를 보존한다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # plan 객체 한 개를 Floor·Body·Ceiling 세 호출이 모두 사용하므로 관리실 바닥과 상부가 다른 평면으로 벌어질 수 없는지 대조했다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # administration 평면 네 끝선이 부모의 업무방 범위와 같고 문턱 생성은 별도 Floor에서 처리하므로 이 평면 선언이 부모 치수를 고치게 하지 않았다.
 */
 ...
 * @evidenceReview spaces/rooms/administration.md # Floor의 roomFloorInput에 administration 한 본체 구획과 문 표를 같이 넘겨 floor owner가 방 본체와 문턱을 함께 방출하는지 확인했다.
 ...
 * @evidenceReview spaces/storey.md#threshold-support # 관리실 바닥 입력이 templeDoorPassages와 slabThickness를 받아 door-administration의 벽 두께 문턱을 같은 방 owner 아래 두며 주랑 바닥에 넘기지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # floor와 slabThickness는 storey 값이며 문턱 목록은 passage 입력에서 administration과 결합되어 이 함수가 독립적인 높이나 문 위치를 발명하지 않는지 확인했다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # roomFloorInput의 반환에 body와 door-administration threshold가 함께 들어가므로 하나의 관리실 바닥 export가 직접 문턱까지 완결한다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 관리실 문턱은 door-administration의 기존 passage로 생성되고 추가 문이나 slab 높이 변경이 이 바닥 export에 없어 부모의 문턱 소유를 유지한다.
 */
 ...
 * @evidenceReview spaces/rooms/administration.md # 반환 공간의 id·kind가 administration이고 parent가 단층 storey라 별도 층이나 숨은 방 레코드를 만들지 않는다.
 ...
 * @evidenceReview spaces/rooms/administration.md#office-volume # levelCell의 plan과 lowCeiling이 본체 범위·3.10m 상한을 정하고 thresholdCells가 같은 administration 아래 직접 문턱 cell을 더하는지 확인했다.
 ...
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # 공간 cell은 administration plan 및 해당 문턱만으로 구성되고 y.lowCeiling을 사용하므로 기록실 쪽으로 확장하는 새 경계가 없다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # body levelCell과 administration thresholdCells를 한 cells 배열로 결합해 관찰의 inside 판정에 필요한 두 구획을 같은 공간에 제공한다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # office-volume의 단일 본체와 기존 door-administration 문턱만 방 cell로 받았으며 천장 상한도 storey 예약을 사용해 부모 room volume 변경이 필요하지 않았다.
 */
 ...
 * @evidenceReview spaces/rooms/administration.md # ceilingBoardFaces가 관리실 평면과 lowCeiling·ceilingBoardReserve를 받아 3.10m 아랫면의 닫힌 천장 판을 형성한다.
 * @evidence principles/core/source-units.md#source-scope-preservation 관리실 평면에 층의 낮은 천장 높이와 널판 예약만 쓰고 노출 보는 만들지 않는다(후속 부재).
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # 이 export는 관리실 평면과 층의 두 높이 매개변수만 전달하고 노출 목재 보는 모델 소유로 남긴다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # ceilingBoardFaces의 administration 소유와 직사각 평면 입력으로 천장 정면과 뒷면이 같은 방 범위에서 방출되는지 확인했다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 관리실 천장은 기존 lowCeiling과 boardReserve를 사용하며 독립 ceilingBoardFaces 구획이라 기록실 천장 치수나 층 예약을 재정의하지 않았다.
 */
