 ...
 * @evidenceReview spaces/rooms/courtyard.md # CourtyardPlan의 네 변이 westCourt·eastCourt·courtBack·courtFront를 그대로 사용해 요구된 단일 열린 중정만 감싼다.
 * @evidence principles/core/source-units.md#source-scope-preservation 기준선으로만 평면을 정한다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # 중정 plan은 templePlan의 네 datum 참조뿐이며 렌더용 확장이나 임의 치수는 없다.
 * @evidence principles/core/source-units.md#source-substantive-completion as const 사각형으로 바닥·cell·분수 중심이 같은 평면을 받는다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # Floor와 courtyard cell, FountainInputs가 한 CourtyardPlan을 공유하므로 수반 중심과 바닥 중심이 갈라지지 않는다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 네 경계는 판정된 7.0×7.85m 중정의 기준선이고 source에서 폭이나 길이를 재결정하지 않아 부모를 고치지 않았다.
 */
 ...
 * @evidenceReview spaces/rooms/courtyard.md # 중정 Floor의 body가 y.courtyard를 완성면으로 받아 주랑의 y.floor보다 0.12m 낮은 slab을 만든다.
 * @evidence principles/core/source-units.md#source-scope-preservation 분수 밑에 별도 지면을 두지 않고 중정 높이는 층 값을 받는다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # floor 입력은 한 body 구획뿐이고 fountain 받침판을 바닥으로 복제하지 않으며 높이도 storey 값을 쓴다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # 한 번의 roomFloorInput에 중정 plan과 낮은 floor가 들어가 주랑 바닥과의 경계에서 단차 입력이 보존된다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 바닥의 courtyard 높이는 기존 storey datum이고 새 지면이나 수반 geometry를 만들지 않아 상위 층 변경이 없다.
 */
 ...
 * @evidenceReview spaces/rooms/courtyard.md # OwnsCurb는 주랑 바닥의 중정 접경 수직 face만 courtyard 소유로 돌려 중정 네 변의 석재 턱을 단독 귀속한다.
 ...
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # top·bottom face와 주랑 외 face를 먼저 거르고 네 중정 경계선·두 높이를 모두 확인해 바닥 내부 면을 잘못 청구하지 않는다.
 ...
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # face의 space·direction·corner를 판정한 boolean이 조립 단계의 owner 선택에 쓰일 수 있는 완결된 경계 판별을 제공한다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 중정 턱은 실제 주랑 slab의 노출 옆면을 재귀속할 뿐 중정 바닥 형상이나 주랑 높이를 변경하지 않는다.
 */
 ...
 * @evidenceReview spaces/rooms/courtyard.md # 공간 레코드 하나가 id·kind courtyard, fidelity exact, 단층 부모와 단일 body cell을 갖는다.
 * @evidence spaces/rooms/courtyard.md#court-volume 하한은 완성면 -0.12m, 상한은 주랑 처마 지지 높이 3.20m의 유한 논리 범위이며 천장·그림자 면을 만들지 않는다.
 * @evidenceReview spaces/rooms/courtyard.md#court-volume # levelCell의 하한 y.courtyard와 상한 courtEave는 관찰용 공간 부피이고 이 export에는 천장 mesh 생성이 없다.
 * @evidence principles/core/source-units.md#source-scope-preservation 논리 상한을 하늘을 막는 geometry로 만들지 않는다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # cell의 courtEave가 논리 분류에만 들어가고 roof나 ceiling 부재는 반환하지 않아 열린 중정의 하늘이 유지된다.
 * @evidence principles/core/source-units.md#source-substantive-completion 한 cell 공간을 돌려주며 관찰의 중심·모서리가 이 cell로 내부성을 판정한다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # courtyard.body 한 cell을 가진 exact 공간이 중정 관찰 pose를 공간 안으로 분류할 완결된 레코드다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 중정 평면과 y.courtyard·courtEave를 기존 설계에서 그대로 소비했고 천장을 만들지 않아 상위 설계 수리는 필요하지 않았다.
 */
 ...
 * @evidenceReview spaces/rooms/courtyard.md # FountainInputs의 X·Z가 중정 양끝의 평균이고 외경·rim·water·jet 높이가 설계의 2.0·0.52·−0.08·+0.65m 관계를 따른다.
 * @evidence principles/core/source-units.md#source-scope-preservation 수반 geometry를 만들지 않고 후속 model/system이 쓸 입력만 돌려준다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # 이 함수는 center와 치수만 반환하며 수반 mesh나 물 애니메이션을 방 소스에 만들지 않는다.
 * @evidence principles/core/source-units.md#source-substantive-completion 중심 좌표와 네 높이를 계산된 값으로 돌려줘 소비자가 치수를 다시 정하지 않는다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # waterHeight와 jetTop이 rimHeight에서 순서대로 산술 계산되어 후속 모델이 물 높이를 새로 선택하지 않는다.
 ...
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 분수 입력의 중앙점·외경·물면·물줄기 범위가 기존 중정 예약과 같아 이 함수에서 공간 크기 수정이 드러나지 않았다.
 */
