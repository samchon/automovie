 ...
 * @evidenceReview obligations/design/space-sources.md#space-source-invalid-topology #030592d # validateBuiltEnvironment 실패 분기에서 부분 조립을 반환하지 않고 위반 경로와 기대값을 포함한 Error를 던진다.
 * @evidenceReview obligations/design/space-sources.md#space-source-stable-identities #8f4bb4a # element ID는 model ID의 접두어 변환에서, boundary ID는 고정 설계 주소에서 나오며 난수나 실행 시각을 읽지 않는다.
 * @evidenceReview obligations/design/space-sources.md#space-source-design-ownership #c0afa1f # 각 방·입면·지붕·문·대지 export를 호출해 합성하고 높이 차이 뒷면처럼 역으로 드러난 결정은 junctions H2의 면 소유 후에 방출한다.
 */
