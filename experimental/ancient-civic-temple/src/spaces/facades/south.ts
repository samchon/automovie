 ...
 * @evidenceReview spaces/facades/south.md # 남측 입면 파일의 파라펫 수평 윗단을 이 상수가 정하고 벽 segment는 templeSouthWalls가 구성한다.
 * @evidenceReview spaces/facades/south.md#south-envelope # Y=4.85m 상단을 이 export 하나로 고정해 서·북 파라펫 및 반환벽 코핑에 같은 값을 공급한다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # 숫자 4.85만 내고 templeWallTrim의 코핑 두께·돌출과 roof patch 높이는 수정하지 않는다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # 네 입면의 flat top 정책이 직접 읽는 유한 literal로 코핑 높이가 미정인 출력 경로가 없다.
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # south-envelope의 4.85m를 그대로 대조했고 self-check의 지붕·코핑 겹침 0에서 상단을 낮출 부모 결함이 나오지 않았다.
 */
 ...
 * @evidenceReview spaces/facades/south.md # 입면의 접지 상대 기단 높이 0.65m를 공통 상수로 내며 실제 grade 접합은 wall-trim이 계산한다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # 돌출·절단 형상은 junctions의 trim profile에 남기고 여기서는 지면 위 rise 하나만 정한다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # plinthFaces가 동일한 0.65m를 외벽 네 면에 적용하므로 현관 도로 높이를 기단 절대 높이로 잘못 복제하지 않는다.
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # south-envelope의 지면 위 0.65m 기준을 source trim 입력에 옮겼고 지면 접합은 공통 wallBottom으로 내려가 부모 바닥 높이를 바꾸지 않았다.
 */
 ...
 * @evidenceReview spaces/facades/south.md # 포치 삼각 막음의 하단 높이만 이 상수가 맡고 기둥·보 형상은 models에 남긴다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # 막음 윗변은 templeSouthWalls의 porch roof top으로 받아 숫자 3.5가 윗변이나 용마루 높이를 새로 정하지 않는다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # pediment WallSpec의 bottom이 이 3.5m 상수를 사용해 열린 포치 전면 위에 삼각 벽을 실제로 둔다.
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # south-envelope의 보 위 삼각 막음 아랫면 Y=3.50m와 일치해 현관 개구를 막는 추가 벽을 요구하지 않았다.
 */
 ...
 * @evidenceReview spaces/facades/south.md # 두 날개 외벽·두 반환벽·후퇴벽·삼각 막음 여섯 WallSpec을 같은 남측 소유로 내며 중앙 문 void만 openings에서 받는다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # 앞쪽 벽과 반환벽의 코핑은 templeParapetTop, 박공 막음 하단은 templePedimentBase, 윗변은 porch roof를 써 후속 기둥 형상을 여기서 만들지 않는다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # entry-back의 door void, end 두 칸의 coping top, 반환벽의 wing/porch split과 pediment의 상단 roof 정책이 각각 WallSpec의 실제 구간을 이룬다.
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 후퇴벽 북면 끝 칸과 반환벽 위쪽, 앞쪽 삼각 막음을 설계된 범위로 나눴고 겹침 전수 검사 0에서 부모 박공 위치를 바꿀 요구가 없었다.
 */
