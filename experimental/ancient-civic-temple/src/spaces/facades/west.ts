 ...
 * @evidenceReview spaces/facades/west.md #9e93c1a # 서측 전 길이 무창 파라펫 실체를 wall.facade-west 하나로 반환하고 봉헌실 내부 면과 두 모서리 joint를 구분한다.
 * @evidenceReview spaces/facades/west.md#west-envelope #0520696 # west 안쪽 지붕 위 부분은 highSplit의 facade-west.parapet-back, 아래는 offering.wall로 나뉘며 제실 flank 표면은 boundaries의 west-spine split이 같은 입면 owner로 낸다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation #e4bc845 # voids 빈 배열을 유지하고 plan은 templeOuterWallPlans, top은 south의 templeParapetTop을 받아 새 창이나 별도 코핑 높이를 만들지 않는다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion #e9c974f # 북·중·남 세 segment와 바깥 실체 bottom, 양끝 joint, 중간 봉헌실/파라펫 뒷면을 채워 무창 외벽 모델을 생성할 입력이 있다.
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work #d9ad066 # 서측 무창·봉헌실 안면·남측과 같은 코핑을 wall spec에 대조했고 겹침 검사는 0으로 부모에 새 출입이나 높이 보정이 필요하지 않았다.
 */
