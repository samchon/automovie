 ...
 * @evidenceReview spaces/building.md # 기준 평면 파일의 plan-datums와 footprint 수치를 이 객체가 맡고, containment 조립은 아래 templeSpaceHierarchy가 맡는 파일 내 분업을 읽었다.
 * @evidenceReview spaces/building.md#plan-datums # west/east-outer ±10.5, yard-front −2.45, south-outer 10.25와 tolerance 0.001이 기준선 표의 이름·부호·값과 같다.
 * @evidenceReview spaces/building.md#footprint # outer X 차 21m와 Z 차 20.5m를 계산해 430.5㎡가 되고 room·court 선은 이 외곽 안에 남는다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # templePlan의 속성은 기준선과 비교 허용값뿐이며 벽이나 mesh를 만드는 호출이 없다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # as const 객체가 하위 벽·방·지붕 모듈에 동일한 수치형 literal을 공급하며 임시 null이나 계산 대기가 없다.
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # plan-datums의 X 16개·Z 16개 선과 외곽 네 끝을 객체 값에 대조했고 self-check의 겹침 0이 이 입력으로 나와 부모 선을 바꿀 필요가 드러나지 않았다.
 */
 ...
 * @evidenceReview spaces/building.md # 기준 평면 파일의 containment가 정한 temple→temple-ground→아홉 공간을 이 함수가 조립하고 datum 수치는 templePlan이 별도로 공급한다.
 * @evidenceReview spaces/building.md#containment # expected 집합의 아홉 ID를 한 번씩 소진하며 parent가 temple-ground가 아니거나 cell이 비거나 shell을 쓰면 오류를 던진다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # caller의 rooms 레코드를 펼쳐 보존하고 두 부모 레코드만 추가해 물리벽이나 방 경계를 이 함수가 복제하지 않는다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # 중복·외래·누락 ID와 부모·cell·shell 실패가 각각 return 이전에 거부되어 빈 위계 반환으로 통과하지 않는다.
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # containment의 한 건물·한 층·아홉 공간 ID를 expected 집합과 반환 배열에 맞대어 새 방이나 보조 복도를 만들 결정이 필요하지 않았다.
 */
