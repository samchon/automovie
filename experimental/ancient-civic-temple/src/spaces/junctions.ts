 ...
 * @evidenceReview spaces/junctions.md # 외벽 접합 파일의 대각 평면을 north/west/east/southWest/southEast 다섯 윤곽으로 내고 다른 접합 치수는 아래 exports에 둔다.
 * @evidenceReview spaces/junctions.md#wall-junctions # 북서·북동·남서·남동의 외점과 내점이 인접 윤곽에서 같은 대각 끝선을 공유해 두 벽의 모서리 체적이 중복되지 않는다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # PlanPoint 배열만 반환하고 코핑·벽 높이·표면 ID는 이 함수가 만들지 않아 접합 계산의 책임을 넘지 않는다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # 네 입면이 바로 plan으로 쓰는 닫힌 꼭짓점 순서 다섯 개를 반환하며 남측은 현관 후퇴로 양분한다.
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 외곽 네 쌍의 대각 끝을 junctions 기준선과 대조했고 1cm 외피 스캔의 겹침 0에서 부모 모서리 규칙을 바꿀 필요가 없었다.
 */
 ...
 * @evidenceReview spaces/junctions.md # 외벽 석재 단면 수치만 이 상수가 내고 대각 벽 윤곽과 T 접합은 같은 파일의 다른 exports가 계산한다.
 * @evidenceReview spaces/junctions.md#plinth-coping # copingThickness 0.16, copingProjection 0.06, plinthProjection 0.04가 설계의 세 독립 단면 치수와 같다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # 4.85m 코핑 상단이나 지면 위 0.65m 기단 높이를 재선택하지 않고 wall-trim에 두께·돌출만 준다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # as const 치수 객체가 copingFaces와 plinthFaces에서 동일하게 쓰여 별도 코핑·기단 박스 상수를 두지 않는다.
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # 설계의 0.16·0.06·0.04m와 객체 값이 같고 self-check 겹침 0이 접합을 받아 부모 단면을 수정하지 않았다.
 */
 ...
 * @evidenceReview spaces/junctions.md # 내부 T 접합의 끝선 값 자체는 templeInteriorWallEnds가 내고 이 함수는 그 네 수치로 plan polygon을 형성한다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # west/east/north/south 인수를 다시 고르지 않고 네 PlanPoint로만 옮겨 내부벽 높이·문을 만들지 않는다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # 서북→동북→동남→서남 순서가 planar-domain의 바깥법선과 맞아 wall prism이 뒤집힌 면 없이 생성된다.
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # T 접합 위치는 caller의 templeInteriorWallEnds에서 받은 그대로 사각 윤곽이 되고 junctions H2에 없는 위치 보정이 없었다.
 */
 ...
 * @evidenceReview spaces/junctions.md # 내부 spine·제실 남벽·업무방 가로벽과 현관 반환부의 접합 값을 별도 이름으로 내어 WallRect 소비자가 한 기준선을 공유한다.
 * @evidenceReview principles/core/source-units.md#source-scope-preservation # 각 끝점은 templePlan의 기존 경계선이고 이 함수에는 새 벽 두께·표면·문 좌표가 없다.
 * @evidenceReview principles/core/source-units.md#source-substantive-completion # spine, sanctuarySouth, crossWall, entryBack, porchReturn의 다섯 반환값을 boundaries와 south facade가 직접 사용한다.
 * @evidenceExcludeReview upstream/design/space-sources.md#design-revision-from-space-source-work # T 가지 벽 끝을 parent의 north-inner/south-inner, west-ring/east-ring, east-room/east-inner와 비교했고 임의 여유를 더하지 않았다.
 */
