 ...
 * 외부 지면은 maps가 아직 없으므로 공급하지 않고, 외벽 하단 계산에는 현재
 * 판정된 건물 측 최저 접점(정문 도로 Y=-0.24m)을 잠정 입력으로 쓴다.
 */
...
export const createViewerPayload = () => {
  const built = createTempleEnvironment({ exteriorContactMinimum: templeLevels.publicRoad });
