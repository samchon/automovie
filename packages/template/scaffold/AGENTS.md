...
- **The user outranks a skill.** A direct user instruction takes precedence over anything in this file or a skill document. An override is something the user said; infer none from a terse request or the cost of complying. Precedence settles authority, grants nothing else, and waives no gate or refusal. Two production directives that contradict each other are a different question; [Production-specific contract](.agents/skills/evidence-graph/work-specific.md#inputs-and-authority) owns it.
...
- **Scope belongs to the user.** Reinterpret the goal or expand the work only when the user delegates that decision. Within the agreed goal, investigate, implement, and verify the necessary substeps with full initiative. A defect that predates the request or a cleanup the task did not call for belongs in your report as a follow-up; leave it out of this change unless the requested result cannot be reached without it. Correcting the earliest owner of the defect you were asked about stays inside that request.
...
- **Finish the turn's work.** A turn ends when the work is done. When your last paragraph is a plan or a promise about work you have not done, take that step now, and take a reversible step the request already covers instead of asking permission for it. Say in one line what you are doing as you go. This governs stopping early and changes nothing about what a result may claim.
