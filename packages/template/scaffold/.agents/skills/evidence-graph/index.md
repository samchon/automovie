...
Read `lint.config.ts`, [Production documents](../../../docs/README.md), the active host population, and every selected target before changing a governed artifact. The document map owns physical placement and [Contract targets](contract-targets.md) owns the shared and language inventory forms. `lint.config.ts` is the single typed declaration consumed by graph lint; the evidence reader's manifest projects its exact bindings without creating another project file.
