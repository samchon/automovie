...
`solveAutoMovieAbsoluteDisplacement` minimizes weighted absolute signed travel with exact fixed-zero attachments. It uses a linear epigraph, compresses coefficient-identical rows, restores original coordinates, and checks every original interval and the numerical objective gap. The caller supplies physical caps as rows. This early API supports the joint oral-enclosure integration; current face construction does not yet call it. L1 travel is a geometric fitting objective, not a biomechanical tissue law.
