...
The deterministic engine evaluates and validates the `@automovie/interface` AST. Geometry and orchestration are TypeScript; jointly constrained displacement uses a pinned, synchronous OSQP WebAssembly kernel. The engine contains no AI or `three.js` dependency.
...
The numerical kernel ships as generated byte data and initializes synchronously on its first solve. Each call owns temporary memory and releases the native solver; there is no file, network, clock or random-state dependency. Node and browser workers must support WebAssembly. Normal package builds consume committed bytes without a C compiler. See [kernel provenance, rebuilding and licenses](vendor/quadratic/README.md) before changing that boundary.
