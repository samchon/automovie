/**
 * Build closed solids from one convex profile by straight extrusion or a
 * transported polyline sweep. Asset authors reach these operations through
 * proceduralMesh; the profile hull is admitted once before any ring is emitted.
 * Inputs stay caller-owned, output buffers are new, and coordinates are metres.
 * Path frames and smooth area-weighted normals come from their shared owners.
 * Changing ring order changes winding and every downstream topology result.
 */
import { IAutoMovieMesh, IAutoMovieVector3 } from "@automovie/interface";

import { convexHull2D } from "../math/hull";
import { finitePoint, finiteVector, positive } from "./proceduralDimensions";
